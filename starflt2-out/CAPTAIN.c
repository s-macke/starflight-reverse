// ====== OVERLAY 'CAPTAIN' ======

#include"interface.h"

// store offset = 0xe420
// overlay size   = 0x1140

// =================================
// =========== DICTIONARY ==========
// =================================
// 1869:            EDIT  codep:0x224c parp:0xb41c size:0x0008 C-string:'EDIT'
// 1870:            USE"  codep:0x224c parp:0xb42d size:0x0006 C-string:'USE_qm_'
// 1871:              SB  codep:0x2214 parp:0xb43a size:0x00e6 C-string:'SB'
// 1872:      UNK_0xb522  codep:0xd415 parp:0xb522 size:0x0026 C-string:'UNK_0xb522'
// 1873:      UNK_0xb54a  codep:0xf60e parp:0xb54a size:0x0007 C-string:'UNK_0xb54a'
// 1874:              WS  codep:0x224c parp:0xb558 size:0x0006 C-string:'WS'
// 1875:              WL  codep:0x224c parp:0xb565 size:0x001c C-string:'WL'
// 1876:              .S  codep:0x224c parp:0xb588 size:0x0010 C-string:'_dot_S'
// 1877:           MUSIC  codep:0x1ab5 parp:0xb5a2 size:0x0014 C-string:'MUSIC'
// 1878:        MUSIC-OV  codep:0x83dd parp:0xb5c3 size:0x0002 C-string:'MUSIC_minus_OV'
// 1879:          INTROS  codep:0x224c parp:0xb5d0 size:0x0006 C-string:'INTROS'
// 1880:           HIMUS  codep:0x224c parp:0xb5e0 size:0x0006 C-string:'HIMUS'
// 1881:          TALLOC  codep:0x224c parp:0xb5f1 size:0x0006 C-string:'TALLOC'
// 1882:         SETABLT  codep:0x224c parp:0xb603 size:0x0006 C-string:'SETABLT'
// 1883:             *OP  codep:0x3b74 parp:0xb611 size:0x0004 C-string:'_star_OP'
// 1884:           *COMM  codep:0x3b74 parp:0xb61f size:0x000b C-string:'_star_COMM'
// 1885:      UNK_0xb62c  codep:0x3b74 parp:0xb62c size:0x02af C-string:'UNK_0xb62c'
// 1886:      UNK_0xb8dd  codep:0x224c parp:0xb8dd size:0x0012 C-string:'UNK_0xb8dd'
// 1887:      UNK_0xb8f1  codep:0x224c parp:0xb8f1 size:0x0012 C-string:'UNK_0xb8f1'
// 1888:      UNK_0xb905  codep:0x224c parp:0xb905 size:0x0012 C-string:'UNK_0xb905'
// 1889:      UNK_0xb919  codep:0x224c parp:0xb919 size:0x0023 C-string:'UNK_0xb919'
// 1890:      UNK_0xb93e  codep:0x224c parp:0xb93e size:0x03d4 C-string:'UNK_0xb93e'
// 1891:      UNK_0xbd14  codep:0x0000 parp:0xbd14 size:0x0066 C-string:'UNK_0xbd14'
// 1892:      UNK_0xbd7c  codep:0x79bd parp:0xbd7c size:0x037b C-string:'UNK_0xbd7c'
// 1893:      UNK_0xc0f9  codep:0x224c parp:0xc0f9 size:0x1129 C-string:'UNK_0xc0f9'
// 1894:      UNK_0xd224  codep:0xc527 parp:0xd224 size:0x0d49 C-string:'UNK_0xd224'
// 1895:      UNK_0xdf6f  codep:0xdc70 parp:0xdf6f size:0x04c5 C-string:'UNK_0xdf6f'
// 1896:      UNK_0xe436  codep:0x1d29 parp:0xe436 size:0x0002 C-string:'UNK_0xe436'
// 1897:      UNK_0xe43a  codep:0x1d29 parp:0xe43a size:0x0008 C-string:'UNK_0xe43a'
// 1898:      UNK_0xe444  codep:0x1d29 parp:0xe444 size:0x0002 C-string:'UNK_0xe444'
// 1899:      UNK_0xe448  codep:0x1d29 parp:0xe448 size:0x0002 C-string:'UNK_0xe448'
// 1900:      UNK_0xe44c  codep:0x224c parp:0xe44c size:0x0010 C-string:'UNK_0xe44c'
// 1901:      UNK_0xe45e  codep:0x7394 parp:0xe45e size:0x0006 C-string:'UNK_0xe45e'
// 1902:      UNK_0xe466  codep:0x224c parp:0xe466 size:0x000a C-string:'UNK_0xe466'
// 1903:      UNK_0xe472  codep:0x224c parp:0xe472 size:0x0008 C-string:'UNK_0xe472'
// 1904:      UNK_0xe47c  codep:0x224c parp:0xe47c size:0x004e C-string:'UNK_0xe47c'
// 1905:      UNK_0xe4cc  codep:0x7394 parp:0xe4cc size:0x0006 C-string:'UNK_0xe4cc'
// 1906:      UNK_0xe4d4  codep:0x7394 parp:0xe4d4 size:0x0006 C-string:'UNK_0xe4d4'
// 1907:      UNK_0xe4dc  codep:0x7394 parp:0xe4dc size:0x0006 C-string:'UNK_0xe4dc'
// 1908:      UNK_0xe4e4  codep:0x7394 parp:0xe4e4 size:0x0006 C-string:'UNK_0xe4e4'
// 1909:      UNK_0xe4ec  codep:0x7420 parp:0xe4ec size:0x0003 C-string:'UNK_0xe4ec'
// 1910:      UNK_0xe4f1  codep:0x7394 parp:0xe4f1 size:0x0006 C-string:'UNK_0xe4f1'
// 1911:      UNK_0xe4f9  codep:0x7394 parp:0xe4f9 size:0x0006 C-string:'UNK_0xe4f9'
// 1912:      UNK_0xe501  codep:0x7420 parp:0xe501 size:0x0003 C-string:'UNK_0xe501'
// 1913:      UNK_0xe506  codep:0x7420 parp:0xe506 size:0x0003 C-string:'UNK_0xe506'
// 1914:      UNK_0xe50b  codep:0x7420 parp:0xe50b size:0x0003 C-string:'UNK_0xe50b'
// 1915:      UNK_0xe510  codep:0x7420 parp:0xe510 size:0x0003 C-string:'UNK_0xe510'
// 1916:      UNK_0xe515  codep:0x7420 parp:0xe515 size:0x0003 C-string:'UNK_0xe515'
// 1917:      UNK_0xe51a  codep:0x7420 parp:0xe51a size:0x000d C-string:'UNK_0xe51a'
// 1918:      UNK_0xe529  codep:0x7420 parp:0xe529 size:0x0017 C-string:'UNK_0xe529'
// 1919:      UNK_0xe542  codep:0x1d29 parp:0xe542 size:0x000e C-string:'UNK_0xe542'
// 1920:      UNK_0xe552  codep:0x224c parp:0xe552 size:0x001a C-string:'UNK_0xe552'
// 1921:      UNK_0xe56e  codep:0x224c parp:0xe56e size:0x000c C-string:'UNK_0xe56e'
// 1922:            .BAL  codep:0x224c parp:0xe583 size:0x0046 C-string:'_dot_BAL'
// 1923:      UNK_0xe5cb  codep:0x224c parp:0xe5cb size:0x002a C-string:'UNK_0xe5cb'
// 1924:      UNK_0xe5f7  codep:0x224c parp:0xe5f7 size:0x000e C-string:'UNK_0xe5f7'
// 1925:      UNK_0xe607  codep:0x224c parp:0xe607 size:0x000e C-string:'UNK_0xe607'
// 1926:      UNK_0xe617  codep:0x224c parp:0xe617 size:0x0032 C-string:'UNK_0xe617'
// 1927:      UNK_0xe64b  codep:0x224c parp:0xe64b size:0x000a C-string:'UNK_0xe64b'
// 1928:      UNK_0xe657  codep:0x224c parp:0xe657 size:0x002a C-string:'UNK_0xe657'
// 1929:      UNK_0xe683  codep:0x224c parp:0xe683 size:0x0060 C-string:'UNK_0xe683'
// 1930:       TD-SCROLL  codep:0x4b3b parp:0xe6f1 size:0x0126 C-string:'TD_minus_SCROLL'
// 1931:      UNK_0xe819  codep:0xdf75 parp:0xe819 size:0x0017 C-string:'UNK_0xe819'
// 1932:      UNK_0xe832  codep:0x224c parp:0xe832 size:0x0014 C-string:'UNK_0xe832'
// 1933:      UNK_0xe848  codep:0x224c parp:0xe848 size:0x0028 C-string:'UNK_0xe848'
// 1934:      UNK_0xe872  codep:0x224c parp:0xe872 size:0x0030 C-string:'UNK_0xe872'
// 1935:      UNK_0xe8a4  codep:0x224c parp:0xe8a4 size:0x002a C-string:'UNK_0xe8a4'
// 1936:      UNK_0xe8d0  codep:0x224c parp:0xe8d0 size:0x008e C-string:'UNK_0xe8d0'
// 1937:      UNK_0xe960  codep:0x224c parp:0xe960 size:0x002a C-string:'UNK_0xe960'
// 1938:      UNK_0xe98c  codep:0x224c parp:0xe98c size:0x00c4 C-string:'UNK_0xe98c'
// 1939:      UNK_0xea52  codep:0x224c parp:0xea52 size:0x001c C-string:'UNK_0xea52'
// 1940:      UNK_0xea70  codep:0x224c parp:0xea70 size:0x0029 C-string:'UNK_0xea70'
// 1941:      UNK_0xea9b  codep:0x224c parp:0xea9b size:0x0284 C-string:'UNK_0xea9b'
// 1942:      UNK_0xed21  codep:0x224c parp:0xed21 size:0x003e C-string:'UNK_0xed21'
// 1943:      UNK_0xed61  codep:0x224c parp:0xed61 size:0x0032 C-string:'UNK_0xed61'
// 1944:       1STOPTION  codep:0x4b3b parp:0xeda1 size:0x0018 C-string:'_1STOPTION'
// 1945:      UNK_0xedbb  codep:0x1d29 parp:0xedbb size:0x0002 C-string:'UNK_0xedbb'
// 1946:      UNK_0xedbf  codep:0x1d29 parp:0xedbf size:0x0002 C-string:'UNK_0xedbf'
// 1947:       DO-OPTION  codep:0x224c parp:0xedcf size:0x006b C-string:'DO_minus_OPTION'
// 1948:    ITEM-OPTIONS  codep:0x4b3b parp:0xee4b size:0x0014 C-string:'ITEM_minus_OPTIONS'
// 1949:      UNK_0xee61  codep:0x224c parp:0xee61 size:0x0051 C-string:'UNK_0xee61'
// 1950:      UNK_0xeeb4  codep:0x224c parp:0xeeb4 size:0x004f C-string:'UNK_0xeeb4'
// 1951:      UNK_0xef05  codep:0x150f parp:0xef05 size:0x0012 C-string:'UNK_0xef05'
// 1952:      UNK_0xef19  codep:0x920f parp:0xef19 size:0x003f C-string:'UNK_0xef19'
// 1953:      UNK_0xef5a  codep:0x224c parp:0xef5a size:0x0024 C-string:'UNK_0xef5a'
// 1954:        (/ITEMS)  codep:0x224c parp:0xef8b size:0x0094 C-string:'_ro__slash_ITEMS_rc_'
// 1955:        >DESCEND  codep:0x224c parp:0xf02c size:0x0031 C-string:'_gt_DESCEND'
// 1956:      UNK_0xf05f  codep:0x224c parp:0xf05f size:0x00ac C-string:'UNK_0xf05f'
// 1957:      UNK_0xf10d  codep:0x224c parp:0xf10d size:0x001b C-string:'UNK_0xf10d'
// 1958:      UNK_0xf12a  codep:0x224c parp:0xf12a size:0x0010 C-string:'UNK_0xf12a'
// 1959:      UNK_0xf13c  codep:0x2045 parp:0xf13c size:0x0008 C-string:'UNK_0xf13c'
// 1960:      UNK_0xf146  codep:0x444e parp:0xf146 size:0x0021 C-string:'UNK_0xf146'
// 1961:  (/LAUNCH/LAND)  codep:0x224c parp:0xf17a size:0x0044 C-string:'_ro__slash_LAUNCH_slash_LAND_rc_'
// 1962:      UNK_0xf1c0  codep:0x1d29 parp:0xf1c0 size:0x0002 C-string:'UNK_0xf1c0'
// 1963:      UNK_0xf1c4  codep:0x224c parp:0xf1c4 size:0x0006 C-string:'UNK_0xf1c4'
// 1964:      UNK_0xf1cc  codep:0x224c parp:0xf1cc size:0x0010 C-string:'UNK_0xf1cc'
// 1965:      UNK_0xf1de  codep:0x224c parp:0xf1de size:0x0010 C-string:'UNK_0xf1de'
// 1966:      UNK_0xf1f0  codep:0x224c parp:0xf1f0 size:0x0020 C-string:'UNK_0xf1f0'
// 1967:      UNK_0xf212  codep:0x224c parp:0xf212 size:0x001a C-string:'UNK_0xf212'
// 1968:      UNK_0xf22e  codep:0x224c parp:0xf22e size:0x001c C-string:'UNK_0xf22e'
// 1969:      UNK_0xf24c  codep:0x224c parp:0xf24c size:0x0018 C-string:'UNK_0xf24c'
// 1970:      UNK_0xf266  codep:0x224c parp:0xf266 size:0x000a C-string:'UNK_0xf266'
// 1971:      UNK_0xf272  codep:0x224c parp:0xf272 size:0x0016 C-string:'UNK_0xf272'
// 1972:      UNK_0xf28a  codep:0x224c parp:0xf28a size:0x0039 C-string:'UNK_0xf28a'
// 1973:      UNK_0xf2c5  codep:0x224c parp:0xf2c5 size:0x003c C-string:'UNK_0xf2c5'
// 1974:      UNK_0xf303  codep:0x224c parp:0xf303 size:0x004e C-string:'UNK_0xf303'
// 1975:      UNK_0xf353  codep:0x224c parp:0xf353 size:0x0030 C-string:'UNK_0xf353'
// 1976:      UNK_0xf385  codep:0x224c parp:0xf385 size:0x0034 C-string:'UNK_0xf385'
// 1977:      UNK_0xf3bb  codep:0x224c parp:0xf3bb size:0x001f C-string:'UNK_0xf3bb'
// 1978:      UNK_0xf3dc  codep:0x224c parp:0xf3dc size:0x0065 C-string:'UNK_0xf3dc'
// 1979:     (/LOG-PLAN)  codep:0x224c parp:0xf451 size:0x00af C-string:'_ro__slash_LOG_minus_PLAN_rc_'
// 1980:      UNK_0xf502  codep:0x204e parp:0xf502 size:0x005e C-string:'UNK_0xf502'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xe436[2] = {0x00, 0x00}; // UNK_0xe436
unsigned char UNK_0xe43a[8] = {0x00, 0x00, 0x29, 0x1d, 0x56, 0x3a, 0x20, 0x58}; // UNK_0xe43a
unsigned char UNK_0xe444[2] = {0x00, 0x00}; // UNK_0xe444
unsigned char UNK_0xe448[2] = {0x00, 0x00}; // UNK_0xe448
unsigned char UNK_0xe542[14] = {0x3a, 0x20, 0x4c, 0x22, 0x16, 0x9c, 0xb8, 0x3b, 0x46, 0x5a, 0x23, 0x6d, 0x90, 0x16}; // UNK_0xe542
unsigned char UNK_0xedbb[2] = {0x3a, 0x20}; // UNK_0xedbb
unsigned char UNK_0xedbf[2] = {0x3a, 0x20}; // UNK_0xedbf
unsigned char UNK_0xf1c0[2] = {0x3a, 0x20}; // UNK_0xf1c0

const unsigned short int cc_SB = 0x0168; // SB


// 0xe432: db 0x12 0x01 '  '

// ================================================
// 0xe434: WORD 'UNK_0xe436' codep=0x1d29 parp=0xe436
// ================================================
// 0xe436: db 0x00 0x00 '  '

// ================================================
// 0xe438: WORD 'UNK_0xe43a' codep=0x1d29 parp=0xe43a
// ================================================
// 0xe43a: db 0x00 0x00 0x29 0x1d 0x56 0x3a 0x20 0x58 '  ) V: X'

// ================================================
// 0xe442: WORD 'UNK_0xe444' codep=0x1d29 parp=0xe444
// ================================================
// 0xe444: db 0x00 0x00 '  '

// ================================================
// 0xe446: WORD 'UNK_0xe448' codep=0x1d29 parp=0xe448
// ================================================
// 0xe448: db 0x00 0x00 '  '

// ================================================
// 0xe44a: WORD 'UNK_0xe44c' codep=0x224c parp=0xe44c
// ================================================

void UNK_0xe44c() // UNK_0xe44c
{
  Push(pp_UNK_0xe448); // UNK_0xe448
  ON(); // ON
}

// 0xe452: db 0x4c 0x22 0x5d 0x17 0x6b 0xb7 0xd9 0x84 0x90 0x16 'L"] k     '

// ================================================
// 0xe45c: WORD 'UNK_0xe45e' codep=0x7394 parp=0xe45e
// ================================================
// 0xe45e: db 0x20 0x16 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe464: WORD 'UNK_0xe466' codep=0x224c parp=0xe466
// ================================================

void UNK_0xe466() // UNK_0xe466
{
  UNK_0xe45e(); // UNK_0xe45e
  C_at_(); // C@
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xe470: WORD 'UNK_0xe472' codep=0x224c parp=0xe472
// ================================================

void UNK_0xe472() // UNK_0xe472
{
  Push(1); // 1
  Push(pp_WBOTTOM); // WBOTTOM
  _ex_(); // !
}


// ================================================
// 0xe47a: WORD 'UNK_0xe47c' codep=0x224c parp=0xe47c
// ================================================

void UNK_0xe47c() // UNK_0xe47c
{
  Push(pp_ETIME); // ETIME
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1188;

  CTINIT(); // CTINIT
  UNK_0x3f09(); // UNK_0x3f09
  UNK_0x4410(); // UNK_0x4410
  Push(pp_FONT_n_); // FONT#
  UNK_0x4550(); // UNK_0x4550
  UNK_0x5222(); // UNK_0x5222
  DOS_minus_DTA(); // DOS-DTA
  UNK_0x4c43(); // UNK_0x4c43
  UNK_0x454e(); // UNK_0x454e
  UNK_0xf146(); // UNK_0xf146
  UNK_0x5da1(); // UNK_0x5da1
  UNK_0xe819(); // UNK_0xe819
  UNK_0xef05(); // UNK_0xef05
  UNK_0x8a2c(); // UNK_0x8a2c
  UNK_0x6565(); // UNK_0x6565
  UNK_0x906f(); // UNK_0x906f

  label1188:
}

// 0xe4a5: db 0x94 0x73 0x1c 0x18 0x01 0x20 0xcd 0x6b 0x94 0x73 0x1c 0x1d 0x01 0x20 0xcd 0x6b 0x94 0x73 0x39 0x00 0xbe 0x00 0x57 0x6e 0x20 0x74 0x14 0x45 0x04 0x94 0x73 0x20 0x07 0x02 0x17 0x49 0x6c ' s     k s     k s9   Wn t E  s    Il'

// ================================================
// 0xe4ca: WORD 'UNK_0xe4cc' codep=0x7394 parp=0xe4cc
// ================================================
// 0xe4cc: db 0x20 0x02 0x02 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe4d2: WORD 'UNK_0xe4d4' codep=0x7394 parp=0xe4d4
// ================================================
// 0xe4d4: db 0x20 0x11 0x02 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe4da: WORD 'UNK_0xe4dc' codep=0x7394 parp=0xe4dc
// ================================================
// 0xe4dc: db 0x20 0x04 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe4e2: WORD 'UNK_0xe4e4' codep=0x7394 parp=0xe4e4
// ================================================
// 0xe4e4: db 0x20 0x09 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe4ea: WORD 'UNK_0xe4ec' codep=0x7420 parp=0xe4ec
// ================================================
// 0xe4ec: db 0x19 0x12 0x07 '   '

// ================================================
// 0xe4ef: WORD 'UNK_0xe4f1' codep=0x7394 parp=0xe4f1
// ================================================
// 0xe4f1: db 0x20 0x0f 0x02 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe4f7: WORD 'UNK_0xe4f9' codep=0x7394 parp=0xe4f9
// ================================================
// 0xe4f9: db 0x20 0x10 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xe4ff: WORD 'UNK_0xe501' codep=0x7420 parp=0xe501
// ================================================
// 0xe501: db 0x24 0x0a 0x01 '$  '

// ================================================
// 0xe504: WORD 'UNK_0xe506' codep=0x7420 parp=0xe506
// ================================================
// 0xe506: db 0x24 0x0b 0x02 '$  '

// ================================================
// 0xe509: WORD 'UNK_0xe50b' codep=0x7420 parp=0xe50b
// ================================================
// 0xe50b: db 0x24 0x11 0x02 '$  '

// ================================================
// 0xe50e: WORD 'UNK_0xe510' codep=0x7420 parp=0xe510
// ================================================
// 0xe510: db 0x24 0x13 0x01 '$  '

// ================================================
// 0xe513: WORD 'UNK_0xe515' codep=0x7420 parp=0xe515
// ================================================
// 0xe515: db 0x24 0x14 0x01 '$  '

// ================================================
// 0xe518: WORD 'UNK_0xe51a' codep=0x7420 parp=0xe51a
// ================================================
// 0xe51a: db 0x24 0x15 0x0f 0x20 0x74 0x14 0x13 0x02 0x20 0x74 0x00 0x13 0x02 '$   t    t   '

// ================================================
// 0xe527: WORD 'UNK_0xe529' codep=0x7420 parp=0xe529
// ================================================
// 0xe529: db 0x17 0x12 0x01 0x20 0x74 0x00 0x15 0x01 0x20 0x74 0x00 0x16 0x01 0x20 0x74 0x00 0x17 0x01 0x20 0x74 0x14 0x43 0x02 '    t    t    t    t C '

// ================================================
// 0xe540: WORD 'UNK_0xe542' codep=0x1d29 parp=0xe542
// ================================================
// 0xe542: db 0x3a 0x20 0x4c 0x22 0x16 0x9c 0xb8 0x3b 0x46 0x5a 0x23 0x6d 0x90 0x16 ': L"   ;FZ#m  '

// ================================================
// 0xe550: WORD 'UNK_0xe552' codep=0x224c parp=0xe552
// ================================================

void UNK_0xe552() // UNK_0xe552
{
  Push(0x003a);
  Push(pp_FILE_n_); // FILE#
  _ex_(); // !
  Push(1); // 1
  Push(pp_RECORD_n_); // RECORD#
  _ex_(); // !
}

// 0xe562: db 0x4c 0x22 0xf2 0x0e 0x8a 0x6d 0xa0 0x79 0x90 0x16 'L"   m y  '

// ================================================
// 0xe56c: WORD 'UNK_0xe56e' codep=0x224c parp=0xe56e
// ================================================

void UNK_0xe56e() // UNK_0xe56e
{
  Push(0xc285);
  MODULE(); // MODULE
  Push(cc__7); // 7
  D_dot_R(); // D.R
}


// ================================================
// 0xe57a: WORD '.BAL' codep=0x224c parp=0xe583
// ================================================

void _dot_BAL() // .BAL
{
  CTINIT(); // CTINIT
  CTERASE(); // CTERASE
  Push(pp_WLEFT); // WLEFT
  Push(Read16(Pop())); // @
  Push(Pop()+3); // 3+
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(Pop()-2); // 2-
  POS_dot_(); // POS.
  PRINT("YOUR ACCOUNT BALANCE IS: ", 25); // (.")
  UNK_0xe56e(); // UNK_0xe56e
  PRINT(" SP", 3); // (.")
  Push(0x2710); Pust(0x0000);
  Push(0xbe96);
  MODULE(); // MODULE
  CTERASE(); // CTERASE
}


// ================================================
// 0xe5c9: WORD 'UNK_0xe5cb' codep=0x224c parp=0xe5cb
// ================================================

void UNK_0xe5cb() // UNK_0xe5cb
{
  Push(pp__ro_THIS_minus_I); // (THIS-I
  _1_dot_5_at_(); // 1.5@
}

// 0xe5d1: db 0x4c 0x22 0xc9 0xe5 0x1f 0x7a 0xb1 0x7a 0x8b 0x75 0x0f 0x65 0x32 0x6d 0xdf 0x79 0x90 0x16 0x4c 0x22 0xc9 0xe5 0x1f 0x7a 0xc9 0x7a 0x8b 0x75 0x0f 0x65 0x32 0x6d 0xdf 0x79 0x90 0x16 'L"   z z u e2m y  L"   z z u e2m y  '

// ================================================
// 0xe5f5: WORD 'UNK_0xe5f7' codep=0x224c parp=0xe5f7
// ================================================

void UNK_0xe5f7() // UNK_0xe5f7
{
  UNK_0xe5cb(); // UNK_0xe5cb
  _gt_C_plus_S(); // >C+S
  TEXT_minus_IN(); // TEXT-IN
  _1_dot_5_at_(); // 1.5@
  ICLOSE(); // ICLOSE
  _gt_C_plus_S(); // >C+S
}


// ================================================
// 0xe605: WORD 'UNK_0xe607' codep=0x224c parp=0xe607
// ================================================

void UNK_0xe607() // UNK_0xe607
{
  _gt_C_plus_S(); // >C+S
  UNK_0xb62c(); // UNK_0xb62c
  _gt_C_plus_S(); // >C+S
  UNK_0xb8dd(); // UNK_0xb8dd
  ICLOSE(); // ICLOSE
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xe615: WORD 'UNK_0xe617' codep=0x224c parp=0xe617
// ================================================

void UNK_0xe617() // UNK_0xe617
{
  _gt_R(); // >R
  _at_CRS(); // @CRS
  Push(cc__3); // 3
  R_gt_(); // R>
  POS_dot_(); // POS.
  CURSORS(); // CURSORS
  Push(pp_ABLT); // ABLT
  _ex_(); // !
  Push(cc__7); // 7
  Push(pp_LBLT); // LBLT
  _ex_(); // !
  Push(0x009a);
  Push(pp_WBLT); // WBLT
  _ex_(); // !
  WHITE(); // WHITE
  GREY1(); // GREY1
  DK_minus_GREE(); // DK-GREE
  _ask_MRC(); // ?MRC
  _ex_COLOR(); // !COLOR
  Push(pp_XORMODE); // XORMODE
  ON(); // ON
  BLT(); // BLT
  _ex_CRS(); // !CRS
}


// ================================================
// 0xe649: WORD 'UNK_0xe64b' codep=0x224c parp=0xe64b
// ================================================

void UNK_0xe64b() // UNK_0xe64b
{
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  UNK_0xe617(); // UNK_0xe617
}


// ================================================
// 0xe655: WORD 'UNK_0xe657' codep=0x224c parp=0xe657
// ================================================

void UNK_0xe657() // UNK_0xe657
{
  CTINIT(); // CTINIT
  Push(pp_WLINES); // WLINES
  Push(Read16(Pop())); // @
  Push(0); // 0

  do // (DO)
  {
  Push(pp_WLEFT); // WLEFT
  Push(Read16(Pop())); // @
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  I(); // I
  Push(cc__7); // 7
  Push(Pop() * Pop()); // *
  _minus_(); // -
  POS_dot_(); // POS.
  TEXT_gt_PA(); // TEXT>PA
  PAD(); // PAD
  _do__dot_(); // $.
  INEXT(); // INEXT

  } while(...); // (LOOP) 0xffe4
}


// ================================================
// 0xe681: WORD 'UNK_0xe683' codep=0x224c parp=0xe683
// ================================================

void UNK_0xe683() // UNK_0xe683
{
  Push(pp__ro_SCROLL); // (SCROLL
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN
  CI(); // CI
  Push(pp__ro_THIS_minus_I); // (THIS-I
  _1_dot_5_ex_(); // 1.5!
  Push(0); // 0

  label1141:
  Push(Pop()+1); // 1+
  INEXT(); // INEXT
  _ask_FIRST(); // ?FIRST
  if (Pop() == 0) goto label1141;

  _gt_R(); // >R
  _gt_1FONT(); // >1FONT
  Push(0x002b);
  Push(cc__3); // 3
  Push(cc__5); // 5
  R_gt_(); // R>
  MIN(); // MIN
  Push(0x0026);
  WINDOW(); // WINDOW
  UNK_0xe657(); // UNK_0xe657
  IPREV(); // IPREV
  UNK_0xe64b(); // UNK_0xe64b
}

// 0xe6b9: db 0x4c 0x22 0xb1 0x7a 0xd1 0xe5 0x84 0xaa 0xdb 0x23 0x49 0xe6 0xfc 0x9e 0x49 0xe6 0x90 0x16 0x4c 0x22 0xc9 0x7a 0xe3 0xe5 0xc9 0xe5 0x1f 0x7a 0x84 0xaa 0xdf 0x79 0xdb 0x23 0x49 0xe6 0x0e 0x9f 0x49 0xe6 0x90 0x16 'L" z     #I   I   L" z     z   y #I   I   '

// ================================================
// 0xe6e3: WORD 'TD-SCROLL' codep=0x4b3b parp=0xe6f1
// ================================================
// 0xe6f1: db  '  H:        L"] 8      zW 2mf 2mu ] &    mu    m u y.  ^#m  L" ? ELEMENTS<  J l   c/z?z u { u y8 Wm  L" l z?z]   ]   L~2  u< 2m u yf /z"u]   _ 4u]   _      y     z`     L"4u   c/z?z  f /z4uP _ "u]   _    t8        @    >  yA      u e2m zA  z>       2  u y  L"   z      . 8      '
// 0xe807: adc    ax,0004
// 0xe80a: sub    ax,D0E7
// 0xe80d: db     63
// 0xe80e: das    
// 0xe80f: jp     E850
// 0xe811: jp     E869
// 0xe813: out    8B,ax
// 0xe815: jnz    E7BE

// ================================================
// 0xe817: WORD 'UNK_0xe819' codep=0xdf75 parp=0xe819
// ================================================
// 0xe819: db 0x79 0x88 0xb9 0xdb 0x23 0xcd 0x0c 0x8b 0x75 0x3c 0xe4 0x83 0x4a 0xfd 0xe6 0x06 0x0e 0x17 0x7c 0xdf 0x79 0x90 0x16 'y   #   u<  J     | y  '

// ================================================
// 0xe830: WORD 'UNK_0xe832' codep=0x224c parp=0xe832
// ================================================

void UNK_0xe832() // UNK_0xe832
{
  _star_SHIP(); // *SHIP
  _gt_C_plus_S(); // >C+S
  INST_minus_X(); // INST-X
  _2_at_(); // 2@
  SWAP(); // SWAP
  ICLOSE(); // ICLOSE
  Push(pp_ILOCAL); // ILOCAL
  Push(Read16(Pop())); // @
  _ask_ICONS_minus_(); // ?ICONS-
}


// ================================================
// 0xe846: WORD 'UNK_0xe848' codep=0x224c parp=0xe848
// ================================================

void UNK_0xe848() // UNK_0xe848
{
  _at_ID(); // @ID
  Push(0x002b);
  _eq_(); // =
  if (Pop() == 0) goto label1131;

  _at_IL(); // @IL
  _at_IH(); // @IH
  _gt_C_plus_S(); // >C+S
  UNK_0xe4ec(); // UNK_0xe4ec
  Push(cc__3); // 3
  Push(Pop() + Pop()); // +
  _2_at_(); // 2@
  D0_eq_(); // D0=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  ICLOSE(); // ICLOSE
  goto label1132;

  label1131:
  Push(0); // 0

  label1132:
}


// ================================================
// 0xe870: WORD 'UNK_0xe872' codep=0x224c parp=0xe872
// ================================================

void UNK_0xe872() // UNK_0xe872
{
  Push(pp_UNK_0xe436); // UNK_0xe436
  _099(); // 099
  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  Push(cc__4); // 4
  _eq_(); // =
  if (Pop() == 0) goto label1130;

  UNK_0xe832(); // UNK_0xe832
  Push(0); // 0

  do // (DO)
  {
  POINT_gt_I(); // POINT>I
  UNK_0xe848(); // UNK_0xe848
  Push(pp_UNK_0xe436); // UNK_0xe436
  _plus__ex_(); // +!

  } while(...); // (LOOP) 0xfff6

  label1130:
  Push(pp_UNK_0xe436); // UNK_0xe436
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(pp_UNK_0xe436); // UNK_0xe436
  _ex_(); // !
}


// ================================================
// 0xe8a2: WORD 'UNK_0xe8a4' codep=0x224c parp=0xe8a4
// ================================================

void UNK_0xe8a4() // UNK_0xe8a4
{
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x0030);
  IFIND(); // IFIND
  if (Pop() == 0) goto label1160;

  IOPEN(); // IOPEN

  label1161:
  Push(0xffff); Pust(0xffff);
  INST_minus_X(); // INST-X
  D_ex_(); // D!
  INEXT(); // INEXT
  _ask_FIRST(); // ?FIRST
  if (Pop() == 0) goto label1161;

  CDROP(); // CDROP

  label1160:
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xe8ce: WORD 'UNK_0xe8d0' codep=0x224c parp=0xe8d0
// ================================================

void UNK_0xe8d0() // UNK_0xe8d0
{
  UNK_0xe832(); // UNK_0xe832
  Push(0); // 0

  do // (DO)
  {
  POINT_gt_I(); // POINT>I
  _at_ID(); // @ID
  Push(0x002b);
  _eq_(); // =
  if (Pop() == 0) goto label1135;

  Push(0x00ff);
  INST_minus_QT(); // INST-QT
  Push(Read16(Pop())); // @
  MIN(); // MIN
  DUP(); // DUP
  Push(pp_ELEM_minus_AM); // ELEM-AM
  _ex_(); // !
  Push(-Pop()); // NEGATE
  INST_minus_QT(); // INST-QT
  _plus__ex_(); // +!
  _at_INST_minus_S(); // @INST-S
  DUP(); // DUP
  Push(pp_UNK_0xe542); // UNK_0xe542
  _ex_(); // !
  Push(cc__6); // 6
  _eq_(); // =
  _at_IL(); // @IL
  _at_IH(); // @IH
  _gt_C_plus_S(); // >C+S
  if (Pop() == 0) goto label1136;

  Push(pp_ELEM_minus_AM); // ELEM-AM
  Push(Read16(Pop())); // @
  UNK_0xe4ec(); // UNK_0xe4ec
  Push(cc__3); // 3
  Push(Pop() + Pop()); // +
  C_ex_(); // C!
  goto label1138;

  label1136:
  Push(cc__3); // 3
  Push(0); // 0

  do // (DO)
  {
  UNK_0xe4ec(); // UNK_0xe4ec
  I(); // I
  Push(Pop() + Pop()); // +
  C_at_(); // C@
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1137;

  Push(pp_UNK_0xe542); // UNK_0xe542
  Push(Read16(Pop())); // @
  UNK_0xe4ec(); // UNK_0xe4ec
  I(); // I
  Push(Pop() + Pop()); // +
  C_ex_(); // C!
  Push(pp_ELEM_minus_AM); // ELEM-AM
  Push(Read16(Pop())); // @
  UNK_0xe4ec(); // UNK_0xe4ec
  Push(cc__4); // 4
  Push(Pop() + Pop()); // +
  I(); // I
  Push(Pop() + Pop()); // +
  C_ex_(); // C!
  LEAVE(); // LEAVE

  label1137:

  } while(...); // (LOOP) 0xffd2

  label1138:
  ICLOSE(); // ICLOSE

  label1135:

  } while(...); // (LOOP) 0xff7c
}


// ================================================
// 0xe95e: WORD 'UNK_0xe960' codep=0x224c parp=0xe960
// ================================================

void UNK_0xe960() // UNK_0xe960
{
  _star_STARSH(); // *STARSH
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x0030);
  IFIND(); // IFIND
  if (Pop() == 0) goto label1134;

  IOPEN(); // IOPEN

  label1139:
  UNK_0xe8d0(); // UNK_0xe8d0
  INEXT(); // INEXT
  _ask_FIRST(); // ?FIRST
  if (Pop() == 0) goto label1139;

  ICLOSE(); // ICLOSE
  IDELETE(); // IDELETE

  label1134:
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xe98a: WORD 'UNK_0xe98c' codep=0x224c parp=0xe98c
// ================================================

void UNK_0xe98c() // UNK_0xe98c
{
  _star_STARSH(); // *STARSH
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x000a);
  IFIND(); // IFIND
  if (Pop() == 0) goto label1156;

  Push(0x0031);
  _ex_INST_minus_S(); // !INST-S

  label1156:
  ICLOSE(); // ICLOSE
  UNK_0xe832(); // UNK_0xe832
  Push(0); // 0

  do // (DO)
  {
  POINT_gt_I(); // POINT>I
  UNK_0xe848(); // UNK_0xe848
  if (Pop() == 0) goto label1157;

  _at_IL(); // @IL
  _at_IH(); // @IH
  _gt_C_plus_S(); // >C+S
  Push(cc__4); // 4
  Push(0); // 0

  do // (DO)
  {
  UNK_0xe4ec(); // UNK_0xe4ec
  Push(cc__3); // 3
  Push(Pop() + Pop()); // +
  I(); // I
  Push(Pop() + Pop()); // +
  C_at_(); // C@
  _ask_DUP(); // ?DUP
  if (Pop() == 0) goto label1162;

  Push(0x001a);
  I(); // I
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1163;

  Push(cc__6); // 6
  goto label1164;

  label1163:
  UNK_0xe4ec(); // UNK_0xe4ec
  I(); // I
  Push(Pop()-1); // 1-
  Push(Pop() + Pop()); // +
  C_at_(); // C@

  label1164:
  ICREATE(); // ICREATE
  _gt_C_plus_S(); // >C+S
  INST_minus_QT(); // INST-QT
  _ex_(); // !
  CI(); // CI
  ICLOSE(); // ICLOSE
  _star_STARSH(); // *STARSH
  _gt_C_plus_S(); // >C+S
  UNK_0xb93e(); // UNK_0xb93e
  ICLOSE(); // ICLOSE

  label1162:

  } while(...); // (LOOP) 0xffbc
  UNK_0xe4ec(); // UNK_0xe4ec
  Push(cc__7); // 7
  Push(0); // 0
  FILL(); // FILL
  ICLOSE(); // ICLOSE

  label1157:

  } while(...); // (LOOP) 0xff9a
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x000a);
  IFIND(); // IFIND
  Push(pp_UNK_0xe436); // UNK_0xe436
  Push(Read16(Pop())); // @
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1158;

  Push(0x0030);
  _ex_INST_minus_S(); // !INST-S

  label1158:
  IFIRST(); // IFIRST
  Push(0x000b);
  Push(0x0031);
  IFIND(); // IFIND
  if (Pop() == 0) goto label1159;

  Push(0x000a);
  _ex_INST_minus_S(); // !INST-S

  label1159:
  ICLOSE(); // ICLOSE
  UNK_0xe8a4(); // UNK_0xe8a4
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xea50: WORD 'UNK_0xea52' codep=0x224c parp=0xea52
// ================================================

void UNK_0xea52() // UNK_0xea52
{
  _gt_R(); // >R
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(cc__6); // 6
  _minus_(); // -
  Push(pp_WLEFT); // WLEFT
  Push(Read16(Pop())); // @
  Push(1); // 1
  Push(pp_WRIGHT); // WRIGHT
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  R_gt_(); // R>
  POLY_minus_WI(); // POLY-WI
}


// ================================================
// 0xea6e: WORD 'UNK_0xea70' codep=0x224c parp=0xea70
// ================================================

void UNK_0xea70() // UNK_0xea70
{
  BLUE(); // BLUE
  _ex_COLOR(); // !COLOR
  Push(cc__5); // 5
  Push(cc__6); // 6
  CTPOS_dot_(); // CTPOS.
  PRINT("SCROLL ITEMS: ^\  QUIT: []", 26); // (.")
}


// ================================================
// 0xea99: WORD 'UNK_0xea9b' codep=0x224c parp=0xea9b
// ================================================

void UNK_0xea9b() // UNK_0xea9b
{
  _gt_1FONT(); // >1FONT
  _gt_SSCT(); // >SSCT
  CTERASE(); // CTERASE
  UNK_0xea70(); // UNK_0xea70
  Push(0xfff9);
  Push(pp_WTOP); // WTOP
  _plus__ex_(); // +!
  Push(cc__4); // 4
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(cc__7); // 7
  Push(Pop() + Pop()); // +
  POS_dot_(); // POS.
  PRINT("ITEM", 4); // (.")
  Push(0x006c);
  Push(pp_XBLT); // XBLT
  _ex_(); // !
  PRINT("VOLUME", 6); // (.")
  Push(0x0088);
  Push(pp_XBLT); // XBLT
  _ex_(); // !
  PRINT("VALUE", 5); // (.")
}

// 0xeae1: db  'L"   t  ]   `  y   zu ]   r  ;] .  m  u ]   r       m  2   u ]   r  m y  L"   t   y       c J z   z  ; ^#m u y  L"]      P     ;     PRESS ANY KEY TO CONTINUE *8   `   e  *8  ? @  >         L"]     YU  A   A  t#m.S z; #m yA ]      b8 e  bLm]   4u   z t m u y l z  <  yYU        +     `     ]         L" t/z '
// 0xec16: pop    si
// 0xec17: db     65
// 0xec18: db     6D
// 0xec19: mov    si,CDA9
// 0xec1c: or     al,7F
// 0xec1e: mov    bx,79DF
// 0xec21: mul    word ptr [bp-33]
// 0xec24: or     al,5D
// 0xec26: pop    ss
// 0xec27: db     6F
// 0xec28: mov    di,84D9
// 0xec2b: push   ss
// 0xec2c: push   ss
// 0xec2d: and    [bp+00],cl
// 0xec30: add    [di+17],bl
// 0xec33: xchg   ax,si
// 0xec34: mov    si,84D9
// 0xec37: nop    
// 0xec38: push   ss
// 0xec39: dec    sp
// 0xec3a: and    bl,[si+5D]
// 0xec3d: scasb
// 0xec3e: or     dx,[bx-6DC5]
// 0xec42: db     0F
// 0xec43: adc    ax,9FE6
// 0xec46: cmp    sp,[di+6C]
// 0xec49: and    bp,[di-48]
// 0xec4c: cmp    bp,[bp+si+5D]
// 0xec4f: and    bp,[di+46]
// 0xec52: pushf  
// 0xec53: push   bx
// 0xec54: push   ax
// 0xec55: dec    cx
// 0xec56: pop    cx
// 0xec57: push   ss
// 0xec58: db     6D
// 0xec59: db     0F
// 0xec5a: in     al,dx
// 0xec5b: sub    [bp-06],si
// 0xec5e: adc    ax,0010
// 0xec61: aas    
// 0xec62: jp     EC73
// 0xec64: in     al,dx
// 0xec65: mov    cl,7A
// 0xec67: lea    di,[bp+si-06]
// 0xec6a: adc    ax,FFF8
// 0xec6d: esc    7,[bx+di+5D]
// 0xec70: pop    ss
// 0xec71: stc    
// 0xec72: jmp    word ptr [di+6C]
// 0xec75: and    bp,[di+2E]
// 0xec78: db     0F
// 0xec79: db     6A
// 0xec7a: pop    bp
// 0xec7b: and    bp,[di-70]
// 0xec7e: push   ss
// 0xec7f: dec    sp
// 0xec80: and    bl,[di+17]
// 0xec83: loopne EC86
// 0xec85: xchg   ax,dx
// 0xec86: db     0F
// 0xec87: pop    bp
// 0xec88: pop    ss
// 0xec89: or     al,[bx+si]
// 0xec8b: pop    bp
// 0xec8c: pop    ss
// 0xec8d: xor    ax,C400
// 0xec90: db     0F
// 0xec91: repnz  
// 0xec92: push   cs
// 0xec93: pop    bp
// 0xec94: pop    ss
// 0xec95: add    byte ptr [si],92
// 0xec98: db     0F
// 0xec99: pop    bp
// 0xec9a: pop    ss
// 0xec9b: or     al,[bx+si]
// 0xec9d: pop    bp
// 0xec9e: pop    ss
// 0xec9f: inc    ax
// 0xeca0: add    ah,al
// 0xeca2: db     0F
// 0xeca3: repnz  
// 0xeca4: push   cs
// 0xeca5: nop    
// 0xeca6: push   ss
// 0xeca7: dec    sp
// 0xeca8: and    cl,al
// 0xecaa: or     ax,15FA
// 0xecad: sbb    [bx+si],al
// 0xecaf: js     ECC3
// 0xecb1: cli    
// 0xecb2: adc    ax,000A
// 0xecb5: esc    4,[bp+di]
// 0xecb7: add    [bx+60],dx
// 0xecba: push   ss
// 0xecbb: push   es
// 0xecbc: add    ah,bl
// 0xecbe: sbb    ax,[bx+di]
// 0xecc0: inc    bp
// 0xecc1: db     60
// 0xecc2: push   ss
// 0xecc3: push   es
// 0xecc4: add    ah,bl
// 0xecc6: sbb    ax,[bx+di]
// 0xecc8: and    [bx+si+4C16],dl
// 0xeccc: and    cl,al
// 0xecce: or     ax,15FA
// 0xecd1: sbb    [bx+si],al
// 0xecd3: js     ECE7
// 0xecd5: cli    
// 0xecd6: adc    ax,000A
// 0xecd9: esc    4,[bp+di]
// 0xecdb: add    [bp+di+60],dx
// 0xecde: push   ss
// 0xecdf: push   es
// 0xece0: add    ah,bl
// 0xece2: sbb    ax,[bx+di]
// 0xece4: dec    si
// 0xece5: db     60
// 0xece6: push   ss
// 0xece7: push   es
// 0xece8: add    ah,bl
// 0xecea: sbb    ax,[bx+di]
// 0xecec: and    [bx+si+4C16],dl
// 0xecf0: and    bh,[bx-14]
// 0xecf3: inc    cx
// 0xecf4: push   cs
// 0xecf5: inc    cx
// 0xecf6: push   cs
// 0xecf7: js     ED0B
// 0xecf9: cli    
// 0xecfa: adc    ax,0004
// 0xecfd: mov    byte ptr [bx+di],20
// 0xed00: db     0F
// 0xed01: loope  ED2A
// 0xed03: retf   
// 0xed04: in     al,dx
// 0xed05: esc    4,[bp+di]
// 0xed07: add    sp,[bx+si]
// 0xed09: sub    ah,[bx+si]
// 0xed0b: inc    cx
// 0xed0c: push   cs
// 0xed0d: inc    cx
// 0xed0e: push   cs
// 0xed0f: js     ED23
// 0xed11: cli    
// 0xed12: adc    ax,0004
// 0xed15: mov    byte ptr [bx+di],20
// 0xed18: db     0F
// 0xed19: loope  ED42
// 0xed1b: cmpsw
// 0xed1c: in     al,dx
// 0xed1d: nop    
// 0xed1e: push   ss

// ================================================
// 0xed1f: WORD 'UNK_0xed21' codep=0x224c parp=0xed21
// ================================================

void UNK_0xed21() // UNK_0xed21
{
  Push(pp_UNK_0xe448); // UNK_0xe448
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1151;

  Push(1); // 1
  Push(cc__5); // 5
  CTPOS_dot_(); // CTPOS.
  WHITE(); // WHITE
  _ex_COLOR(); // !COLOR
  PRINT("REVISING...", 11); // (.")

  label1151:
  UNK_0xb919(); // UNK_0xb919
  UNK_0xb905(); // UNK_0xb905
  _star_STARSH(); // *STARSH
  UNK_0xb8f1(); // UNK_0xb8f1
  Push(1); // 1
  Push(cc__5); // 5
  CTPOS_dot_(); // CTPOS.
  Push(0x000b);
  BLACK(); // BLACK
  POLY_minus_ER(); // POLY-ER
  _star_STARSH(); // *STARSH
  UNK_0xe607(); // UNK_0xe607
  UNK_0xe683(); // UNK_0xe683
}


// ================================================
// 0xed5f: WORD 'UNK_0xed61' codep=0x224c parp=0xed61
// ================================================

void UNK_0xed61() // UNK_0xed61
{
  UNK_0xe44c(); // UNK_0xe44c
  Push(0xccf0);
  MODULE(); // MODULE
  Pop(); Pop();// 2DROP
  Push(0xb6ec);
  MODULE(); // MODULE
  UNK_0xc0f9(); // UNK_0xc0f9
}

// 0xed75: db 0x4c 0x22 0x4a 0xe4 0xbb 0x74 0x38 0x0c 0x16 0x16 0xff 0xff 0xff 0xff 0x07 0x40 0xfa 0x15 0x08 0x00 0xa1 0xeb 0x60 0x16 0x04 0x00 0x5f 0xed 0x90 0x16 'L"J  t8        @      `   _   '

// ================================================
// 0xed93: WORD '1STOPTION' codep=0x4b3b parp=0xeda1
// ================================================
// 0xeda1: db 0x05 0x00 0x61 0xed 0x1c 0x00 0x61 0xed 0x1b 0x00 0x3b 0xec 0x1a 0x00 0x77 0xed 0x44 0x00 0x61 0xed 0x09 0x00 0x61 0xed '  a   a   ;   w D a   a '

// ================================================
// 0xedb9: WORD 'UNK_0xedbb' codep=0x1d29 parp=0xedbb
// ================================================
// 0xedbb: db 0x3a 0x20 ': '

// ================================================
// 0xedbd: WORD 'UNK_0xedbf' codep=0x1d29 parp=0xedbf
// ================================================
// 0xedbf: db 0x3a 0x20 ': '

// ================================================
// 0xedc1: WORD 'DO-OPTION' codep=0x224c parp=0xedcf
// ================================================

void DO_minus_OPTION() // DO-OPTION
{
  Push(pp_UNK_0xedbb); // UNK_0xedbb
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1152;

  UNK_0xed61(); // UNK_0xed61
  goto label1153;

  label1152:
  _at_INST_minus_C(); // @INST-C
  _1STOPTION(); // 1STOPTION

  label1153:
  Push(1); // 1
}

// 0xede5: db 0x4c 0x22 0xdc 0x1b 0x09 0x20 0x4a 0x45 0x54 0x54 0x49 0x53 0x4f 0x4e 0x20 0x0f 0x90 0x16 0x4c 0x22 0xdc 0x1b 0x09 0x20 0x50 0x4c 0x41 0x59 0x42 0x41 'L"    JETTISON    L"    PLAYBA'
// 0xee03: inc    bx
// 0xee04: dec    bx
// 0xee05: sbb    al,9F
// 0xee07: esc    4,[bp+di]
// 0xee09: push   es
// 0xee0a: and    [di+52],al
// 0xee0d: inc    cx
// 0xee0e: push   bx
// 0xee0f: inc    bp
// 0xee10: cs:    
// 0xee11: db     0F
// 0xee12: nop    
// 0xee13: push   ss
// 0xee14: dec    sp
// 0xee15: and    bh,[bp+di+3874]
// 0xee19: or     al,16
// 0xee1b: push   ss
// 0xee1c: ???    di
// 0xee1e: ???    di
// 0xee20: pop    es
// 0xee21: inc    ax
// 0xee22: cli    
// 0xee23: adc    ax,0012
// 0xee26: esc    4,[bp+di]
// 0xee28: pop    es
// 0xee29: and    [bx+si+49],dl
// 0xee2c: inc    bx
// 0xee2d: dec    bx
// 0xee2e: push   bp
// 0xee2f: push   ax
// 0xee30: and    [bx],cl
// 0xee32: db     60
// 0xee33: push   ss
// 0xee34: add    al,00
// 0xee36: in     ax,ED
// 0xee38: nop    
// 0xee39: push   ss

// ================================================
// 0xee3a: WORD 'ITEM-OPTIONS' codep=0x4b3b parp=0xee4b
// ================================================
// 0xee4b: db 0x04 0x00 0xe7 0xed 0x1c 0x00 0xe7 0xed 0x1b 0x00 0xf9 0xed 0x44 0x00 0xe7 0xed 0x1a 0x00 0x16 0xee '            D       '

// ================================================
// 0xee5f: WORD 'UNK_0xee61' codep=0x224c parp=0xee61
// ================================================

void UNK_0xee61() // UNK_0xee61
{
  Push(pp_WLEFT); // WLEFT
  Push(Read16(Pop())); // @
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(cc__7); // 7
  _minus_(); // -
  POS_dot_(); // POS.
  CTINIT(); // CTINIT
  _at_INST_minus_C(); // @INST-C
  ITEM_minus_OPTIONS(); // ITEM-OPTIONS
  Push(pp_UNK_0xedbf); // UNK_0xedbf
  _ex_(); // !
  Push(pp_UNK_0xedbb); // UNK_0xedbb
  _099(); // 099
  BLUE(); // BLUE
  _ex_COLOR(); // !COLOR
  Push(cc__4); // 4
  Push(cc__6); // 6
  CTPOS_dot_(); // CTPOS.
  PRINT("^\ SELECT OPTION OR [ QUIT ]", 28); // (.")
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(cc__6); // 6
  _minus_(); // -
  UNK_0xe617(); // UNK_0xe617
}


// ================================================
// 0xeeb2: WORD 'UNK_0xeeb4' codep=0x224c parp=0xeeb4
// ================================================

void UNK_0xeeb4() // UNK_0xeeb4
{
  UNK_0xe5f7(); // UNK_0xe5f7
  _at_INST_minus_C(); // @INST-C
  ICLOSE(); // ICLOSE
  Push(0x000b);
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1145;

  UNK_0xe5f7(); // UNK_0xe5f7
  BLACK(); // BLACK
  UNK_0xe64b(); // UNK_0xe64b
  UNK_0xea52(); // UNK_0xea52
  UNK_0xee61(); // UNK_0xee61

  label1148:
  XYSCAN(); // XYSCAN
  Pop(); // DROP
  _ask_DUP(); // ?DUP
  if (Pop() == 0) goto label1146;

  Push(-Pop()); // NEGATE
  Push(pp_UNK_0xedbb); // UNK_0xedbb
  Push(Read16(Pop())); // @
  Push(Pop() + Pop()); // +
  DUP(); // DUP
  Push(0); // 0
  Push(pp_UNK_0xedbf); // UNK_0xedbf
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  WITHIN(); // WITHIN
  if (Pop() == 0) goto label1155;

  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  Push(pp_UNK_0xedbb); // UNK_0xedbb
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  Push(cc__7); // 7
  Push(Pop() * Pop()); // *
  _minus_(); // -
  Push(Pop()+1); // 1+

// ================================================
// 0xef03: WORD 'UNK_0xef05' codep=0x150f parp=0xef05
// ================================================
  UNK_0xe617(); // UNK_0xe617
  DUP(); // DUP
  Push(pp_UNK_0xedbb); // UNK_0xedbb
  _ex_(); // !
  Push(pp_WTOP); // WTOP
  Push(Read16(Pop())); // @
  SWAP(); // SWAP
  Push(Pop()+1); // 1+
  Push(cc__7); // 7
  Push(Pop() * Pop()); // *

// ================================================
// 0xef17: WORD 'UNK_0xef19' codep=0x920f parp=0xef19
// ================================================
  _minus_(); // -
  Push(Pop()+1); // 1+
  UNK_0xe617(); // UNK_0xe617
  goto label1146;

  label1155:
  Pop(); // DROP

  label1146:
  _ask_TRIG(); // ?TRIG
  if (Pop() == 0) goto label1147;

  DO_minus_OPTION(); // DO-OPTION
  goto label1154;

  label1147:
  Push(0); // 0

  label1154:
  Push(pp_FQUIT); // FQUIT
  Push(Read16(Pop())); // @
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1148;

  Push(cc__3); // 3
  _star_CLOSE(); // *CLOSE
  Push(pp_SCROLL_minus_); // SCROLL-
  Push(Read16(Pop())); // @
  Push(1); // 1
  _gt_(); // >
  if (Pop() == 0) goto label1149;

  UNK_0xea9b(); // UNK_0xea9b
  UNK_0xed21(); // UNK_0xed21

  label1149:
  goto label1150;

  label1145:
  BEEP(); // BEEP

  label1150:
}


// ================================================
// 0xef58: WORD 'UNK_0xef5a' codep=0x224c parp=0xef5a
// ================================================

void UNK_0xef5a() // UNK_0xef5a
{
  CTINIT(); // CTINIT
  PRINT("STARSHIP HOLD IS EMPTY.", 23); // (.")
  Push(0x07d0);
  MS(); // MS
}


// ================================================
// 0xef7e: WORD '(/ITEMS)' codep=0x224c parp=0xef8b
// ================================================

void _ro__slash_ITEMS_rc_() // (/ITEMS)
{
  _dot_BAL(); // .BAL
  CTERASE(); // CTERASE
  UNK_0xe872(); // UNK_0xe872
  Push(pp_UNK_0xe436); // UNK_0xe436
  Push(Read16(Pop())); // @
  DUP(); // DUP
  _star_STARSH(); // *STARSH
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x000a);
  IFIND(); // IFIND
  Push(pp_UNK_0xe43a); // UNK_0xe43a
  _ex_(); // !
  CI(); // CI
  Push(Pop() | Pop()); // OR
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1133;

  Push(0x002b);
  Push(cc__3); // 3
  Push(cc__7); // 7
  Push(0x0026);
  WINDOW(); // WINDOW
  UNK_0xe472(); // UNK_0xe472
  UNK_0xea9b(); // UNK_0xea9b
  UNK_0xb905(); // UNK_0xb905
  Push(pp_ESC_minus_EN); // ESC-EN
  _099(); // 099
  if (Pop() == 0) goto label1140;

  UNK_0xe98c(); // UNK_0xe98c

  label1140:
  _star_STARSH(); // *STARSH
  UNK_0xb8f1(); // UNK_0xb8f1
  _star_STARSH(); // *STARSH
  UNK_0xe607(); // UNK_0xe607
  UNK_0xe683(); // UNK_0xe683

  label1143:
  XYSCAN(); // XYSCAN
  Pop(); // DROP
  TD_minus_SCROLL(); // TD-SCROLL
  _ask_TRIG(); // ?TRIG
  if (Pop() == 0) goto label1142;

  UNK_0xeeb4(); // UNK_0xeeb4
  Push(pp_FQUIT); // FQUIT
  _099(); // 099

  label1142:
  _ask_QUIT(); // ?QUIT
  Push(pp_SCROLL_minus_); // SCROLL-
  Push(Read16(Pop())); // @
  Push(2); // 2
  _st_(); // <
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1143;

  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
  UNK_0xb919(); // UNK_0xb919
  Push(pp_ESC_minus_EN); // ESC-EN
  ON(); // ON
  UNK_0xe64b(); // UNK_0xe64b
  goto label1144;

  label1133:
  Pop(); // DROP
  UNK_0xef5a(); // UNK_0xef5a

  label1144:
  _gt_SSCT(); // >SSCT
  CTERASE(); // CTERASE
  UNK_0xe960(); // UNK_0xe960
}


// ================================================
// 0xf01f: WORD '>DESCEND' codep=0x224c parp=0xf02c
// ================================================

void _gt_DESCEND() // >DESCEND
{
  UNK_0xe552(); // UNK_0xe552
  UNK_0x3f09(); // UNK_0x3f09
  UNK_0x4c08(); // UNK_0x4c08
  UNK_0x5543(); // UNK_0x5543
  UNK_0x4350(); // UNK_0x4350
  UNK_0xb54a(); // UNK_0xb54a
  UNK_0x627a(); // UNK_0x627a
  UNK_0x2ae7(); // UNK_0x2ae7
  UNK_0x655e(); // UNK_0x655e
  UNK_0x5d6f(); // UNK_0x5d6f
  UNK_0x4819(); // UNK_0x4819
  UNK_0xf13c(); // UNK_0xf13c
  goto label1191;
  UNK_0x1f55(); // UNK_0x1f55
  UNK_0xbd7c(); // UNK_0xbd7c
  UNK_0x92e6(); // UNK_0x92e6
  UNK_0x5d0e(); // UNK_0x5d0e
  UNK_0xef19(); // UNK_0xef19
  UNK_0xf502(); // UNK_0xf502
  UNK_0xbd14(); // UNK_0xbd14
  UNK_0x0ae6(); // UNK_0x0ae6
  UNK_0xdf6f(); // UNK_0xdf6f
  UNK_0x907b(); // UNK_0x907b
  UNK_0x4c18(); // UNK_0x4c18

// ================================================
// 0xf05d: WORD 'UNK_0xf05f' codep=0x224c parp=0xf05f
// ================================================
  UNK_0xd224(); // UNK_0xd224

void UNK_0xf05f() // UNK_0xf05f
{
  CTINIT(); // CTINIT
  CTERASE(); // CTERASE
  PRINT("READY TO LAUNCH", 15); // (.")
  CTCR(); // CTCR
  PRINT("CONFIRM: [N Y]", 14); // (.")
  Y_slash_N(); // Y/N
  if (Pop() == 0) goto label1182;

  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(pp__ro_PLANET); // (PLANET
  _1_dot_5_at_(); // 1.5@
  D0_eq_(); // D0=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1183;

  Push(pp__i_LAUNCH); // 'LAUNCH
  Push(Read16(Pop())); // @
  MODULE(); // MODULE
  DUP(); // DUP
  if (Pop() == 0) goto label1186;

  UNK_0xe47c(); // UNK_0xe47c
  Push(0xbd8b);
  MODULE(); // MODULE
  Push(0xd272);
  MODULE(); // MODULE
  Push(0xbd67);
  MODULE(); // MODULE
  Push(0xbd9b);
  Push(pp__i_EXTERN); // 'EXTERN
  _ex_(); // !

  label1186:
  goto label1187;

  label1183:
  Push(0xd272);
  MODULE(); // MODULE
  Push(1); // 1

  label1187:
  if (Pop() == 0) goto label1184;

  UNK_0xe552(); // UNK_0xe552
  UNK_0x3f09(); // UNK_0x3f09
  UNK_0x4c08(); // UNK_0x4c08
  UNK_0x4e43(); // UNK_0x4e43
  UNK_0x2046(); // UNK_0x2046
  UNK_0xb522(); // UNK_0xb522
  UNK_0x627a(); // UNK_0x627a
  UNK_0x7ce7(); // UNK_0x7ce7

  label1184:
  Push(pp_STIME); // STIME
  _2_at_(); // 2@
  Push(0x03e8); Pust(0x0000);
  D_st_(); // D<
  Pop(); // DROP
  goto label1185;

  label1182:
  CTERASE(); // CTERASE
  PRINT("CANCELLED", 9); // (.")

  label1185:
}


// ================================================
// 0xf10b: WORD 'UNK_0xf10d' codep=0x224c parp=0xf10d
// ================================================

void UNK_0xf10d() // UNK_0xf10d
{
  CTINIT(); // CTINIT
  CTERASE(); // CTERASE
  PRINT("WE'RE NOT IN ORBIT", 18); // (.")
}


// ================================================
// 0xf128: WORD 'UNK_0xf12a' codep=0x224c parp=0xf12a
// ================================================

void UNK_0xf12a() // UNK_0xf12a
{
  Push(pp__ask_PORT); // ?PORT
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1179;

  CTINIT(); // CTINIT
  CTERASE(); // CTERASE
  PRINT("WE CAN'T LAND HERE", 18); // (.")

// ================================================
// 0xf13a: WORD 'UNK_0xf13c' codep=0x2045 parp=0xf13c
// ================================================

// ================================================
// 0xf144: WORD 'UNK_0xf146' codep=0x444e parp=0xf146
// ================================================
  Push(0); // 0
  goto label1180;

  label1179:
  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  Push(1); // 1
  _eq_(); // =
  _ask_DUP(); // ?DUP
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1180;

  UNK_0xf10d(); // UNK_0xf10d
  Push(0); // 0

  label1180:
}


// ================================================
// 0xf167: WORD '(/LAUNCH/LAND)' codep=0x224c parp=0xf17a
// ================================================

void _ro__slash_LAUNCH_slash_LAND_rc_() // (/LAUNCH/LAND)
{
  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  Push(1); // 1
  _eq_(); // =
  if (Pop() == 0) goto label1176;

  Push(cc__5); // 5
  Push(0xb77c);
  MODULE(); // MODULE
  goto label1190;

  label1176:
  Push(1); // 1

  label1190:
  if (Pop() == 0) goto label1177;

  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  DUP(); // DUP
  if (Pop() == 0) Push(1); else Push(0); // 0=
  SWAP(); // SWAP
  Push(cc__5); // 5
  _eq_(); // =
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1178;

  UNK_0xf05f(); // UNK_0xf05f
  Push(0); // 0
  goto label1189;

  label1178:
  UNK_0xf12a(); // UNK_0xf12a

  label1189:
  goto label1181;

  label1177:
  Push(0); // 0

  label1181:
}


// ================================================
// 0xf1be: WORD 'UNK_0xf1c0' codep=0x1d29 parp=0xf1c0
// ================================================
// 0xf1c0: db 0x3a 0x20 ': '

// ================================================
// 0xf1c2: WORD 'UNK_0xf1c4' codep=0x224c parp=0xf1c4
// ================================================

void UNK_0xf1c4() // UNK_0xf1c4
{
  Push(pp_UNK_0xf1c0); // UNK_0xf1c0
  _plus__ex_(); // +!
}


// ================================================
// 0xf1ca: WORD 'UNK_0xf1cc' codep=0x224c parp=0xf1cc
// ================================================

void UNK_0xf1cc() // UNK_0xf1cc
{
  UNK_0xe4f9(); // UNK_0xe4f9
  C_at_(); // C@
  Push(2); // 2
  _st_(); // <
  DUP(); // DUP
  if (Pop() == 0) Push(1); else Push(0); // NOT
  UNK_0xf1c4(); // UNK_0xf1c4
}


// ================================================
// 0xf1dc: WORD 'UNK_0xf1de' codep=0x224c parp=0xf1de
// ================================================

void UNK_0xf1de() // UNK_0xf1de
{
  UNK_0xe4f1(); // UNK_0xe4f1
  C_at_(); // C@
  Push(cc__3); // 3
  _gt_(); // >
  DUP(); // DUP
  if (Pop() == 0) Push(1); else Push(0); // NOT
  UNK_0xf1c4(); // UNK_0xf1c4
}


// ================================================
// 0xf1ee: WORD 'UNK_0xf1f0' codep=0x224c parp=0xf1f0
// ================================================

void UNK_0xf1f0() // UNK_0xf1f0
{
  UNK_0xe4cc(); // UNK_0xe4cc
  Push(Read16(Pop())); // @
  DUP(); // DUP
  Push(0x0045);
  _gt_(); // >
  OVER(); // OVER
  Push(0x0083);
  _st_(); // <
  Push(Pop() + Pop()); // +
  UNK_0xf1c4(); // UNK_0xf1c4
  Push(0x00c8);
  _gt_(); // >
}


// ================================================
// 0xf210: WORD 'UNK_0xf212' codep=0x224c parp=0xf212
// ================================================

void UNK_0xf212() // UNK_0xf212
{
  UNK_0xe4d4(); // UNK_0xe4d4
  Push(Read16(Pop())); // @
  DUP(); // DUP
  Push(cc__4); // 4
  _st_(); // <
  OVER(); // OVER
  Push(cc__3); // 3
  _st_(); // <
  Push(Pop() + Pop()); // +
  UNK_0xf1c4(); // UNK_0xf1c4
  Push(2); // 2
  _gt_(); // >
}


// ================================================
// 0xf22c: WORD 'UNK_0xf22e' codep=0x224c parp=0xf22e
// ================================================

void UNK_0xf22e() // UNK_0xf22e
{
  Push(pp_ATMO); // ATMO
  Push(Read16(Pop())); // @
  Push(0x000f);
  Push(0x0011);
  WITHIN(); // WITHIN
  Push(pp_HYDRO); // HYDRO
  Push(Read16(Pop())); // @
  Push(2); // 2
  _eq_(); // =
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xf24a: WORD 'UNK_0xf24c' codep=0x224c parp=0xf24c
// ================================================

void UNK_0xf24c() // UNK_0xf24c
{
  Push(pp_HYDRO); // HYDRO
  Push(Read16(Pop())); // @
  Push(cc__5); // 5
  _eq_(); // =
  Push(pp_ATMO); // ATMO
  Push(Read16(Pop())); // @
  Push(0x0015);
  _eq_(); // =
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xf264: WORD 'UNK_0xf266' codep=0x224c parp=0xf266
// ================================================

void UNK_0xf266() // UNK_0xf266
{
  UNK_0xe4dc(); // UNK_0xe4dc
  C_at_(); // C@
  _0_gt_(); // 0>
  UNK_0xf1c4(); // UNK_0xf1c4
}


// ================================================
// 0xf270: WORD 'UNK_0xf272' codep=0x224c parp=0xf272
// ================================================

void UNK_0xf272() // UNK_0xf272
{
  UNK_0xe4e4(); // UNK_0xe4e4
  C_at_(); // C@
  DUP(); // DUP
  _0_gt_(); // 0>
  SWAP(); // SWAP
  Push(0x0032);
  _slash_(); // /
  MAX(); // MAX
  UNK_0xf1c4(); // UNK_0xf1c4
}


// ================================================
// 0xf288: WORD 'UNK_0xf28a' codep=0x224c parp=0xf28a
// ================================================

void UNK_0xf28a() // UNK_0xf28a
{
  CTINIT(); // CTINIT
  CTERASE(); // CTERASE
  PRINT("NAME THIS PLANET: ", 18); // (.")
  CTCR(); // CTCR
  UNK_0xe51a(); // UNK_0xe51a
  Push(0x000f);
  Push(cc_BL); // BL
  FILL(); // FILL
  Push(0x000f);
  Push(0xb6fd);
  MODULE(); // MODULE
  COUNT(); // COUNT
  UNK_0xe51a(); // UNK_0xe51a
  SWAP(); // SWAP
  CMOVE(); // CMOVE
}


// ================================================
// 0xf2c3: WORD 'UNK_0xf2c5' codep=0x224c parp=0xf2c5
// ================================================

void UNK_0xf2c5() // UNK_0xf2c5
{
  _star_OP(); // *OP
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x002d);
  IFIND(); // IFIND
  Pop(); // DROP
  IOPEN(); // IOPEN
  Push(0x0024);
  Push(0); // 0
  Push(1); // 1
  _star_CREATE(); // *CREATE
  Push(pp__ro_SYSTEM); // (SYSTEM
  _at__gt_C_plus_S(); // @>C+S
  INST_minus_X(); // INST-X
  _2_at_(); // 2@
  ICLOSE(); // ICLOSE
  INST_minus_X(); // INST-X
  D_ex_(); // D!
  Push(pp__ro_ORBIT_rc_); // (ORBIT)
  _at__gt_C_plus_S(); // @>C+S
  INST_minus_QT(); // INST-QT
  Push(Read16(Pop())); // @
  ICLOSE(); // ICLOSE
  UNK_0xe510(); // UNK_0xe510
  C_ex_(); // C!
}


// ================================================
// 0xf301: WORD 'UNK_0xf303' codep=0x224c parp=0xf303
// ================================================

void UNK_0xf303() // UNK_0xf303
{
  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
  UNK_0xf1de(); // UNK_0xf1de
  UNK_0xf1cc(); // UNK_0xf1cc
  Push(Pop()*2); // 2*
  Push(Pop() | Pop()); // OR
  UNK_0xf1f0(); // UNK_0xf1f0
  Push(cc__4); // 4
  Push(Pop() * Pop()); // *
  Push(Pop() | Pop()); // OR
  UNK_0xe466(); // UNK_0xe466
  Push(0x0040);
  Push(Pop() * Pop()); // *
  Push(Pop() | Pop()); // OR
  UNK_0xf212(); // UNK_0xf212
  Push(cc__8); // 8
  Push(Pop() * Pop()); // *
  Push(Pop() | Pop()); // OR
  UNK_0xf22e(); // UNK_0xf22e
  Push(Pop()<<4); // 16*
  Push(Pop() | Pop()); // OR
  UNK_0xf24c(); // UNK_0xf24c
  Push(0x0020);
  Push(Pop() * Pop()); // *
  Push(Pop() | Pop()); // OR
  Push(pp_PAST); // PAST
  Push(Read16(Pop())); // @
  Push(0x0080);
  Push(Pop() * Pop()); // *
  Push(Pop() | Pop()); // OR
  UNK_0xf266(); // UNK_0xf266
  UNK_0xf272(); // UNK_0xf272
  ICLOSE(); // ICLOSE
  UNK_0xe515(); // UNK_0xe515
  C_ex_(); // C!
}


// ================================================
// 0xf351: WORD 'UNK_0xf353' codep=0x224c parp=0xf353
// ================================================

void UNK_0xf353() // UNK_0xf353
{
  UNK_0xe515(); // UNK_0xe515
  C_at_(); // C@
  if (Pop() == 0) goto label1174;

  Push(pp_UNK_0xe444); // UNK_0xe444
  Push(Read16(Pop())); // @
  Push(cc__8); // 8
  MIN(); // MIN
  Push(0x000a);
  Push(Pop() * Pop()); // *
  Push(1); // 1
  Push(pp_UNK_0xe444); // UNK_0xe444
  _plus__ex_(); // +!
  SAVE_minus_OV(); // SAVE-OV
  goto label1175;

  label1174:
  Push(pp_UNK_0xf1c0); // UNK_0xf1c0
  Push(Read16(Pop())); // @
  Push(cc__5); // 5
  Push(Pop() * Pop()); // *

  label1175:
  UNK_0xe501(); // UNK_0xe501
  C_ex_(); // C!
}


// ================================================
// 0xf383: WORD 'UNK_0xf385' codep=0x224c parp=0xf385
// ================================================

void UNK_0xf385() // UNK_0xf385
{
  Push(pp_UNK_0xf1c0); // UNK_0xf1c0
  _099(); // 099
  UNK_0xf2c5(); // UNK_0xf2c5
  Push(pp_STARDAT); // STARDAT
  Push(Read16(Pop())); // @
  UNK_0xe506(); // UNK_0xe506
  _ex_(); // !
  CI_i_(); // CI'
  _gt_C_plus_S(); // >C+S
  Push(1); // 1
  INST_minus_QT(); // INST-QT
  _plus__ex_(); // +!
  INST_minus_QT(); // INST-QT
  Push(Read16(Pop())); // @
  ICLOSE(); // ICLOSE
  UNK_0xe50b(); // UNK_0xe50b
  _ex_(); // !
  UNK_0xf303(); // UNK_0xf303
  UNK_0xf353(); // UNK_0xf353
  UNK_0xf28a(); // UNK_0xf28a
  ICLOSE(); // ICLOSE
  ICLOSE(); // ICLOSE
  ICLOSE(); // ICLOSE
  Push(pp__ask_EVAL); // ?EVAL
  ON(); // ON
}


// ================================================
// 0xf3b9: WORD 'UNK_0xf3bb' codep=0x224c parp=0xf3bb
// ================================================

void UNK_0xf3bb() // UNK_0xf3bb
{
  UNK_0xf385(); // UNK_0xf385
  CTCR(); // CTCR
  PRINT("RECOMMENDATION LOGGED.", 22); // (.")
}


// ================================================
// 0xf3da: WORD 'UNK_0xf3dc' codep=0x224c parp=0xf3dc
// ================================================

void UNK_0xf3dc() // UNK_0xf3dc
{
  Push(pp__ro_ORBIT_rc_); // (ORBIT)
  _at__gt_C_plus_S(); // @>C+S
  INST_minus_QT(); // INST-QT
  Push(Read16(Pop())); // @
  BIT(); // BIT
  DUP(); // DUP
  ICLOSE(); // ICLOSE
  Push(pp__ro_SYSTEM); // (SYSTEM
  _at__gt_C_plus_S(); // @>C+S
  UNK_0xe529(); // UNK_0xe529
  C_at_(); // C@
  Push(Pop() & Pop()); // AND
  DUP(); // DUP
  ROT(); // ROT
  UNK_0xe529(); // UNK_0xe529
  C_at_(); // C@
  Push(Pop() | Pop()); // OR
  Push(pp_PAST); // PAST
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1171;

  Pop(); // DROP
  goto label1173;

  label1171:
  UNK_0xe529(); // UNK_0xe529
  C_ex_(); // C!

  label1173:
  ICLOSE(); // ICLOSE
  if (Pop() == 0) goto label1172;

  PRINT("THIS PLANET HAS ALREADY BEEN LOGGED.", 36); // (.")

  label1172:
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xf441: WORD '(/LOG-PLAN)' codep=0x224c parp=0xf451
// ================================================

void _ro__slash_LOG_minus_PLAN_rc_() // (/LOG-PLAN)
{
  CTINIT(); // CTINIT
  CTERASE(); // CTERASE
  Push(pp__minus_AIN); // -AIN
  _099(); // 099
  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  Push(2); // 2
  _st_(); // <
  if (Pop() == 0) goto label1165;

  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN
  Push(0x000c);
  Push(0); // 0
  IFIND(); // IFIND
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
  if (Pop() == 0) goto label1166;

  PRINT("THE PLANET THAT STARPORT ORBITS IS", 34); // (.")
  GCR(); // GCR
  PRINT("DEFINITELY NOT COLONIZABLE!", 27); // (.")
  goto label1170;

  label1166:
  Push(pp_CONTEXT); // CONTEXT
  Push(Read16(Pop())); // @
  Push(2); // 2
  _st_(); // <
  if (Pop() == 0) goto label1167;

  PRINT("RECOMMEND THIS PLANET FOR", 25); // (.")
  CTCR(); // CTCR
  PRINT("COLONIZATION? [N Y]", 19); // (.")

// ================================================
// 0xf500: WORD 'UNK_0xf502' codep=0x204e parp=0xf502
// ================================================
  Y_slash_N(); // Y/N
  CTCR(); // CTCR
  if (Pop() == 0) goto label1169;

  UNK_0xf3dc(); // UNK_0xf3dc
  if (Pop() == 0) goto label1169;

  Push(0xc2cd);
  MODULE(); // MODULE
  Push(pp_STARDAT); // STARDAT
  Push(Read16(Pop())); // @
  Push(pp_PORTDAT); // PORTDAT
  _ex_(); // !
  UNK_0xf3bb(); // UNK_0xf3bb

  label1169:
  goto label1170;

  label1167:
  UNK_0xf10d(); // UNK_0xf10d

  label1170:
  goto label1168;

  label1165:
  UNK_0xf10d(); // UNK_0xf10d

  label1168:
}

// 0xf530: db 0x43 0x41 0x50 0x2d 0x56 0x4f 0x43 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x20 0x66 0x6f 0x72 0x20 0x43 0x41 0x50 0x54 0x41 0x49 0x4e 0x20 0x2d 0x2d 0x2d 0x00 'CAP-VOC_________________________ for CAPTAIN --- '
  