// ====== OVERLAY 'PORTMENU-OV' ======
// store offset = 0xece0
// overlay size   = 0x0880

#include"../emul/cpu.h"
#include"../emul/starflt2.h"


// =================================
// =========== DICTIONARY ==========
// =================================
// 1869:      UNK_0x3f09  codep:0x224c parp:0x3f09 size:0xadeb C-string:'UNK_0x3f09'
// 1870:      UNK_0xecf6  codep:0x7420 parp:0xecf6 size:0x0003 C-string:'UNK_0xecf6'
// 1871:      UNK_0xecfb  codep:0x7420 parp:0xecfb size:0x0003 C-string:'UNK_0xecfb'
// 1872:      UNK_0xed00  codep:0x7420 parp:0xed00 size:0x0003 C-string:'UNK_0xed00'
// 1873:      UNK_0xed05  codep:0x7420 parp:0xed05 size:0x0007 C-string:'UNK_0xed05'
// 1874:      UNK_0xed0e  codep:0x224c parp:0xed0e size:0x0044 C-string:'UNK_0xed0e'
// 1875:      UNK_0xed54  codep:0x224c parp:0xed54 size:0x000c C-string:'UNK_0xed54'
// 1876:      UNK_0xed62  codep:0x224c parp:0xed62 size:0x0028 C-string:'UNK_0xed62'
// 1877:      UNK_0xed8c  codep:0x1d29 parp:0xed8c size:0x0002 C-string:'UNK_0xed8c'
// 1878:      UNK_0xed90  codep:0x224c parp:0xed90 size:0x005e C-string:'UNK_0xed90'
// 1879:            MANX  codep:0x1d29 parp:0xedf7 size:0x0002 C-string:'MANX'
// 1880:            MANY  codep:0x1d29 parp:0xee02 size:0x0002 C-string:'MANY'
// 1881:      UNK_0xee06  codep:0x224c parp:0xee06 size:0x0010 C-string:'UNK_0xee06'
// 1882:      UNK_0xee18  codep:0x224c parp:0xee18 size:0x000e C-string:'UNK_0xee18'
// 1883:      UNK_0xee28  codep:0x224c parp:0xee28 size:0x000e C-string:'UNK_0xee28'
// 1884:      UNK_0xee38  codep:0x224c parp:0xee38 size:0x000e C-string:'UNK_0xee38'
// 1885:      UNK_0xee48  codep:0x224c parp:0xee48 size:0x000e C-string:'UNK_0xee48'
// 1886:      UNK_0xee58  codep:0x224c parp:0xee58 size:0x000e C-string:'UNK_0xee58'
// 1887:      UNK_0xee68  codep:0x224c parp:0xee68 size:0x0008 C-string:'UNK_0xee68'
// 1888:         |REGION  codep:0x224c parp:0xee7c size:0x0052 C-string:'_v_REGION'
// 1889:      UNK_0xeed0  codep:0x1d29 parp:0xeed0 size:0x0002 C-string:'UNK_0xeed0'
// 1890:      UNK_0xeed4  codep:0x1d29 parp:0xeed4 size:0x000f C-string:'UNK_0xeed4'
// 1891:      UNK_0xeee5  codep:0x224c parp:0xeee5 size:0x0028 C-string:'UNK_0xeee5'
// 1892:      UNK_0xef0f  codep:0x1d29 parp:0xef0f size:0x0004 C-string:'UNK_0xef0f'
// 1893:      UNK_0xef15  codep:0x224c parp:0xef15 size:0x000c C-string:'UNK_0xef15'
// 1894:      UNK_0xef23  codep:0x224c parp:0xef23 size:0x0012 C-string:'UNK_0xef23'
// 1895:      UNK_0xef37  codep:0x224c parp:0xef37 size:0x000f C-string:'UNK_0xef37'
// 1896:      UNK_0xef48  codep:0x224c parp:0xef48 size:0x0017 C-string:'UNK_0xef48'
// 1897:      UNK_0xef61  codep:0x224c parp:0xef61 size:0x000e C-string:'UNK_0xef61'
// 1898:      UNK_0xef71  codep:0x224c parp:0xef71 size:0x0014 C-string:'UNK_0xef71'
// 1899:      UNK_0xef87  codep:0x224c parp:0xef87 size:0x0010 C-string:'UNK_0xef87'
// 1900:      UNK_0xef99  codep:0x224c parp:0xef99 size:0x0009 C-string:'UNK_0xef99'
// 1901:      UNK_0xefa4  codep:0x224c parp:0xefa4 size:0x0010 C-string:'UNK_0xefa4'
// 1902:      UNK_0xefb6  codep:0x224c parp:0xefb6 size:0x001a C-string:'UNK_0xefb6'
// 1903:         (.DOOR)  codep:0x4b3b parp:0xefdc size:0x0020 C-string:'_ro__dot_DOOR_rc_'
// 1904:      UNK_0xeffe  codep:0x224c parp:0xeffe size:0x000e C-string:'UNK_0xeffe'
// 1905:      UNK_0xf00e  codep:0x224c parp:0xf00e size:0x001e C-string:'UNK_0xf00e'
// 1906:      UNK_0xf02e  codep:0x224c parp:0xf02e size:0x004a C-string:'UNK_0xf02e'
// 1907:          STPORT  codep:0x2214 parp:0xf083 size:0x000c C-string:'STPORT'
// 1908:      UNK_0xf091  codep:0x224c parp:0xf091 size:0x000e C-string:'UNK_0xf091'
// 1909:      UNK_0xf0a1  codep:0x224c parp:0xf0a1 size:0x0014 C-string:'UNK_0xf0a1'
// 1910:      UNK_0xf0b7  codep:0x224c parp:0xf0b7 size:0x0063 C-string:'UNK_0xf0b7'
// 1911:      UNK_0xf11c  codep:0x224c parp:0xf11c size:0x007a C-string:'UNK_0xf11c'
// 1912:      UNK_0xf198  codep:0x224c parp:0xf198 size:0x0024 C-string:'UNK_0xf198'
// 1913:      UNK_0xf1be  codep:0x1d29 parp:0xf1be size:0x0002 C-string:'UNK_0xf1be'
// 1914:      UNK_0xf1c2  codep:0x1d29 parp:0xf1c2 size:0x0002 C-string:'UNK_0xf1c2'
// 1915:      UNK_0xf1c6  codep:0x1d29 parp:0xf1c6 size:0x000c C-string:'UNK_0xf1c6'
// 1916:      UNK_0xf1d4  codep:0x1d29 parp:0xf1d4 size:0x009a C-string:'UNK_0xf1d4'
// 1917:      UNK_0xf270  codep:0x1d29 parp:0xf270 size:0x0038 C-string:'UNK_0xf270'
// 1918:      UNK_0xf2aa  codep:0x224c parp:0xf2aa size:0x001c C-string:'UNK_0xf2aa'
// 1919:      UNK_0xf2c8  codep:0x224c parp:0xf2c8 size:0x003e C-string:'UNK_0xf2c8'
// 1920:      UNK_0xf308  codep:0x224c parp:0xf308 size:0x0034 C-string:'UNK_0xf308'
// 1921:      UNK_0xf33e  codep:0x224c parp:0xf33e size:0x0006 C-string:'UNK_0xf33e'
// 1922:      UNK_0xf346  codep:0x224c parp:0xf346 size:0x000a C-string:'UNK_0xf346'
// 1923:      UNK_0xf352  codep:0x224c parp:0xf352 size:0x000c C-string:'UNK_0xf352'
// 1924:      UNK_0xf360  codep:0x224c parp:0xf360 size:0x001c C-string:'UNK_0xf360'
// 1925:      UNK_0xf37e  codep:0x224c parp:0xf37e size:0x0050 C-string:'UNK_0xf37e'
// 1926:      UNK_0xf3d0  codep:0x224c parp:0xf3d0 size:0x003e C-string:'UNK_0xf3d0'
// 1927:      UNK_0xf410  codep:0x224c parp:0xf410 size:0x000a C-string:'UNK_0xf410'
// 1928:      UNK_0xf41c  codep:0x224c parp:0xf41c size:0x000c C-string:'UNK_0xf41c'
// 1929:      UNK_0xf42a  codep:0x224c parp:0xf42a size:0x005a C-string:'UNK_0xf42a'
// 1930:      UNK_0xf486  codep:0x224c parp:0xf486 size:0x000c C-string:'UNK_0xf486'
// 1931:         DO-ROOM  codep:0x4b3b parp:0xf49e size:0x0020 C-string:'DO_dash_ROOM'
// 1932:      UNK_0xf4c0  codep:0x224c parp:0xf4c0 size:0x0032 C-string:'UNK_0xf4c0'
// 1933:           XWALK  codep:0x4b3b parp:0xf4fc size:0x000c C-string:'XWALK'
// 1934:         WALKIES  codep:0x224c parp:0xf514 size:0x0030 C-string:'WALKIES'
// 1935:            PPIC  codep:0x224c parp:0xf54d size:0x0000 C-string:'PPIC'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xed8c[2] = {0x00, 0x00}; // UNK_0xed8c
unsigned char MANX[2] = {0x06, 0x00}; // MANX
unsigned char MANY[2] = {0x52, 0x00}; // MANY
unsigned char UNK_0xeed0[2] = {0x01, 0x00}; // UNK_0xeed0
unsigned char UNK_0xeed4[15] = {0x07, 0x04, 0x12, 0x1d, 0x27, 0x2d, 0x3c, 0x47, 0x51, 0x5c, 0x66, 0x6e, 0x7c, 0x88, 0x92}; // UNK_0xeed4
unsigned char UNK_0xef0f[4] = {0x56, 0x3a, 0x20, 0x56}; // UNK_0xef0f
unsigned char UNK_0xf1be[2] = {0x3a, 0x20}; // UNK_0xf1be
unsigned char UNK_0xf1c2[2] = {0xff, 0xff}; // UNK_0xf1c2
unsigned char UNK_0xf1c6[12] = {0x00, 0x03, 0x03, 0x00, 0x02, 0x05, 0x00, 0x02, 0x02, 0x0f, 0x03, 0x03}; // UNK_0xf1c6
unsigned char UNK_0xf1d4[154] = {}; // UNK_0xf1d4
unsigned char UNK_0xf270[56] = {0xc4, 0x00, 0x02, 0x03, 0x1e, 0x01, 0x01, 0x04, 0x14, 0x00, 0x00, 0x05, 0xe2, 0xff, 0x01, 0x04, 0x3c, 0x00, 0x02, 0x03, 0xc4, 0x00, 0xfe, 0x08, 0x1e, 0x01, 0xff, 0x09, 0x14, 0x00, 0x00, 0x0a, 0xe2, 0xff, 0xff, 0x09, 0x3c, 0x00, 0xfe, 0x08, 0x9c, 0x03, 0x00, 0x06, 0x9c, 0x03, 0x00, 0x07, 0x9c, 0xfd, 0x00, 0x01, 0x9c, 0xfd, 0x00, 0x02}; // UNK_0xf270

const unsigned short int cc_STPORT = 0xf086; // STPORT


// 0xecf2: db 0x88 0x00 '  '

// ================================================
// 0xecf4: WORD 'UNK_0xecf6' codep=0x7420 parp=0xecf6
// ================================================
// 0xecf6: db 0x14 0x14 0x01 '   '

// ================================================
// 0xecf9: WORD 'UNK_0xecfb' codep=0x7420 parp=0xecfb
// ================================================
// 0xecfb: db 0x10 0x20 0x02 '   '

// ================================================
// 0xecfe: WORD 'UNK_0xed00' codep=0x7420 parp=0xed00
// ================================================
// 0xed00: db 0x10 0x1f 0x01 '   '

// ================================================
// 0xed03: WORD 'UNK_0xed05' codep=0x7420 parp=0xed05
// ================================================
// 0xed05: db 0x14 0x43 0x02 0x29 0x1d 0x3a 0x20 ' C ) : '

// ================================================
// 0xed0c: WORD 'UNK_0xed0e' codep=0x224c parp=0xed0e
// ================================================

void UNK_0xed0e() // UNK_0xed0e
{
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(0x000a);
  Push(Pop() + Pop()); // +
  RRND(); // RRND
  Push(0x5a3d); // pointer to YBLT
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  Push(Read16(regsp)); // DUP
  Push(0x000c);
  _dash_(); // -
  SWAP(); // SWAP
  RRND(); // RRND
  _2DUP(); // 2DUP
  SWAP(); // SWAP
  Push(Pop()*2); // 2*
  SWAP(); // SWAP
  _gt_HIDDEN(); // >HIDDEN
  L_at_PIXEL(); // L@PIXEL
  _gt_DISPLA(); // >DISPLA
  _ex_COLOR(); // !COLOR
  LPLOT(); // LPLOT
  Push(0x1421);
  Push(0x7e64);
  RRND(); // RRND
  Push(0); // 0
  Push(0x637e); // pointer to STIME
  D_ex_(); // D!
}


// ================================================
// 0xed52: WORD 'UNK_0xed54' codep=0x224c parp=0xed54
// ================================================

void UNK_0xed54() // UNK_0xed54
{
  BEEPON_2(); // BEEPON_2
  Push(Read16(regsp)); // DUP
  Push(Pop()*2); // 2*
  RRND(); // RRND
  TONE(); // TONE
}


// ================================================
// 0xed60: WORD 'UNK_0xed62' codep=0x224c parp=0xed62
// ================================================

void UNK_0xed62() // UNK_0xed62
{
  SetColor("GREY2");
  _ex_COLOR(); // !COLOR
  Push(0x5a56); // pointer to XORMODE
  _099(); // 099
  Push(0x0096);
  Push(cc_MPS); // MPS
  Push(2); // 2
  MAX(); // MAX
  Push(Pop() * Pop()); // *
  Push(0x0064);

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  UNK_0xed54(); // UNK_0xed54
  UNK_0xed0e(); // UNK_0xed0e
  i++;
  } while(i<imax); // (LOOP) 0xfff8

  BEEPOFF(); // BEEPOFF
}


// ================================================
// 0xed8a: WORD 'UNK_0xed8c' codep=0x1d29 parp=0xed8c
// ================================================
// 0xed8c: db 0x00 0x00 '  '

// ================================================
// 0xed8e: WORD 'UNK_0xed90' codep=0x224c parp=0xed90
// ================================================

void UNK_0xed90() // UNK_0xed90
{
  Push(0x6002); // pointer to STARDAT
  Push(Read16(Pop())); // @
  Push(0xed8c); // pointer to UNK_0xed8c
  Push(Read16(Pop())); // @
  _dash_(); // -
  Push(0x0014);
  Push(Pop() * Pop()); // *
  _gt_R(); // >R
  Push2Words("*PERSON");
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN

  label2:
  Push(0x6601); // IFIELD(UNK_0xecfb)
  Push(Read8(Pop())&0xFF); // C@
  Push(Read16(regsp)); // DUP
  Push(cc__8); // 8
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) Push(1); else Push(0); // NOT
  SWAP(); // SWAP
  Push(2); // 2
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1;
  Push(h); // I
  Push(0x6600); // IFIELD(UNK_0xed00)
  Push(Read8(Pop())&0xFF); // C@
  Push(Pop() + Pop()); // +
  Push(0x0064);
  MIN(); // MIN
  Push(0x6600); // IFIELD(UNK_0xed00)
  C_ex__2(); // C!_2

  label1:
  INEXT(); // INEXT
  _ask_FIRST(); // ?FIRST
  if (Pop() == 0) goto label2;
  ICLOSE(); // ICLOSE
  ICLOSE(); // ICLOSE
  R_gt_(); // R>
  Pop(); // DROP
  Push(0x6002); // pointer to STARDAT
  Push(Read16(Pop())); // @
  Push(0xed8c); // pointer to UNK_0xed8c
  _ex__2(); // !_2
}


// ================================================
// 0xedee: WORD 'MANX' codep=0x1d29 parp=0xedf7
// ================================================
// 0xedf7: db 0x06 0x00 '  '

// ================================================
// 0xedf9: WORD 'MANY' codep=0x1d29 parp=0xee02
// ================================================
// 0xee02: db 0x52 0x00 'R '

// ================================================
// 0xee04: WORD 'UNK_0xee06' codep=0x224c parp=0xee06
// ================================================

void UNK_0xee06() // UNK_0xee06
{
  Push(0x5e5d); // pointer to ?>OP
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  if (Pop() == 0) goto label1;
  BEEP(); // BEEP

  label1:
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xee16: WORD 'UNK_0xee18' codep=0x224c parp=0xee18
// ================================================

void UNK_0xee18() // UNK_0xee18
{
  UNK_0xee06(); // UNK_0xee06
  if (Pop() == 0) return;
  Push(0xcbcf);
  MODULE(); // MODULE
}


// ================================================
// 0xee26: WORD 'UNK_0xee28' codep=0x224c parp=0xee28
// ================================================

void UNK_0xee28() // UNK_0xee28
{
  UNK_0xee06(); // UNK_0xee06
  if (Pop() == 0) return;
  Push(0xcb98);
  MODULE(); // MODULE
}


// ================================================
// 0xee36: WORD 'UNK_0xee38' codep=0x224c parp=0xee38
// ================================================

void UNK_0xee38() // UNK_0xee38
{
  UNK_0xee06(); // UNK_0xee06
  if (Pop() == 0) return;
  Push(0xc2db);
  MODULE(); // MODULE
}


// ================================================
// 0xee46: WORD 'UNK_0xee48' codep=0x224c parp=0xee48
// ================================================

void UNK_0xee48() // UNK_0xee48
{
  UNK_0xee06(); // UNK_0xee06
  if (Pop() == 0) return;
  Push(0xcca7);
  MODULE(); // MODULE
}


// ================================================
// 0xee56: WORD 'UNK_0xee58' codep=0x224c parp=0xee58
// ================================================

void UNK_0xee58() // UNK_0xee58
{
  UNK_0xee06(); // UNK_0xee06
  if (Pop() == 0) return;
  Push(0xcce1);
  MODULE(); // MODULE
}


// ================================================
// 0xee66: WORD 'UNK_0xee68' codep=0x224c parp=0xee68
// ================================================

void UNK_0xee68() // UNK_0xee68
{
  Push(0xcd3b);
  MODULE(); // MODULE
}


// ================================================
// 0xee70: WORD '|REGION' codep=0x224c parp=0xee7c
// ================================================

void _v_REGION() // |REGION
{
  if (Pop() == 0) goto label1;
  H_gt_D(); // H>D
  goto label2;

  label1:
  D_gt_H(); // D>H

  label2:
  Push(cc_AX); // AX
  _099(); // 099
  Push(0x5a3d); // pointer to YBLT
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(cc_BX); // BX
  Push(Pop()+1); // 1+
  C_ex__2(); // C!_2
  Push(cc_DX); // DX
  Push(Pop()+1); // 1+
  C_ex__2(); // C!_2
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(cc_BX); // BX
  C_ex__2(); // C!_2
  Push(cc_DX); // DX
  C_ex__2(); // C!_2
  Push(0x5a3d); // pointer to YBLT
  Push(Read16(Pop())); // @
  Push(0x000c);
  _dash_(); // -
  Push(cc_CX); // CX
  Push(Pop()+1); // 1+
  C_ex__2(); // C!_2
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(0x0014);
  Push(Pop() + Pop()); // +
  Push(cc_CX); // CX
  C_ex__2(); // C!_2
  GRCALL(); // GRCALL
}


// ================================================
// 0xeece: WORD 'UNK_0xeed0' codep=0x1d29 parp=0xeed0
// ================================================
// 0xeed0: db 0x01 0x00 '  '

// ================================================
// 0xeed2: WORD 'UNK_0xeed4' codep=0x1d29 parp=0xeed4
// ================================================
// 0xeed4: db 0x07 0x04 0x12 0x1d 0x27 0x2d 0x3c 0x47 0x51 0x5c 0x66 0x6e 0x7c 0x88 0x92 '    '-<GQ\fn|  '

// ================================================
// 0xeee3: WORD 'UNK_0xeee5' codep=0x224c parp=0xeee5
// ================================================

void UNK_0xeee5() // UNK_0xeee5
{
  if (Read16(regsp) != 0) Push(Read16(regsp)); // ?DUP
  if (Pop() == 0) goto label1;
  Push(Pop()*2); // 2*
  Push(0x0050);
  SWAP(); // SWAP
  _dash_(); // -
  Push(0x00c7);
  POS_dot_(); // POS.
  CTINIT(); // CTINIT
  Push(Read16(regsp)); // DUP
  Push(Pop()-1); // 1-
  Push(Read8(Pop())&0xFF); // C@
  Exec(TYPE); // call of word 0x2690 '(TYPE)'
  return;

  label1:
  Pop(); // DROP
}


// ================================================
// 0xef0d: WORD 'UNK_0xef0f' codep=0x1d29 parp=0xef0f
// ================================================
// 0xef0f: db 0x56 0x3a 0x20 0x56 'V: V'

// ================================================
// 0xef13: WORD 'UNK_0xef15' codep=0x224c parp=0xef15
// ================================================

void UNK_0xef15() // UNK_0xef15
{
  TIME(); // TIME
  _2_at_(); // 2@
  Push(0xef0f); // pointer to UNK_0xef0f
  _2_at_(); // 2@
  D_gt_(); // D>
}


// ================================================
// 0xef21: WORD 'UNK_0xef23' codep=0x224c parp=0xef23
// ================================================

void UNK_0xef23() // UNK_0xef23
{
  TIME(); // TIME
  _2_at_(); // 2@
  Push(0x9c40); Push(0x0000);
  D_plus_(); // D+
  Push(0xef0f); // pointer to UNK_0xef0f
  D_ex_(); // D!
}


// ================================================
// 0xef35: WORD 'UNK_0xef37' codep=0x224c parp=0xef37
// ================================================

void UNK_0xef37() // UNK_0xef37
{

  UNK_0x3f09("OPERATIONS");
}


// ================================================
// 0xef46: WORD 'UNK_0xef48' codep=0x224c parp=0xef48
// ================================================

void UNK_0xef48() // UNK_0xef48
{

  UNK_0x3f09("SHIP CONFIGURATION");
}


// ================================================
// 0xef5f: WORD 'UNK_0xef61' codep=0x224c parp=0xef61
// ================================================

void UNK_0xef61() // UNK_0xef61
{

  UNK_0x3f09("PERSONNEL");
}


// ================================================
// 0xef6f: WORD 'UNK_0xef71' codep=0x224c parp=0xef71
// ================================================

void UNK_0xef71() // UNK_0xef71
{

  UNK_0x3f09("CREW ASSIGNMENT");
}


// ================================================
// 0xef85: WORD 'UNK_0xef87' codep=0x224c parp=0xef87
// ================================================

void UNK_0xef87() // UNK_0xef87
{

  UNK_0x3f09("TRADE DEPOT");
}


// ================================================
// 0xef97: WORD 'UNK_0xef99' codep=0x224c parp=0xef99
// ================================================

void UNK_0xef99() // UNK_0xef99
{

  UNK_0x3f09("BANK");
}


// ================================================
// 0xefa2: WORD 'UNK_0xefa4' codep=0x224c parp=0xefa4
// ================================================

void UNK_0xefa4() // UNK_0xefa4
{

  UNK_0x3f09("DOCKING BAY");
}


// ================================================
// 0xefb4: WORD 'UNK_0xefb6' codep=0x224c parp=0xefb6
// ================================================

void UNK_0xefb6() // UNK_0xefb6
{
  Push(0x00c7);
  Push(0x002c);
  Push(0x00c0);
  Push(0x0072);
  SetColor("BLACK");
  POLY_dash_WI(); // POLY-WI
  Push(0); // 0
  Push(0); // 0
}


// ================================================
// 0xefd0: WORD '(.DOOR)' codep=0x4b3b parp=0xefdc
// ================================================
// 0xefdc: db 0x07 0x00 0xb6 0xef 0x01 0x00 0x37 0xef 0x02 0x00 0x99 0xef 0x03 0x00 0x61 0xef 0x04 0x00 0x71 0xef 0x05 0x00 0x87 0xef 0x06 0x00 0x48 0xef 0x07 0x00 0xa4 0xef '      7       a   q       H     '

// ================================================
// 0xeffc: WORD 'UNK_0xeffe' codep=0x224c parp=0xeffe
// ================================================

void UNK_0xeffe() // UNK_0xeffe
{
  _at_CRS(); // @CRS
  Push(0xeed0); // pointer to UNK_0xeed0
  Push(Read16(Pop())); // @
  Pop();
  switch(Pop()) // (.DOOR)
  {
  case 1:
    UNK_0xef37(); // UNK_0xef37
    break;
  case 2:
    UNK_0xef99(); // UNK_0xef99
    break;
  case 3:
    UNK_0xef61(); // UNK_0xef61
    break;
  case 4:
    UNK_0xef71(); // UNK_0xef71
    break;
  case 5:
    UNK_0xef87(); // UNK_0xef87
    break;
  case 6:
    UNK_0xef48(); // UNK_0xef48
    break;
  case 7:
    UNK_0xefa4(); // UNK_0xefa4
    break;
  default:
    UNK_0xefb6(); // UNK_0xefb6
    break;

  }
  UNK_0xeee5(); // UNK_0xeee5
  _ex_CRS(); // !CRS
}


// ================================================
// 0xf00c: WORD 'UNK_0xf00e' codep=0x224c parp=0xf00e
// ================================================

void UNK_0xf00e() // UNK_0xf00e
{
  Push(2); // 2
  Push(Pop() * Pop()); // *
  Push(0xeed4); // pointer to UNK_0xeed4
  Push(Pop()+1); // 1+
  Push(Pop() + Pop()); // +
  _gt_R(); // >R
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(h); // I
  Push(Read8(Pop())&0xFF); // C@
  R_gt_(); // R>
  Push(Pop()+1); // 1+
  Push(Read8(Pop())&0xFF); // C@
  WITHIN(); // WITHIN
}


// ================================================
// 0xf02c: WORD 'UNK_0xf02e' codep=0x224c parp=0xf02e
// ================================================

void UNK_0xf02e() // UNK_0xf02e
{
  Push(0xeed0); // pointer to UNK_0xeed0
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1;
  Push(0xeed4); // pointer to UNK_0xeed4
  Push(Read8(Pop())&0xFF); // C@
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  UNK_0xf00e(); // UNK_0xf00e
  if (Pop() == 0) goto label2;
  Push(i); // I
  Push(Pop()+1); // 1+
  Push(0xeed0); // pointer to UNK_0xeed0
  _ex__2(); // !_2
  UNK_0xeffe(); // UNK_0xeffe
  LEAVE(); // LEAVE

  label2:
  i++;
  } while(i<imax); // (LOOP) 0xffea

  return;

  label1:
  Push(0xeed0); // pointer to UNK_0xeed0
  Push(Read16(Pop())); // @
  Push(Pop()-1); // 1-
  UNK_0xf00e(); // UNK_0xf00e
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) return;
  Push(0xeed0); // pointer to UNK_0xeed0
  _099(); // 099
  UNK_0xeffe(); // UNK_0xeffe
}

// 0xf072: db 0x29 0x1d 0x3a 0x20 0x05 0x41 ') :  A'

// ================================================
// 0xf078: WORD 'STPORT' codep=0x2214 parp=0xf083
// ================================================
// 0xf083: db 0x86 0xf0 0x00 0x70 0x6f 0x72 0x74 0x2e 0x72 0x61 0x77 0x00 '   port.raw '

// ================================================
// 0xf08f: WORD 'UNK_0xf091' codep=0x224c parp=0xf091
// ================================================

void UNK_0xf091() // UNK_0xf091
{
  SAVE_dash_BU(); // SAVE-BU
  Push(cc_STPORT); // STPORT
  Push(0x6084); // pointer to PIC^
  Push(Read16(Pop())); // @
  _ro_LDS_rc_(); // (LDS)
  MOUNTB(); // MOUNTB
}


// ================================================
// 0xf09f: WORD 'UNK_0xf0a1' codep=0x224c parp=0xf0a1
// ================================================

void UNK_0xf0a1() // UNK_0xf0a1
{
  CTINIT(); // CTINIT
  Push(0xaa3b);
  Push(0x460e); // pointer to ASKMOUN
  _ex__2(); // !_2
  Push(0xaa1d);
  Push(0x6c5b); // pointer to 'VERSIO
  _ex__2(); // !_2
}


// ================================================
// 0xf0b5: WORD 'UNK_0xf0b7' codep=0x224c parp=0xf0b7
// ================================================

void UNK_0xf0b7() // UNK_0xf0b7
{
  _gt_DISPLA(); // >DISPLA
  DARK(); // DARK
  _gt_SSCT(); // >SSCT
  CTINIT(); // CTINIT
  _ask_CGA(); // ?CGA
  if (Pop() == 0) goto label1;

  UNK_0x3f09("WAIT..");
  _dot_TTY(); // .TTY

  label1:
  Push(0x6084); // pointer to PIC^
  Push(Read16(Pop())); // @
  Push(0); // 0
  Push(0x542b); // pointer to XBUF-SE
  Push(Read16(Pop())); // @
  Push(0); // 0
  Push(0x3ded);
  LCMOVE(); // LCMOVE
  Push(0x5f1b); // pointer to ?EGA
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label2;
  _gt_HIDDEN(); // >HIDDEN
  goto label3;

  label2:
  _gt_DISPLA(); // >DISPLA

  label3:
  DARK(); // DARK
  Push(0xb9f9);
  MODULE(); // MODULE
  Push(0); // 0
  Push(0x0096);
  _dot_RAW(); // .RAW
  UNK_0xf0a1(); // UNK_0xf0a1
  Push(0x5f1b); // pointer to ?EGA
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label4;
  SCR_dash_RES(); // SCR-RES
  goto label5;

  label4:
  SAVE_dash_SC(); // SAVE-SC

  label5:
  H_gt_D(); // H>D
  _gt_DISPLA(); // >DISPLA
}


// ================================================
// 0xf11a: WORD 'UNK_0xf11c' codep=0x224c parp=0xf11c
// ================================================

void UNK_0xf11c() // UNK_0xf11c
{
  Push(0x5ecd); // pointer to ESC-EN
  ON_2(); // ON_2
  Push(0xc1f8);
  Push(0x60e5); // pointer to 'CREW-C
  _ex__2(); // !_2
  Push(0xc1c7);
  Push(0x610f); // pointer to 'TREATM
  _ex__2(); // !_2
  Push(0x3a48);
  Push(0x60ae); // pointer to '.VITAL
  _ex__2(); // !_2
  Push(0xc39d);
  Push(0x60d7); // pointer to 'VEHICL
  _ex__2(); // !_2
  Push(0xc0f9);
  Push(0x60c9); // pointer to '.VEHIC
  _ex__2(); // !_2
  Push(0xc3cf);
  Push(0x6101); // pointer to 'REPAIR
  _ex__2(); // !_2
  Push(0xc0e7);
  Push(0x60bb); // pointer to '.DATE
  _ex__2(); // !_2
  Push(0xca0d);
  MODULE(); // MODULE
  Push(0x0032);
  Push(0x5ea4); // pointer to %VAL
  _ex__2(); // !_2
  Push2Words("*SHIP");
  _gt_C_plus_S(); // >C+S
  Push(cc__7); // 7
  Push(0x65f5); // IFIELD(UNK_0xecf6)
  Push(Read8(Pop())&0xFF); // C@
  Push(cc__7); // 7
  Push(Pop() & Pop()); // AND
  _dash_(); // -
  Push(0x6161); // pointer to E-USE
  _ex__2(); // !_2
  Push(0x6624); // IFIELD(UNK_0xed05)
  Push(Read16(Pop())); // @
  ICLOSE(); // ICLOSE
  Push(0x000a);
  U_star_(); // U*
  Push(0x62fe); // pointer to 10*CARG
  D_ex_(); // D!
  Push(0x55d2); // pointer to ?TV
  ON_2(); // ON_2
  Push(0x5de2); // pointer to ?RECALL
  _099(); // 099
  UNK_0xed62(); // UNK_0xed62
}


// ================================================
// 0xf196: WORD 'UNK_0xf198' codep=0x224c parp=0xf198
// ================================================

void UNK_0xf198() // UNK_0xf198
{
  Push(0xd4cc);
  MODULE(); // MODULE
  if (Pop() == 0) return;
  UNK_0xf11c(); // UNK_0xf11c
  Push(0xc8c3);
  MODULE(); // MODULE
  Push(0xd491);
  MODULE(); // MODULE
  UNK_0xf091(); // UNK_0xf091
  Push(0xc2cd);
  MODULE(); // MODULE
  UNK_0xed90(); // UNK_0xed90
}


// ================================================
// 0xf1bc: WORD 'UNK_0xf1be' codep=0x1d29 parp=0xf1be
// ================================================
// 0xf1be: db 0x3a 0x20 ': '

// ================================================
// 0xf1c0: WORD 'UNK_0xf1c2' codep=0x1d29 parp=0xf1c2
// ================================================
// 0xf1c2: db 0xff 0xff '  '

// ================================================
// 0xf1c4: WORD 'UNK_0xf1c6' codep=0x1d29 parp=0xf1c6
// ================================================
// 0xf1c6: db 0x00 0x03 0x03 0x00 0x02 0x05 0x00 0x02 0x02 0x0f 0x03 0x03 '            '

// ================================================
// 0xf1d2: WORD 'UNK_0xf1d4' codep=0x1d29 parp=0xf1d4
// ================================================
// 0xf1d4: db  '     A  >          A             A  \           p -T             0             0                          \           p ZT  @                             '

// ================================================
// 0xf26e: WORD 'UNK_0xf270' codep=0x1d29 parp=0xf270
// ================================================
// 0xf270: db 0xc4 0x00 0x02 0x03 0x1e 0x01 0x01 0x04 0x14 0x00 0x00 0x05 0xe2 0xff 0x01 0x04 0x3c 0x00 0x02 0x03 0xc4 0x00 0xfe 0x08 0x1e 0x01 0xff 0x09 0x14 0x00 0x00 0x0a 0xe2 0xff 0xff 0x09 0x3c 0x00 0xfe 0x08 0x9c 0x03 0x00 0x06 0x9c 0x03 0x00 0x07 0x9c 0xfd 0x00 0x01 0x9c 0xfd 0x00 0x02 '                <                   <                   '

// ================================================
// 0xf2a8: WORD 'UNK_0xf2aa' codep=0x224c parp=0xf2aa
// ================================================

void UNK_0xf2aa() // UNK_0xf2aa
{
  Push(0xf1d4); // pointer to UNK_0xf1d4
  Push(0x000c);
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  Push(0x8040);
  Push(Pop() ^ Pop()); // XOR
  Push(0xf1d4); // pointer to UNK_0xf1d4
  Push(0x000c);
  Push(Pop() + Pop()); // +
  _ex__2(); // !_2
}


// ================================================
// 0xf2c6: WORD 'UNK_0xf2c8' codep=0x224c parp=0xf2c8
// ================================================

void UNK_0xf2c8() // UNK_0xf2c8
{
  _at_CRS(); // @CRS
  Push(0xf1c6); // pointer to UNK_0xf1c6
  Push(0xf1be); // pointer to UNK_0xf1be
  Push(Read16(Pop())); // @
  Push(cc__3); // 3
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(Read16(regsp)); // DUP
  Push(Read16(regsp)); // DUP
  Push(Read8(Pop())&0xFF); // C@
  _ex_COLOR(); // !COLOR
  Push(Pop()+1); // 1+
  Push(Read8(Pop())&0xFF); // C@
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(Pop()+2); // 2+
  Push(Read8(Pop())&0xFF); // C@
  Push(0x5a48); // pointer to XBLT
  _plus__ex__2(); // +!_2
  Push(cc__dash_2); // -2
  Push(0x5a3d); // pointer to YBLT
  _plus__ex__2(); // +!_2
  Push(2); // 2
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(0xf1c2); // pointer to UNK_0xf1c2
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
  _ex_CRS(); // !CRS
}


// ================================================
// 0xf306: WORD 'UNK_0xf308' codep=0x224c parp=0xf308
// ================================================

void UNK_0xf308() // UNK_0xf308
{
  _at_DS(); // @DS
  Push(0x5a84); // pointer to BLTSEG
  _ex__2(); // !_2
  Push(cc__9); // 9
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(0x000c);
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  CTINIT(); // CTINIT
  Push(0xf1d4); // pointer to UNK_0xf1d4
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  Push(0xedf7); // pointer to MANX
  Push(Read16(Pop())); // @
  Push(0xee02); // pointer to MANY
  Push(Read16(Pop())); // @
  POS_dot_(); // POS.
  Push(0); // 0
  _v_REGION(); // |REGION
  BLT(); // BLT
  Push(0xf1be); // pointer to UNK_0xf1be
  _099(); // 099
  UNK_0xf2c8(); // UNK_0xf2c8
}


// ================================================
// 0xf33c: WORD 'UNK_0xf33e' codep=0x224c parp=0xf33e
// ================================================

void UNK_0xf33e() // UNK_0xf33e
{
  Push(1); // 1
  _v_REGION(); // |REGION
}


// ================================================
// 0xf344: WORD 'UNK_0xf346' codep=0x224c parp=0xf346
// ================================================

void UNK_0xf346() // UNK_0xf346
{
  Push(0); // 0
  _v_REGION(); // |REGION
  BLT(); // BLT
  UNK_0xf2c8(); // UNK_0xf2c8
}


// ================================================
// 0xf350: WORD 'UNK_0xf352' codep=0x224c parp=0xf352
// ================================================

void UNK_0xf352() // UNK_0xf352
{
  Push(0); // 0
  Push(0x03e8);
  Push(1); // 1
  _gt_SND(); // >SND
}


// ================================================
// 0xf35e: WORD 'UNK_0xf360' codep=0x224c parp=0xf360
// ================================================

void UNK_0xf360() // UNK_0xf360
{
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(2); // 2
  MAX(); // MAX
  Push(0x009b);
  MIN(); // MIN
  Push(Read16(regsp)); // DUP
  Push(0x5a48); // pointer to XBLT
  _ex__2(); // !_2
  Push(0xedf7); // pointer to MANX
  _ex__2(); // !_2
  SAVE_dash_OV(); // SAVE-OV
}


// ================================================
// 0xf37c: WORD 'UNK_0xf37e' codep=0x224c parp=0xf37e
// ================================================

void UNK_0xf37e() // UNK_0xf37e
{
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  Push(0x000e);
  Push(Pop() * Pop()); // *
  Push(0xf1d4); // pointer to UNK_0xf1d4
  Push(Pop() + Pop()); // +
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(0x5a3d); // pointer to YBLT
  Push(Read16(Pop())); // @
  _2SWAP(); // 2SWAP
  Push(0x5a48); // pointer to XBLT
  _plus__ex__2(); // +!_2
  Push(0x5a3d); // pointer to YBLT
  _plus__ex__2(); // +!_2
  UNK_0xf360(); // UNK_0xf360
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(0x5a3d); // pointer to YBLT
  Push(Read16(Pop())); // @
  _2SWAP(); // 2SWAP
  POS_dot_(); // POS.
  DISPLAY(); // DISPLAY
  UNK_0xf33e(); // UNK_0xf33e
  POS_dot_(); // POS.
  UNK_0xf346(); // UNK_0xf346
  Push(Read16(regsp)); // DUP
  _0_st_(); // 0<
  if (Pop() == 0) goto label1;
  UNK_0xf352(); // UNK_0xf352

  label1:
  ABS(); // ABS
  Push(cc__3); // 3
  Push(cc__4); // 4
  _star__slash_(); // */
  MS(); // MS
}


// ================================================
// 0xf3ce: WORD 'UNK_0xf3d0' codep=0x224c parp=0xf3d0
// ================================================

void UNK_0xf3d0() // UNK_0xf3d0
{
  UNK_0xef23(); // UNK_0xef23
  Push(0xf1be); // pointer to UNK_0xf1be
  _ex__2(); // !_2
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(0xf270); // pointer to UNK_0xf270
  OVER(); // OVER
  Push(Pop() + Pop()); // +
  Push(i); // I
  Push(cc__4); // 4
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  _gt_R(); // >R
  Push(i); // I
  _plus__dash__at_(); // +-@
  Push(i); // I
  Push(Pop()+1); // 1+
  _plus__dash__at_(); // +-@
  Push(i); // I
  Push(Pop()+2); // 2+
  _plus__dash__at_(); // +-@
  R_gt_(); // R>
  Push(cc__3); // 3
  Push(Pop() + Pop()); // +
  Push(Read8(Pop())&0xFF); // C@
  UNK_0xf37e(); // UNK_0xf37e
  i++;
  } while(i<imax); // (LOOP) 0xffd4

  Pop(); // DROP
  UNK_0xf02e(); // UNK_0xf02e
}


// ================================================
// 0xf40e: WORD 'UNK_0xf410' codep=0x224c parp=0xf410
// ================================================

void UNK_0xf410() // UNK_0xf410
{
  Push(0); // 0
  Push(cc__5); // 5
  Push(1); // 1
  UNK_0xf3d0(); // UNK_0xf3d0
}


// ================================================
// 0xf41a: WORD 'UNK_0xf41c' codep=0x224c parp=0xf41c
// ================================================

void UNK_0xf41c() // UNK_0xf41c
{
  Push(0x0014);
  Push(cc__5); // 5
  Push(2); // 2
  UNK_0xf3d0(); // UNK_0xf3d0
}


// ================================================
// 0xf428: WORD 'UNK_0xf42a' codep=0x224c parp=0xf42a
// ================================================

void UNK_0xf42a() // UNK_0xf42a
{
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  Push(0x5a77); // pointer to ABLT
  Push(Read16(Pop())); // @
  Push(0xf1d4); // pointer to UNK_0xf1d4
  _eq_(); // =
  if (Pop() == 0) goto label1;
  Push(0); // 0
  Push(0x05dc);
  RRND(); // RRND
  if (Pop() == 0) Push(1); else Push(0); // 0=
  UNK_0xef15(); // UNK_0xef15
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label2;
  Push(cc__4); // 4
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  UNK_0xf2aa(); // UNK_0xf2aa
  DISPLAY(); // DISPLAY
  UNK_0xf33e(); // UNK_0xf33e
  UNK_0xf346(); // UNK_0xf346
  Push(0x0032);
  MS(); // MS
  UNK_0xf2aa(); // UNK_0xf2aa
  DISPLAY(); // DISPLAY
  UNK_0xf33e(); // UNK_0xf33e
  UNK_0xf346(); // UNK_0xf346
  UNK_0xf352(); // UNK_0xf352
  Push(0x0096);
  MS(); // MS
  i++;
  } while(i<imax); // (LOOP) 0xffe0


  label2:
  return;

  label1:
  Push(0xf1be); // pointer to UNK_0xf1be
  _099(); // 099
  Push2Words("NULL");
  Push2Words("NULL");
  UNK_0xf37e(); // UNK_0xf37e
}


// ================================================
// 0xf484: WORD 'UNK_0xf486' codep=0x224c parp=0xf486
// ================================================

void UNK_0xf486() // UNK_0xf486
{
  UNK_0xf0b7(); // UNK_0xf0b7
  UNK_0xf308(); // UNK_0xf308
  UNK_0xefb6(); // UNK_0xefb6
  Pop(); Pop();// 2DROP
  UNK_0xeffe(); // UNK_0xeffe
}


// ================================================
// 0xf492: WORD 'DO-ROOM' codep=0x4b3b parp=0xf49e
// ================================================
// 0xf49e: db 0x07 0x00 0x48 0x3a 0x03 0x00 0x18 0xee 0x04 0x00 0x28 0xee 0x06 0x00 0x48 0xee 0x05 0x00 0x58 0xee 0x02 0x00 0x38 0xee 0x01 0x00 0x68 0xee 0x07 0x00 0x98 0xf1 '  H:      (   H   X   8   h     '

// ================================================
// 0xf4be: WORD 'UNK_0xf4c0' codep=0x224c parp=0xf4c0
// ================================================

void UNK_0xf4c0() // UNK_0xf4c0
{
  Push(0x5a48); // pointer to XBLT
  Push(Read16(Pop())); // @
  Push(0xedf7); // pointer to MANX
  _ex__2(); // !_2
  Push(0xeed0); // pointer to UNK_0xeed0
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  if (Pop() == 0) goto label1;
  Push(0x5456); // pointer to HUB
  _099(); // 099
  Push(0x5ecd); // pointer to ESC-EN
  _099(); // 099

  label1:
  Pop();
  switch(Pop()) // DO-ROOM
  {
  case 3:
    UNK_0xee18(); // UNK_0xee18
    break;
  case 4:
    UNK_0xee28(); // UNK_0xee28
    break;
  case 6:
    UNK_0xee48(); // UNK_0xee48
    break;
  case 5:
    UNK_0xee58(); // UNK_0xee58
    break;
  case 2:
    UNK_0xee38(); // UNK_0xee38
    break;
  case 1:
    UNK_0xee68(); // UNK_0xee68
    break;
  case 7:
    UNK_0xf198(); // UNK_0xf198
    break;
  default:
    NOP(); // NOP
    break;

  }
  Push(0xeed0); // pointer to UNK_0xeed0
  Push(Read16(Pop())); // @
  if (Pop() == 0) return;
  Push(0x5456); // pointer to HUB
  ON_2(); // ON_2
  Push(0x5ecd); // pointer to ESC-EN
  ON_2(); // ON_2
  UNK_0xf486(); // UNK_0xf486
  UNK_0xef23(); // UNK_0xef23
}


// ================================================
// 0xf4f2: WORD 'XWALK' codep=0x4b3b parp=0xf4fc
// ================================================
// 0xf4fc: db 0x02 0x00 0x48 0x3a 0x01 0x00 0x10 0xf4 0xff 0xff 0x1c 0xf4 '  H:        '

// ================================================
// 0xf508: WORD 'WALKIES' codep=0x224c parp=0xf514
// ================================================
// entry

void WALKIES() // WALKIES
{
  Push(cc__5); // 5
  Push(0x5c2c); // pointer to CONTEXT_3
  _ex__2(); // !_2
  UNK_0xf091(); // UNK_0xf091
  UNK_0xef23(); // UNK_0xef23
  UNK_0xf486(); // UNK_0xf486

  label3:
  XYSCAN(); // XYSCAN
  _2DUP(); // 2DUP
  Pop();
  switch(Pop()) // XWALK
  {
  case 1:
    UNK_0xf410(); // UNK_0xf410
    break;
  case 65535:
    UNK_0xf41c(); // UNK_0xf41c
    break;
  default:
    NOP(); // NOP
    break;

  }
  Pop(); // DROP
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  UNK_0xf42a(); // UNK_0xf42a

  label1:
  _ask_TRIG(); // ?TRIG
  if (Pop() == 0) goto label2;
  UNK_0xf4c0(); // UNK_0xf4c0
  _i_KEY(); // 'KEY
  Pop(); // DROP

  label2:
  goto label3;
}


// ================================================
// 0xf544: WORD 'PPIC' codep=0x224c parp=0xf54d
// ================================================
// entry

void PPIC() // PPIC
{
  UNK_0xf091(); // UNK_0xf091
  UNK_0xf486(); // UNK_0xf486
}

// 0xf553: db 0x50 0x4d 0x2d 0x56 0x4f 0x43 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x00 'PM-VOC_______ '
  