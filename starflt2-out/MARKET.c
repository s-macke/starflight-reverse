...
void UNK_0xeada() // UNK_0xeada
{
  ...
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  Push(0x6202); // pointer to SELLING
  Push(Read16(Pop())); // @
...
{
  Push(0x0083);
  Push(0xee83); // pointer to UNK_0xee83
  Push(Read16(Pop())); // @
  Push(cc__7); // 7
  Push(Pop() * Pop()); // *
  _dash_(); // -
...
{
  Push(0x6210); // pointer to BARTERI
  Push(Read16(Pop())); // @
  Push(0x621d); // pointer to ?REPLY
  Push(Read16(Pop())); // @
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) return;
