// ====== OVERLAY 'SEED-OV' ======

#include"interface.h"

// store offset = 0xec10
// overlay size   = 0x0950

// =================================
// =========== DICTIONARY ==========
// =================================
// 1869:      UNK_0xec36  codep:0x2214 parp:0xec36 size:0x0002 C-string:'UNK_0xec36'
// 1870:      UNK_0xec3a  codep:0x2214 parp:0xec3a size:0x0002 C-string:'UNK_0xec3a'
// 1871:      UNK_0xec3e  codep:0x2214 parp:0xec3e size:0x0002 C-string:'UNK_0xec3e'
// 1872:      UNK_0xec42  codep:0x1d29 parp:0xec42 size:0x0002 C-string:'UNK_0xec42'
// 1873:      UNK_0xec46  codep:0x1d29 parp:0xec46 size:0x0002 C-string:'UNK_0xec46'
// 1874:      UNK_0xec4a  codep:0x7394 parp:0xec4a size:0x0016 C-string:'UNK_0xec4a'
// 1875:      UNK_0xec62  codep:0x7394 parp:0xec62 size:0x0006 C-string:'UNK_0xec62'
// 1876:      UNK_0xec6a  codep:0x7394 parp:0xec6a size:0x0006 C-string:'UNK_0xec6a'
// 1877:      UNK_0xec72  codep:0x7394 parp:0xec72 size:0x0016 C-string:'UNK_0xec72'
// 1878:      UNK_0xec8a  codep:0x7394 parp:0xec8a size:0x0006 C-string:'UNK_0xec8a'
// 1879:      UNK_0xec92  codep:0x7394 parp:0xec92 size:0x0006 C-string:'UNK_0xec92'
// 1880:      UNK_0xec9a  codep:0x7420 parp:0xec9a size:0x0003 C-string:'UNK_0xec9a'
// 1881:      UNK_0xec9f  codep:0x7420 parp:0xec9f size:0x0003 C-string:'UNK_0xec9f'
// 1882:      UNK_0xeca4  codep:0x7420 parp:0xeca4 size:0x0003 C-string:'UNK_0xeca4'
// 1883:      UNK_0xeca9  codep:0x7394 parp:0xeca9 size:0x0006 C-string:'UNK_0xeca9'
// 1884:      UNK_0xecb1  codep:0x7394 parp:0xecb1 size:0x0006 C-string:'UNK_0xecb1'
// 1885:      UNK_0xecb9  codep:0x7394 parp:0xecb9 size:0x0006 C-string:'UNK_0xecb9'
// 1886:      UNK_0xecc1  codep:0x7394 parp:0xecc1 size:0x0006 C-string:'UNK_0xecc1'
// 1887:      UNK_0xecc9  codep:0x7420 parp:0xecc9 size:0x0003 C-string:'UNK_0xecc9'
// 1888:      UNK_0xecce  codep:0x7420 parp:0xecce size:0x0003 C-string:'UNK_0xecce'
// 1889:      UNK_0xecd3  codep:0x7420 parp:0xecd3 size:0x0003 C-string:'UNK_0xecd3'
// 1890:      UNK_0xecd8  codep:0x7420 parp:0xecd8 size:0x0003 C-string:'UNK_0xecd8'
// 1891:      UNK_0xecdd  codep:0x7420 parp:0xecdd size:0x000f C-string:'UNK_0xecdd'
// 1892:      UNK_0xecee  codep:0x224c parp:0xecee size:0x0008 C-string:'UNK_0xecee'
// 1893:      UNK_0xecf8  codep:0x224c parp:0xecf8 size:0x001c C-string:'UNK_0xecf8'
// 1894:      UNK_0xed16  codep:0x224c parp:0xed16 size:0x000c C-string:'UNK_0xed16'
// 1895:      UNK_0xed24  codep:0x224c parp:0xed24 size:0x0006 C-string:'UNK_0xed24'
// 1896:      UNK_0xed2c  codep:0x224c parp:0xed2c size:0x0006 C-string:'UNK_0xed2c'
// 1897:      UNK_0xed34  codep:0x224c parp:0xed34 size:0x0010 C-string:'UNK_0xed34'
// 1898:      UNK_0xed46  codep:0x224c parp:0xed46 size:0x000a C-string:'UNK_0xed46'
// 1899:      UNK_0xed52  codep:0x224c parp:0xed52 size:0x001c C-string:'UNK_0xed52'
// 1900:      UNK_0xed70  codep:0x224c parp:0xed70 size:0x001c C-string:'UNK_0xed70'
// 1901:      UNK_0xed8e  codep:0x224c parp:0xed8e size:0x001e C-string:'UNK_0xed8e'
// 1902:      UNK_0xedae  codep:0x224c parp:0xedae size:0x000a C-string:'UNK_0xedae'
// 1903:      UNK_0xedba  codep:0x224c parp:0xedba size:0x000a C-string:'UNK_0xedba'
// 1904:      UNK_0xedc6  codep:0x224c parp:0xedc6 size:0x0010 C-string:'UNK_0xedc6'
// 1905:      UNK_0xedd8  codep:0x224c parp:0xedd8 size:0x001c C-string:'UNK_0xedd8'
// 1906:      UNK_0xedf6  codep:0x224c parp:0xedf6 size:0x0010 C-string:'UNK_0xedf6'
// 1907:      UNK_0xee08  codep:0x224c parp:0xee08 size:0x0010 C-string:'UNK_0xee08'
// 1908:      UNK_0xee1a  codep:0x224c parp:0xee1a size:0x0062 C-string:'UNK_0xee1a'
// 1909:      UNK_0xee7e  codep:0x224c parp:0xee7e size:0x0012 C-string:'UNK_0xee7e'
// 1910:      UNK_0xee92  codep:0x224c parp:0xee92 size:0x002c C-string:'UNK_0xee92'
// 1911:      UNK_0xeec0  codep:0x224c parp:0xeec0 size:0x001c C-string:'UNK_0xeec0'
// 1912:      UNK_0xeede  codep:0x1d29 parp:0xeede size:0x000e C-string:'UNK_0xeede'
// 1913:      UNK_0xeeee  codep:0x224c parp:0xeeee size:0x001a C-string:'UNK_0xeeee'
// 1914:      UNK_0xef0a  codep:0x224c parp:0xef0a size:0x0008 C-string:'UNK_0xef0a'
// 1915:      UNK_0xef14  codep:0x224c parp:0xef14 size:0x0038 C-string:'UNK_0xef14'
// 1916:      UNK_0xef4e  codep:0x224c parp:0xef4e size:0x0044 C-string:'UNK_0xef4e'
// 1917:      UNK_0xef94  codep:0x224c parp:0xef94 size:0x003e C-string:'UNK_0xef94'
// 1918:      UNK_0xefd4  codep:0x224c parp:0xefd4 size:0x0010 C-string:'UNK_0xefd4'
// 1919:      UNK_0xefe6  codep:0x224c parp:0xefe6 size:0x002e C-string:'UNK_0xefe6'
// 1920:      UNK_0xf016  codep:0x224c parp:0xf016 size:0x0012 C-string:'UNK_0xf016'
// 1921:      UNK_0xf02a  codep:0x224c parp:0xf02a size:0x0018 C-string:'UNK_0xf02a'
// 1922:      UNK_0xf044  codep:0x224c parp:0xf044 size:0x004a C-string:'UNK_0xf044'
// 1923:      UNK_0xf090  codep:0x224c parp:0xf090 size:0x0020 C-string:'UNK_0xf090'
// 1924:      UNK_0xf0b2  codep:0x224c parp:0xf0b2 size:0x0026 C-string:'UNK_0xf0b2'
// 1925:      UNK_0xf0da  codep:0x224c parp:0xf0da size:0x0044 C-string:'UNK_0xf0da'
// 1926:      UNK_0xf120  codep:0x224c parp:0xf120 size:0x003c C-string:'UNK_0xf120'
// 1927:      UNK_0xf15e  codep:0x224c parp:0xf15e size:0x0002 C-string:'UNK_0xf15e'
// 1928:      UNK_0xf162  codep:0x224c parp:0xf162 size:0x002c C-string:'UNK_0xf162'
// 1929:      UNK_0xf190  codep:0x224c parp:0xf190 size:0x0010 C-string:'UNK_0xf190'
// 1930:      UNK_0xf1a2  codep:0x224c parp:0xf1a2 size:0x0016 C-string:'UNK_0xf1a2'
// 1931:      UNK_0xf1ba  codep:0x224c parp:0xf1ba size:0x001e C-string:'UNK_0xf1ba'
// 1932:      UNK_0xf1da  codep:0x224c parp:0xf1da size:0x000c C-string:'UNK_0xf1da'
// 1933:      UNK_0xf1e8  codep:0x224c parp:0xf1e8 size:0x000a C-string:'UNK_0xf1e8'
// 1934:      UNK_0xf1f4  codep:0x224c parp:0xf1f4 size:0x0026 C-string:'UNK_0xf1f4'
// 1935:      UNK_0xf21c  codep:0x224c parp:0xf21c size:0x0084 C-string:'UNK_0xf21c'
// 1936:      UNK_0xf2a2  codep:0x224c parp:0xf2a2 size:0x0018 C-string:'UNK_0xf2a2'
// 1937:      UNK_0xf2bc  codep:0x224c parp:0xf2bc size:0x001c C-string:'UNK_0xf2bc'
// 1938:      UNK_0xf2da  codep:0x224c parp:0xf2da size:0x000e C-string:'UNK_0xf2da'
// 1939:      UNK_0xf2ea  codep:0x224c parp:0xf2ea size:0x000e C-string:'UNK_0xf2ea'
// 1940:      UNK_0xf2fa  codep:0x224c parp:0xf2fa size:0x0004 C-string:'UNK_0xf2fa'
// 1941:      UNK_0xf300  codep:0x224c parp:0xf300 size:0x002c C-string:'UNK_0xf300'
// 1942:      UNK_0xf32e  codep:0x224c parp:0xf32e size:0x000a C-string:'UNK_0xf32e'
// 1943:      UNK_0xf33a  codep:0x224c parp:0xf33a size:0x000e C-string:'UNK_0xf33a'
// 1944:      UNK_0xf34a  codep:0x224c parp:0xf34a size:0x0028 C-string:'UNK_0xf34a'
// 1945:      UNK_0xf374  codep:0x224c parp:0xf374 size:0x0042 C-string:'UNK_0xf374'
// 1946:      UNK_0xf3b8  codep:0x224c parp:0xf3b8 size:0x0006 C-string:'UNK_0xf3b8'
// 1947:      UNK_0xf3c0  codep:0x224c parp:0xf3c0 size:0x0012 C-string:'UNK_0xf3c0'
// 1948:      UNK_0xf3d4  codep:0x224c parp:0xf3d4 size:0x0024 C-string:'UNK_0xf3d4'
// 1949:      UNK_0xf3fa  codep:0x224c parp:0xf3fa size:0x0022 C-string:'UNK_0xf3fa'
// 1950:      UNK_0xf41e  codep:0x224c parp:0xf41e size:0x001a C-string:'UNK_0xf41e'
// 1951:      UNK_0xf43a  codep:0x224c parp:0xf43a size:0x0044 C-string:'UNK_0xf43a'
// 1952:      UNK_0xf480  codep:0x224c parp:0xf480 size:0x000c C-string:'UNK_0xf480'
// 1953:      UNK_0xf48e  codep:0x224c parp:0xf48e size:0x0030 C-string:'UNK_0xf48e'
// 1954:      UNK_0xf4c0  codep:0x224c parp:0xf4c0 size:0x0044 C-string:'UNK_0xf4c0'
// 1955:      UNK_0xf506  codep:0x224c parp:0xf506 size:0x001a C-string:'UNK_0xf506'
// 1956:          POPULA  codep:0x224c parp:0xf52b size:0x0035 C-string:'POPULA'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xec42[2] = {0x3a, 0x20}; // UNK_0xec42
unsigned char UNK_0xec46[2] = {0x01, 0x00}; // UNK_0xec46
unsigned char UNK_0xeede[14] = {0x05, 0x41, 0x4c, 0x4c, 0x4f, 0x54, 0x20, 0x3b, 0x2d, 0x12, 0x41, 0x5c, 0xae, 0x0b}; // UNK_0xeede

const unsigned short int cc_UNK_0xec36 = 0x001f; // UNK_0xec36
const unsigned short int cc_UNK_0xec3a = 0x0015; // UNK_0xec3a
const unsigned short int cc_UNK_0xec3e = 0x0021; // UNK_0xec3e


// 0xec22: db 0x95 0x00 0x14 0x22 0x20 0x00 0x14 0x22 0x43 0x00 0x14 0x22 0x44 0x00 0x14 0x22 0x1c 0x00 '   "   "C  "D  "  '

// ================================================
// 0xec34: WORD 'UNK_0xec36' codep=0x2214 parp=0xec36
// ================================================
// 0xec36: db 0x1f 0x00 '  '

// ================================================
// 0xec38: WORD 'UNK_0xec3a' codep=0x2214 parp=0xec3a
// ================================================
// 0xec3a: db 0x15 0x00 '  '

// ================================================
// 0xec3c: WORD 'UNK_0xec3e' codep=0x2214 parp=0xec3e
// ================================================
// 0xec3e: db 0x21 0x00 '! '

// ================================================
// 0xec40: WORD 'UNK_0xec42' codep=0x1d29 parp=0xec42
// ================================================
// 0xec42: db 0x3a 0x20 ': '

// ================================================
// 0xec44: WORD 'UNK_0xec46' codep=0x1d29 parp=0xec46
// ================================================
// 0xec46: db 0x01 0x00 '  '

// ================================================
// 0xec48: WORD 'UNK_0xec4a' codep=0x7394 parp=0xec4a
// ================================================
// 0xec4a: db 0x20 0x09 0x01 0x17 0x49 0x6c 0x94 0x73 0x20 0x0a 0x01 0x17 0x49 0x6c 0x94 0x73 0x20 0x0b 0x01 0x17 0x49 0x6c '    Il s    Il s    Il'

// ================================================
// 0xec60: WORD 'UNK_0xec62' codep=0x7394 parp=0xec62
// ================================================
// 0xec62: db 0x20 0x0c 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xec68: WORD 'UNK_0xec6a' codep=0x7394 parp=0xec6a
// ================================================
// 0xec6a: db 0x20 0x0d 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xec70: WORD 'UNK_0xec72' codep=0x7394 parp=0xec72
// ================================================
// 0xec72: db 0x20 0x0e 0x01 0x17 0x49 0x6c 0x94 0x73 0x20 0x0f 0x01 0x17 0x49 0x6c 0x94 0x73 0x20 0x10 0x01 0x17 0x49 0x6c '    Il s    Il s    Il'

// ================================================
// 0xec88: WORD 'UNK_0xec8a' codep=0x7394 parp=0xec8a
// ================================================
// 0xec8a: db 0x20 0x04 0x01 0x17 0x49 0x6c '    Il'

// ================================================
// 0xec90: WORD 'UNK_0xec92' codep=0x7394 parp=0xec92
// ================================================
// 0xec92: db 0x15 0x13 0x01 0x32 0xd1 0x6a '   2 j'

// ================================================
// 0xec98: WORD 'UNK_0xec9a' codep=0x7420 parp=0xec9a
// ================================================
// 0xec9a: db 0x15 0x0b 0x01 '   '

// ================================================
// 0xec9d: WORD 'UNK_0xec9f' codep=0x7420 parp=0xec9f
// ================================================
// 0xec9f: db 0x15 0x0c 0x01 '   '

// ================================================
// 0xeca2: WORD 'UNK_0xeca4' codep=0x7420 parp=0xeca4
// ================================================
// 0xeca4: db 0x15 0x11 0x03 '   '

// ================================================
// 0xeca7: WORD 'UNK_0xeca9' codep=0x7394 parp=0xeca9
// ================================================
// 0xeca9: db 0x15 0x16 0x01 0x32 0xd1 0x6a '   2 j'

// ================================================
// 0xecaf: WORD 'UNK_0xecb1' codep=0x7394 parp=0xecb1
// ================================================
// 0xecb1: db 0x15 0x14 0x01 0x32 0xd1 0x6a '   2 j'

// ================================================
// 0xecb7: WORD 'UNK_0xecb9' codep=0x7394 parp=0xecb9
// ================================================
// 0xecb9: db 0x44 0x1c 0x01 0x22 0x07 0x6f 'D  " o'

// ================================================
// 0xecbf: WORD 'UNK_0xecc1' codep=0x7394 parp=0xecc1
// ================================================
// 0xecc1: db 0x44 0x1b 0x01 0x22 0x07 0x6f 'D  " o'

// ================================================
// 0xecc7: WORD 'UNK_0xecc9' codep=0x7420 parp=0xecc9
// ================================================
// 0xecc9: db 0x44 0x15 0x01 'D  '

// ================================================
// 0xeccc: WORD 'UNK_0xecce' codep=0x7420 parp=0xecce
// ================================================
// 0xecce: db 0x44 0x16 0x01 'D  '

// ================================================
// 0xecd1: WORD 'UNK_0xecd3' codep=0x7420 parp=0xecd3
// ================================================
// 0xecd3: db 0x44 0x17 0x01 'D  '

// ================================================
// 0xecd6: WORD 'UNK_0xecd8' codep=0x7420 parp=0xecd8
// ================================================
// 0xecd8: db 0x44 0x18 0x01 'D  '

// ================================================
// 0xecdb: WORD 'UNK_0xecdd' codep=0x7420 parp=0xecdd
// ================================================
// 0xecdd: db 0x44 0x19 0x01 0x4c 0x22 0x20 0x0f 0x5d 0x17 0x00 0x01 0xc4 0x4b 0x90 0x16 'D  L"  ]    K  '

// ================================================
// 0xecec: WORD 'UNK_0xecee' codep=0x224c parp=0xecee
// ================================================

void UNK_0xecee() // UNK_0xecee
{
  Push(0); // 0
  Push(2); // 2
  RRND(); // RRND
}


// ================================================
// 0xecf6: WORD 'UNK_0xecf8' codep=0x224c parp=0xecf8
// ================================================

void UNK_0xecf8() // UNK_0xecf8
{
  Push(1); // 1
  Push(0x0064);
  RRND(); // RRND
}

// 0xed02: db 0x4c 0x22 0xec 0xec 0xfa 0x15 0x08 0x00 0xb8 0x3b 0x60 0x16 0x04 0x00 0x2e 0x0f 0x90 0x16 'L"       ;`   .   '

// ================================================
// 0xed14: WORD 'UNK_0xed16' codep=0x224c parp=0xed16
// ================================================

void UNK_0xed16() // UNK_0xed16
{
  _2DUP(); // 2DUP
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label945;

  _2DUP(); // 2DUP

  label945:
}


// ================================================
// 0xed22: WORD 'UNK_0xed24' codep=0x224c parp=0xed24
// ================================================

void UNK_0xed24() // UNK_0xed24
{
  Push(pp__ro_TRADER); // (TRADER
  _at__gt_C_plus_S(); // @>C+S
}


// ================================================
// 0xed2a: WORD 'UNK_0xed2c' codep=0x224c parp=0xed2c
// ================================================

void UNK_0xed2c() // UNK_0xed2c
{
  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
}


// ================================================
// 0xed32: WORD 'UNK_0xed34' codep=0x224c parp=0xed34
// ================================================

void UNK_0xed34() // UNK_0xed34
{
  UNK_0xed16(); // UNK_0xed16
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label946;

  Push(pp_THIS_minus_RE); // THIS-RE
  _1_dot_5_at_(); // 1.5@
  IINSERT(); // IINSERT

  label946:
}


// ================================================
// 0xed44: WORD 'UNK_0xed46' codep=0x224c parp=0xed46
// ================================================

void UNK_0xed46() // UNK_0xed46
{
  UNK_0xed24(); // UNK_0xed24
  UNK_0xecb1(); // UNK_0xecb1
  C_at_(); // C@
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xed50: WORD 'UNK_0xed52' codep=0x224c parp=0xed52
// ================================================

void UNK_0xed52() // UNK_0xed52
{
  Push(pp_XABS); // XABS
  Push(Read16(Pop())); // @
  Push(0x0012);
  _slash_(); // /
  Push(0x0012);
  Push(Pop() * Pop()); // *
  DUP(); // DUP
  Push(0x0013);
  Push(Pop() + Pop()); // +
  RRND(); // RRND
}


// ================================================
// 0xed6e: WORD 'UNK_0xed70' codep=0x224c parp=0xed70
// ================================================

void UNK_0xed70() // UNK_0xed70
{
  Push(pp_YABS); // YABS
  Push(Read16(Pop())); // @
  Push(0x001e);
  _slash_(); // /
  Push(0x001e);
  Push(Pop() * Pop()); // *
  DUP(); // DUP
  Push(0x001f);
  Push(Pop() + Pop()); // +
  RRND(); // RRND
}


// ================================================
// 0xed8c: WORD 'UNK_0xed8e' codep=0x224c parp=0xed8e
// ================================================

void UNK_0xed8e() // UNK_0xed8e
{
  UNK_0xed52(); // UNK_0xed52
  UNK_0xed70(); // UNK_0xed70
}

// 0xed94: db 0x4c 0x22 0xaf 0x64 0x2f 0x7a 0x78 0xec 0x92 0x0c 0x80 0xec 0x92 0x0c 0x7f 0x0e 0x92 0x0f 0xe7 0x0f 0xdf 0x79 0x90 0x16 'L" d/zx             y  '

// ================================================
// 0xedac: WORD 'UNK_0xedae' codep=0x224c parp=0xedae
// ================================================

void UNK_0xedae() // UNK_0xedae
{
  INST_minus_X(); // INST-X
  Push(Read16(Pop())); // @
  INST_minus_Y(); // INST-Y
  Push(Read16(Pop())); // @
}


// ================================================
// 0xedb8: WORD 'UNK_0xedba' codep=0x224c parp=0xedba
// ================================================

void UNK_0xedba() // UNK_0xedba
{
  INST_minus_Y(); // INST-Y
  _ex_(); // !
  INST_minus_X(); // INST-X
  _ex_(); // !
}


// ================================================
// 0xedc4: WORD 'UNK_0xedc6' codep=0x224c parp=0xedc6
// ================================================

void UNK_0xedc6() // UNK_0xedc6
{
  _2DUP(); // 2DUP
  _ask_INVIS(); // ?INVIS
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(pp_MOVED); // MOVED
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() | Pop()); // OR
}


// ================================================
// 0xedd6: WORD 'UNK_0xedd8' codep=0x224c parp=0xedd8
// ================================================

void UNK_0xedd8() // UNK_0xedd8
{
  Push(0x6a82);
  SETLARR(); // SETLARR
  SWAP(); // SWAP
  Push(pp_XCON); // XCON
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  _minus_(); // -
  SWAP(); // SWAP
  Push(pp_YCON); // YCON
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  _minus_(); // -
}


// ================================================
// 0xedf4: WORD 'UNK_0xedf6' codep=0x224c parp=0xedf6
// ================================================

void UNK_0xedf6() // UNK_0xedf6
{
  _2DUP(); // 2DUP
  UNK_0xedd8(); // UNK_0xedd8
  ACELLAD(); // ACELLAD
  A_at_(); // A@
  Push(0x001f);
  _gt_(); // >
}


// ================================================
// 0xee06: WORD 'UNK_0xee08' codep=0x224c parp=0xee08
// ================================================

void UNK_0xee08() // UNK_0xee08
{
  _2DUP(); // 2DUP
  UNK_0xedd8(); // UNK_0xedd8
  ACELLAD(); // ACELLAD
  A_at_(); // A@
  Push(0x0040);
  _st_(); // <
}


// ================================================
// 0xee18: WORD 'UNK_0xee1a' codep=0x224c parp=0xee1a
// ================================================

void UNK_0xee1a() // UNK_0xee1a
{
  _2DUP(); // 2DUP
  UNK_0xedd8(); // UNK_0xedd8
  ACELLAD(); // ACELLAD
  A_at_(); // A@
  Push(0x003f);
  _gt_(); // >
}

// 0xee2a: db 0x4c 0x22 0x3a 0x5f 0xae 0x0b 0x41 0x0e 0x5d 0x17 0xe0 0x01 0x2d 0x12 0xfa 0x15 0x0a 0x00 0x5d 0x17 0xc0 0x03 0xf2 0x0e 0x92 0x0f 0x5d 0x17 0xe0 0x01 0x94 0xed 0xb3 0x0e 0xf2 0x0e 0x4c 0x0f 0xe7 0x0f 0xb3 0x0e 0xf2 0x0e 0x4c 0x0f 0x72 0x0f 0x90 0x16 0x4c 0x22 0x22 0x75 0x5d 0x17 0x29 0x00 0x5f 0x12 0xfa 0x15 0x12 0x00 0x34 0x75 0x41 0x0e 0x7f 0x3b 0x46 0x12 0xfa 0x15 0x04 0x00 0x15 0x10 0xf8 0x74 0x90 0x16 'L":_  A ]   -     ]       ]         L       L r   L""u] ) _     4uA ;F        t  '

// ================================================
// 0xee7c: WORD 'UNK_0xee7e' codep=0x224c parp=0xee7e
// ================================================

void UNK_0xee7e() // UNK_0xee7e
{
  Push(pp_THIS_minus_RE); // THIS-RE
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN
  Push(0xee5e);
  ALL(); // ALL
  Push(2); // 2
  _star_CLOSE(); // *CLOSE
}


// ================================================
// 0xee90: WORD 'UNK_0xee92' codep=0x224c parp=0xee92
// ================================================

void UNK_0xee92() // UNK_0xee92
{
  Push(cc_TRUE); // TRUE
  _gt_R(); // >R
  CI_i_(); // CI'
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN

  label992:
  _2DUP(); // 2DUP
  UNK_0xedae(); // UNK_0xedae
  D_eq_(); // D=
  if (Pop() == 0) goto label991;

  R_gt_(); // R>
  Pop(); // DROP
  Push(cc_FALSE); // FALSE
  _gt_R(); // >R

  label991:
  INEXT(); // INEXT
  _ask_FIRST(); // ?FIRST
  if (Pop() == 0) goto label992;

  Push(2); // 2
  _star_CLOSE(); // *CLOSE
  R_gt_(); // R>
}


// ================================================
// 0xeebe: WORD 'UNK_0xeec0' codep=0x224c parp=0xeec0
// ================================================

void UNK_0xeec0() // UNK_0xeec0
{
  UNK_0xedf6(); // UNK_0xedf6
  _gt_R(); // >R
  UNK_0xedc6(); // UNK_0xedc6
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  _gt_R(); // >R
  UNK_0xedf6(); // UNK_0xedf6
  Push(pp_MOVED); // MOVED
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  R_gt_(); // R>
  Push(Pop() | Pop()); // OR
}


// ================================================
// 0xeedc: WORD 'UNK_0xeede' codep=0x1d29 parp=0xeede
// ================================================
// 0xeede: db 0x05 0x41 0x4c 0x4c 0x4f 0x54 0x20 0x3b 0x2d 0x12 0x41 0x5c 0xae 0x0b ' ALLOT ;- A\  '

// ================================================
// 0xeeec: WORD 'UNK_0xeeee' codep=0x224c parp=0xeeee
// ================================================

void UNK_0xeeee() // UNK_0xeeee
{
  UNK_0xed2c(); // UNK_0xed2c
  UNK_0xec8a(); // UNK_0xec8a
  C_at_(); // C@
  ICLOSE(); // ICLOSE
  Push(0x0059);
  SWAP(); // SWAP
  _at_RECORD(); // @RECORD
  Push(pp_UNK_0xeede); // UNK_0xeede
  Push(0x000e);
  CMOVE(); // CMOVE
}


// ================================================
// 0xef08: WORD 'UNK_0xef0a' codep=0x224c parp=0xef0a
// ================================================

void UNK_0xef0a() // UNK_0xef0a
{
  Push(cc__minus_2); // -2
  Push(cc__3); // 3
  RRND(); // RRND
}


// ================================================
// 0xef12: WORD 'UNK_0xef14' codep=0x224c parp=0xef14
// ================================================

void UNK_0xef14() // UNK_0xef14
{
  UNK_0xedba(); // UNK_0xedba
  UNK_0xecb9(); // UNK_0xecb9
  C_at_(); // C@
  INST_minus_QT(); // INST-QT
  _ex_(); // !
  Push(1); // 1
  UNK_0xecdd(); // UNK_0xecdd
  C_ex_(); // C!
  Push(0); // 0
  UNK_0xecd8(); // UNK_0xecd8
  C_ex_(); // C!
  Push(1); // 1
  UNK_0xecd3(); // UNK_0xecd3
  C_ex_(); // C!
  UNK_0xecc1(); // UNK_0xecc1
  C_at_(); // C@
  Push(1); // 1
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label989;

  UNK_0xef0a(); // UNK_0xef0a
  UNK_0xecc9(); // UNK_0xecc9
  C_ex_(); // C!
  UNK_0xef0a(); // UNK_0xef0a
  UNK_0xecce(); // UNK_0xecce
  C_ex_(); // C!

  label989:
}


// ================================================
// 0xef4c: WORD 'UNK_0xef4e' codep=0x224c parp=0xef4e
// ================================================

void UNK_0xef4e() // UNK_0xef4e
{
  Push(0x000d);
  _slash_(); // /
  DUP(); // DUP
  Push(pp_UNK_0xeede); // UNK_0xeede
  Push(Pop() + Pop()); // +
  C_at_(); // C@
  _ask_DUP(); // ?DUP
  if (Pop() == 0) goto label983;

  SWAP(); // SWAP
  Push(cc__7); // 7
  Push(Pop() + Pop()); // +
  Push(pp_UNK_0xeede); // UNK_0xeede
  Push(Pop() + Pop()); // +
  C_at_(); // C@
  UNK_0xecf8(); // UNK_0xecf8
  _gt_(); // >
  if (Pop() == 0) goto label984;

  Push(0x0044);
  SWAP(); // SWAP
  ICREATE(); // ICREATE
  Push(1); // 1
  goto label986;

  label984:
  Pop(); // DROP
  Push(0); // 0

  label986:
  goto label985;

  label983:
  Pop(); // DROP
  Push(0); // 0

  label985:
}


// ================================================
// 0xef92: WORD 'UNK_0xef94' codep=0x224c parp=0xef94
// ================================================

void UNK_0xef94() // UNK_0xef94
{
  UNK_0xeeee(); // UNK_0xeeee
  Push(0x005b);
  Push(0); // 0

  do // (DO)
  {
  UNK_0xed8e(); // UNK_0xed8e
  UNK_0xeec0(); // UNK_0xeec0
  _gt_R(); // >R
  UNK_0xee08(); // UNK_0xee08
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label982;

  I(); // I
  UNK_0xef4e(); // UNK_0xef4e
  if (Pop() == 0) goto label987;

  _2DUP(); // 2DUP
  _gt_C_plus_S(); // >C+S
  UNK_0xed34(); // UNK_0xed34
  UNK_0xef14(); // UNK_0xef14
  ICLOSE(); // ICLOSE
  goto label990;

  label987:
  Pop(); Pop();// 2DROP

  label990:
  goto label988;

  label982:
  Pop(); Pop();// 2DROP

  label988:

  } while(...); // (LOOP) 0xffd0
}


// ================================================
// 0xefd2: WORD 'UNK_0xefd4' codep=0x224c parp=0xefd4
// ================================================

void UNK_0xefd4() // UNK_0xefd4
{
  UNK_0xed2c(); // UNK_0xed2c
  UNK_0xec8a(); // UNK_0xec8a
  C_at_(); // C@
  ICLOSE(); // ICLOSE
  if (Pop() == 0) goto label981;

  UNK_0xef94(); // UNK_0xef94

  label981:
}


// ================================================
// 0xefe4: WORD 'UNK_0xefe6' codep=0x224c parp=0xefe6
// ================================================

void UNK_0xefe6() // UNK_0xefe6
{
  UNK_0xec62(); // UNK_0xec62
  C_at_(); // C@
  Push(cc__6); // 6
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label975;


  label978:
  Push(1); // 1
  Push(0x0017);
  RRND(); // RRND
  DUP(); // DUP
  Push(cc__6); // 6
  _eq_(); // =
  if (Pop() == 0) goto label976;

  Pop(); // DROP
  goto label978;

  label976:
  goto label977;

  label975:
  Push(cc__6); // 6

  label977:
}


// ================================================
// 0xf014: WORD 'UNK_0xf016' codep=0x224c parp=0xf016
// ================================================

void UNK_0xf016() // UNK_0xf016
{
  Push(1); // 1
  UNK_0xecee(); // UNK_0xecee
  if (Pop() == 0) goto label949;

  _minus_(); // -
  goto label950;

  label949:
  Push(Pop() + Pop()); // +

  label950:
}


// ================================================
// 0xf028: WORD 'UNK_0xf02a' codep=0x224c parp=0xf02a
// ================================================

void UNK_0xf02a() // UNK_0xf02a
{
  Push(pp_PAST); // PAST
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label938;

  Push(cc__minus_1); // -1
  goto label939;

  label938:
  Push(pp_STARDAT); // STARDAT
  Push(Read16(Pop())); // @

  label939:
  INST_minus_DA(); // INST-DA
  _ex_(); // !
}


// ================================================
// 0xf042: WORD 'UNK_0xf044' codep=0x224c parp=0xf044
// ================================================

void UNK_0xf044() // UNK_0xf044
{
  Push(0); // 0
  Push(0x0064);
  RRND(); // RRND
  DUP(); // DUP
  Push(0x0032);
  _st_(); // <
  if (Pop() == 0) goto label972;

  Pop(); // DROP
  UNK_0xec62(); // UNK_0xec62
  C_at_(); // C@
  goto label979;

  label972:
  DUP(); // DUP
  Push(0x004b);
  _st_(); // <
  if (Pop() == 0) goto label973;

  Pop(); // DROP
  UNK_0xec6a(); // UNK_0xec6a
  C_at_(); // C@
  goto label979;

  label973:
  Push(0x005a);
  _st_(); // <
  if (Pop() == 0) goto label974;

  UNK_0xec72(); // UNK_0xec72
  C_at_(); // C@
  goto label979;

  label974:
  UNK_0xefe6(); // UNK_0xefe6

  label979:
}


// ================================================
// 0xf08e: WORD 'UNK_0xf090' codep=0x224c parp=0xf090
// ================================================

void UNK_0xf090() // UNK_0xf090
{
  Push(0x001a);
  SWAP(); // SWAP
  ICREATE(); // ICREATE
  _2DUP(); // 2DUP
  UNK_0xed34(); // UNK_0xed34
  _gt_C_plus_S(); // >C+S
  UNK_0xedba(); // UNK_0xedba
  Push(0x000a);
  Push(0x0064);
  RRND(); // RRND
  INST_minus_QT(); // INST-QT
  _ex_(); // !
}


// ================================================
// 0xf0b0: WORD 'UNK_0xf0b2' codep=0x224c parp=0xf0b2
// ================================================

void UNK_0xf0b2() // UNK_0xf0b2
{
  Push(0); // 0
  Push(pp_UNK_0xec46); // UNK_0xec46
  Push(Read16(Pop())); // @
  RRND(); // RRND
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label948;

  UNK_0xf016(); // UNK_0xf016
  SWAP(); // SWAP
  UNK_0xf016(); // UNK_0xf016
  SWAP(); // SWAP
  Push(0x0017);
  UNK_0xf090(); // UNK_0xf090
  ICLOSE(); // ICLOSE
  goto label951;

  label948:
  Pop(); Pop();// 2DROP

  label951:
}


// ================================================
// 0xf0d8: WORD 'UNK_0xf0da' codep=0x224c parp=0xf0da
// ================================================

void UNK_0xf0da() // UNK_0xf0da
{
  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
  UNK_0xec4a(); // UNK_0xec4a
  C_at_(); // C@
  _ask_DUP(); // ?DUP
  if (Pop() == 0) goto label970;

  Push(0x000a);
  _slash_(); // /
  Push(0); // 0
  SWAP(); // SWAP
  RRND(); // RRND
  Push(0); // 0

  do // (DO)
  {
  UNK_0xed52(); // UNK_0xed52
  UNK_0xed70(); // UNK_0xed70
  UNK_0xee1a(); // UNK_0xee1a
  _gt_R(); // >R
  UNK_0xedc6(); // UNK_0xedc6
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label971;

  UNK_0xf044(); // UNK_0xf044
  UNK_0xf090(); // UNK_0xf090
  ICLOSE(); // ICLOSE
  goto label980;

  label971:
  Pop(); Pop();// 2DROP

  label980:

  } while(...); // (LOOP) 0xffe0

  label970:
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xf11e: WORD 'UNK_0xf120' codep=0x224c parp=0xf120
// ================================================

void UNK_0xf120() // UNK_0xf120
{
  Push(pp_THIS_minus_RE); // THIS-RE
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN
  Push(0); // 0
  _gt_R(); // >R

  label942:
  Push(0x0029);
  Push(0); // 0
  IFIND(); // IFIND
  R_at_(); // R@
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label940;

  _2DUP(); // 2DUP
  UNK_0xedae(); // UNK_0xedae
  D_eq_(); // D=
  if (Pop() == 0) goto label941;

  R_gt_(); // R>
  Pop(); // DROP
  Push(1); // 1
  _gt_R(); // >R

  label941:
  INEXT(); // INEXT
  goto label942;

  label940:
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
  R_gt_(); // R>
}


// ================================================
// 0xf15c: WORD 'UNK_0xf15e' codep=0x224c parp=0xf15e
// ================================================

void UNK_0xf15e() // UNK_0xf15e
{
}


// ================================================
// 0xf160: WORD 'UNK_0xf162' codep=0x224c parp=0xf162
// ================================================

void UNK_0xf162() // UNK_0xf162
{
  UNK_0xed8e(); // UNK_0xed8e
  UNK_0xedf6(); // UNK_0xedf6
  DUP(); // DUP
  _gt_R(); // >R
  if (Pop() == 0) goto label961;

  ROT(); // ROT
  Push(0x0029);
  SWAP(); // SWAP
  ICREATE(); // ICREATE
  _2DUP(); // 2DUP
  UNK_0xed34(); // UNK_0xed34
  _gt_C_plus_S(); // >C+S
  _2DUP(); // 2DUP
  UNK_0xedba(); // UNK_0xedba
  ICLOSE(); // ICLOSE
  goto label962;

  label961:
  Pop(); // DROP

  label962:
  R_gt_(); // R>
}


// ================================================
// 0xf18e: WORD 'UNK_0xf190' codep=0x224c parp=0xf190
// ================================================

void UNK_0xf190() // UNK_0xf190
{
  _at_INST_minus_C(); // @INST-C
  Push(0x001b);
  _eq_(); // =
  if (Pop() == 0) goto label937;

  UNK_0xf02a(); // UNK_0xf02a

  label937:
}


// ================================================
// 0xf1a0: WORD 'UNK_0xf1a2' codep=0x224c parp=0xf1a2
// ================================================

void UNK_0xf1a2() // UNK_0xf1a2
{
  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
  Push(0x0027);
  _eq_SPECIE(); // =SPECIE
  Push(0x003a);
  _eq_SPECIE(); // =SPECIE
  Push(Pop() | Pop()); // OR
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xf1b8: WORD 'UNK_0xf1ba' codep=0x224c parp=0xf1ba
// ================================================

void UNK_0xf1ba() // UNK_0xf1ba
{
  Push(0x001e);
  _slash_(); // /
  Push(0x001e);
  Push(Pop() * Pop()); // *
  SWAP(); // SWAP
  Push(0x0012);
  _slash_(); // /
  Push(0x0012);
  Push(Pop() * Pop()); // *
  SWAP(); // SWAP
}


// ================================================
// 0xf1d8: WORD 'UNK_0xf1da' codep=0x224c parp=0xf1da
// ================================================

void UNK_0xf1da() // UNK_0xf1da
{
  Push(pp_THIS_minus_RE); // THIS-RE
  _at__gt_C_plus_S(); // @>C+S
  UNK_0xedae(); // UNK_0xedae
  UNK_0xf1ba(); // UNK_0xf1ba
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xf1e6: WORD 'UNK_0xf1e8' codep=0x224c parp=0xf1e8
// ================================================

void UNK_0xf1e8() // UNK_0xf1e8
{
  UNK_0xedae(); // UNK_0xedae
  UNK_0xf1ba(); // UNK_0xf1ba
  _2OVER(); // 2OVER
  D_eq_(); // D=
}


// ================================================
// 0xf1f2: WORD 'UNK_0xf1f4' codep=0x224c parp=0xf1f4
// ================================================

void UNK_0xf1f4() // UNK_0xf1f4
{
  Push(0x001c);
  _eq_(); // =
  if (Pop() == 0) goto label944;

  Push(cc__3); // 3
  goto label947;

  label944:
  Push(cc__3); // 3

  label947:
  Push(0x0029);
  SWAP(); // SWAP
  ICREATE(); // ICREATE
  _2DUP(); // 2DUP
  UNK_0xed34(); // UNK_0xed34
  _gt_C_plus_S(); // >C+S
  UNK_0xedba(); // UNK_0xedba
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xf21a: WORD 'UNK_0xf21c' codep=0x224c parp=0xf21c
// ================================================

void UNK_0xf21c() // UNK_0xf21c
{
  _gt_R(); // >R
  Push(1); // 1
  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN

  label934:
  _ask_LAST(); // ?LAST
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label932;

  R_at_(); // R@
  Push(0); // 0
  IFIND(); // IFIND
  DUP(); // DUP
  _gt_R(); // >R
  if (Pop() == 0) goto label933;

  UNK_0xf1da(); // UNK_0xf1da
  UNK_0xf1e8(); // UNK_0xf1e8
  if (Pop() == 0) goto label935;

  UNK_0xedae(); // UNK_0xedae
  Push(0x0029);
  Push(cc__4); // 4
  _ask_CLASS_slash_(); // ?CLASS/
  _gt_R(); // >R
  Pop(); Pop();// 2DROP
  R_gt_(); // R>
  if (Pop() == 0) goto label936;

  _2DUP(); // 2DUP
  UNK_0xf0b2(); // UNK_0xf0b2

  label936:
  UNK_0xf190(); // UNK_0xf190
  IEXTRAC(); // IEXTRAC
  IPREV(); // IPREV
  Push(pp_THIS_minus_RE); // THIS-RE
  _1_dot_5_at_(); // 1.5@
  IINSERT(); // IINSERT
  I_i_(); // I'
  Push(0x0029);
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  _gt_R(); // >R
  UNK_0xf120(); // UNK_0xf120
  if (Pop() == 0) Push(1); else Push(0); // NOT
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label943;

  I_i_(); // I'
  UNK_0xf1f4(); // UNK_0xf1f4
  goto label935;

  label943:
  Pop(); Pop();// 2DROP

  label935:
  Pop(); Pop();// 2DROP

  label933:
  INEXT(); // INEXT
  R_gt_(); // R>
  goto label934;

  label932:
  ICLOSE(); // ICLOSE
  ICLOSE(); // ICLOSE
  R_gt_(); // R>
  Pop(); // DROP
}


// ================================================
// 0xf2a0: WORD 'UNK_0xf2a2' codep=0x224c parp=0xf2a2
// ================================================

void UNK_0xf2a2() // UNK_0xf2a2
{
  Push(pp__ro_SYSTEM); // (SYSTEM
  _1_dot_5_at_(); // 1.5@
  _star_GD(); // *GD
  D_eq_(); // D=
  if (Pop() == 0) goto label929;

  Push(pp_PAST); // PAST
  Push(Read16(Pop())); // @
  goto label930;

  label929:
  Push(cc_TRUE); // TRUE

  label930:
}


// ================================================
// 0xf2ba: WORD 'UNK_0xf2bc' codep=0x224c parp=0xf2bc
// ================================================

void UNK_0xf2bc() // UNK_0xf2bc
{
  UNK_0xf2a2(); // UNK_0xf2a2
  UNK_0xf1a2(); // UNK_0xf1a2
  if (Pop() == 0) goto label953;

  Push(pp_PAST); // PAST
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND

  label953:
  if (Pop() == 0) goto label954;

  Push(0x001b);
  UNK_0xf21c(); // UNK_0xf21c

  label954:
}


// ================================================
// 0xf2d8: WORD 'UNK_0xf2da' codep=0x224c parp=0xf2da
// ================================================

void UNK_0xf2da() // UNK_0xf2da
{
  UNK_0xf2a2(); // UNK_0xf2a2
  if (Pop() == 0) goto label952;

  Push(0x001c);
  UNK_0xf21c(); // UNK_0xf21c

  label952:
}


// ================================================
// 0xf2e8: WORD 'UNK_0xf2ea' codep=0x224c parp=0xf2ea
// ================================================

void UNK_0xf2ea() // UNK_0xf2ea
{
  UNK_0xf2a2(); // UNK_0xf2a2
  if (Pop() == 0) goto label931;

  Push(0x0029);
  UNK_0xf21c(); // UNK_0xf21c

  label931:
}


// ================================================
// 0xf2f8: WORD 'UNK_0xf2fa' codep=0x224c parp=0xf2fa
// ================================================

void UNK_0xf2fa() // UNK_0xf2fa
{
  Push(cc_TRUE); // TRUE
}


// ================================================
// 0xf2fe: WORD 'UNK_0xf300' codep=0x224c parp=0xf300
// ================================================

void UNK_0xf300() // UNK_0xf300
{
  _star_ARTH(); // *ARTH
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN
  Push(0x001b);
  Push(0); // 0
  IFIND(); // IFIND
  UNK_0xf1a2(); // UNK_0xf1a2
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label965;

  UNK_0xedba(); // UNK_0xedba
  IEXTRAC(); // IEXTRAC
  UNK_0xed34(); // UNK_0xed34
  goto label966;

  label965:
  Pop(); Pop();// 2DROP

  label966:
  UNK_0xf02a(); // UNK_0xf02a
  Push(2); // 2
  _star_CLOSE(); // *CLOSE
}


// ================================================
// 0xf32c: WORD 'UNK_0xf32e' codep=0x224c parp=0xf32e
// ================================================

void UNK_0xf32e() // UNK_0xf32e
{
  UNK_0xf016(); // UNK_0xf016
  SWAP(); // SWAP
  UNK_0xf016(); // UNK_0xf016
  SWAP(); // SWAP
}


// ================================================
// 0xf338: WORD 'UNK_0xf33a' codep=0x224c parp=0xf33a
// ================================================

void UNK_0xf33a() // UNK_0xf33a
{
  UNK_0xf2fa(); // UNK_0xf2fa
  if (Pop() == 0) goto label964;

  _2DUP(); // 2DUP
  UNK_0xf300(); // UNK_0xf300
  UNK_0xee7e(); // UNK_0xee7e

  label964:
}


// ================================================
// 0xf348: WORD 'UNK_0xf34a' codep=0x224c parp=0xf34a
// ================================================

void UNK_0xf34a() // UNK_0xf34a
{
  Push(0); // 0
  Push(0x0140);
  ROT(); // ROT
  IFLD_at_(); // IFLD@
  DUP(); // DUP
  if (Pop() == 0) goto label956;

  _minus_(); // -
  Push(0x0040);
  _slash_(); // /
  RRND(); // RRND
  if (Pop() == 0) Push(1); else Push(0); // 0=
  goto label957;

  label956:
  _gt_R(); // >R
  Pop(); Pop();// 2DROP
  R_gt_(); // R>

  label957:
}


// ================================================
// 0xf372: WORD 'UNK_0xf374' codep=0x224c parp=0xf374
// ================================================

void UNK_0xf374() // UNK_0xf374
{
  Push(pp__ro_PLANET); // (PLANET
  _at__gt_C_plus_S(); // @>C+S
  Push(0xec52);
  UNK_0xf34a(); // UNK_0xf34a
  if (Pop() == 0) goto label958;

  Push(2); // 2
  UNK_0xf162(); // UNK_0xf162
  if (Pop() == 0) goto label968;

  UNK_0xf15e(); // UNK_0xf15e

  label968:
  UNK_0xf0b2(); // UNK_0xf0b2

  label958:
  Push(0xec5a);
  UNK_0xf34a(); // UNK_0xf34a
  if (Pop() == 0) goto label959;

  UNK_0xf1a2(); // UNK_0xf1a2
  if (Pop() == 0) goto label960;

  Push(cc__6); // 6
  goto label967;

  label960:
  Push(1); // 1

  label967:
  UNK_0xf162(); // UNK_0xf162
  if (Pop() == 0) goto label963;

  UNK_0xf15e(); // UNK_0xf15e
  UNK_0xf33a(); // UNK_0xf33a

  label963:
  Pop(); Pop();// 2DROP

  label959:
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xf3b6: WORD 'UNK_0xf3b8' codep=0x224c parp=0xf3b8
// ================================================

void UNK_0xf3b8() // UNK_0xf3b8
{
  UNK_0xf32e(); // UNK_0xf32e
  UNK_0xeec0(); // UNK_0xeec0
}


// ================================================
// 0xf3be: WORD 'UNK_0xf3c0' codep=0x224c parp=0xf3c0
// ================================================

void UNK_0xf3c0() // UNK_0xf3c0
{
  Push(pp_UNK_0xec42); // UNK_0xec42
  ON(); // ON
  Push(cc_UNK_0xec36); // UNK_0xec36
  UNK_0xed24(); // UNK_0xed24
  UNK_0xec92(); // UNK_0xec92
  C_at_(); // C@
  ICLOSE(); // ICLOSE
  ICREATE(); // ICREATE
}


// ================================================
// 0xf3d2: WORD 'UNK_0xf3d4' codep=0x224c parp=0xf3d4
// ================================================

void UNK_0xf3d4() // UNK_0xf3d4
{
  UNK_0xf3b8(); // UNK_0xf3b8
  _gt_R(); // >R
  UNK_0xee92(); // UNK_0xee92
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label993;

  UNK_0xf3c0(); // UNK_0xf3c0
  _2DUP(); // 2DUP
  _gt_C_plus_S(); // >C+S
  _2SWAP(); // 2SWAP
  UNK_0xedba(); // UNK_0xedba
  ICLOSE(); // ICLOSE
  goto label994;

  label993:
  Pop(); Pop();// 2DROP
  _0_dot_(); // 0.

  label994:
}


// ================================================
// 0xf3f8: WORD 'UNK_0xf3fa' codep=0x224c parp=0xf3fa
// ================================================

void UNK_0xf3fa() // UNK_0xf3fa
{
  Push(0); // 0
  UNK_0xed46(); // UNK_0xed46
  Push(cc__4); // 4
  Push(Pop() * Pop()); // *
  RRND(); // RRND
  DUP(); // DUP
  _gt_FLAG(); // >FLAG
  UNK_0xed24(); // UNK_0xed24
  _at_INST_minus_S(); // @INST-S
  ICLOSE(); // ICLOSE
  _gt_FLAG(); // >FLAG
  Push(Pop() & Pop()); // AND
  Push(pp_PAST); // PAST
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
}


// ================================================
// 0xf41c: WORD 'UNK_0xf41e' codep=0x224c parp=0xf41e
// ================================================

void UNK_0xf41e() // UNK_0xf41e
{
  Push(pp_UNK_0xec42); // UNK_0xec42
  _099(); // 099
  UNK_0xed8e(); // UNK_0xed8e
  ROT(); // ROT
  Push(0); // 0

  do // (DO)
  {
  UNK_0xf32e(); // UNK_0xf32e
  _2DUP(); // 2DUP
  UNK_0xf3d4(); // UNK_0xf3d4
  UNK_0xed34(); // UNK_0xed34

  } while(...); // (LOOP) 0xfff6
}


// ================================================
// 0xf438: WORD 'UNK_0xf43a' codep=0x224c parp=0xf43a
// ================================================

void UNK_0xf43a() // UNK_0xf43a
{
  UNK_0xecf8(); // UNK_0xecf8
  Push(0x003c);
  _st_(); // <
  _gt_R(); // >R
  UNK_0xf3b8(); // UNK_0xf3b8
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  UNK_0xed24(); // UNK_0xed24
  UNK_0xeca9(); // UNK_0xeca9
  C_at_(); // C@
  _at_INST_minus_S(); // @INST-S
  _gt_R(); // >R
  ICLOSE(); // ICLOSE
  _gt_FLAG(); // >FLAG
  Push(Pop() & Pop()); // AND
  R_gt_(); // R>
  Push(cc__7); // 7
  _eq_(); // =
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label995;

  Push(cc_UNK_0xec3e); // UNK_0xec3e
  Push(0); // 0
  ICREATE(); // ICREATE
  _2DUP(); // 2DUP
  UNK_0xed34(); // UNK_0xed34
  _gt_C_plus_S(); // >C+S
  UNK_0xedba(); // UNK_0xedba
  ICLOSE(); // ICLOSE
  goto label996;

  label995:
  Pop(); Pop();// 2DROP

  label996:
}


// ================================================
// 0xf47e: WORD 'UNK_0xf480' codep=0x224c parp=0xf480
// ================================================

void UNK_0xf480() // UNK_0xf480
{
  Push(cc_UNK_0xec3a); // UNK_0xec3a
  UNK_0xed24(); // UNK_0xed24
  _at_INST_minus_S(); // @INST-S
  ICLOSE(); // ICLOSE
  ICREATE(); // ICREATE
}


// ================================================
// 0xf48c: WORD 'UNK_0xf48e' codep=0x224c parp=0xf48e
// ================================================

void UNK_0xf48e() // UNK_0xf48e
{
  Push(1); // 1
  UNK_0xec9f(); // UNK_0xec9f
  C_ex_(); // C!
  UNK_0xec92(); // UNK_0xec92
  C_at_(); // C@
  Push(Pop()-1); // 1-
  Push(1); // 1
  MAX(); // MAX
  UNK_0xec9a(); // UNK_0xec9a
  C_ex_(); // C!
  Push(pp_THIS_minus_RE); // THIS-RE
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN
  Push(0x001f);
  Push(0); // 0
  IFIND(); // IFIND
  Pop(); // DROP
  CI(); // CI
  Push(2); // 2
  _star_CLOSE(); // *CLOSE
  UNK_0xeca4(); // UNK_0xeca4
  _1_dot_5_ex_(); // 1.5!
}


// ================================================
// 0xf4be: WORD 'UNK_0xf4c0' codep=0x224c parp=0xf4c0
// ================================================

void UNK_0xf4c0() // UNK_0xf4c0
{
  Push(0); // 0
  UNK_0xed46(); // UNK_0xed46
  _3_star_(); // 3*
  RRND(); // RRND
  DUP(); // DUP
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(pp_UNK_0xec42); // UNK_0xec42
  Push(Read16(Pop())); // @
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label997;

  Push(0); // 0

  do // (DO)
  {
  UNK_0xed8e(); // UNK_0xed8e
  UNK_0xf3b8(); // UNK_0xf3b8
  if (Pop() == 0) goto label998;

  UNK_0xf480(); // UNK_0xf480
  _2DUP(); // 2DUP
  UNK_0xed34(); // UNK_0xed34
  _gt_C_plus_S(); // >C+S
  UNK_0xedba(); // UNK_0xedba
  UNK_0xf48e(); // UNK_0xf48e
  ICLOSE(); // ICLOSE
  goto label1000;

  label998:
  Pop(); Pop();// 2DROP

  label1000:

  } while(...); // (LOOP) 0xffe2
  goto label999;

  label997:
  Pop(); // DROP

  label999:
}


// ================================================
// 0xf504: WORD 'UNK_0xf506' codep=0x224c parp=0xf506
// ================================================

void UNK_0xf506() // UNK_0xf506
{
  MAXSPAC(); // MAXSPAC
  _1_dot_5_at_(); // 1.5@
  _at_NEWSPA(); // @NEWSPA
  Push(0x017c);
  Push(0); // 0
  D_plus_(); // D+
  D_st_(); // D<
  Push(pp__ask_CRITIC); // ?CRITIC
  Push(Read16(Pop())); // @
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xf520: WORD 'POPULA' codep=0x224c parp=0xf52b
// ================================================

void POPULA() // POPULA
{
  UNK_0xf2ea(); // UNK_0xf2ea
  UNK_0xf2da(); // UNK_0xf2da
  UNK_0xf2bc(); // UNK_0xf2bc
  UNK_0xf506(); // UNK_0xf506
  if (Pop() == 0) goto label955;

  UNK_0xf374(); // UNK_0xf374
  UNK_0xf3fa(); // UNK_0xf3fa
  if (Pop() == 0) goto label969;

  UNK_0xf41e(); // UNK_0xf41e
  UNK_0xf43a(); // UNK_0xf43a
  UNK_0xf4c0(); // UNK_0xf4c0
  goto label1001;

  label969:
  Pop(); // DROP

  label1001:
  UNK_0xf0da(); // UNK_0xf0da
  Push(pp_PLHI); // PLHI
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label955;

  UNK_0xefd4(); // UNK_0xefd4

  label955:
}

// 0xf559: db 0x53 0x45 0x45 0x44 0x5f 0x5f 0x5f 0x00 'SEED___ '
  