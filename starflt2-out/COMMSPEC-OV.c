// store offset = 0xe540
// overlay size   = 0x1020
// name = 'XCOM-VOC_________'

// =================================
// =========== DICTIONARY ==========
// =================================
// 1869:           ENG-O bitfield: 0 0 0  codep:0x83dd parp:0xbc8c size:0x0002
// 1870:           OV/DA bitfield: 0 0 0  codep:0x224c parp:0xbc98 size:0x0006
// 1871:           OV/RE bitfield: 0 0 0  codep:0x224c parp:0xbca8 size:0x0006
// 1872:           OVDBA bitfield: 0 0 0  codep:0x224c parp:0xbcb8 size:0x0006
// 1873:          SETTER bitfield: 0 0 0  codep:0x1ab5 parp:0xbcc9 size:0x0014
// 1874:        PLSET-OV bitfield: 0 0 0  codep:0x83dd parp:0xbcea size:0x040d
// 1875:      UNK_0xc0f9 bitfield: 0 0 0  codep:0x224c parp:0xc0f9 size:0x245b
// 1876:      UNK_0xe556 bitfield: 0 0 0  codep:0x224c parp:0xe556 size:0x0028
// 1877:      UNK_0xe580 bitfield: 0 0 0  codep:0x224c parp:0xe580 size:0x0008
// 1878:      UNK_0xe58a bitfield: 0 0 0  codep:0x224c parp:0xe58a size:0x0012
// 1879:      UNK_0xe59e bitfield: 0 0 0  codep:0x224c parp:0xe59e size:0x0042
// 1880:      UNK_0xe5e2 bitfield: 0 0 0  codep:0x224c parp:0xe5e2 size:0x0010
// 1881:      UNK_0xe5f4 bitfield: 0 0 0  codep:0x224c parp:0xe5f4 size:0x0034
// 1882:      UNK_0xe62a bitfield: 0 0 0  codep:0x7420 parp:0xe62a size:0x000d
// 1883:      UNK_0xe639 bitfield: 0 0 0  codep:0x7420 parp:0xe639 size:0x0003
// 1884:      UNK_0xe63e bitfield: 0 0 0  codep:0x1d29 parp:0xe63e size:0x0002
// 1885:      UNK_0xe642 bitfield: 0 0 0  codep:0x224c parp:0xe642 size:0x0014
// 1886:      UNK_0xe658 bitfield: 0 0 0  codep:0x224c parp:0xe658 size:0x0072
// 1887:      UNK_0xe6cc bitfield: 0 0 0  codep:0x224c parp:0xe6cc size:0x000e
// 1888:          ?VOWEL bitfield: 0 0 0  codep:0x4b3b parp:0xe6e5 size:0x0168
// 1889:      UNK_0xe84f bitfield: 0 0 0  codep:0x224c parp:0xe84f size:0x00e4
// 1890:      UNK_0xe935 bitfield: 0 0 0  codep:0x224c parp:0xe935 size:0x011a
// 1891:      UNK_0xea51 bitfield: 0 0 0  codep:0x224c parp:0xea51 size:0x0010
// 1892:      UNK_0xea63 bitfield: 0 0 0  codep:0x224c parp:0xea63 size:0x0037
// 1893:      UNK_0xea9c bitfield: 0 0 0  codep:0x7420 parp:0xea9c size:0x00b9
// 1894:      GORZEK>UHL bitfield: 0 0 0  codep:0x224c parp:0xeb64 size:0x0132
// 1895:          NPCREW bitfield: 0 0 0  codep:0x4b3b parp:0xeca1 size:0x0174
// 1896:      UNK_0xee17 bitfield: 0 0 0  codep:0x224c parp:0xee17 size:0x042b
// 1897:       #>PROTEST bitfield: 0 0 0  codep:0x4b3b parp:0xf250 size:0x0010
// 1898:           #>PRO bitfield: 0 0 0  codep:0x224c parp:0xf26a size:0x0241
// 1899:      UNK_0xf4ad bitfield: 0 0 0  codep:0x224c parp:0xf4ad size:0x0036
// 1900:         SA-CASE bitfield: 0 0 0  codep:0x4b3b parp:0xf4ef size:0x002c
// 1901:      UNK_0xf51d bitfield: 0 0 0  codep:0x224c parp:0xf51d size:0x0014
// 1902:         (XCOMM) bitfield: 0 0 0  codep:0x224c parp:0xf53d size:0x0023

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xe63e[2] = {0x3a, 0x20};



// 0xe552: db 0x01 0x01 '  '
  
// ================================================
// 0xe554: WORD 'UNK_0xe556' codep=0x224c parp=0xe556
// ================================================

void UNK_0xe556()
{
  Push(0xb76b); MODULE();
  
}

// 0xe55e: db 0x4c 0x22 0x5f 0x61 0xae 0x0b 0x01 0x57 0x16 0x6d 0x90 0x16 0x4c 0x22 0x5d 0x17 0x16 0xbf 0xd9 0x84 0x90 0x16 0x4c 0x22 0x5d 0x17 0x26 0xbf 0xd9 0x84 0x90 0x16 'L"_a   W m  L"]       L"] &     '
  
// ================================================
// 0xe57e: WORD 'UNK_0xe580' codep=0x224c parp=0xe580
// ================================================

void UNK_0xe580()
{
  Push(0xbf4a); MODULE();
  
}


// ================================================
// 0xe588: WORD 'UNK_0xe58a' codep=0x224c parp=0xe58a
// ================================================

void UNK_0xe58a()
{
  *PLIST >C+S();
  IOPEN();
  Push(0x000b); SWAP IFIND();
  DROP 
}


// ================================================
// 0xe59c: WORD 'UNK_0xe59e' codep=0x224c parp=0xe59e
// ================================================

void UNK_0xe59e()
{
  pp_@,0*1; @ 
  if (Pop() == 0) goto label924;
  BEEPON();
  Push(0x00c8); Push(cc_MPS); 1- * Push(0x01f4); MAX();
  Push(0x05dc); MIN();
  Push(0); 
  do // (DO)
  {
  I TONE Push(0x000a); Push(cc_MPS); * Push(0); 
  do // (DO)
  {
  NOP();
  
  } while(...); // (LOOP) 0xfffc
  
  } while(...); // (LOOP) 0xffe8
  BEEPOFF 
  label924:
  
}


// ================================================
// 0xe5e0: WORD 'UNK_0xe5e2' codep=0x224c parp=0xe5e2
// ================================================

void UNK_0xe5e2()
{
  >C+S();
  PHRASE$ pp_LSCAN $!();
  pp_LSCAN COUNT();
  ICLOSE();
  
}


// ================================================
// 0xe5f2: WORD 'UNK_0xe5f4' codep=0x224c parp=0xe5f4
// ================================================

void UNK_0xe5f4()
{
  Push(0xbe96); MODULE();
  
}

// 0xe5fc: db 0x4c 0x22 0x2e 0x0f 0x5d 0x17 0x0b 0x00 0xc4 0x4b 0x72 0x0f 0x5d 0x17 0xfa 0x00 0xad 0x11 0x41 0x0e 0xb8 0x27 0x90 0x16 0x20 0x74 0x34 0x12 0x01 0x20 0x74 0x34 0x13 0x01 0x20 0x74 0x14 0x43 0x02 0x20 0x74 0x34 0x14 0x01 'L". ]    Kr ]     A  '   t4   t4   t C  t4  '
  
// ================================================
// 0xe628: WORD 'UNK_0xe62a' codep=0x7420 parp=0xe62a
// ================================================
// 0xe62a: db 0x34 0x0b 0x01 0x20 0x74 0x10 0x0b 0x0f 0x20 0x74 0x10 0x20 0x02 '4   t    t   '
  
// ================================================
// 0xe637: WORD 'UNK_0xe639' codep=0x7420 parp=0xe639
// ================================================
// 0xe639: db 0x3d 0x1b 0x01 '=  '
  
// ================================================
// 0xe63c: WORD 'UNK_0xe63e' codep=0x1d29 parp=0xe63e
// ================================================
// 0xe63e: db 0x3a 0x20 ': '
  
// ================================================
// 0xe640: WORD 'UNK_0xe642' codep=0x224c parp=0xe642
// ================================================

void UNK_0xe642()
{
  CTINIT();
  Push(0x5bbc); Pust(0x0002); UNK_0xe5e2();
  .TTY();
  Push(0x01f4); MS();
  
}


// ================================================
// 0xe656: WORD 'UNK_0xe658' codep=0x224c parp=0xe658
// ================================================

void UNK_0xe658()
{
  pp_THIS-BU @ Push(2); < 
  if (Pop() == 0) goto label919;
  BLUE pp_THIS-BU @ .ON();
  CTINIT();
  UNK_0xe642();
  
  if (Pop() == 0) goto label920;
  UNK_0x3f09();
  UNK_0x5906 UNK_0x5347 UNK_0x6030 UNK_0x0818 UNK_0x0702 
  label920:
  UNK_0x3f09();
  UNK_0x4e05 UNK_0x2e51 .TTY();
  TTY-SCR();
  
  goto label921;
  
  label919:
  DROP 
  label921:
  BLACK pp_THIS-BU @ .ON();
  
}

// 0xe699: db 0x4c 0x22 0xe7 0x5e 0x65 0x6d 0xd2 0x9f 0x07 0x3f 0x0a 0x52 0x45 0x43 0x45 0x49 0x56 0x49 0x4e 0x47 0x3a 0xf1 0x9f 0x5d 0x17 0xf4 0x01 0xef 0x2a 0x90 0x16 0x4c 0x22 0x99 0xe6 0x6a 0xe5 0x74 0xe5 0x5d 0x17 0x6f 0xbf 0xd9 0x84 0xdf 0x79 0x90 0x16 'L" ^em   ? RECEIVING:  ]    *  L"  j t ] o    y  '
  
// ================================================
// 0xe6ca: WORD 'UNK_0xe6cc' codep=0x224c parp=0xe6cc
// ================================================

void UNK_0xe6cc()
{
  XYSCAN();
  DROP ?DUP 
  if (Pop() == 0) goto label916;
  NEW-BUT();
  
  label916:
  
}


// ================================================
// 0xe6da: WORD '?VOWEL' codep=0x4b3b parp=0xe6e5
// ================================================
// 0xe6e5: db  '  " A 0 E 0 I 0 O 0 U 0 L"      L" \Wm] d ~   L"]       L" l z?z]   ]   L~  L"    ( ?z    L~     t      x   `   2    y`        u y  L"  2 ?z    L~2   L"     m]   ~   L"]   A #  m#    m  L" u l @       u`    y  L"i    t#m t                   L"]        z t mnu l z<  y      L".S z       y  '
  // 0xe807: dec    sp
// 0xe808: and    bl,[di+2FB6]
// 0xe80c: jp     E799
// 0xe80e: out    7B,ax
// 0xe810: out    90,ax
// 0xe812: push   ss
// 0xe813: dec    sp
// 0xe814: and    dl,[bx+di]
// 0xe816: db     6C
// 0xe817: pop    ds
// 0xe818: jp     E859
// 0xe81a: jp     E89B
// 0xe81c: push   cs
// 0xe81d: pop    bp
// 0xe81e: pop    ss
// 0xe81f: jbe    E7DA
// 0xe821: esc    1,[si+175D]
// 0xe825: or     ax,[bx+si]
// 0xe827: repnz  
// 0xe828: push   cs
// 0xe829: dec    sp
// 0xe82a: jle    E826
// 0xe82c: adc    ax,000A
// 0xe82f: aas    
// 0xe830: jp     E87E
// 0xe832: jle    E894
// 0xe834: push   ss
// 0xe835: or     [bx+si],al
// 0xe837: esc    6,[di]
// 0xe839: iret   
// 0xe83a: push   cx
// 0xe83b: aas    
// 0xe83c: jp     E7CE
// 0xe83e: push   ss
// 0xe83f: dec    sp
// 0xe840: and    dl,[0016]
// 0xe844: pop    sp
// 0xe845: add    al,[bx+si]
// 0xe847: loopne E82E
// 0xe849: mov    ax,90E6
// 0xe84c: push   ss

// ================================================
// 0xe84d: WORD 'UNK_0xe84f' codep=0x224c parp=0xe84f
// ================================================

void UNK_0xe84f()
{
  'KEY();
  DROP Push(0x000a); .BTN-TE();
  INIT-BU();
  TIME 2@ Push(0xea60); Pust(0x0000); D+ pp_TIRED-T D!();
  
  label917:
  UNK_0xe6cc();
  pp_FTRIG @ TIME 2@ pp_TIRED-T 2@ D>();
  OR 
  if (Pop() == 0) goto label917;
  pp_THIS-BU @ DUP Push(2); = pp_FTRIG @ * pp_(STOP-C !();
  ?TRIG();
  0= 2* + 0= 
  if (Pop() == 0) goto label918;
  DROP UNK_0xe580();
  Push(1); 
  goto label922;
  
  label918:
  UNK_0xe580();
  DROP Push(0); 
  label922:
  DUP UNK_0xe658();
  Push(cc_9); .BTN-TE();
  
}

// 0xe8bd: db 0x4c 0x22 0x5d 0x17 0xf4 0x01 0x5d 0x17 0xe8 0x03 0xc4 0x4b 0xf7 0xe7 0x97 0x3b 0xd7 0xe7 0x5d 0x50 0x57 0x6d 0x90 0x16 0x4c 0x22 0xad 0x74 0xae 0x0b 0x59 0x55 0x16 0x6d 0x60 0xb9 0xf0 0x7e 0x07 0x76 0xdf 0x79 0xfa 0x15 0x04 0x00 0x1e 0x7f 0x90 0x16 0x4c 0x22 0x5d 0x17 0xdc 0x05 0xef 0x2a 0x90 0x16 0x4c 0x22 0x46 0x9c 0x16 0x16 0x70 0x5c 0x02 0x00 0xe0 0xe5 0xf1 0x9f 0x2d 0xe6 0xcd 0x0c 0xe6 0x06 0xdc 0x1b 0x0b 0x20 0x52 0x45 0x50 0x4f 0x52 0x54 0x49 0x4e 0x47 0x2e 0x90 0x16 0x4c 0x22 0x2e 0x0f 0x07 0xe8 0x2e 0x0f 0x07 0xe8 0x5d 0x17 0x12 0x00 0x07 0xe8 0x7f 0x3b 0x0a 0x7a 0x90 0x16 'L"]   ]    K   ;  ]PWm  L" t  YU m`  ~ v y       L"]    *  L"F   p\      -         REPORTING.  L".   .   ]     ; z  '
  
// ================================================
// 0xe933: WORD 'UNK_0xe935' codep=0x224c parp=0xe935
// ================================================

void UNK_0xe935()
{
  pp_XORMODE 099();
  pp_(AORIGI @>C+S();
  UNK_0xe62a C@ I>C();
  ?CGA();
  
  if (Pop() == 0) goto label915;
  >FLAG();
  
  label915:
  DUP !COLOR();
  pp_CTCOLOR !();
  ICLOSE();
  
}

// 0xe957: db 0x4c 0x22 0x5d 0x17 0x44 0x00 0x5d 0x17 0x28 0x00 0x13 0xe8 0xfa 0x15 0x44 0x00 0x33 0xe9 0x3f 0xe8 0x5d 0x17 0x9c 0xff 0x20 0x0f 0x4d 0xe8 0xfa 0x15 0x2e 0x00 0xd5 0xe8 0xbd 0xe8 0x9c 0xe5 0x5d 0x17 0xec 0xb6 'L"] D ] (     D 3 ? ]     M   .       ]   '
  // 0xe981: esc    1,[si-1A96]
// 0xe985: jz     E96C
// 0xe987: jnp    E970
// 0xe989: esc    7,[bx+di+5D]
// 0xe98c: pop    ss
// 0xe98d: esc    1,[bx+si]
// 0xe98f: mov    ch,ah
// 0xe991: aas    
// 0xe992: jp     E9C7
// 0xe994: jmp    00F4
// 0xe997: inc    bp
// 0xe998: mov    si,84D9
// 0xe99b: jg     E9D8
// 0xe99d: or     bh,[bp+si+60]
// 0xe9a0: push   ss
// 0xe9a1: add    al,00
// 0xe9a3: esc    7,[bx+di+60]
// 0xe9a6: push   ss
// 0xe9a7: add    al,00
// 0xe9a9: esc    7,[bx+di+3D]
// 0xe9ac: db     0F
// 0xe9ad: or     bh,[bp+si-70]
// 0xe9b0: push   ss
// 0xe9b1: dec    sp
// 0xe9b2: and    dh,[bp+di]
// 0xe9b4: jmp    FFCD
// 0xe9b7: mov    [si+02],ds
// 0xe9ba: add    al,ah
// 0xe9bc: in     ax,B8
// 0xe9be: out    90,al
// 0xe9c0: push   ss
// 0xe9c1: dec    sp
// 0xe9c2: and    ch,[bp+si-1B]
// 0xe9c5: jz     E9AC
// 0xe9c7: pop    bp
// 0xe9c8: pop    ss
// 0xe9c9: sub    ax,5400
// 0xe9cc: in     ax,FA
// 0xe9ce: adc    ax,004C
// 0xe9d1: mov    cl,E9
// 0xe9d3: adc    [si+1F],bp
// 0xe9d6: jp     EA17
// 0xe9d8: jp     EA37
// 0xe9da: pop    ss
// 0xe9db: or     ax,[bx+si]
// 0xe9dd: pop    bp
// 0xe9de: pop    ss
// 0xe9df: db     0F
// 0xe9e0: add    [si+7E],cl
// 0xe9e3: xor    cl,[7A3F]
// 0xe9e7: pop    bp
// 0xe9e8: pop    ss
// 0xe9e9: sbb    al,00
// 0xe9eb: pop    bp
// 0xe9ec: pop    ss
// 0xe9ed: sub    ax,4C00
// 0xe9f0: jle    EA24
// 0xe9f2: push   cs
// 0xe9f3: pop    bp
// 0xe9f4: pop    ss
// 0xe9f5: pushf  
// 0xe9f6: call   far [di+17]
// 0xe9f9: db     64
// 0xe9fa: add    [di-18],cl
// 0xe9fd: cli    
// 0xe9fe: adc    ax,0016
// 0xea01: aad    E8
// 0xea03: pop    bp
// 0xea04: pop    ss
// 0xea05: in     al,dx
// 0xea06: mov    dh,D9
// 0xea08: test   [bx+si+61],cl
// 0xea0b: push   di
// 0xea0c: db     6D
// 0xea0d: pushf  
// 0xea0e: in     ax,1D
// 0xea10: jmp    0073
// 0xea13: add    al,00
// 0xea15: esc    7,[bx+di+3D]
// 0xea18: db     0F
// 0xea19: or     bh,[bp+si-21]
// 0xea1c: jns    E9AE
// 0xea1e: push   ss
// 0xea1f: dec    sp
// 0xea20: and    dh,dl
// 0xea22: db     61
// 0xea23: scasb
// 0xea24: or     cx,[bx+2D3B]
// 0xea28: adc    dl,[bx+si+4C16]
// 0xea2c: and    bl,[di+2FB6]
// 0xea30: jp     EA46
// 0xea32: out    92,al
// 0xea34: or     al,23
// 0xea36: out    92,al
// 0xea38: or     al,5D
// 0xea3a: pop    ss
// 0xea3b: db     65
// 0xea3c: add    [di],ah
// 0xea3e: inc    ax
// 0xea3f: nop    
// 0xea40: push   ss
// 0xea41: dec    sp
// 0xea42: and    ch,[1F53]
// 0xea46: jp     EA66
// 0xea48: out    AE,al
// 0xea4a: or     bx,di
// 0xea4c: jns    E9DE
// 0xea4e: push   ss

// ================================================
// 0xea4f: WORD 'UNK_0xea51' codep=0x224c parp=0xea51
// ================================================

void UNK_0xea51()
{
  UNK_0xe935();
  INEXT();
  Push(0xbe45); MODULE();
  UNK_0xe5f4();
  WUP();
  
}


// ================================================
// 0xea61: WORD 'UNK_0xea63' codep=0x224c parp=0xea63
// ================================================

void UNK_0xea63()
{
  Push(cc_6); Push(0x003a); Push(1); @RECORD();
  Push(0x0048); + C!();
  UPDATE();
  SAVE-BU();
  Push(0x5d19); Pust(0x0002); UNK_0xe5e2();
  Push(0x001c); Push(0x0036); @RECORD();
  SWAP CMOVE();
  UPDATE();
  SET-CUR();
  
}

// 0xea95: db 0x20 0x74 0x10 0x1a 0x01 ' t   '
  
// ================================================
// 0xea9a: WORD 'UNK_0xea9c' codep=0x7420 parp=0xea9c
// ================================================
// 0xea9c: db  '   L"    >]       ? SCIENCE:            m ? NAVIGATING:                m  `]                   m  z]        pJ      pJ m ? MEDICINE:      ;r        ;r  m] d    m2   ]     2  m   .      '
  
// ================================================
// 0xeb55: WORD 'GORZEK>UHL' codep=0x224c parp=0xeb64
// ================================================

void GORZEK>UHL()
{
  Push(0x0036); UNK_0xe556();
  
  if (Pop() == 0) goto label914;
  Push(0x0064); UNK_0xe58a();
  IOPEN();
  UNK_0xe935();
  Push(0xbe45); MODULE();
  Push(0x5d96); Pust(0x0002); UNK_0xe5e2();
  .TTY();
  Push(0x0000); Push(0x0000); UNK_0xe84f();
  
  if (Pop() == 0) goto label923;
  Push(0x2710); Pust(0x0000); UNK_0xea51();
  UNK_0xe59e();
  Push(0x2710); Pust(0x0000); UNK_0xea51();
  Push(0x2710); Pust(0x0000); UNK_0xea51();
  pp_(ENCOUN @>C+S();
  Push(0); UNK_0xe639 C!();
  IOPEN();
  Push(0x0019); Push(0x0014); IFIND();
  DROP -ICON();
  pp_TERMINA ON();
  pp_(STOP-C ON();
  Push(2); *CLOSE();
  UNK_0xea63();
  pp_UNK_0xe63e ON();
  
  goto label925;
  
  label923:
  UNK_0x3f09();
  UNK_0x430b UNK_0x4e43 UNK_0x4545 UNK_0x4c4e UNK_0x4447 .TTY();
  
  label925:
  Push(cc_3); *CLOSE();
  
  label914:
  
}

// 0xebf4: db 0x4c 0x22 0x2d 0xe6 0x65 0x6d 0x2d 0xe6 0xe7 0x0f 0x5d 0x17 0x0e 0x00 0x98 0x09 0x9d 0x6d 0x3d 0x0f 0x20 0x0f 0xb8 0x15 0x5d 0x17 0x8f 0xbf 0xd9 0x84 0xdb 0x23 0xcd 0x0c 'L"- em-   ]      m=     ]      #  '
  // 0xec16: mov    ah,0D
// 0xec18: sub    ax,CDE6
// 0xec1b: or     al,72
// 0xec1d: db     0F
// 0xec1e: push   ax
// 0xec1f: push   cs
// 0xec20: mov    ch,[di+2D]
// 0xec23: out    92,al
// 0xec25: or     al,90
// 0xec27: push   cs
// 0xec28: jb     EC39
// 0xec2a: sub    ax,0AE6
// 0xec2d: db     6D
// 0xec2e: rcl    byte ptr [di],1
// 0xec30: esc    4,??
// 0xec32: nop    
// 0xec33: push   ss
// 0xec34: dec    sp
// 0xec35: and    bl,[di+17]
// 0xec38: sbb    al,00
// 0xec3a: pop    bp
// 0xec3b: pop    ss
// 0xec3c: or     ax,A900
// 0xec3f: sbb    word ptr [bx],9E7A
// 0xec43: test   ax,0BAE
// 0xec46: inc    cx
// 0xec47: push   cs
// 0xec48: lodsw
// 0xec49: jz     EC61
// 0xec4b: db     6D
// 0xec4c: mov    si,[di-21]
// 0xec4f: jns    EC04
// 0xec51: push   cs
// 0xec52: inc    cx
// 0xec53: jmp    1307:122D
// 0xec58: cli    
// 0xec59: adc    ax,0010
// 0xec5c: adc    [si+1F],bp
// 0xec5f: jp     EC9D
// 0xec61: mov    cx,E713
// 0xec64: esc    7,[bx+di+60]
// 0xec67: push   ss
// 0xec68: add    al,00
// 0xec6a: esc    6,[di]
// 0xec6c: nop    
// 0xec6d: push   ss
// 0xec6e: dec    sp
// 0xec6f: and    dh,[bx+di+327A]
// 0xec73: out    92,al
// 0xec75: or     al,A7
// 0xec77: cmp    si,bp
// 0xec79: adc    bh,dl
// 0xec7b: adc    ax,FFF4
// 0xec7e: cs:    
// 0xec7f: db     0F
// 0xec80: nop    
// 0xec81: push   ss
// 0xec82: dec    sp
// 0xec83: and    cl,cl
// 0xec85: jp     ECB9
// 0xec87: out    92,al
// 0xec89: or     al,A7
// 0xec8b: cmp    si,bp
// 0xec8d: adc    bh,dl
// 0xec8f: adc    ax,FFF4
// 0xec92: cs:    
// 0xec93: db     0F
// 0xec94: nop    
// 0xec95: push   ss

// ================================================
// 0xec96: WORD 'NPCREW' codep=0x4b3b parp=0xeca1
// ================================================
// 0xeca1: db  '  "   p     L"2   A A =    J  .         ]     2  m`   2   L"  F  k z?z2    ;         z`   . ]        ]        = ]        ]        ]   =   -     2       ]   =   ]    O  -   .         L"]     ?z3 ] E   ; z  L"]     ?z z ;    3 ] E    z   N        ; z  L"   m#  m#    m  ~   L"]   . .     L"]   .       L"j t     * W     M          ; t          =  z ; a m'
  // 0xee03: esc    7,[bx+di-70]
// 0xee06: push   ss
// 0xee07: dec    sp
// 0xee08: and    dl,dl
// 0xee0a: db     61
// 0xee0b: scasb
// 0xee0c: or     bx,[di+17]
// 0xee0f: adc    ax,[bx+si]
// 0xee11: sub    ax,9012
// 0xee14: push   ss

// ================================================
// 0xee15: WORD 'UNK_0xee17' codep=0x224c parp=0xee17
// ================================================

void UNK_0xee17()
{
  Push(cc_TRUE); *ASSIGN >C+S();
  INST-SI Push(0x0011); + Push(cc_6); Push(0); 
  do // (DO)
  {
  I 3*();
  OVER + @>C+S();
  Push(cc_6); =SPECIE();
  UNK_0xea9c C@ >FLAG();
  AND 
  if (Pop() == 0) goto label911;
  SWAP NOT SWAP LEAVE 
  label911:
  ICLOSE();
  
  } while(...); // (LOOP) 0xffda
  DROP ICLOSE();
  
}

// 0xee59: db  'L"]     ?z3 ] E      N    ; z  L"2    ]        L"   ^        L" ? FOR X0,000 SHYNEUM PENNIES?    L" ? THE   ] (   Cs]    '      MAP  L"  4^      L"   ; K  L"]   ]    K  L"  A ]   ` ] 0 r   ]   r  m] 0 r   ]   r  m]       L"  3 ]              L"j t RV  = _    ScJ ;  ]      K      p     7     M   \     ]       >   ]      ;] s     ]     ] . r    z     t m u y l z<  y  `     2      ;~ `   2 y  y  L"  r^         ^        L" ?"OF, SAY XX UNITS OF SHYNEUM MIGHT      ^        L"  ]   ` ] 0 r   ]   r  m] 0 r   ]   r  m  L"  3 ]   4         L"   d/z?z  =  z  L"j t RV   ;_   ]    K      >   A ` ]     M   * ]     A  ;1      ;    `   2      ;~ `   y  y  L"j t . + = + > =  z    #   #     r 5 A    mDV m y  L"] - T        _         '      L"RV   ;_       =  K  j t   . o     M            ; t          4 Y =  z`            y  L" I S zbt]   r ]       P  r /z4u ;_          u  J  y;\   2  y  L"       u   z] U    y  @     =  z  L"0PA   IY m'
  // 0xf200: nop    
// 0xf201: push   ss
// 0xf202: dec    sp
// 0xf203: and    dh,ah
// 0xf205: db     F1
// 0xf206: pop    bp
// 0xf207: pop    ss
// 0xf208: db     63
// 0xf209: add    al,dl
// 0xf20b: db     F1
// 0xf20c: mov    unknown_seg_reg,cx
// 0xf20e: pop    ds
// 0xf20f: jp     F26E
// 0xf211: pop    ss
// 0xf212: jmp    CBD6
// 0xf215: test   [di+17],bl
// 0xf218: mov    ax,D9C1
// 0xf21b: test   bh,bl
// 0xf21d: jns    F1F1
// 0xf21f: lahf   
// 0xf220: pop    bp
// 0xf221: pop    ss
// 0xf222: db     63
// 0xf223: add    al,dl
// 0xf225: db     F1
// 0xf226: nop    
// 0xf227: push   ss
// 0xf228: dec    sp
// 0xf229: and    dh,ah
// 0xf22b: db     F1
// 0xf22c: pop    bp
// 0xf22d: pop    ss
// 0xf22e: db     62
// 0xf22f: add    al,dl
// 0xf231: db     F1
// 0xf232: cs:    
// 0xf233: db     0F
// 0xf234: and    al,5F
// 0xf236: and    bp,[di-70]
// 0xf239: push   ss
// 0xf23a: dec    sp
// 0xf23b: and    ah,[si]
// 0xf23d: pop    di
// 0xf23e: db     65
// 0xf23f: db     6D
// 0xf240: nop    
// 0xf241: push   ss

// ================================================
// 0xf242: WORD '#>PROTEST' codep=0x4b3b parp=0xf250
// ================================================
// 0xf250: db 0x03 0x00 0x3c 0xf2 0x01 0x00 0x04 0xf2 0x02 0x00 0x2a 0xf2 0x04 0x00 0x48 0x3a '  <       *   H:'
  
// ================================================
// 0xf260: WORD '#>PRO' codep=0x224c parp=0xf26a
// ================================================

void #>PRO()
{
  UNK_0xee17();
  NOT 
  if (Pop() == 0) goto label912;
  #>PROTEST 
  goto label913;
  
  label912:
  DROP 
  label913:
  
}

// 0xf27c: db  'L"   `      L"j t |     ] d M   4    O  HP]  O mF    `         `        2  I2 ? `   t < Wm   y  L"   a      L"  ?a      L"j t     ]    * a    {a         ^Wm    ]    *< Wm \Wm{  y  L"   a      L" ?%OF 1/X OF YOUR SHYNEUM. DO YOU AGREE?  L"   ; K      = `    ;A ] 0 r < 2  ;r  m  L"RV  = -   J h 0   <     ] d M   (  ;i  t  ; z  L  ;         b      `   2  \Wm ;KU m  L"HP    ] E      N    '
  // 0xf400: mov    cl,7A
// 0xf402: nop    
// 0xf403: push   ss
// 0xf404: sub    [di],bx
// 0xf406: add    [bx+si],al
// 0xf408: dec    sp
// 0xf409: and    al,[si]
// 0xf40b: hlt    
// 0xf40c: scasb
// 0xf40d: or     ax,[bx]
// 0xf40f: adc    sp,bx
// 0xf411: push   cx
// 0xf412: scasb
// 0xf413: or     cx,[bx+di-0AEE]
// 0xf417: adc    bh,dl
// 0xf419: adc    ax,0026
// 0xf41c: pop    bp
// 0xf41d: pop    ss
// 0xf41e: db     C9
// 0xf41f: add    [bx+si+3FE5],cl
// 0xf423: jp     F40F
// 0xf425: repz   
// 0xf426: jmp    7FF3:EAF3
// 0xf42b: cmp    cx,[bp+si]
// 0xf42d: jp     F3E8
// 0xf42f: pop    di
// 0xf430: push   di
// 0xf431: db     6D
// 0xf432: sbb    [si+57],bl
// 0xf435: db     6D
// 0xf436: cmp    al,E6
// 0xf438: push   di
// 0xf439: db     6D
// 0xf43a: add    al,F4
// 0xf43c: push   di
// 0xf43d: db     6D
// 0xf43e: lea    ax,[bp+di+1690]
// 0xf442: dec    sp
// 0xf443: and    bl,[di+17]
// 0xf446: xlat   
// 0xf447: add    [bx+si+3FE5],cl
// 0xf44b: jp     F437
// 0xf44d: repz   
// 0xf44e: jg     F48B
// 0xf450: or     bh,[bp+si+20]
// 0xf453: db     0F
// 0xf454: pop    bp
// 0xf455: pop    ss
// 0xf456: db     64
// 0xf457: add    [di-18],cl
// 0xf45a: cli    
// 0xf45b: adc    ax,0006
// 0xf45e: jz     F4B1
// 0xf460: push   di
// 0xf461: db     6D
// 0xf462: mov    cx,575F
// 0xf465: db     6D
// 0xf466: sbb    [si+57],bl
// 0xf469: db     6D
// 0xf46a: cmp    al,E6
// 0xf46c: push   di
// 0xf46d: db     6D
// 0xf46e: nop    
// 0xf46f: push   ss
// 0xf470: sub    [di],bx
// 0xf472: add    bx,ax
// 0xf474: jmp    11A0
// 0xf477: add    cl,[bx-11]
// 0xf47a: mov    al,dh
// 0xf47c: sub    [di],bx
// 0xf47e: add    cl,[bp+si]
// 0xf480: hlt    
// 0xf481: inc    sp
// 0xf482: hlt    
// 0xf483: sub    [di],bx
// 0xf485: add    bx,[bp+si]
// 0xf487: db     F1
// 0xf488: db     64
// 0xf489: jmp    F471
// 0xf48b: lock   
// 0xf48c: sub    [di],bx
// 0xf48e: add    bl,[bx+di-17]
// 0xf491: shr    bp,1
// 0xf493: sub    [di],bx
// 0xf495: add    bl,[bx+di-17]
// 0xf498: shr    bp,1
// 0xf49a: sub    [di],bx
// 0xf49c: add    dh,dh
// 0xf49e: repnz  
// 0xf49f: mov    dh,dl
// 0xf4a1: sub    [di],bx
// 0xf4a3: add    [bp+si],di
// 0xf4a5: db     F1
// 0xf4a6: sub    [di],bx
// 0xf4a8: add    [si+4CF3],dx

// ================================================
// 0xf4ab: WORD 'UNK_0xf4ad' codep=0x224c parp=0xf4ad
// ================================================

void UNK_0xf4ad()
{
  ?DUP 
  if (Pop() == 0) goto label927;
  pp_UNK_0xe63e 099();
  COUNT();
  2* OVER + SWAP 
  do // (DO)
  {
  I @EXECUTE pp_UNK_0xe63e @ 
  if (Pop() == 0) goto label928;
  LEAVE 
  label928:
  UNK_0xc0f9();
  Push(2); 
  } while(...); // (+LOOP) 0xffec
  
  label927:
  
}

// 0xf4db: db 0x16 0x16 0x3c 0x62 0x02 0x00 0xe0 0xe5 '  <b    '
  
// ================================================
// 0xf4e3: WORD 'SA-CASE' codep=0x4b3b parp=0xf4ef
// ================================================
// 0xf4ef: db 0x0a 0x00 0x22 0x0f 0x01 0x00 0x72 0xf4 0x02 0x00 0x72 0xf4 0x07 0x00 0x77 0xf4 0x06 '  "   r   r   w  '
  // 0xf500: add    [bx+si+08F4],ch
// 0xf504: add    [bp+0BF4],cl
// 0xf508: add    [di+0AF4],dl
// 0xf50c: add    [bp+di+15F4],ah
// 0xf510: add    [si+16F4],bl
// 0xf514: add    [bp-0C],bh
// 0xf517: adc    al,[bx+si]
// 0xf519: test   sp,si

// ================================================
// 0xf51b: WORD 'UNK_0xf51d' codep=0x224c parp=0xf51d
// ================================================

  label926:
  
void UNK_0xf51d()
{
  TIME 2@ pp_OK-TALK 2@ D>();
  ?TERMINAL OR 
  if (Pop() == 0) goto label926;
  
}


// ================================================
// 0xf531: WORD '(XCOMM)' codep=0x224c parp=0xf53d
// ================================================

void (XCOMM)()
{
  UNK_0xf51d();
  pp_(AORIGI @>C+S();
  @INST-S();
  ICLOSE();
  SA-CASE UNK_0xf4ad();
  UNK_0xf51d();
  
}

// 0xf54f: db 0x58 0x43 0x4f 0x4d 0x2d 0x56 0x4f 0x43 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x00 'XCOM-VOC_________ '
  