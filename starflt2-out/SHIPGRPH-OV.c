// ====== OVERLAY 'SHIPGRPH-OV' ======
// store offset = 0xecb0
// overlay size   = 0x08b0

#include"../emul/cpu.h"
#include"../emul/starflt2.h"


// =================================
// =========== DICTIONARY ==========
// =================================
// 1869:      UNK_0xecc6  codep:0x224c parp:0xecc6 size:0x000e C-string:'UNK_0xecc6'
// 1870:      UNK_0xecd6  codep:0x224c parp:0xecd6 size:0x0009 C-string:'UNK_0xecd6'
// 1871:      UNK_0xece1  codep:0x224c parp:0xece1 size:0x000d C-string:'UNK_0xece1'
// 1872:      UNK_0xecf0  codep:0x224c parp:0xecf0 size:0x0009 C-string:'UNK_0xecf0'
// 1873:      UNK_0xecfb  codep:0x224c parp:0xecfb size:0x000e C-string:'UNK_0xecfb'
// 1874:          A>TEXT  codep:0x4b3b parp:0xed14 size:0x0018 C-string:'A_gt_TEXT'
// 1875:      UNK_0xed2e  codep:0x7420 parp:0xed2e size:0x0003 C-string:'UNK_0xed2e'
// 1876:      UNK_0xed33  codep:0x7420 parp:0xed33 size:0x0003 C-string:'UNK_0xed33'
// 1877:      UNK_0xed38  codep:0x7420 parp:0xed38 size:0x0003 C-string:'UNK_0xed38'
// 1878:      UNK_0xed3d  codep:0x7420 parp:0xed3d size:0x0003 C-string:'UNK_0xed3d'
// 1879:      UNK_0xed42  codep:0x7420 parp:0xed42 size:0x0003 C-string:'UNK_0xed42'
// 1880:      UNK_0xed47  codep:0x7420 parp:0xed47 size:0x0003 C-string:'UNK_0xed47'
// 1881:      UNK_0xed4c  codep:0x7420 parp:0xed4c size:0x0003 C-string:'UNK_0xed4c'
// 1882:      UNK_0xed51  codep:0x7420 parp:0xed51 size:0x0008 C-string:'UNK_0xed51'
// 1883:      UNK_0xed5b  codep:0x7420 parp:0xed5b size:0x0003 C-string:'UNK_0xed5b'
// 1884:      UNK_0xed60  codep:0x7420 parp:0xed60 size:0x0003 C-string:'UNK_0xed60'
// 1885:      UNK_0xed65  codep:0x224c parp:0xed65 size:0x000a C-string:'UNK_0xed65'
// 1886:      UNK_0xed71  codep:0x1d29 parp:0xed71 size:0x0044 C-string:'UNK_0xed71'
// 1887:      UNK_0xedb7  codep:0x1d29 parp:0xedb7 size:0x0044 C-string:'UNK_0xedb7'
// 1888:      UNK_0xedfd  codep:0x1d29 parp:0xedfd size:0x0032 C-string:'UNK_0xedfd'
// 1889:      UNK_0xee31  codep:0x1d29 parp:0xee31 size:0x0022 C-string:'UNK_0xee31'
// 1890:      UNK_0xee55  codep:0x1d29 parp:0xee55 size:0x0002 C-string:'UNK_0xee55'
// 1891:      UNK_0xee59  codep:0x1d29 parp:0xee59 size:0x0006 C-string:'UNK_0xee59'
// 1892:         (.MASS)  codep:0x224c parp:0xee6b size:0x008c C-string:'_ro__dot_MASS_rc_'
// 1893:          (.ACC)  codep:0x224c parp:0xef02 size:0x0036 C-string:'_ro__dot_ACC_rc_'
// 1894:      UNK_0xef3a  codep:0x224c parp:0xef3a size:0x0074 C-string:'UNK_0xef3a'
// 1895:      UNK_0xefb0  codep:0x224c parp:0xefb0 size:0x0082 C-string:'UNK_0xefb0'
// 1896:      UNK_0xf034  codep:0x224c parp:0xf034 size:0x0010 C-string:'UNK_0xf034'
// 1897:      UNK_0xf046  codep:0x224c parp:0xf046 size:0x0070 C-string:'UNK_0xf046'
// 1898:      UNK_0xf0b8  codep:0x1d29 parp:0xf0b8 size:0x0010 C-string:'UNK_0xf0b8'
// 1899:      UNK_0xf0ca  codep:0x1d29 parp:0xf0ca size:0x0010 C-string:'UNK_0xf0ca'
// 1900:      UNK_0xf0dc  codep:0x1d29 parp:0xf0dc size:0x0002 C-string:'UNK_0xf0dc'
// 1901:      UNK_0xf0e0  codep:0x224c parp:0xf0e0 size:0x0024 C-string:'UNK_0xf0e0'
// 1902:      UNK_0xf106  codep:0x224c parp:0xf106 size:0x0012 C-string:'UNK_0xf106'
// 1903:         (.PODS)  codep:0x224c parp:0xf124 size:0x0056 C-string:'_ro__dot_PODS_rc_'
// 1904:      UNK_0xf17c  codep:0x1d29 parp:0xf17c size:0x0004 C-string:'UNK_0xf17c'
// 1905:      UNK_0xf182  codep:0x1d29 parp:0xf182 size:0x0004 C-string:'UNK_0xf182'
// 1906:      UNK_0xf188  codep:0x1d29 parp:0xf188 size:0x0008 C-string:'UNK_0xf188'
// 1907:      UNK_0xf192  codep:0x224c parp:0xf192 size:0x0040 C-string:'UNK_0xf192'
// 1908:      UNK_0xf1d4  codep:0x224c parp:0xf1d4 size:0x0036 C-string:'UNK_0xf1d4'
// 1909:         (.SHIP)  codep:0x224c parp:0xf216 size:0x0016 C-string:'_ro__dot_SHIP_rc_'
// 1910:         (BALANC  codep:0x224c parp:0xf238 size:0x003d C-string:'_ro_BALANC'
// 1911:      UNK_0xf277  codep:0x224c parp:0xf277 size:0x00f1 C-string:'UNK_0xf277'
// 1912:      UNK_0xf36a  codep:0x224c parp:0xf36a size:0x000a C-string:'UNK_0xf36a'
// 1913:      UNK_0xf376  codep:0x224c parp:0xf376 size:0x0100 C-string:'UNK_0xf376'
// 1914:      UNK_0xf478  codep:0x224c parp:0xf478 size:0x00d1 C-string:'UNK_0xf478'
// 1915:         (.CONFI  codep:0x224c parp:0xf555 size:0x0000 C-string:'_ro__dot_CONFI'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xed71[68] = {0x00, 0x7c, 0x00, 0x00, 0x80, 0x7f, 0x00, 0x00, 0xff, 0x7f, 0x00, 0xf8, 0xff, 0x7f, 0x00, 0xfc, 0x86, 0x61, 0x00, 0x1e, 0xff, 0x7f, 0x00, 0xff, 0xff, 0x7f, 0x80, 0xff, 0xff, 0x7f, 0xc0, 0xff, 0xff, 0x7f, 0xe0, 0xff, 0xff, 0x7f, 0xf0, 0xff, 0xff, 0x7f, 0xf8, 0xff, 0xff, 0x7f, 0xfc, 0xff, 0xff, 0x07, 0xfc, 0xff, 0x3f, 0x00, 0xfc, 0xff, 0x00, 0x00, 0xfc, 0x3f, 0x00, 0x00, 0xfc, 0x1f, 0x00, 0x00, 0xfe, 0x3f}; // UNK_0xed71
unsigned char UNK_0xedb7[68] = {0x00, 0x00, 0xfe, 0x3f, 0x00, 0x00, 0xfc, 0x1f, 0x00, 0x00, 0xfc, 0x3f, 0x3f, 0x00, 0xfc, 0xff, 0xff, 0x07, 0xfc, 0xff, 0xff, 0x7f, 0xfc, 0xff, 0xff, 0x7f, 0xf8, 0xff, 0xff, 0x7f, 0xf0, 0xff, 0xff, 0x7f, 0xe0, 0xff, 0xff, 0x7f, 0xc0, 0xff, 0xff, 0x7f, 0x80, 0xff, 0xff, 0x7f, 0x00, 0xff, 0x86, 0x61, 0x00, 0x1e, 0xff, 0x7f, 0x00, 0xfc, 0xff, 0x7f, 0x00, 0xf8, 0x80, 0x7f, 0x00, 0x00, 0x00, 0x7c, 0x00, 0x00}; // UNK_0xedb7
unsigned char UNK_0xedfd[50] = {0xfe, 0x7f, 0x00, 0x00, 0x7f, 0x00, 0xfc, 0xff, 0x00, 0x00, 0xff, 0x7f, 0xff, 0xff, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f, 0xff, 0xff, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f, 0xff, 0xff, 0x7f, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x7f, 0x00, 0xfc, 0x7f, 0x00, 0x00, 0xfe, 0x00, 0x00}; // UNK_0xedfd
unsigned char UNK_0xee31[34] = {0x01, 0x1e, 0x1f, 0xf0, 0xdc, 0x83, 0xc3, 0x3c, 0x7c, 0xc6, 0xe3, 0x37, 0x3f, 0xff, 0xff, 0xf3, 0xf3, 0x3f, 0x3f, 0xff, 0x7e, 0xf3, 0xc3, 0x37, 0x63, 0x3c, 0x3d, 0xcc, 0xf8, 0xc1, 0x01, 0x1f, 0x00, 0xe0}; // UNK_0xee31
unsigned char UNK_0xee55[2] = {0x00, 0xf0}; // UNK_0xee55
unsigned char UNK_0xee59[6] = {0x00, 0x04, 0x04, 0x03, 0x03, 0x02}; // UNK_0xee59
unsigned char UNK_0xf0b8[16] = {0x76, 0x76, 0x79, 0x79, 0x7c, 0x7c, 0x7f, 0x7f, 0x82, 0x82, 0x85, 0x85, 0x88, 0x88, 0x8b, 0x8b}; // UNK_0xf0b8
unsigned char UNK_0xf0ca[16] = {0x50, 0x5a, 0x50, 0x5a, 0x51, 0x59, 0x51, 0x59, 0x51, 0x59, 0x51, 0x59, 0x51, 0x59, 0x51, 0x59}; // UNK_0xf0ca
unsigned char UNK_0xf0dc[2] = {0x3a, 0x20}; // UNK_0xf0dc
unsigned char UNK_0xf17c[4] = {0x3f, 0x38, 0x00, 0xff}; // UNK_0xf17c
unsigned char UNK_0xf182[4] = {0x3f, 0xfc, 0x00, 0xce}; // UNK_0xf182
unsigned char UNK_0xf188[8] = {0x78, 0x30, 0xf8, 0xff, 0xff, 0xf8, 0x30, 0x78}; // UNK_0xf188



// 0xecc2: db 0x8b 0x00 '  '

// ================================================
// 0xecc4: WORD 'UNK_0xecc6' codep=0x224c parp=0xecc6
// ================================================

void UNK_0xecc6() // UNK_0xecc6
{
  PRINT("VERY POOR", 9); // (.")
}


// ================================================
// 0xecd4: WORD 'UNK_0xecd6' codep=0x224c parp=0xecd6
// ================================================

void UNK_0xecd6() // UNK_0xecd6
{
  PRINT("POOR", 4); // (.")
}


// ================================================
// 0xecdf: WORD 'UNK_0xece1' codep=0x224c parp=0xece1
// ================================================

void UNK_0xece1() // UNK_0xece1
{
  PRINT("MODERATE", 8); // (.")
}


// ================================================
// 0xecee: WORD 'UNK_0xecf0' codep=0x224c parp=0xecf0
// ================================================

void UNK_0xecf0() // UNK_0xecf0
{
  PRINT("GOOD", 4); // (.")
}


// ================================================
// 0xecf9: WORD 'UNK_0xecfb' codep=0x224c parp=0xecfb
// ================================================

void UNK_0xecfb() // UNK_0xecfb
{
  PRINT("EXCELLENT", 9); // (.")
}


// ================================================
// 0xed09: WORD 'A>TEXT' codep=0x4b3b parp=0xed14
// ================================================
// 0xed14: db 0x05 0x00 0x48 0x3a 0x01 0x00 0xc6 0xec 0x02 0x00 0xd6 0xec 0x03 0x00 0xe1 0xec 0x04 0x00 0xf0 0xec 0x05 0x00 0xfb 0xec '  H:                    '

// ================================================
// 0xed2c: WORD 'UNK_0xed2e' codep=0x7420 parp=0xed2e
// ================================================
// 0xed2e: db 0x14 0x11 0x02 '   '

// ================================================
// 0xed31: WORD 'UNK_0xed33' codep=0x7420 parp=0xed33
// ================================================
// 0xed33: db 0x14 0x13 0x02 '   '

// ================================================
// 0xed36: WORD 'UNK_0xed38' codep=0x7420 parp=0xed38
// ================================================
// 0xed38: db 0x14 0x19 0x02 '   '

// ================================================
// 0xed3b: WORD 'UNK_0xed3d' codep=0x7420 parp=0xed3d
// ================================================
// 0xed3d: db 0x14 0x1b 0x02 '   '

// ================================================
// 0xed40: WORD 'UNK_0xed42' codep=0x7420 parp=0xed42
// ================================================
// 0xed42: db 0x14 0x1d 0x02 '   '

// ================================================
// 0xed45: WORD 'UNK_0xed47' codep=0x7420 parp=0xed47
// ================================================
// 0xed47: db 0x14 0x1f 0x02 '   '

// ================================================
// 0xed4a: WORD 'UNK_0xed4c' codep=0x7420 parp=0xed4c
// ================================================
// 0xed4c: db 0x14 0x25 0x02 ' % '

// ================================================
// 0xed4f: WORD 'UNK_0xed51' codep=0x7420 parp=0xed51
// ================================================
// 0xed51: db 0x14 0x27 0x02 0x20 0x74 0x14 0x45 0x04 ' '  t E '

// ================================================
// 0xed59: WORD 'UNK_0xed5b' codep=0x7420 parp=0xed5b
// ================================================
// 0xed5b: db 0x14 0x56 0x01 ' V '

// ================================================
// 0xed5e: WORD 'UNK_0xed60' codep=0x7420 parp=0xed60
// ================================================
// 0xed60: db 0x14 0x57 0x01 ' W '

// ================================================
// 0xed63: WORD 'UNK_0xed65' codep=0x224c parp=0xed65
// ================================================

void UNK_0xed65() // UNK_0xed65
{
  Push(Pop()+1); // 1+
  Push(Read8(Pop())&0xFF); // C@
  Push(cc__7); // 7
  Push(Pop() & Pop()); // AND
}


// ================================================
// 0xed6f: WORD 'UNK_0xed71' codep=0x1d29 parp=0xed71
// ================================================
// 0xed71: db 0x00 0x7c 0x00 0x00 0x80 0x7f 0x00 0x00 0xff 0x7f 0x00 0xf8 0xff 0x7f 0x00 0xfc 0x86 0x61 0x00 0x1e 0xff 0x7f 0x00 0xff 0xff 0x7f 0x80 0xff 0xff 0x7f 0xc0 0xff 0xff 0x7f 0xe0 0xff 0xff 0x7f 0xf0 0xff 0xff 0x7f 0xf8 0xff 0xff 0x7f 0xfc 0xff 0xff 0x07 0xfc 0xff 0x3f 0x00 0xfc 0xff 0x00 0x00 0xfc 0x3f 0x00 0x00 0xfc 0x1f 0x00 0x00 0xfe 0x3f ' |            a                           ?      ?       ?'

// ================================================
// 0xedb5: WORD 'UNK_0xedb7' codep=0x1d29 parp=0xedb7
// ================================================
// 0xedb7: db 0x00 0x00 0xfe 0x3f 0x00 0x00 0xfc 0x1f 0x00 0x00 0xfc 0x3f 0x3f 0x00 0xfc 0xff 0xff 0x07 0xfc 0xff 0xff 0x7f 0xfc 0xff 0xff 0x7f 0xf8 0xff 0xff 0x7f 0xf0 0xff 0xff 0x7f 0xe0 0xff 0xff 0x7f 0xc0 0xff 0xff 0x7f 0x80 0xff 0xff 0x7f 0x00 0xff 0x86 0x61 0x00 0x1e 0xff 0x7f 0x00 0xfc 0xff 0x7f 0x00 0xf8 0x80 0x7f 0x00 0x00 0x00 0x7c 0x00 0x00 '   ?       ??                             a            |  '

// ================================================
// 0xedfb: WORD 'UNK_0xedfd' codep=0x1d29 parp=0xedfd
// ================================================
// 0xedfd: db 0xfe 0x7f 0x00 0x00 0x7f 0x00 0xfc 0xff 0x00 0x00 0xff 0x7f 0xff 0xff 0x7f 0xff 0xff 0xff 0xff 0xff 0xff 0x7f 0xff 0xff 0x7f 0xff 0xff 0xff 0xff 0xff 0xff 0x7f 0xff 0xff 0x7f 0xff 0xff 0xff 0xff 0xff 0xff 0x7f 0x00 0xfc 0x7f 0x00 0x00 0xfe 0x00 0x00 '                                        '

// ================================================
// 0xee2f: WORD 'UNK_0xee31' codep=0x1d29 parp=0xee31
// ================================================
// 0xee31: db 0x01 0x1e 0x1f 0xf0 0xdc 0x83 0xc3 0x3c 0x7c 0xc6 0xe3 0x37 0x3f 0xff 0xff 0xf3 0xf3 0x3f 0x3f 0xff 0x7e 0xf3 0xc3 0x37 0x63 0x3c 0x3d 0xcc 0xf8 0xc1 0x01 0x1f 0x00 0xe0 '       <|  7?    ?? ~  7c<=       '

// ================================================
// 0xee53: WORD 'UNK_0xee55' codep=0x1d29 parp=0xee55
// ================================================
// 0xee55: db 0x00 0xf0 '  '

// ================================================
// 0xee57: WORD 'UNK_0xee59' codep=0x1d29 parp=0xee59
// ================================================
// 0xee59: db 0x00 0x04 0x04 0x03 0x03 0x02 '      '

// ================================================
// 0xee5f: WORD '(.MASS)' codep=0x224c parp=0xee6b
// ================================================
// entry

void _ro__dot_MASS_rc_() // (.MASS)
{
  Push(0x0032);
  Push(0x65f4); // IFIELD(UNK_0xed33)
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(0x0032);
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(0x65f2); // IFIELD(UNK_0xed2e)
  UNK_0xed65(); // UNK_0xed65
  Push(Read16(regsp)); // DUP
  Push(Pop() * Pop()); // *
  Push(cc__9); // 9
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(0x6600); // IFIELD(UNK_0xed47)
  Push(Read16(Pop())); // @
  _plus_BIT(); // +BIT
  Push(0x000a);
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(0x6638); // IFIELD(UNK_0xed60)
  Push(Read8(Pop())&0xFF); // C@
  Push(0x000a);
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(0x6637); // IFIELD(UNK_0xed5b)
  Push(Read8(Pop())&0xFF); // C@
  Push(0x000a);
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(0x65fe); // IFIELD(UNK_0xed42)
  UNK_0xed65(); // UNK_0xed65
  _0_gt_(); // 0>
  Push(0x65fc); // IFIELD(UNK_0xed3d)
  UNK_0xed65(); // UNK_0xed65
  _0_gt_(); // 0>
  Push(Pop() + Pop()); // +
  Push(0x65fa); // IFIELD(UNK_0xed38)
  UNK_0xed65(); // UNK_0xed65
  _0_gt_(); // 0>
  Push(Pop() + Pop()); // +
  Push(cc__5); // 5
  Push(Pop() * Pop()); // *
  Push(Pop() + Pop()); // +
  Push(0x6608); // IFIELD(UNK_0xed51)
  _ex__2(); // !_2
  _at_CRS(); // @CRS
  CTINIT(); // CTINIT
  Push(cc__4); // 4
  SetColor("BLACK");
  Push(0x0069);
  Push(0x0036);
  _2DUP(); // 2DUP
  POS_dot_(); // POS.
  SWAP(); // SWAP
  POS_dot_PXT(); // POS.PXT
  Push(0x6608); // IFIELD(UNK_0xed51)
  Push(Read16(Pop())); // @
  Push(cc__4); // 4
  _dot_R(); // .R
  _ex_CRS(); // !CRS
}


// ================================================
// 0xeef7: WORD '(.ACC)' codep=0x224c parp=0xef02
// ================================================
// entry

void _ro__dot_ACC_rc_() // (.ACC)
{
  Push(0x65f4); // IFIELD(UNK_0xed33)
  UNK_0xed65(); // UNK_0xed65
  Push(Read16(regsp)); // DUP
  Push(0x6606); // IFIELD(UNK_0xed4c)
  _ex__2(); // !_2
  Push(0xee59); // pointer to UNK_0xee59
  Push(Pop() + Pop()); // +
  Push(Read8(Pop())&0xFF); // C@
  Push(0x5703); // pointer to -END
  _ex__2(); // !_2
  _at_CRS(); // @CRS
  CTINIT(); // CTINIT
  Push(cc__9); // 9
  SetColor("BLACK");
  Push(0x0072);
  Push(0x002c);
  _2DUP(); // 2DUP
  POS_dot_(); // POS.
  SWAP(); // SWAP
  POS_dot_PXT(); // POS.PXT
  Push(0x6606); // IFIELD(UNK_0xed4c)
  Push(Read16(Pop())); // @
  Pop();
  switch(Pop()) // A>TEXT
  {
  case 1:
    UNK_0xecc6(); // UNK_0xecc6
    break;
  case 2:
    UNK_0xecd6(); // UNK_0xecd6
    break;
  case 3:
    UNK_0xece1(); // UNK_0xece1
    break;
  case 4:
    UNK_0xecf0(); // UNK_0xecf0
    break;
  case 5:
    UNK_0xecfb(); // UNK_0xecfb
    break;
  default:
    NOP(); // NOP
    break;

  }
  _ex_CRS(); // !CRS
}


// ================================================
// 0xef38: WORD 'UNK_0xef3a' codep=0x224c parp=0xef3a
// ================================================

void UNK_0xef3a() // UNK_0xef3a
{
  _at_CRS(); // @CRS
  CTINIT(); // CTINIT
  Push(cc__6); // 6
  SetColor("BLACK");
  Push(0x007e);
  Push(0x0022);
  _2DUP(); // 2DUP
  POS_dot_(); // POS.
  SWAP(); // SWAP
  POS_dot_PXT(); // POS.PXT
  Push2Words("*STARSH");
  _gt_C_plus_S(); // >C+S
  IOPEN(); // IOPEN
  Push(0x000b);
  Push(0x000a);
  IFIND(); // IFIND
  if (Pop() == 0) goto label1;
  IOPEN(); // IOPEN
  Push(0x001a);
  Push(cc__6); // 6
  IFIND(); // IFIND
  if (Pop() == 0) goto label2;
  Push(0x65ec); // IFIELD(INST-QT)
  Push(Read16(Pop())); // @
  goto label3;

  label2:
  Push(0); // 0

  label3:
  ICLOSE(); // ICLOSE
  goto label4;

  label1:
  Push(0); // 0

  label4:
  ICLOSE(); // ICLOSE
  ICLOSE(); // ICLOSE
  Push(0x000a);
  _slash_MOD(); // /MOD
  Push(cc__3); // 3
  _dot_R(); // .R
  PRINT(".", 1); // (.")
  Push(0); // 0
  _dot_R(); // .R
  PRINT("M", 1); // (.")
  Push(cc__3); // 3
  Push(Read16(regsp)); // DUP
  Push(0x5a3d); // pointer to YBLT
  _plus__ex__2(); // +!_2
  _dot_(); // .
  _ex_CRS(); // !CRS
}


// ================================================
// 0xefae: WORD 'UNK_0xefb0' codep=0x224c parp=0xefb0
// ================================================

void UNK_0xefb0() // UNK_0xefb0
{
  SetColor("DK-BLUE");
  _ex_COLOR(); // !COLOR
  Push(0x5a56); // pointer to XORMODE
  _099(); // 099
  Push(0x0011);
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(0x0020);
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(0x0055);
  Push(0x006a);
  POS_dot_(); // POS.
  Push(0xed71); // pointer to UNK_0xed71
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
  Push(0x0055);
  Push(0x004f);
  POS_dot_(); // POS.
  Push(0xedb7); // pointer to UNK_0xedb7
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
  Push(0x000a);
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(0x0028);
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(0x0065);
  Push(0x0059);
  POS_dot_(); // POS.
  Push(0xedfd); // pointer to UNK_0xedfd
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
  Push(0x0016);
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(0x000c);
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(0x008d);
  Push(0x005f);
  POS_dot_(); // POS.
  Push(0xee31); // pointer to UNK_0xee31
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
}


// ================================================
// 0xf032: WORD 'UNK_0xf034' codep=0x224c parp=0xf034
// ================================================

void UNK_0xf034() // UNK_0xf034
{
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  _2OVER(); // 2OVER
  LLINE(); // LLINE
  i++;
  } while(i<imax); // (LOOP) 0xfffa

  Pop(); Pop();// 2DROP
}


// ================================================
// 0xf044: WORD 'UNK_0xf046' codep=0x224c parp=0xf046
// ================================================

void UNK_0xf046() // UNK_0xf046
{
  Push(0x65fa); // IFIELD(UNK_0xed38)
  UNK_0xed65(); // UNK_0xed65
  _0_gt_(); // 0>
  SetColor("RED");
  Push(Pop() * Pop()); // *
  _ex_COLOR(); // !COLOR
  Push(0x5a56); // pointer to XORMODE
  _099(); // 099
  Push(0x0053);
  Push(0x003c);
  Push(0x0053);
  Push(0x006d);
  Push(0x0069);
  Push(0x006d);
  Push(0x0086);
  Push(0x0063);
  Push(0x0096);
  Push(0x0063);
  Push(0x009b);
  Push(0x005e);
  Push(0x009b);
  Push(0x004b);
  Push(0x0096);
  Push(0x0046);
  Push(0x0084);
  Push(0x0046);
  Push(0x0068);
  Push(0x003c);
  Push(0x0053);
  Push(0x003c);
  Push(0x000a);
  UNK_0xf034(); // UNK_0xf034
}


// ================================================
// 0xf0b6: WORD 'UNK_0xf0b8' codep=0x1d29 parp=0xf0b8
// ================================================
// 0xf0b8: db 0x76 0x76 0x79 0x79 0x7c 0x7c 0x7f 0x7f 0x82 0x82 0x85 0x85 0x88 0x88 0x8b 0x8b 'vvyy||        '

// ================================================
// 0xf0c8: WORD 'UNK_0xf0ca' codep=0x1d29 parp=0xf0ca
// ================================================
// 0xf0ca: db 0x50 0x5a 0x50 0x5a 0x51 0x59 0x51 0x59 0x51 0x59 0x51 0x59 0x51 0x59 0x51 0x59 'PZPZQYQYQYQYQYQY'

// ================================================
// 0xf0da: WORD 'UNK_0xf0dc' codep=0x1d29 parp=0xf0dc
// ================================================
// 0xf0dc: db 0x3a 0x20 ': '

// ================================================
// 0xf0de: WORD 'UNK_0xf0e0' codep=0x224c parp=0xf0e0
// ================================================

void UNK_0xf0e0() // UNK_0xf0e0
{
  Push(0xf0dc); // pointer to UNK_0xf0dc
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(0xf0b8); // pointer to UNK_0xf0b8
  Push(Pop() + Pop()); // +
  Push(Read8(Pop())&0xFF); // C@
  Push(0x5a48); // pointer to XBLT
  _ex__2(); // !_2
  Push(0xf0ca); // pointer to UNK_0xf0ca
  Push(Pop() + Pop()); // +
  Push(Read8(Pop())&0xFF); // C@
  Push(0x5a3d); // pointer to YBLT
  _ex__2(); // !_2
  BLT(); // BLT
  Push(1); // 1
  Push(0xf0dc); // pointer to UNK_0xf0dc
  _plus__ex__2(); // +!_2
}


// ================================================
// 0xf104: WORD 'UNK_0xf106' codep=0x224c parp=0xf106
// ================================================

void UNK_0xf106() // UNK_0xf106
{
  if (Read16(regsp) != 0) Push(Read16(regsp)); // ?DUP
  if (Pop() == 0) return;
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  UNK_0xf0e0(); // UNK_0xf0e0
  i++;
  } while(i<imax); // (LOOP) 0xfffc

}


// ================================================
// 0xf118: WORD '(.PODS)' codep=0x224c parp=0xf124
// ================================================
// entry

void _ro__dot_PODS_rc_() // (.PODS)
{
  _at_CRS(); // @CRS
  Push(0xf0dc); // pointer to UNK_0xf0dc
  _099(); // 099
  Push(0x5a56); // pointer to XORMODE
  _099(); // 099
  _at_DS(); // @DS
  Push(0x5a84); // pointer to BLTSEG
  _ex__2(); // !_2
  Push(0xee55); // pointer to UNK_0xee55
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  Push(2); // 2
  Push(Read16(regsp)); // DUP
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  SetColor("GREY1");
  _ex_COLOR(); // !COLOR
  Push(0x6600); // IFIELD(UNK_0xed47)
  Push(Read16(Pop())); // @
  _plus_BIT(); // +BIT
  UNK_0xf106(); // UNK_0xf106
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  Push(0x6637); // IFIELD(UNK_0xed5b)
  Push(Read8(Pop())&0xFF); // C@
  UNK_0xf106(); // UNK_0xf106
  SetColor("RED");
  _ex_COLOR(); // !COLOR
  Push(0x6638); // IFIELD(UNK_0xed60)
  Push(Read8(Pop())&0xFF); // C@
  UNK_0xf106(); // UNK_0xf106
  SetColor("BLACK");
  _ex_COLOR(); // !COLOR
  Push(0x0010);
  Push(0xf0dc); // pointer to UNK_0xf0dc
  Push(Read16(Pop())); // @
  _dash_(); // -
  UNK_0xf106(); // UNK_0xf106
  _ex_CRS(); // !CRS
}


// ================================================
// 0xf17a: WORD 'UNK_0xf17c' codep=0x1d29 parp=0xf17c
// ================================================
// 0xf17c: db 0x3f 0x38 0x00 0xff '?8  '

// ================================================
// 0xf180: WORD 'UNK_0xf182' codep=0x1d29 parp=0xf182
// ================================================
// 0xf182: db 0x3f 0xfc 0x00 0xce '?   '

// ================================================
// 0xf186: WORD 'UNK_0xf188' codep=0x1d29 parp=0xf188
// ================================================
// 0xf188: db 0x78 0x30 0xf8 0xff 0xff 0xf8 0x30 0x78 'x0    0x'

// ================================================
// 0xf190: WORD 'UNK_0xf192' codep=0x224c parp=0xf192
// ================================================

void UNK_0xf192() // UNK_0xf192
{
  Push(0x65fc); // IFIELD(UNK_0xed3d)
  UNK_0xed65(); // UNK_0xed65
  _0_gt_(); // 0>
  SetColor("LT-BLUE");
  Push(Pop() * Pop()); // *
  _ex_COLOR(); // !COLOR
  Push(0x5a56); // pointer to XORMODE
  _099(); // 099
  Push(cc__3); // 3
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(cc__9); // 9
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(0x0060);
  Push(0x006b);
  POS_dot_(); // POS.
  Push(0xf17c); // pointer to UNK_0xf17c
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
  Push(0x0040);
  Push(0x5a3d); // pointer to YBLT
  _ex__2(); // !_2
  Push(0xf182); // pointer to UNK_0xf182
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  BLT(); // BLT
}


// ================================================
// 0xf1d2: WORD 'UNK_0xf1d4' codep=0x224c parp=0xf1d4
// ================================================

void UNK_0xf1d4() // UNK_0xf1d4
{
  Push(0x65fe); // IFIELD(UNK_0xed42)
  UNK_0xed65(); // UNK_0xed65
  if (Pop() == 0) goto label1;
  SetColor("LT-BLUE");
  goto label2;

  label1:
  SetColor("DK-BLUE");

  label2:
  _ex_COLOR(); // !COLOR
  Push(0x5a56); // pointer to XORMODE
  _099(); // 099
  Push(cc__8); // 8
  Push(0x5a61); // pointer to LBLT
  _ex__2(); // !_2
  Push(cc__8); // 8
  Push(0x5a6c); // pointer to WBLT
  _ex__2(); // !_2
  Push(0xf188); // pointer to UNK_0xf188
  Push(0x5a77); // pointer to ABLT
  _ex__2(); // !_2
  Push(0x006b);
  Push(0x0058);
  POS_dot_(); // POS.
  BLT(); // BLT
}


// ================================================
// 0xf20a: WORD '(.SHIP)' codep=0x224c parp=0xf216
// ================================================
// entry

void _ro__dot_SHIP_rc_() // (.SHIP)
{
  _at_CRS(); // @CRS
  _at_DS(); // @DS
  Push(0x5a84); // pointer to BLTSEG
  _ex__2(); // !_2
  UNK_0xefb0(); // UNK_0xefb0
  UNK_0xf046(); // UNK_0xf046
  UNK_0xf1d4(); // UNK_0xf1d4
  UNK_0xf192(); // UNK_0xf192
  _ro__dot_PODS_rc_(); // (.PODS)
  _ex_CRS(); // !CRS
}


// ================================================
// 0xf22c: WORD '(BALANC' codep=0x224c parp=0xf238
// ================================================
// entry

void _ro_BALANC() // (BALANC
{
  _at_CRS(); // @CRS
  Push(0); // 0
  _0MESS(); // 0MESS
  Push(cc__5); // 5
  Push(0x0017);
  POS_dot_(); // POS.
  CTINIT(); // CTINIT
  PRINT("YOUR ACCOUNT BALANCE IS: ", 25); // (.")
  Push(0xc285);
  MODULE(); // MODULE
  D_dot_(); // D.
  PRINT("SP", 2); // (.")
  _ex_CRS(); // !CRS
}


// ================================================
// 0xf275: WORD 'UNK_0xf277' codep=0x224c parp=0xf277
// ================================================

void UNK_0xf277() // UNK_0xf277
{
  SetColor("DK-BLUE");
  _ex_COLOR(); // !COLOR
  Push(0x009d);
  Push(1); // 1
  Push(2); // 2
  Push(1); // 1
  LLINE(); // LLINE
  Push(0x009d);
  Push(0x000b);
  Push(2); // 2
  Push(0x000b);
  LLINE(); // LLINE
  Push(0x009d);
  Push(0x001a);
  Push(2); // 2
  Push(0x001a);
  LLINE(); // LLINE
  Push(0x009d);
  Push(0x00b6);
  Push(2); // 2
  Push(0x00b6);
  LLINE(); // LLINE
  Push(2); // 2
  Push(0x00c6);
  Push(0x009d);
  Push(0x00c6);
  LLINE(); // LLINE
  Push(1); // 1
  Push(0x00c6);
  Push(1); // 1
  Push(1); // 1
  LLINE(); // LLINE
  Push(0x009e);
  Push(0x00c6);
  Push(0x009e);
  Push(1); // 1
  LLINE(); // LLINE
  Push(0x0050);
  Push(0x00b5);
  Push(0x0050);
  Push(0x001b);
  LLINE(); // LLINE
  Push(0x0015);
  Push(2); // 2
  OVER(); // OVER
  Push(0x000a);
  LLINE(); // LLINE
  Push(0x003b);
  Push(2); // 2
  OVER(); // OVER
  Push(0x000a);
  LLINE(); // LLINE
  Push(0x0063);
  Push(2); // 2
  OVER(); // OVER
  Push(0x000a);
  LLINE(); // LLINE
  Push(0x007f);
  Push(2); // 2
  OVER(); // OVER
  Push(0x000a);
  LLINE(); // LLINE
  Push(0x0015);
  Push(0x00c3);
  SetColor("DK-BLUE");
  _dot_1LOGO(); // .1LOGO
  SetColor("BLUE");
  _ex_COLOR(); // !COLOR
  _gt_3FONT(); // >3FONT
  Push(0x0027);
  Push(0x00c2);
  POS_dot_(); // POS.
  PRINT("SHIP", 4); // (.")
  Push(cc__6); // 6
  Push(0x5a48); // pointer to XBLT
  _plus__ex__2(); // +!_2
  PRINT("CONFIGURATION", 13); // (.")
}


// ================================================
// 0xf368: WORD 'UNK_0xf36a' codep=0x224c parp=0xf36a
// ================================================

void UNK_0xf36a() // UNK_0xf36a
{
  Push(0x0056);
  Push(0x5a48); // pointer to XBLT
  _ex__2(); // !_2
}


// ================================================
// 0xf374: WORD 'UNK_0xf376' codep=0x224c parp=0xf376
// ================================================

void UNK_0xf376() // UNK_0xf376
{
  CTINIT(); // CTINIT
  Push(cc__7); // 7
  Push(0x00ae);
  POS_dot_(); // POS.
  PRINT("CARGO PODS", 10); // (.")
  UNK_0xf36a(); // UNK_0xf36a
  PRINT("TOTAL PODS:", 11); // (.")
  Push(cc__7); // 7
  Push(0x00a4);
  POS_dot_(); // POS.
  PRINT("ENGINES", 7); // (.")
  UNK_0xf36a(); // UNK_0xf36a
  PRINT("ENGINES:", 8); // (.")
  Push(cc__7); // 7
  Push(0x009a);
  POS_dot_(); // POS.
  PRINT("SHIELDING", 9); // (.")
  UNK_0xf36a(); // UNK_0xf36a
  PRINT("SHIELDS :", 9); // (.")
  Push(cc__7); // 7
  Push(0x0090);
  POS_dot_(); // POS.
  PRINT("ARMOR", 5); // (.")
  UNK_0xf36a(); // UNK_0xf36a
  PRINT("ARMOR   :", 9); // (.")
  Push(cc__7); // 7
  Push(0x0086);
  POS_dot_(); // POS.
  PRINT("MISSILE LAUNCHER", 16); // (.")
  UNK_0xf36a(); // UNK_0xf36a
  PRINT("MISSILES:", 9); // (.")
  Push(cc__7); // 7
  Push(0x007c);
  POS_dot_(); // POS.
  PRINT("LASER CANNON", 12); // (.")
  UNK_0xf36a(); // UNK_0xf36a
  PRINT("LASERS  :", 9); // (.")
  Push(cc__7); // 7
  Push(0x0072);
  POS_dot_(); // POS.
  PRINT("BLASTO PODS", 11); // (.")
  Push(cc__7); // 7
  Push(0x0068);
  POS_dot_(); // POS.
  PRINT("JUMP PODS", 9); // (.")
}


// ================================================
// 0xf476: WORD 'UNK_0xf478' codep=0x224c parp=0xf478
// ================================================

void UNK_0xf478() // UNK_0xf478
{
  UNK_0xf376(); // UNK_0xf376
  Push(cc__5); // 5
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(0x000f);
  Push(i); // I
  Push(0x000a);
  Push(Pop() * Pop()); // *
  Push(0x002c);
  Push(Pop() + Pop()); // +
  POS_dot_(); // POS.
  PRINT("CLASS ", 6); // (.")
  Push(cc__5); // 5
  Push(i); // I
  _dash_(); // -
  Push(0); // 0
  _dot_R(); // .R
  PRINT("...", 3); // (.")
  i++;
  } while(i<imax); // (LOOP) 0xffd1

  Push(cc__6); // 6
  Push(cc__8); // 8
  POS_dot_(); // POS.
  PRINT("BUY", 3); // (.")
  Push(0x0020);
  Push(cc__8); // 8
  POS_dot_(); // POS.
  PRINT("SELL", 4); // (.")
  Push(0x0044);
  Push(cc__8); // 8
  POS_dot_(); // POS.
  PRINT("REPAIR", 6); // (.")
  Push(0x006a);
  Push(cc__8); // 8
  POS_dot_(); // POS.
  PRINT("NAME", 4); // (.")
  Push(0x0087);
  Push(cc__8); // 8
  POS_dot_(); // POS.
  PRINT("EXIT", 4); // (.")
  Push(0x0056);
  Push(0x0036);
  POS_dot_(); // POS.
  PRINT("MASS:     TONS", 14); // (.")
  Push(0x0056);
  Push(0x002c);
  POS_dot_(); // POS.
  PRINT("ACCEL: ", 7); // (.")
  Push(0x0056);
  Push(0x0022);
  POS_dot_(); // POS.
  PRINT("SHYNEUM:", 8); // (.")
  _ro__dot_MASS_rc_(); // (.MASS)
  _ro__dot_ACC_rc_(); // (.ACC)
  UNK_0xef3a(); // UNK_0xef3a
  _ro_BALANC(); // (BALANC
}


// ================================================
// 0xf549: WORD '(.CONFI' codep=0x224c parp=0xf555
// ================================================
// entry

void _ro__dot_CONFI() // (.CONFI
{
  UNK_0xf277(); // UNK_0xf277
  UNK_0xf478(); // UNK_0xf478
  _ro__dot_SHIP_rc_(); // (.SHIP)
}

// 0xf55d: db 0x53 0x48 0x49 0x00 'SHI '
  