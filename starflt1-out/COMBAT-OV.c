// ====== OVERLAY 'COMBAT-OV' ======
// store offset = 0xdd30
// overlay size   = 0x1830

#include"cpu.h"
#include"starflt1.h"


// =================================
// =========== DICTIONARY ==========
// =================================
// 1703:      UNK_0xdd46  codep:0x224c parp:0xdd46 size:0x0012 C-string:'UNK_0xdd46'
// 1704:      UNK_0xdd5a  codep:0x224c parp:0xdd5a size:0x0008 C-string:'UNK_0xdd5a'
// 1705:      UNK_0xdd64  codep:0x224c parp:0xdd64 size:0x000a C-string:'UNK_0xdd64'
// 1706:      UNK_0xdd70  codep:0x224c parp:0xdd70 size:0x0006 C-string:'UNK_0xdd70'
// 1707:      UNK_0xdd78  codep:0x224c parp:0xdd78 size:0x001e C-string:'UNK_0xdd78'
// 1708:      UNK_0xdd98  codep:0x73ea parp:0xdd98 size:0x001e C-string:'UNK_0xdd98'
// 1709:      UNK_0xddb8  codep:0x744d parp:0xddb8 size:0x0008 C-string:'UNK_0xddb8'
// 1710:      UNK_0xddc2  codep:0x744d parp:0xddc2 size:0x0003 C-string:'UNK_0xddc2'
// 1711:      UNK_0xddc7  codep:0x744d parp:0xddc7 size:0x0003 C-string:'UNK_0xddc7'
// 1712:      UNK_0xddcc  codep:0x744d parp:0xddcc size:0x0034 C-string:'UNK_0xddcc'
// 1713:      UNK_0xde02  codep:0x744d parp:0xde02 size:0x0017 C-string:'UNK_0xde02'
// 1714:      UNK_0xde1b  codep:0x744d parp:0xde1b size:0x001c C-string:'UNK_0xde1b'
// 1715:      UNK_0xde39  codep:0x1d29 parp:0xde39 size:0x0002 C-string:'UNK_0xde39'
// 1716:      UNK_0xde3d  codep:0x1d29 parp:0xde3d size:0x0002 C-string:'UNK_0xde3d'
// 1717:      UNK_0xde41  codep:0x1d29 parp:0xde41 size:0x0034 C-string:'UNK_0xde41'
// 1718:      UNK_0xde77  codep:0x224c parp:0xde77 size:0x0022 C-string:'UNK_0xde77'
// 1719:      UNK_0xde9b  codep:0x224c parp:0xde9b size:0x000e C-string:'UNK_0xde9b'
// 1720:      UNK_0xdeab  codep:0x224c parp:0xdeab size:0x0008 C-string:'UNK_0xdeab'
// 1721:      UNK_0xdeb5  codep:0x224c parp:0xdeb5 size:0x0054 C-string:'UNK_0xdeb5'
// 1722:            X=-1  codep:0x4a4f parp:0xdf12 size:0x0010 C-string:'X_eq__dash_1'
// 1723:             X=0  codep:0x4a4f parp:0xdf2a size:0x0010 C-string:'X_eq_0'
// 1724:             X=1  codep:0x4a4f parp:0xdf42 size:0x0010 C-string:'X_eq_1'
// 1725:       KEY>ORIEN  codep:0x4a4f parp:0xdf60 size:0x00db C-string:'KEY_gt_ORIEN'
// 1726:      UNK_0xe03d  codep:0x1d29 parp:0xe03d size:0x0016 C-string:'UNK_0xe03d'
// 1727:      UNK_0xe055  codep:0x224c parp:0xe055 size:0x0014 C-string:'UNK_0xe055'
// 1728:      UNK_0xe06b  codep:0x224c parp:0xe06b size:0x0054 C-string:'UNK_0xe06b'
// 1729:      UNK_0xe0c1  codep:0x1d29 parp:0xe0c1 size:0x0002 C-string:'UNK_0xe0c1'
// 1730:      UNK_0xe0c5  codep:0x1d29 parp:0xe0c5 size:0x0004 C-string:'UNK_0xe0c5'
// 1731:      UNK_0xe0cb  codep:0x224c parp:0xe0cb size:0x009a C-string:'UNK_0xe0cb'
// 1732:      UNK_0xe167  codep:0x224c parp:0xe167 size:0x008e C-string:'UNK_0xe167'
// 1733:      UNK_0xe1f7  codep:0x1d29 parp:0xe1f7 size:0x0002 C-string:'UNK_0xe1f7'
// 1734:      UNK_0xe1fb  codep:0x2214 parp:0xe1fb size:0x0002 C-string:'UNK_0xe1fb'
// 1735:      UNK_0xe1ff  codep:0x2214 parp:0xe1ff size:0x0006 C-string:'UNK_0xe1ff'
// 1736:      UNK_0xe207  codep:0x2214 parp:0xe207 size:0x0017 C-string:'UNK_0xe207'
// 1737:           CURRX  codep:0xe211 parp:0xe228 size:0x0001 C-string:'CURRX'
// 1738:           CURRY  codep:0xe211 parp:0xe233 size:0x0001 C-string:'CURRY'
// 1739:           DESTX  codep:0xe211 parp:0xe23e size:0x0001 C-string:'DESTX'
// 1740:           DESTY  codep:0xe211 parp:0xe249 size:0x0001 C-string:'DESTY'
// 1741:           MORIG  codep:0xe211 parp:0xe254 size:0x0001 C-string:'MORIG'
// 1742:          MCLASS  codep:0xe211 parp:0xe260 size:0x0001 C-string:'MCLASS'
// 1743:          DELTAX  codep:0xe211 parp:0xe26c size:0x0001 C-string:'DELTAX'
// 1744:          DELTAY  codep:0xe211 parp:0xe278 size:0x0001 C-string:'DELTAY'
// 1745:      UNK_0xe27b  codep:0x224c parp:0xe27b size:0x000c C-string:'UNK_0xe27b'
// 1746:          MARRAY  codep:0x1d29 parp:0xe292 size:0x01ce C-string:'MARRAY'
// 1747:      UNK_0xe462  codep:0x224c parp:0xe462 size:0x000e C-string:'UNK_0xe462'
// 1748:      UNK_0xe472  codep:0x224c parp:0xe472 size:0x000e C-string:'UNK_0xe472'
// 1749:      UNK_0xe482  codep:0x224c parp:0xe482 size:0x0016 C-string:'UNK_0xe482'
// 1750:      UNK_0xe49a  codep:0x224c parp:0xe49a size:0x0006 C-string:'UNK_0xe49a'
// 1751:      UNK_0xe4a2  codep:0x224c parp:0xe4a2 size:0x000a C-string:'UNK_0xe4a2'
// 1752:      UNK_0xe4ae  codep:0x224c parp:0xe4ae size:0x001a C-string:'UNK_0xe4ae'
// 1753:       SCALE>MIS  codep:0x4a4f parp:0xe4d6 size:0x0010 C-string:'SCALE_gt_MIS'
// 1754:      UNK_0xe4e8  codep:0x224c parp:0xe4e8 size:0x0016 C-string:'UNK_0xe4e8'
// 1755:      UNK_0xe500  codep:0x224c parp:0xe500 size:0x0034 C-string:'UNK_0xe500'
// 1756:      UNK_0xe536  codep:0x224c parp:0xe536 size:0x000e C-string:'UNK_0xe536'
// 1757:      UNK_0xe546  codep:0x224c parp:0xe546 size:0x0046 C-string:'UNK_0xe546'
// 1758:      UNK_0xe58e  codep:0x224c parp:0xe58e size:0x001c C-string:'UNK_0xe58e'
// 1759:      UNK_0xe5ac  codep:0x224c parp:0xe5ac size:0x000e C-string:'UNK_0xe5ac'
// 1760:      UNK_0xe5bc  codep:0x224c parp:0xe5bc size:0x001e C-string:'UNK_0xe5bc'
// 1761:      UNK_0xe5dc  codep:0x224c parp:0xe5dc size:0x0016 C-string:'UNK_0xe5dc'
// 1762:      UNK_0xe5f4  codep:0x224c parp:0xe5f4 size:0x0014 C-string:'UNK_0xe5f4'
// 1763:      UNK_0xe60a  codep:0x224c parp:0xe60a size:0x00a0 C-string:'UNK_0xe60a'
// 1764:      UNK_0xe6ac  codep:0x1d29 parp:0xe6ac size:0x0002 C-string:'UNK_0xe6ac'
// 1765:      UNK_0xe6b0  codep:0x1d29 parp:0xe6b0 size:0x007a C-string:'UNK_0xe6b0'
// 1766:      UNK_0xe72c  codep:0x224c parp:0xe72c size:0x0016 C-string:'UNK_0xe72c'
// 1767:      UNK_0xe744  codep:0x224c parp:0xe744 size:0x0052 C-string:'UNK_0xe744'
// 1768:      UNK_0xe798  codep:0x224c parp:0xe798 size:0x00d4 C-string:'UNK_0xe798'
// 1769:      UNK_0xe86e  codep:0x224c parp:0xe86e size:0x0071 C-string:'UNK_0xe86e'
// 1770:      UNK_0xe8e1  codep:0x224c parp:0xe8e1 size:0x002e C-string:'UNK_0xe8e1'
// 1771:      UNK_0xe911  codep:0x224c parp:0xe911 size:0x0016 C-string:'UNK_0xe911'
// 1772:      UNK_0xe929  codep:0x224c parp:0xe929 size:0x0022 C-string:'UNK_0xe929'
// 1773:      UNK_0xe94d  codep:0x224c parp:0xe94d size:0x002e C-string:'UNK_0xe94d'
// 1774:      UNK_0xe97d  codep:0x224c parp:0xe97d size:0x0036 C-string:'UNK_0xe97d'
// 1775:      UNK_0xe9b5  codep:0x224c parp:0xe9b5 size:0x004a C-string:'UNK_0xe9b5'
// 1776:      UNK_0xea01  codep:0x224c parp:0xea01 size:0x00b8 C-string:'UNK_0xea01'
// 1777:      UNK_0xeabb  codep:0x224c parp:0xeabb size:0x0058 C-string:'UNK_0xeabb'
// 1778:      UNK_0xeb15  codep:0x224c parp:0xeb15 size:0x004c C-string:'UNK_0xeb15'
// 1779:      UNK_0xeb63  codep:0x224c parp:0xeb63 size:0x0056 C-string:'UNK_0xeb63'
// 1780:      UNK_0xebbb  codep:0x1d29 parp:0xebbb size:0x0002 C-string:'UNK_0xebbb'
// 1781:      UNK_0xebbf  codep:0x1d29 parp:0xebbf size:0x0002 C-string:'UNK_0xebbf'
// 1782:      UNK_0xebc3  codep:0x224c parp:0xebc3 size:0x0022 C-string:'UNK_0xebc3'
// 1783:      UNK_0xebe7  codep:0x224c parp:0xebe7 size:0x0022 C-string:'UNK_0xebe7'
// 1784:      UNK_0xec0b  codep:0x224c parp:0xec0b size:0x0032 C-string:'UNK_0xec0b'
// 1785:       SET-SCALE  codep:0x224c parp:0xec4b size:0x014d C-string:'SET_dash_SCALE'
// 1786:      UNK_0xed9a  codep:0x224c parp:0xed9a size:0x001a C-string:'UNK_0xed9a'
// 1787:       ?APPROACH  codep:0x224c parp:0xedc2 size:0x000a C-string:'_ask_APPROACH'
// 1788:      UNK_0xedce  codep:0x224c parp:0xedce size:0x002e C-string:'UNK_0xedce'
// 1789:        <COMBAT>  codep:0xb869 parp:0xee09 size:0x00b9 C-string:'_st_COMBAT_gt_'
// 1790:      UNK_0xeec4  codep:0x224c parp:0xeec4 size:0x0012 C-string:'UNK_0xeec4'
// 1791:      UNK_0xeed8  codep:0x224c parp:0xeed8 size:0x0016 C-string:'UNK_0xeed8'
// 1792:      UNK_0xeef0  codep:0x224c parp:0xeef0 size:0x0016 C-string:'UNK_0xeef0'
// 1793:      UNK_0xef08  codep:0x224c parp:0xef08 size:0x0016 C-string:'UNK_0xef08'
// 1794:      UNK_0xef20  codep:0x224c parp:0xef20 size:0x0010 C-string:'UNK_0xef20'
// 1795:          COMBAT  codep:0x224c parp:0xef3b size:0x006c C-string:'COMBAT'
// 1796:        ORIENT>O  codep:0x1d29 parp:0xefb4 size:0x0501 C-string:'ORIENT_gt_O'
// 1797:             WAR  codep:0x224c parp:0xf4bd size:0x0022 C-string:'WAR'
// 1798:        CLEAR-CO  codep:0x224c parp:0xf4ec size:0x0000 C-string:'CLEAR_dash_CO'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xde39[2] = {0x00, 0x00}; // UNK_0xde39
unsigned char UNK_0xde3d[2] = {0x00, 0x00}; // UNK_0xde3d
unsigned char UNK_0xde41[52] = {0x00, 0x00, 0x29, 0x1d, 0x56, 0x3a, 0x20, 0x20, 0x29, 0x1d, 0x3a, 0x20, 0x29, 0x1d, 0x56, 0x3a, 0x20, 0x45, 0x29, 0x1d, 0x00, 0x00, 0x4c, 0x22, 0xb3, 0x0e, 0x92, 0x0f, 0x41, 0x0e, 0xb3, 0x0f, 0xb4, 0x0d, 0x92, 0x0f, 0x41, 0x0e, 0xb3, 0x0f, 0x20, 0x0f, 0x90, 0x0e, 0x20, 0x0f, 0x65, 0x10, 0xa5, 0x4b, 0x90, 0x16}; // UNK_0xde41
unsigned char UNK_0xe03d[22] = {0x56, 0x3a, 0x20, 0x3c, 0x4c, 0x22, 0x98, 0x2a, 0x38, 0x0c, 0x35, 0xe0, 0x3b, 0xe0, 0x38, 0x0c, 0x65, 0x10, 0x46, 0x40, 0x90, 0x16}; // UNK_0xe03d
unsigned char UNK_0xe0c1[2] = {0x3a, 0x20}; // UNK_0xe0c1
unsigned char UNK_0xe0c5[4] = {0x56, 0x3a, 0x20, 0xa0}; // UNK_0xe0c5
unsigned char UNK_0xe1f7[2] = {0x3a, 0x20}; // UNK_0xe1f7
unsigned char MARRAY[462] = {}; // MARRAY
unsigned char UNK_0xe6ac[2] = {0x00, 0x00}; // UNK_0xe6ac
unsigned char UNK_0xe6b0[122] = {0x00, 0x00, 0x4c, 0x22, 0xf5, 0xe1, 0xae, 0x0b, 0xb4, 0x0d, 0xf9, 0xe1, 0x20, 0x0f, 0xb8, 0x15, 0x50, 0x0e, 0x70, 0xe4, 0xf0, 0x0d, 0x80, 0xe4, 0x39, 0x40, 0xfa, 0x15, 0x04, 0x00, 0x4a, 0x17, 0xd0, 0x15, 0xee, 0xff, 0xde, 0x0d, 0xf5, 0xe1, 0xae, 0x0b, 0x90, 0x0e, 0xf5, 0xe1, 0x76, 0x6d, 0x90, 0x16, 0x4c, 0x22, 0xaa, 0xe6, 0xae, 0x0b, 0xf9, 0xe1, 0x46, 0x12, 0xfa, 0x15, 0x34, 0x00, 0x58, 0x51, 0xb2, 0xe6, 0xf5, 0xe1, 0xfd, 0x4b, 0x5e, 0xe2, 0xf5, 0x4b, 0x52, 0xe2, 0xf5, 0x4b, 0x47, 0xe2, 0xfd, 0x4b, 0x3c, 0xe2, 0xfd, 0x4b, 0x31, 0xe2, 0xfd, 0x4b, 0x26, 0xe2, 0xfd, 0x4b, 0x44, 0xe5, 0x2e, 0x0f, 0xaa, 0xe6, 0x11, 0x4c, 0x2e, 0x0f, 0x08, 0xe6, 0x22, 0xe6, 0x60, 0x16, 0x08, 0x00, 0xde, 0x0d, 0xde, 0x0d, 0xde, 0x0d, 0x90, 0x16}; // UNK_0xe6b0
unsigned char UNK_0xebbb[2] = {0x3a, 0x20}; // UNK_0xebbb
unsigned char UNK_0xebbf[2] = {0x3a, 0x20}; // UNK_0xebbf
unsigned char ORIENT_gt_O[1281] = {}; // ORIENT>O

const unsigned short int cc_UNK_0xe1fb = 0x0014; // UNK_0xe1fb
const unsigned short int cc_UNK_0xe1ff = 0x0016; // UNK_0xe1ff
const unsigned short int cc_UNK_0xe207 = 0x0000; // UNK_0xe207


// 0xdd42: db 0x7f 0x01 ' '

// ================================================
// 0xdd44: WORD 'UNK_0xdd46' codep=0x224c parp=0xdd46
// ================================================

void UNK_0xdd46() // UNK_0xdd46
{
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) Push(1); else Push(0); // 0=
}

// 0xdd4c: db 0x4c 0x22 0x2e 0x0f 0x5d 0x17 0x64 0x00 0xd8 0x4a 0x90 0x16 'L". ] d  J  '

// ================================================
// 0xdd58: WORD 'UNK_0xdd5a' codep=0x224c parp=0xdd5a
// ================================================

void UNK_0xdd5a() // UNK_0xdd5a
{
  Push(0); // 0
  Push(2); // 2
  RRND(); // RRND
}


// ================================================
// 0xdd62: WORD 'UNK_0xdd64' codep=0x224c parp=0xdd64
// ================================================

void UNK_0xdd64() // UNK_0xdd64
{
  Push(pp_XABS); // XABS
  Push(Read16(Pop())); // @
  Push(pp_YABS); // YABS
  Push(Read16(Pop())); // @
}


// ================================================
// 0xdd6e: WORD 'UNK_0xdd70' codep=0x224c parp=0xdd70
// ================================================

void UNK_0xdd70() // UNK_0xdd70
{
  _at_IX(); // @IX
  _at_IY(); // @IY
}


// ================================================
// 0xdd76: WORD 'UNK_0xdd78' codep=0x224c parp=0xdd78
// ================================================

void UNK_0xdd78() // UNK_0xdd78
{
  Push(0x63fc); // IFIELD(INST-X)
  Push(Read16(Pop())); // @
  Push(0x63fe); // IFIELD(INST-Y)
  Push(Read16(Pop())); // @
}

// 0xdd82: db 0x4c 0x22 0xc2 0x51 0xf2 0x79 0x90 0x16 0x14 0x22 0x19 0x00 0xea 0x73 0x19 0x0b 0x01 0x49 0x91 0x64 'L" Q y   "   s   I d'

// ================================================
// 0xdd96: WORD 'UNK_0xdd98' codep=0x73ea parp=0xdd98
// ================================================
// 0xdd98: db 0x19 0x0e 0x02 0x49 0x91 0x64 0xea 0x73 0x19 0x13 0x01 0x49 0x91 0x64 0xea 0x73 0x19 0x12 0x01 0x49 0x91 0x64 0xea 0x73 0x19 0x14 0x01 0x49 0x91 0x64 '   I d s   I d s   I d s   I d'

// ================================================
// 0xddb6: WORD 'UNK_0xddb8' codep=0x744d parp=0xddb8
// ================================================
// 0xddb8: db 0x19 0x0c 0x01 0x4d 0x74 0x19 0x12 0x02 '   Mt   '

// ================================================
// 0xddc0: WORD 'UNK_0xddc2' codep=0x744d parp=0xddc2
// ================================================
// 0xddc2: db 0x19 0x14 0x02 '   '

// ================================================
// 0xddc5: WORD 'UNK_0xddc7' codep=0x744d parp=0xddc7
// ================================================
// 0xddc7: db 0x19 0x11 0x01 '   '

// ================================================
// 0xddca: WORD 'UNK_0xddcc' codep=0x744d parp=0xddcc
// ================================================
// 0xddcc: db 0x19 0x0b 0x01 0x14 0x22 0x14 0x00 0x4d 0x74 0x14 0x11 0x01 0x4d 0x74 0x14 0x12 0x01 0x4d 0x74 0x14 0x13 0x01 0x4d 0x74 0x14 0x14 0x01 0x4d 0x74 0x14 0x15 0x01 0x4d 0x74 0x14 0x16 0x01 0x4d 0x74 0x14 0x17 0x01 0x4d 0x74 0x14 0x18 0x01 0x4d 0x74 0x14 0x19 0x01 '    "  Mt   Mt   Mt   Mt   Mt   Mt   Mt   Mt   Mt   '

// ================================================
// 0xde00: WORD 'UNK_0xde02' codep=0x744d parp=0xde02
// ================================================
// 0xde02: db 0x14 0x1a 0x01 0x4d 0x74 0x14 0x1b 0x01 0x4d 0x74 0x14 0x1c 0x01 0x4d 0x74 0x14 0x1d 0x01 0x4d 0x74 0x14 0x1e 0x01 '   Mt   Mt   Mt   Mt   '

// ================================================
// 0xde19: WORD 'UNK_0xde1b' codep=0x744d parp=0xde1b
// ================================================
// 0xde1b: db 0x14 0x30 0x02 0x4d 0x74 0x14 0x45 0x04 0x4d 0x74 0x14 0x32 0x02 0x4d 0x74 0x14 0x49 0x01 0x4d 0x74 0x11 0x17 0x03 0x4d 0x74 0x10 0x1b 0x01 ' 0 Mt E Mt 2 Mt I Mt   Mt   '

// ================================================
// 0xde37: WORD 'UNK_0xde39' codep=0x1d29 parp=0xde39
// ================================================
// 0xde39: db 0x00 0x00 '  '

// ================================================
// 0xde3b: WORD 'UNK_0xde3d' codep=0x1d29 parp=0xde3d
// ================================================
// 0xde3d: db 0x00 0x00 '  '

// ================================================
// 0xde3f: WORD 'UNK_0xde41' codep=0x1d29 parp=0xde41
// ================================================
// 0xde41: db 0x00 0x00 0x29 0x1d 0x56 0x3a 0x20 0x20 0x29 0x1d 0x3a 0x20 0x29 0x1d 0x56 0x3a 0x20 0x45 0x29 0x1d 0x00 0x00 0x4c 0x22 0xb3 0x0e 0x92 0x0f 0x41 0x0e 0xb3 0x0f 0xb4 0x0d 0x92 0x0f 0x41 0x0e 0xb3 0x0f 0x20 0x0f 0x90 0x0e 0x20 0x0f 0x65 0x10 0xa5 0x4b 0x90 0x16 '  ) V:  ) : ) V: E)   L"    A       A         e  K  '

// ================================================
// 0xde75: WORD 'UNK_0xde77' codep=0x224c parp=0xde77
// ================================================

void UNK_0xde77() // UNK_0xde77
{
  Push(Read8(Pop())&0xFF); // C@
  Push(cc__7); // 7
  Push(Pop() & Pop()); // AND
}

// 0xde7f: db 0x4c 0x22 0x92 0x0c 0x41 0x0e 0x5d 0x17 0x7f 0x00 0xf5 0x12 0xf2 0x0e 0x5d 0x17 0x80 0x00 0xf5 0x12 0x07 0x13 0xb3 0x0f 0x90 0x16 'L"  A ]      ]           '

// ================================================
// 0xde99: WORD 'UNK_0xde9b' codep=0x224c parp=0xde9b
// ================================================

void UNK_0xde9b() // UNK_0xde9b
{

  label1:
  Push(cc__dash_1); // -1
  Push(2); // 2
  RRND(); // RRND
  if (Read16(sp) != 0) Push(Read16(sp)); // ?DUP
  if (Pop() == 0) goto label1;
}


// ================================================
// 0xdea9: WORD 'UNK_0xdeab' codep=0x224c parp=0xdeab
// ================================================

void UNK_0xdeab() // UNK_0xdeab
{
  Push(cc__dash_1); // -1
  Push(2); // 2
  RRND(); // RRND
}


// ================================================
// 0xdeb3: WORD 'UNK_0xdeb5' codep=0x224c parp=0xdeb5
// ================================================

void UNK_0xdeb5() // UNK_0xdeb5
{
  _2OVER(); // 2OVER
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label1;
  Pop(); Pop();// 2DROP
  Push2Words("0.");
  return;

  label1:
  _2OVER(); // 2OVER
  ROT(); // ROT
  SWAP(); // SWAP
  _dash_(); // -
  Push(Read16(sp)); // DUP
  if (Pop() == 0) goto label2;
  Push(Read16(sp)); // DUP
  ABS(); // ABS
  _slash_(); // /

  label2:
  _gt_R(); // >R
  _dash_(); // -
  Push(Read16(sp)); // DUP
  if (Pop() == 0) goto label3;
  Push(Read16(sp)); // DUP
  ABS(); // ABS
  _slash_(); // /

  label3:
  R_gt_(); // R>
}

// 0xdeef: db 0x4c 0x22 0x1e 0x75 0x5d 0x17 0x19 0x00 0x5f 0x12 0xfa 0x15 0x08 0x00 0xca 0xdd 0x60 0x16 0x04 0x00 0x28 0xde 0x92 0x0c 0x90 0x16 'L" u]   _       `   (     '

// ================================================
// 0xdf09: WORD 'X=-1' codep=0x4a4f parp=0xdf12
// ================================================
// 0xdf12: db 0x03 0x00 0x6e 0x3a 0xff 0xff 0x85 0x3b 0x01 0x00 0x75 0x3b 0x00 0x00 0x7d 0x3b '  n:   ;  u;  };'

// ================================================
// 0xdf22: WORD 'X=0' codep=0x4a4f parp=0xdf2a
// ================================================
// 0xdf2a: db 0x03 0x00 0x6e 0x3a 0xff 0xff 0x8d 0x3b 0x01 0x00 0x3f 0x0f 0x00 0x00 0xf1 0xde '  n:   ;  ?     '

// ================================================
// 0xdf3a: WORD 'X=1' codep=0x4a4f parp=0xdf42
// ================================================
// 0xdf42: db 0x03 0x00 0x6e 0x3a 0xff 0xff 0x95 0x3b 0x01 0x00 0x30 0x0f 0x00 0x00 0x22 0x0f '  n:   ;  0   " '

// ================================================
// 0xdf52: WORD 'KEY>ORIEN' codep=0x4a4f parp=0xdf60
// ================================================
// 0xdf60: db  '  n:      *   B L"S   D  T  {;-   A S vm  L"L s;F   L"n b W s;F   L" P    D     L" P    D     L" P    D   >     L"    A      ;  L F ;   ]   F     L"    A      ;  L F   Mt4  L"6^  D   L"L ] F F   L" b I y     yD   h;    '

// ================================================
// 0xe03b: WORD 'UNK_0xe03d' codep=0x1d29 parp=0xe03d
// ================================================
// 0xe03d: db 0x56 0x3a 0x20 0x3c 0x4c 0x22 0x98 0x2a 0x38 0x0c 0x35 0xe0 0x3b 0xe0 0x38 0x0c 0x65 0x10 0x46 0x40 0x90 0x16 'V: <L" *8 5 ; 8 e F@  '

// ================================================
// 0xe053: WORD 'UNK_0xe055' codep=0x224c parp=0xe055
// ================================================

void UNK_0xe055() // UNK_0xe055
{
  TIME(); // TIME
  _2_at_(); // 2@
  Push(pp_UNK_0xe03d); // UNK_0xe03d
  D_ex_(); // D!
}

// 0xe05f: db 0x4c 0x22 0x4c 0xdd 0x83 0x3b 0x46 0x12 0x90 0x16 'L"L  ;F   '

// ================================================
// 0xe069: WORD 'UNK_0xe06b' codep=0x224c parp=0xe06b
// ================================================

void UNK_0xe06b() // UNK_0xe06b
{
  Push(0x63fb); // IFIELD(UNK_0xddb8)
  Push(Read8(Pop())&0xFF); // C@
  UNK_0xdd46(); // UNK_0xdd46
}

// 0xe073: db 0x14 0x22 0x0b 0x00 0x14 0x22 0x05 0x00 0x4c 0x22 0xb4 0x0d 0x76 0xdd 0x62 0xdd 0xb3 0x0e 0x92 0x0f 0x58 0x10 0xa2 0x0e 0x46 0x12 0xb4 0x0d 0x92 0x0f 0x58 0x10 0x90 0x0e 0xf2 0x0e 0x90 0x0e 0x46 0x12 0xf5 0x12 0x90 0x16 0x4c 0x22 0x73 0xe0 0x7b 0xe0 0x07 0x13 0x90 0x16 0x4c 0x22 0x77 0xe0 0x7b 0xe0 0x90 0x16 0x4c 0x22 0x9f 0xe0 0x07 0x13 0xa9 0xe0 0x07 0x13 0xf5 0x12 0x90 0x16 ' "   "  L"  v b     X   F     X       F     L"s {     L"w {   L"            '

// ================================================
// 0xe0bf: WORD 'UNK_0xe0c1' codep=0x1d29 parp=0xe0c1
// ================================================
// 0xe0c1: db 0x3a 0x20 ': '

// ================================================
// 0xe0c3: WORD 'UNK_0xe0c5' codep=0x1d29 parp=0xe0c5
// ================================================
// 0xe0c5: db 0x56 0x3a 0x20 0xa0 'V:  '

// ================================================
// 0xe0c9: WORD 'UNK_0xe0cb' codep=0x224c parp=0xe0cb
// ================================================

void UNK_0xe0cb() // UNK_0xe0cb
{
  Push(pp_UNK_0xe0c1); // UNK_0xe0c1
  _st__ex__gt_(); // <!>
  TIME(); // TIME
  _2_at_(); // 2@
  Push(pp_UNK_0xe0c5); // UNK_0xe0c5
  _2_ex_(); // 2!
}

// 0xe0d9: db  'L" *8   8       e F@  L" Q y  u D    D    y  L" Q y  u D    D    y  L" T  . _   L" T   ;_   L" T  ]   _   )   L"L ] Z F     E  ;S E   D   '

// ================================================
// 0xe165: WORD 'UNK_0xe167' codep=0x224c parp=0xe167
// ================================================

void UNK_0xe167() // UNK_0xe167
{
  Push(pp_UNK_0xde41); // UNK_0xde41
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1;
  Push(cc__dash_1); // -1
  Push(1); // 1
  RRND(); // RRND
  goto label2;

  label1:
  Push(0); // 0

  label2:
  Push(Pop() + Pop()); // +
}

// 0xe17f: db 0x4c 0x22 0x1e 0xde 0x92 0x0c 0x9b 0x3b 0xf5 0x12 0x44 0xdd 0x90 0x16 0x4c 0x22 0x19 0xde 0xae 0x0b 0x44 0xdd 0x7f 0xe1 0xf5 0x12 0xfa 0x5c 0xae 0x0b 0x89 0x12 0xf5 0x12 0x90 0x16 0x4c 0x22 0x7f 0x5d 0xae 0x0b 0x2e 0x0f 0x5f 0x12 0xfa 0x15 0x0a 0x00 0x5d 0x17 0x05 0xbf 0x60 0x16 0x06 0x00 0x5d 0x17 0xf3 0xbe 0x90 0x16 0x4c 0x22 0x7f 0x5d 0xae 0x0b 0x73 0x3b 0x5f 0x12 0xfa 0x15 0x0a 0x00 0x5d 0x17 0x4b 0xbe 0x60 0x16 0x06 0x00 0x5d 0x17 0x2b 0xbe 0x90 0x16 0x4c 0x22 0x23 0xde 0xae 0x0b 0x44 0xdd 0x8d 0xe1 0x3e 0x13 0xfa 0x15 0x08 0x00 0xa3 0xe1 0x60 0x16 0x04 0x00 0xbf 0xe1 0x90 0x16 'L"     ;  D   L"    D     \        L"]  . _     ]   `   ]     L"]  s;_     ] K `   ] +   L"#   D   >       `       '

// ================================================
// 0xe1f5: WORD 'UNK_0xe1f7' codep=0x1d29 parp=0xe1f7
// ================================================
// 0xe1f7: db 0x3a 0x20 ': '

// ================================================
// 0xe1f9: WORD 'UNK_0xe1fb' codep=0x2214 parp=0xe1fb
// ================================================
// 0xe1fb: db 0x14 0x00 '  '

// ================================================
// 0xe1fd: WORD 'UNK_0xe1ff' codep=0x2214 parp=0xe1ff
// ================================================
// 0xe1ff: db 0x16 0x00 0x14 0x22 0x01 0x00 '   "  '

// ================================================
// 0xe205: WORD 'UNK_0xe207' codep=0x2214 parp=0xe207
// ================================================
// 0xe207: db 0x00 0x00 0x4c 0x22 0x4f 0x06 0xaf 0x21 0xfa 0x1b '  L"O  !  '
// 0xe211: call   1649
  Push(Read8(Pop())&0xFF); // C@
  Push(pp_UNK_0xe1f7); // UNK_0xe1f7
  Push(Read16(Pop())); // @
  Push(Pop() + Pop()); // +
}


// ================================================
// 0xe21e: WORD 'CURRX' codep=0xe211 parp=0xe228
// ================================================
// 0xe228: db 0x00 ' '

// ================================================
// 0xe229: WORD 'CURRY' codep=0xe211 parp=0xe233
// ================================================
// 0xe233: db 0x04 ' '

// ================================================
// 0xe234: WORD 'DESTX' codep=0xe211 parp=0xe23e
// ================================================
// 0xe23e: db 0x08 ' '

// ================================================
// 0xe23f: WORD 'DESTY' codep=0xe211 parp=0xe249
// ================================================
// 0xe249: db 0x0a ' '

// ================================================
// 0xe24a: WORD 'MORIG' codep=0xe211 parp=0xe254
// ================================================
// 0xe254: db 0x0c ' '

// ================================================
// 0xe255: WORD 'MCLASS' codep=0xe211 parp=0xe260
// ================================================
// 0xe260: db 0x0d ' '

// ================================================
// 0xe261: WORD 'DELTAX' codep=0xe211 parp=0xe26c
// ================================================
// 0xe26c: db 0x0e ' '

// ================================================
// 0xe26d: WORD 'DELTAY' codep=0xe211 parp=0xe278
// ================================================
// 0xe278: db 0x12 ' '

// ================================================
// 0xe279: WORD 'UNK_0xe27b' codep=0x224c parp=0xe27b
// ================================================

void UNK_0xe27b() // UNK_0xe27b
{
  MCLASS(); // MCLASS
  Push(Read8(Pop())&0xFF); // C@
  Push(0x000f);
  Push(Pop() & Pop()); // AND
}


// ================================================
// 0xe287: WORD 'MARRAY' codep=0x1d29 parp=0xe292
// ================================================
// 0xe292: db  '                                                                                                                                                                                                                                                                                                                                                                                                                                                        D     Mt#  Mt0  Mt$  M'

// ================================================
// 0xe460: WORD 'UNK_0xe462' codep=0x224c parp=0xe462
// ================================================

void UNK_0xe462() // UNK_0xe462
{
  Push(pp_MARRAY); // MARRAY
  Push(cc_UNK_0xe1fb); // UNK_0xe1fb
  Push(cc_UNK_0xe1ff); // UNK_0xe1ff
  Push(Pop() * Pop()); // *
  Push(0); // 0
  FILL_1(); // FILL_1
}


// ================================================
// 0xe470: WORD 'UNK_0xe472' codep=0x224c parp=0xe472
// ================================================

void UNK_0xe472() // UNK_0xe472
{
  Push(cc_UNK_0xe1ff); // UNK_0xe1ff
  Push(Pop() * Pop()); // *
  Push(pp_MARRAY); // MARRAY
  Push(Pop() + Pop()); // +
  Push(pp_UNK_0xe1f7); // UNK_0xe1f7
  _st__ex__gt_(); // <!>
}


// ================================================
// 0xe480: WORD 'UNK_0xe482' codep=0x224c parp=0xe482
// ================================================

void UNK_0xe482() // UNK_0xe482
{
  CURRX(); // CURRX
  Push(Read16(Pop())); // @
  CURRY(); // CURRY
  Push(Read16(Pop())); // @
}

// 0xe48c: db 0x4c 0x22 0x3c 0xe2 0xae 0x0b 0x47 0xe2 0xae 0x0b 0x90 0x16 'L"<   G     '

// ================================================
// 0xe498: WORD 'UNK_0xe49a' codep=0x224c parp=0xe49a
// ================================================

void UNK_0xe49a() // UNK_0xe49a
{
  PRINT(".", 1); // (.")
}


// ================================================
// 0xe4a0: WORD 'UNK_0xe4a2' codep=0x224c parp=0xe4a2
// ================================================

void UNK_0xe4a2() // UNK_0xe4a2
{
  _gt_2FONT(); // >2FONT
  PRINT(".", 1); // (.")
  _gt_1FONT(); // >1FONT
}


// ================================================
// 0xe4ac: WORD 'UNK_0xe4ae' codep=0x224c parp=0xe4ae
// ================================================

void UNK_0xe4ae() // UNK_0xe4ae
{
  CURRX(); // CURRX
  Push(Read16(Pop())); // @
  Push(2); // 2
  MOD(); // MOD
  if (Pop() == 0) goto label1;
  PRINT("+", 1); // (.")
  return;

  label1:
  PRINT("*", 1); // (.")
}


// ================================================
// 0xe4c8: WORD 'SCALE>MIS' codep=0x4a4f parp=0xe4d6
// ================================================
// 0xe4d6: db 0x03 0x00 0x48 0x3a 0x02 0x00 0x9a 0xe4 0x04 0x00 0xa2 0xe4 0x08 0x00 0xae 0xe4 '  H:            '

// ================================================
// 0xe4e6: WORD 'UNK_0xe4e8' codep=0x224c parp=0xe4e8
// ================================================

void UNK_0xe4e8() // UNK_0xe4e8
{
  MORIG(); // MORIG
  Push(Read8(Pop())&0xFF); // C@
  Push(cc_UNK_0xe207); // UNK_0xe207
  _eq_(); // =
  if (Pop() == 0) goto label1;
  SetColor("RED");
  return;

  label1:
  SetColor("LT-BLUE");
}


// ================================================
// 0xe4fe: WORD 'UNK_0xe500' codep=0x224c parp=0xe500
// ================================================

void UNK_0xe500() // UNK_0xe500
{
  Push(cc_UNK_0xe1fb); // UNK_0xe1fb
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  UNK_0xe472(); // UNK_0xe472
  UNK_0xe27b(); // UNK_0xe27b
  if (Pop() == 0) goto label1;
  UNK_0xe482(); // UNK_0xe482
  WLD_gt_SCR(); // WLD>SCR
  SCR_gt_BLT(); // SCR>BLT
  SWAP(); // SWAP
  Push(cc__3); // 3
  Push(Pop() + Pop()); // +
  SWAP(); // SWAP
  POS_dot_(); // POS.
  UNK_0xe4e8(); // UNK_0xe4e8
  _ex_COLOR(); // !COLOR
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  Push(Read16(Pop())); // @
  Pop();
  switch(Pop()) // SCALE>MIS
  {
  case 2:
    UNK_0xe49a(); // UNK_0xe49a
    break;
  case 4:
    UNK_0xe4a2(); // UNK_0xe4a2
    break;
  case 8:
    UNK_0xe4ae(); // UNK_0xe4ae
    break;
  default:
    NOP(); // NOP
    break;

  }

  label1:
  i++;
  } while(i<imax); // (LOOP) 0xffda

  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
}


// ================================================
// 0xe534: WORD 'UNK_0xe536' codep=0x224c parp=0xe536
// ================================================

void UNK_0xe536() // UNK_0xe536
{
  _gt_R(); // >R
  Push(0); // 0
  SWAP(); // SWAP
  Push(1); // 1
  R_gt_(); // R>
  M_star__slash_(); // M*/
}


// ================================================
// 0xe544: WORD 'UNK_0xe546' codep=0x224c parp=0xe546
// ================================================

void UNK_0xe546() // UNK_0xe546
{
  DESTX(); // DESTX
  Push(Read16(Pop())); // @
  CURRX(); // CURRX
  Push(Read16(Pop())); // @
  _dash_(); // -
  Push(Read16(sp)); // DUP
  ABS(); // ABS
  DESTY(); // DESTY
  Push(Read16(Pop())); // @
  CURRY(); // CURRY
  Push(Read16(Pop())); // @
  _dash_(); // -
  Push(Read16(sp)); // DUP
  ABS(); // ABS
  ROT(); // ROT
  MAX(); // MAX
  _gt_R(); // >R
  R_at_(); // R@
  UNK_0xe536(); // UNK_0xe536
  DELTAY(); // DELTAY
  _2_ex_(); // 2!
  R_gt_(); // R>
  UNK_0xe536(); // UNK_0xe536
  DELTAX(); // DELTAX
  _2_ex_(); // 2!
  Push(0x8000);
  Push(Read16(sp)); // DUP
  CURRX(); // CURRX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  CURRY(); // CURRY
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
}


// ================================================
// 0xe58c: WORD 'UNK_0xe58e' codep=0x224c parp=0xe58e
// ================================================

void UNK_0xe58e() // UNK_0xe58e
{
  Push(pp_ILOCAL); // ILOCAL
  Push(Read16(Pop())); // @
  UNK_0xe167(); // UNK_0xe167
  if (Read16(sp) != 0) Push(Read16(sp)); // ?DUP
  if (Pop() == 0) return;
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  POINT_gt_ICON(); // POINT>ICON
  _dot_ICON(); // .ICON
  i++;
  } while(i<imax); // (LOOP) 0xfff8

}


// ================================================
// 0xe5aa: WORD 'UNK_0xe5ac' codep=0x224c parp=0xe5ac
// ================================================

void UNK_0xe5ac() // UNK_0xe5ac
{
  Push(0xec78);
  Push(Read16(sp)); // DUP
  Push(pp_ANCHOR_dash_CONTOUR); // ANCHOR-CONTOUR
  _2_ex_(); // 2!
  ORGLIST(); // ORGLIST
}


// ================================================
// 0xe5ba: WORD 'UNK_0xe5bc' codep=0x224c parp=0xe5bc
// ================================================

void UNK_0xe5bc() // UNK_0xe5bc
{
  CURRY(); // CURRY
  _2_at_(); // 2@
  DELTAY(); // DELTAY
  _2_at_(); // 2@
  D_plus_(); // D+
  CURRY(); // CURRY
  _2_ex_(); // 2!
  CURRX(); // CURRX
  _2_at_(); // 2@
  DELTAX(); // DELTAX
  _2_at_(); // 2@
  D_plus_(); // D+
  CURRX(); // CURRX
  _2_ex_(); // 2!
}


// ================================================
// 0xe5da: WORD 'UNK_0xe5dc' codep=0x224c parp=0xe5dc
// ================================================

void UNK_0xe5dc() // UNK_0xe5dc
{
  Push(pp__ask_NEB); // ?NEB
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1;
  SetColor("DK-GREEN");
  goto label2;

  label1:
  SetColor("BLACK");

  label2:
  _ex_COLOR(); // !COLOR
  BFILL(); // BFILL
}


// ================================================
// 0xe5f2: WORD 'UNK_0xe5f4' codep=0x224c parp=0xe5f4
// ================================================

void UNK_0xe5f4() // UNK_0xe5f4
{
  UNK_0xe5ac(); // UNK_0xe5ac
  _gt_MAINVIEW(); // >MAINVIEW
  UNK_0xe5dc(); // UNK_0xe5dc
  VCLIPSET(); // VCLIPSET
  UNK_0xe500(); // UNK_0xe500
  UNK_0xe58e(); // UNK_0xe58e
  V_gt_DISPLAY(); // V>DISPLAY
  _gt_DISPLAY(); // >DISPLAY
  DCLIPSET(); // DCLIPSET
}


// ================================================
// 0xe608: WORD 'UNK_0xe60a' codep=0x224c parp=0xe60a
// ================================================

void UNK_0xe60a() // UNK_0xe60a
{
  MORIG(); // MORIG
  Push(Read8(Pop())&0xFF); // C@
  Push(cc_UNK_0xe207); // UNK_0xe207
  _eq_(); // =
  if (Pop() == 0) goto label1;
  Push(pp_UNK_0xde3d); // UNK_0xde3d
  goto label2;

  label1:
  Push(pp_UNK_0xde39); // UNK_0xde39

  label2:
  _st__plus__ex__gt_(); // <+!>
}

// 0xe622: db  'L"4     . ]       % P  &  ]   CR      F:    ]   \   Q&   "  L"] d ]    U  8O_ A     ] \ vm        % P P    J &s;CR      F:    Z \   Q&  '

// ================================================
// 0xe6aa: WORD 'UNK_0xe6ac' codep=0x1d29 parp=0xe6ac
// ================================================
// 0xe6ac: db 0x00 0x00 '  '

// ================================================
// 0xe6ae: WORD 'UNK_0xe6b0' codep=0x1d29 parp=0xe6b0
// ================================================
// 0xe6b0: db 0x00 0x00 0x4c 0x22 0xf5 0xe1 0xae 0x0b 0xb4 0x0d 0xf9 0xe1 0x20 0x0f 0xb8 0x15 0x50 0x0e 0x70 0xe4 0xf0 0x0d 0x80 0xe4 0x39 0x40 0xfa 0x15 0x04 0x00 0x4a 0x17 0xd0 0x15 0xee 0xff 0xde 0x0d 0xf5 0xe1 0xae 0x0b 0x90 0x0e 0xf5 0xe1 0x76 0x6d 0x90 0x16 0x4c 0x22 0xaa 0xe6 0xae 0x0b 0xf9 0xe1 0x46 0x12 0xfa 0x15 0x34 0x00 0x58 0x51 0xb2 0xe6 0xf5 0xe1 0xfd 0x4b 0x5e 0xe2 0xf5 0x4b 0x52 0xe2 0xf5 0x4b 0x47 0xe2 0xfd 0x4b 0x3c 0xe2 0xfd 0x4b 0x31 0xe2 0xfd 0x4b 0x26 0xe2 0xfd 0x4b 0x44 0xe5 0x2e 0x0f 0xaa 0xe6 0x11 0x4c 0x2e 0x0f 0x08 0xe6 0x22 0xe6 0x60 0x16 0x08 0x00 0xde 0x0d 0xde 0x0d 0xde 0x0d 0x90 0x16 '  L"            P p     9@    J               vm  L"      F   4 XQ     K^  KR  KG  K<  K1  K&  KD .    L.   " `           '

// ================================================
// 0xe72a: WORD 'UNK_0xe72c' codep=0x224c parp=0xe72c
// ================================================

void UNK_0xe72c() // UNK_0xe72c
{
  Push(cc__dash_1); // -1
  UNK_0xe60a(); // UNK_0xe60a
  Push(pp_UNK_0xe1f7); // UNK_0xe1f7
  Push(Read16(Pop())); // @
  Push(cc_UNK_0xe1ff); // UNK_0xe1ff
  Push(0); // 0
  FILL_1(); // FILL_1
  Push(cc__dash_1); // -1
  Push(pp_UNK_0xe6ac); // UNK_0xe6ac
  _st__plus__ex__gt_(); // <+!>
}


// ================================================
// 0xe742: WORD 'UNK_0xe744' codep=0x224c parp=0xe744
// ================================================

void UNK_0xe744() // UNK_0xe744
{
  UNK_0xe482(); // UNK_0xe482
  Push(pp_ILOCAL); // ILOCAL
  Push(Read16(Pop())); // @
  _ask_ICONS_dash_AT(); // ?ICONS-AT
  Push(Read16(sp)); // DUP
  if (Pop() == 0) return;
  Push(0); // 0
  OVER(); // OVER
  _gt_R(); // >R
  SWAP(); // SWAP
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  SWAP(); // SWAP
  POINT_gt_ICON(); // POINT>ICON
  MORIG(); // MORIG
  Push(Read8(Pop())&0xFF); // C@
  Push(cc_UNK_0xe207); // UNK_0xe207
  _eq_(); // =
  if (Pop() == 0) goto label1;
  _at_ID(); // @ID
  Push(0x001b);
  Push(0x0023);
  WITHIN(); // WITHIN
  if (Pop() == 0) goto label2;
  Pop(); // DROP
  Push(1); // 1

  label2:
  goto label3;

  label1:
  Pop(); // DROP
  Push(1); // 1

  label3:
  i++;
  } while(i<imax); // (LOOP) 0xffd2

  R_gt_(); // R>
  UNK_0xdd46(); // UNK_0xdd46
  Push(Pop() & Pop()); // AND
}


// ================================================
// 0xe796: WORD 'UNK_0xe798' codep=0x224c parp=0xe798
// ================================================

void UNK_0xe798() // UNK_0xe798
{
  Push(cc_UNK_0xe1fb); // UNK_0xe1fb
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  UNK_0xe472(); // UNK_0xe472
  UNK_0xe27b(); // UNK_0xe27b
  if (Pop() == 0) goto label1;
  UNK_0xe482(); // UNK_0xe482
  _ask_INVIS(); // ?INVIS
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label2;
  UNK_0xe72c(); // UNK_0xe72c
  goto label1;

  label2:
  UNK_0xe744(); // UNK_0xe744
  if (Pop() == 0) goto label1;
  Push(1); // 1
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  _plus__ex_(); // +!

  label1:
  i++;
  } while(i<imax); // (LOOP) 0xffd8

}

// 0xe7ca: db  'L" # Vvm #]   r  Vvm #] ( r  Vvm #] < r  Vvm=  Vvm #  P vmP   vmP {;r vm   ;r vm   V    P  ;r   P {;r   P          @ @9@ V  s;_          V   ;r A          V      '

// ================================================
// 0xe86c: WORD 'UNK_0xe86e' codep=0x224c parp=0xe86e
// ================================================

void UNK_0xe86e() // UNK_0xe86e
{
  Push(pp_XORMODE); // XORMODE
  Push(Read16(Pop())); // @
  Push(pp_XORMODE); // XORMODE
  OFF(); // OFF
  CI(); // CI
  _ask_ICON_eq_I(); // ?ICON=I
  if (Pop() == 0) Push(1); else Push(0); // 0=
  ABORT("ERR: MOVE-ICON", 14);// (ABORT")
  POINT_gt_ICON(); // POINT>ICON
  UNK_0xdd78(); // UNK_0xdd78
  _ex_IY(); // !IY
  _ex_IX(); // !IX
  UNK_0xe5ac(); // UNK_0xe5ac
  UNK_0xe5f4(); // UNK_0xe5f4
  Push(pp_XORMODE); // XORMODE
  _st__ex__gt_(); // <!>
}

// 0xe89f: db 0x4c 0x22 0xb3 0xde 0xb3 0x0e 0x72 0x0f 0xc5 0x74 0x76 0x6d 0x72 0x0f 0xb7 0x74 0x76 0x6d 0x90 0x16 0x4c 0x22 0x0d 0x40 0x0d 0x40 0x39 0x40 0xfa 0x15 0x0c 0x00 0xde 0x0d 0x99 0xde 0x99 0xde 0x60 0x16 0x04 0x00 0xb3 0xde 0xc6 0x11 0xb3 0x0e 0x72 0x0f 0xc5 0x74 0x76 0x6d 0xc6 0x11 0x72 0x0f 0xb7 0x74 0x76 0x6d 0x90 0x16 'L"    r  tvmr  tvm  L" @ @9@          `         r  tvm  r  tvm  '

// ================================================
// 0xe8df: WORD 'UNK_0xe8e1' codep=0x224c parp=0xe8e1
// ================================================

void UNK_0xe8e1() // UNK_0xe8e1
{
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  Push(Read16(Pop())); // @
  _gt_R(); // >R
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  OFF(); // OFF
  Push(cc_UNK_0xe1fb); // UNK_0xe1fb
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  UNK_0xe472(); // UNK_0xe472
  UNK_0xe27b(); // UNK_0xe27b
  if (Pop() == 0) goto label1;
  UNK_0xe744(); // UNK_0xe744
  if (Pop() == 0) goto label1;
  LEAVE(); // LEAVE

  label1:
  i++;
  } while(i<imax); // (LOOP) 0xffec

  R_gt_(); // R>
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  _st__ex__gt_(); // <!>
}


// ================================================
// 0xe90f: WORD 'UNK_0xe911' codep=0x224c parp=0xe911
// ================================================

void UNK_0xe911() // UNK_0xe911
{
  Push(pp_UNK_0xde41); // UNK_0xde41
  Push(Read16(Pop())); // @
  if (Pop() == 0) return;
  UNK_0xdeab(); // UNK_0xdeab
  Push(Pop() + Pop()); // +
  SWAP(); // SWAP
  UNK_0xdeab(); // UNK_0xdeab
  Push(Pop() + Pop()); // +
  SWAP(); // SWAP
}


// ================================================
// 0xe927: WORD 'UNK_0xe929' codep=0x224c parp=0xe929
// ================================================

void UNK_0xe929() // UNK_0xe929
{
  UNK_0xe27b(); // UNK_0xe27b
  Push(cc__6); // 6
  _eq_(); // =
  if (Pop() == 0) return;
  UNK_0xdd64(); // UNK_0xdd64
  UNK_0xe911(); // UNK_0xe911
  SWAP(); // SWAP
  DESTX(); // DESTX
  D_ex_(); // D!
  UNK_0xe546(); // UNK_0xe546
  UNK_0xe744(); // UNK_0xe744
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) return;
  UNK_0xe5bc(); // UNK_0xe5bc
}


// ================================================
// 0xe94b: WORD 'UNK_0xe94d' codep=0x224c parp=0xe94d
// ================================================

void UNK_0xe94d() // UNK_0xe94d
{
  Push(cc_UNK_0xe1fb); // UNK_0xe1fb
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  UNK_0xe472(); // UNK_0xe472
  MCLASS(); // MCLASS
  Push(Read8(Pop())&0xFF); // C@
  if (Pop() == 0) goto label1;
  UNK_0xe929(); // UNK_0xe929
  UNK_0xe5bc(); // UNK_0xe5bc

  label1:
  i++;
  } while(i<imax); // (LOOP) 0xffee

}

// 0xe969: db 0x4c 0x22 0x5d 0x17 0x0a 0x00 0x5d 0x17 0xd8 0xbd 0xce 0x84 0x3f 0xde 0x76 0x6d 0x90 0x16 'L"]   ]     ? vm  '

// ================================================
// 0xe97b: WORD 'UNK_0xe97d' codep=0x224c parp=0xe97d
// ================================================

void UNK_0xe97d() // UNK_0xe97d
{
  UNK_0xe94d(); // UNK_0xe94d
  UNK_0xe5f4(); // UNK_0xe5f4
  UNK_0xe798(); // UNK_0xe798
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  Push(Read16(Pop())); // @
  if (Read16(sp) != 0) Push(Read16(sp)); // ?DUP
  if (Pop() == 0) return;
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  UNK_0xe8e1(); // UNK_0xe8e1
  UNK_0xe482(); // UNK_0xe482
  UNK_0xe27b(); // UNK_0xe27b
  MORIG(); // MORIG
  Push(Read8(Pop())&0xFF); // C@
  Push(1); // 1
  Push(0xc8c3);
  MODULE(); // MODULE
  UNK_0xe72c(); // UNK_0xe72c
  Push(cc__dash_1); // -1
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  _plus__ex_(); // +!
  i++;
  } while(i<imax); // (LOOP) 0xffe4

  UNK_0xe5f4(); // UNK_0xe5f4
}


// ================================================
// 0xe9b3: WORD 'UNK_0xe9b5' codep=0x224c parp=0xe9b5
// ================================================

void UNK_0xe9b5() // UNK_0xe9b5
{
  _at_INST_dash_SPECIES(); // @INST-SPECIES
  Push(0x0018);
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  UNK_0xdd78(); // UNK_0xdd78
  UNK_0xdeb5(); // UNK_0xdeb5
  SWAP(); // SWAP
  Pop();
  switch(Pop()) // KEY>ORIEN
  {
  case 65535:
    X_eq__dash_1(); // X=-1
    break;
  case 0:
    X_eq_0(); // X=0
    break;
  case 1:
    X_eq_1(); // X=1
    break;
  default:
    UNRAVEL(); // UNRAVEL
    break;

  }
  Push(Read16(sp)); // DUP
  Push(0x63fa); // IFIELD(UNK_0xddcc)
  C_ex_(); // C!
  Push(0x0023);
  Push(Pop() + Pop()); // +
  _ex_ID(); // !ID

  label1:
  Pop(); Pop();// 2DROP
}

// 0xe9dd: db 0x4c 0x22 0x62 0xdd 0xb3 0xe9 0xca 0xdd 0x92 0x0c 0x7b 0x3b 0x72 0x0f 0x9b 0x3b 0x9f 0x11 0x41 0x0e 0xca 0xdd 0x6a 0x6d 0x5d 0x17 0x23 0x00 0x72 0x0f 0x52 0x9b 0x90 0x16 'L"b       {;r  ;  A   jm] # r R   '

// ================================================
// 0xe9ff: WORD 'UNK_0xea01' codep=0x224c parp=0xea01
// ================================================

void UNK_0xea01() // UNK_0xea01
{
  Push(pp_XABS); // XABS
  Push(Read16(Pop())); // @
  Push(0x0012);
  _dash_(); // -
  Push(pp_XVIS); // XVIS
  _st__ex__gt_(); // <!>
  Push(pp_YABS); // YABS
  Push(Read16(Pop())); // @
  Push(0x001e);
  _dash_(); // -
  Push(pp_YVIS); // YVIS
  _st__ex__gt_(); // <!>
}

// 0xea1f: db  'L"n  b  {;L     r     r     L" Z     Q  2   (   ]   r R    Z K    L"  A          ]       D     L" Z                          2 =         4       ^       '

// ================================================
// 0xeab9: WORD 'UNK_0xeabb' codep=0x224c parp=0xeabb
// ================================================

void UNK_0xeabb() // UNK_0xeabb
{
  UNK_0xea01(); // UNK_0xea01
  _gt_2ICONF(); // >2ICONF
  Push(2); // 2
  Push(Read16(sp)); // DUP
  Push(pp_XLLDEST); // XLLDEST
  _st__ex__gt_(); // <!>
  Push(pp_YLLDEST); // YLLDEST
  _st__ex__gt_(); // <!>
  Push(cc__4); // 4
  Push(Read16(sp)); // DUP
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  _st__ex__gt_(); // <!>
  Push(pp_YWLD_c_YPIX); // YWLD:YPIX
  _st__ex__gt_(); // <!>
  Push(1); // 1
  Push(Read16(sp)); // DUP
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  Push(pp_YWLD_c_YPIX); // YWLD:YPIX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  Push(pp_XVIS); // XVIS
  Push(Read16(Pop())); // @
  Push(0x000a);
  Push(Pop() + Pop()); // +
  Push(pp_YVIS); // YVIS
  Push(Read16(Pop())); // @
  Push(0x000f);
  Push(Pop() + Pop()); // +
  Push(pp_XVIS); // XVIS
  Push(Read16(Pop())); // @
  Push(0x001a);
  Push(Pop() + Pop()); // +
  Push(pp_YVIS); // YVIS
  Push(Read16(Pop())); // @
  Push(0x002b);
  Push(Pop() + Pop()); // +
  _ex_VISWINDOW(); // !VISWINDOW
}


// ================================================
// 0xeb13: WORD 'UNK_0xeb15' codep=0x224c parp=0xeb15
// ================================================

void UNK_0xeb15() // UNK_0xeb15
{
  UNK_0xea01(); // UNK_0xea01
  _gt_3ICONF(); // >3ICONF
  Push(1); // 1
  Push(Read16(sp)); // DUP
  Push(pp_XLLDEST); // XLLDEST
  _st__ex__gt_(); // <!>
  Push(pp_YLLDEST); // YLLDEST
  _st__ex__gt_(); // <!>
  Push(2); // 2
  Push(Read16(sp)); // DUP
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  _st__ex__gt_(); // <!>
  Push(pp_YWLD_c_YPIX); // YWLD:YPIX
  _st__ex__gt_(); // <!>
  Push(1); // 1
  Push(Read16(sp)); // DUP
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  Push(pp_YWLD_c_YPIX); // YWLD:YPIX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  Push(pp_XVIS); // XVIS
  Push(Read16(Pop())); // @
  Push(pp_YVIS); // YVIS
  Push(Read16(Pop())); // @
  Push(pp_XVIS); // XVIS
  Push(Read16(Pop())); // @
  Push(0x0022);
  Push(Pop() + Pop()); // +
  Push(pp_YVIS); // YVIS
  Push(Read16(Pop())); // @
  Push(0x003a);
  Push(Pop() + Pop()); // +
  _ex_VISWINDOW(); // !VISWINDOW
}


// ================================================
// 0xeb61: WORD 'UNK_0xeb63' codep=0x224c parp=0xeb63
// ================================================

void UNK_0xeb63() // UNK_0xeb63
{
  UNK_0xea01(); // UNK_0xea01
  _gt_1ICONFONT(); // >1ICONFONT
  Push2Words("0.");
  Push(pp_XLLDEST); // XLLDEST
  _st__ex__gt_(); // <!>
  Push(pp_YLLDEST); // YLLDEST
  _st__ex__gt_(); // <!>
  Push(cc__8); // 8
  Push(Read16(sp)); // DUP
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  _st__ex__gt_(); // <!>
  Push(pp_YWLD_c_YPIX); // YWLD:YPIX
  _st__ex__gt_(); // <!>
  Push(1); // 1
  Push(Read16(sp)); // DUP
  Push(pp_XWLD_c_XPIX); // XWLD:XPIX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  Push(pp_YWLD_c_YPIX); // YWLD:YPIX
  Push(Pop()+2); // 2+
  _st__ex__gt_(); // <!>
  Push(pp_XVIS); // XVIS
  Push(Read16(Pop())); // @
  Push(0x000e);
  Push(Pop() + Pop()); // +
  Push(pp_YVIS); // YVIS
  Push(Read16(Pop())); // @
  Push(0x0017);
  Push(Pop() + Pop()); // +
  Push(pp_XVIS); // XVIS
  Push(Read16(Pop())); // @
  Push(0x0016);
  Push(Pop() + Pop()); // +
  Push(pp_YVIS); // YVIS
  Push(Read16(Pop())); // @
  Push(0x002a);
  Push(Pop() + Pop()); // +
  _ex_VISWINDOW(); // !VISWINDOW
}


// ================================================
// 0xebb9: WORD 'UNK_0xebbb' codep=0x1d29 parp=0xebbb
// ================================================
// 0xebbb: db 0x3a 0x20 ': '

// ================================================
// 0xebbd: WORD 'UNK_0xebbf' codep=0x1d29 parp=0xebbf
// ================================================
// 0xebbf: db 0x3a 0x20 ': '

// ================================================
// 0xebc1: WORD 'UNK_0xebc3' codep=0x224c parp=0xebc3
// ================================================

void UNK_0xebc3() // UNK_0xebc3
{
  Push(pp_XABS); // XABS
  Push(Read16(Pop())); // @
  _at_IX(); // @IX
  _dash_(); // -
  ABS(); // ABS
  Push(pp_YABS); // YABS
  Push(Read16(Pop())); // @
  _at_IY(); // @IY
  _dash_(); // -
  ABS(); // ABS
  MAX(); // MAX
  Push(pp_UNK_0xebbf); // UNK_0xebbf
  Push(Read16(Pop())); // @
  MIN(); // MIN
  Push(pp_UNK_0xebbf); // UNK_0xebbf
  _st__ex__gt_(); // <!>
}


// ================================================
// 0xebe5: WORD 'UNK_0xebe7' codep=0x224c parp=0xebe7
// ================================================

void UNK_0xebe7() // UNK_0xebe7
{
  Push(pp_XABS); // XABS
  Push(Read16(Pop())); // @
  _at_IX(); // @IX
  _dash_(); // -
  ABS(); // ABS
  Push(pp_YABS); // YABS
  Push(Read16(Pop())); // @
  _at_IY(); // @IY
  _dash_(); // -
  ABS(); // ABS
  MAX(); // MAX
  Push(pp_UNK_0xebbb); // UNK_0xebbb
  Push(Read16(Pop())); // @
  MAX(); // MAX
  Push(pp_UNK_0xebbb); // UNK_0xebbb
  _st__ex__gt_(); // <!>
}


// ================================================
// 0xec09: WORD 'UNK_0xec0b' codep=0x224c parp=0xec0b
// ================================================

void UNK_0xec0b() // UNK_0xec0b
{
  Push(pp_UNK_0xebbb); // UNK_0xebbb
  OFF(); // OFF
  Push(0x03e8);
  Push(pp_UNK_0xebbf); // UNK_0xebbf
  _st__ex__gt_(); // <!>
  Push(pp_ILOCAL); // ILOCAL
  Push(Read16(Pop())); // @
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  POINT_gt_ICON(); // POINT>ICON
  _at_ID(); // @ID
  Push(0x0023);
  Push(0x002c);
  WITHIN(); // WITHIN
  if (Pop() == 0) goto label1;
  UNK_0xebc3(); // UNK_0xebc3
  UNK_0xebe7(); // UNK_0xebe7

  label1:
  i++;
  } while(i<imax); // (LOOP) 0xffe6

}


// ================================================
// 0xec3d: WORD 'SET-SCALE' codep=0x224c parp=0xec4b
// ================================================
// entry

void SET_dash_SCALE() // SET-SCALE
{
  UNK_0xec0b(); // UNK_0xec0b
  Push(pp_UNK_0xebbb); // UNK_0xebbb
  Push(Read16(Pop())); // @
  Push(Read16(sp)); // DUP
  Push(cc__5); // 5
  _st_(); // <
  if (Pop() == 0) goto label1;
  Pop(); // DROP
  UNK_0xeb63(); // UNK_0xeb63
  goto label2;

  label1:
  Push(cc__5); // 5
  Push(cc__8); // 8
  WITHIN(); // WITHIN
  if (Pop() == 0) goto label3;
  UNK_0xeabb(); // UNK_0xeabb
  goto label2;

  label3:
  UNK_0xeb15(); // UNK_0xeb15

  label2:
  UNK_0xe5f4(); // UNK_0xe5f4
}

// 0xec79: db  'L"  I   L"] D r    ;r     L"n b     0u]   _      ;`         s;  jmS  m  S   L"9@  A     9? LASER MISSED US!G   L"n b         8O  O      b       N \\    % 9? LASER PARTIALLY DEFLECTED G `   9? LASER CLASS G      '    ]     s;  jmS  m  S   L"n b   L"h   {;  jmn   l   L"h   s;  jmn   l   '

// ================================================
// 0xed98: WORD 'UNK_0xed9a' codep=0x224c parp=0xed9a
// ================================================

void UNK_0xed9a() // UNK_0xed9a
{
  if (Pop() == 0) goto label1;
  UNK_0xdd5a(); // UNK_0xdd5a
  if (Pop() == 0) goto label2;
  UNK_0xde9b(); // UNK_0xde9b
  goto label3;

  label2:
  Push(0); // 0

  label3:
  return;

  label1:
  UNK_0xde9b(); // UNK_0xde9b
}


// ================================================
// 0xedb4: WORD '?APPROACH' codep=0x224c parp=0xedc2
// ================================================

void _ask_APPROACH() // ?APPROACH
{
  Push(0x6400); // IFIELD(UNK_0xddc7)
  Push(Read8(Pop())&0xFF); // C@
  Push(cc__3); // 3
  _eq_(); // =
}


// ================================================
// 0xedcc: WORD 'UNK_0xedce' codep=0x224c parp=0xedce
// ================================================

void UNK_0xedce() // UNK_0xedce
{
  Push(0x63fc); // IFIELD(INST-X)
  Push(Read16(Pop())); // @
  Push(1); // 1
  UNK_0xed9a(); // UNK_0xed9a
  Push(Read16(sp)); // DUP
  _gt_R(); // >R
  Push(Pop() + Pop()); // +
  Push(0x63fc); // IFIELD(INST-X)
  _ex__3(); // !_3
  Push(0x63fe); // IFIELD(INST-Y)
  Push(Read16(Pop())); // @
  R_gt_(); // R>
  UNK_0xed9a(); // UNK_0xed9a
  Push(Pop() + Pop()); // +
  Push(0x63fe); // IFIELD(INST-Y)
  _ex__3(); // !_3
  UNK_0xdd70(); // UNK_0xdd70
  UNK_0xe9b5(); // UNK_0xe9b5
  UNK_0xe86e(); // UNK_0xe86e
  Push(0); // 0
  Push(0x6400); // IFIELD(UNK_0xddc7)
  C_ex_(); // C!
}


// ================================================
// 0xedfc: WORD '<COMBAT>' codep=0xb869 parp=0xee09
// ================================================
// 0xee09: db  '   v |                      L"  9? !!HULL DESTROYED1173r       - 9         !   k % C      L"O         '8  | ;                                            r       r     r                 '

// ================================================
// 0xeec2: WORD 'UNK_0xeec4' codep=0x224c parp=0xeec4
// ================================================

void UNK_0xeec4() // UNK_0xeec4
{
  Push(0x641f); // IFIELD(UNK_0xde1b)
  Push(Read16(Pop())); // @
  Push(0x6409); // IFIELD(UNK_0xde02)
  UNK_0xde77(); // UNK_0xde77
  Push(0x01f4);
  Push(Pop() * Pop()); // *
  _st_(); // <
}


// ================================================
// 0xeed6: WORD 'UNK_0xeed8' codep=0x224c parp=0xeed8
// ================================================

void UNK_0xeed8() // UNK_0xeed8
{
  Push(pp__h_VES); // ^VES
  Push(Read16(Pop())); // @
  POINT_gt_ICON(); // POINT>ICON
  _at_IL(); // @IL
  _at_IH(); // @IH
  _gt_C_plus_S(); // >C+S
}

// 0xeee6: db 0x4c 0x22 0xae 0x55 0xc5 0x6d 0x90 0x16 'L" U m  '

// ================================================
// 0xeeee: WORD 'UNK_0xeef0' codep=0x224c parp=0xeef0
// ================================================

void UNK_0xeef0() // UNK_0xeef0
{
  Push(0xee09);
  DISTRACT(); // DISTRACT
  Func13("<COMBAT>");
  if (Pop() == 0) Push(1); else Push(0); // 0=
  UNK_0xe06b(); // UNK_0xe06b
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) return;
  UNK_0xedce(); // UNK_0xedce
}


// ================================================
// 0xef06: WORD 'UNK_0xef08' codep=0x224c parp=0xef08
// ================================================

void UNK_0xef08() // UNK_0xef08
{
  Push(0x6403); // IFIELD(UNK_0xddc2)
  Push(Read16(Pop())); // @
  LoadData("UNK_0xdd98"); // from 'VESSEL      '
  Push(Read16(Pop())); // @
  _st_(); // <
  if (Pop() == 0) return;
  Push(cc__5); // 5
  LoadData("UNK_0xdd98"); // from 'VESSEL      '
  _plus__ex_(); // +!
}


// ================================================
// 0xef1e: WORD 'UNK_0xef20' codep=0x224c parp=0xef20
// ================================================

void UNK_0xef20() // UNK_0xef20
{
  UNK_0xeec4(); // UNK_0xeec4
  if (Pop() == 0) return;
  Push(0x0014);
  Push(0x641f); // IFIELD(UNK_0xde1b)
  _plus__ex_(); // +!
}


// ================================================
// 0xef30: WORD 'COMBAT' codep=0x224c parp=0xef3b
// ================================================
// entry

void COMBAT() // COMBAT
{
  SET_dash_SCALE(); // SET-SCALE
  Push(pp_ILOCAL); // ILOCAL
  Push(Read16(Pop())); // @
  Push(1); // 1
  _gt_(); // >
  if (Pop() == 0) goto label1;
  UNK_0xeed8(); // UNK_0xeed8
  _at_ID(); // @ID
  Push(0x0023);
  Push(0x002b);
  WITHIN(); // WITHIN
  if (Pop() == 0) goto label2;
  UNK_0xeef0(); // UNK_0xeef0
  UNK_0xef08(); // UNK_0xef08
  goto label3;

  label2:
  _at_ID(); // @ID
  Push(0x001b);
  Push(0x0023);
  WITHIN(); // WITHIN
  if (Pop() == 0) goto label3;
  UNK_0xef20(); // UNK_0xef20

  label3:
  ICLOSE(); // ICLOSE
  Push(pp__h_VES); // ^VES
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  Push(pp_ILOCAL); // ILOCAL
  Push(Read16(Pop())); // @
  MOD(); // MOD
  Push(pp__h_VES); // ^VES
  _ex__3(); // !_3
  UNK_0xe97d(); // UNK_0xe97d

  label1:
  Push(pp_UNK_0xebbf); // UNK_0xebbf
  Push(Read16(Pop())); // @
  Push(0x0016);
  _gt_(); // >
  Push(pp__ask_COMBAT); // ?COMBAT
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) return;
  Push(pp__i_UNNEST); // 'UNNEST
  Push(Read16(Pop())); // @
  MODULE(); // MODULE
}


// ================================================
// 0xefa7: WORD 'ORIENT>O' codep=0x1d29 parp=0xefb4
// ================================================
// 0xefb4: db  '                                L"        r 5 (   E   r 8       ]  r        ]  r C  m  L"    .     L"Pl y z]   ]    ~2  z]   ]    ~2   L"   t  ]      a m u u y  L"A    a8      @XQ @ a m `8     e  ` m  L" a8 ,@  L" a8 ]        ;-          tvm y y y  ) V:  L"  ]        J] 2 L   =  J        L" Q y-  I y2     M 8     A X    Kr       A X      Kr    u y  n C  m  L"]  'I vm Y  A . -   F     P     ] # ] + W@  & C 8 n W A I   F     I  Kn M  m`   2     `   2   L"I   ]  '    L" O  9? NO TARGET PRESENTG   L". ]  Pvm  L"7     5 F     b       u    ;V S     L"b     M  m   O   M 8     ]     . V S     L"a   . uu  2      ] m ] m   t m t m  r 2   r B `       L"~           e     |   U b M 8 W  ;F           ` 7         ` +         `    O  9? WEAPON NOT AVAILABLEG `     `    O  9? WEAPONS NOT ARMED G `   8O  9? OUT OF FUEL! G   L" ^7:9   L"]   >_vm] 6 h_vm] H:0_vm] H: ^vm]   Z_vm] H: ^vm]   L_vmXQ   m] H: _vm] _ v_vm [ mD^qH ^qH  L"]   >_vm] 6 h_vm] H:0_vm] N  ^vm]   Z_vm]   L_vm] H: _vm] _ v_vm] V "_vm  S  m Z HD^ H y  L" \  ]   _   L"    ^ (    ( jm; _             I ]   CRL   `     `     P           >                 L"       O  9? HIT SPACE BAR TO FIRE WEAPONG 9? HIT RETURN TO EXIT COMBAT G ]    Yvm  i ?     ! 9? CLOAKING DEVICE NOW ACTIVEG ] ?     S  Z m<  Q yy   ] H        '

// ================================================
// 0xf4b5: WORD 'WAR' codep=0x224c parp=0xf4bd
// ================================================
// entry

void WAR() // WAR
{
  Push(pp_CONTEXT_dash_ID_n_); // CONTEXT-ID#
  Push(Read16(Pop())); // @
  Push(cc__4); // 4
  _eq_(); // =
  if (Pop() == 0) goto label1;
  Push(0xf352);
  Push(0xf3bc);
  Push(0xf414);
  DOTASKS(); // DOTASKS
  return;

  label1:
  BEEP(); // BEEP
}


// ================================================
// 0xf4df: WORD 'CLEAR-CO' codep=0x224c parp=0xf4ec
// ================================================
// entry

void CLEAR_dash_CO() // CLEAR-CO
{
  UNK_0xe462(); // UNK_0xe462
  Push(pp_UNK_0xe6ac); // UNK_0xe6ac
  OFF(); // OFF
  Push(pp_UNK_0xe6b0); // UNK_0xe6b0
  OFF(); // OFF
  Push(pp_UNK_0xde39); // UNK_0xde39
  OFF(); // OFF
  Push(pp_UNK_0xde3d); // UNK_0xde3d
  OFF(); // OFF
  UNK_0xe055(); // UNK_0xe055
  Push(0xc03f);
  MODULE(); // MODULE
  Push(pp_UNK_0xde41); // UNK_0xde41
  OFF(); // OFF
  Push(0); // 0
  UNK_0xe0cb(); // UNK_0xe0cb
  Push(0x0006);
  Push(0xbdd8);
  MODULE(); // MODULE
  Push(pp__ask_ROD); // ?ROD
  _st__ex__gt_(); // <!>
}

// 0xf51e: db 0x43 0x4f 0x4d 0x42 0x41 0x54 0x2d 0x56 0x4f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x2d 0x2d 0x2d 0x2d 0x5c 0x20 0x43 0x4f 0x4d 0x42 0x41 0x54 0x20 0x20 0x43 0x4d 0x50 0x2d 0x28 0x20 0x72 0x66 0x67 0x32 0x35 0x73 0x65 0x70 0x38 0x37 0x29 0x2d 0x32 0x33 0x00 'COMBAT-VO_______________________----\ COMBAT  CMP-( rfg25sep87)-23 '
  