// ====== OVERLAY 'MOVE-OV' ======
// store offset = 0xe680
// overlay size   = 0x0ee0

#include"../emul/cpu.h"
#include"../emul/starflt1.h"


// =================================
// =========== DICTIONARY ==========
// =================================
// 1703:      UNK_0xa98d  codep:0x224c parp:0xa98d size:0x0006 C-string:'UNK_0xa98d'
// 1704:      UNK_0xa995  codep:0x224c parp:0xa995 size:0x0006 C-string:'UNK_0xa995'
// 1705:      UNK_0xa99d  codep:0x224c parp:0xa99d size:0x0006 C-string:'UNK_0xa99d'
// 1706:      UNK_0xa9a5  codep:0x224c parp:0xa9a5 size:0x000a C-string:'UNK_0xa9a5'
// 1707:      UNK_0xa9b1  codep:0x224c parp:0xa9b1 size:0x000a C-string:'UNK_0xa9b1'
// 1708:      UNK_0xa9bd  codep:0x224c parp:0xa9bd size:0x0006 C-string:'UNK_0xa9bd'
// 1709:      UNK_0xa9c5  codep:0x224c parp:0xa9c5 size:0x0006 C-string:'UNK_0xa9c5'
// 1710:      UNK_0xa9cd  codep:0x224c parp:0xa9cd size:0x0006 C-string:'UNK_0xa9cd'
// 1711:      UNK_0xa9d5  codep:0x224c parp:0xa9d5 size:0x0004 C-string:'UNK_0xa9d5'
// 1712:      UNK_0xa9db  codep:0x224c parp:0xa9db size:0x3cb9 C-string:'UNK_0xa9db'
// 1713:      UNK_0xe696  codep:0x224c parp:0xe696 size:0x002e C-string:'UNK_0xe696'
// 1714:      UNK_0xe6c6  codep:0x224c parp:0xe6c6 size:0x0018 C-string:'UNK_0xe6c6'
// 1715:      UNK_0xe6e0  codep:0x224c parp:0xe6e0 size:0x0020 C-string:'UNK_0xe6e0'
// 1716:      UNK_0xe702  codep:0x224c parp:0xe702 size:0x0020 C-string:'UNK_0xe702'
// 1717:      UNK_0xe724  codep:0x224c parp:0xe724 size:0x0026 C-string:'UNK_0xe724'
// 1718:      UNK_0xe74c  codep:0x224c parp:0xe74c size:0x0022 C-string:'UNK_0xe74c'
// 1719:      UNK_0xe770  codep:0x224c parp:0xe770 size:0x0022 C-string:'UNK_0xe770'
// 1720:      UNK_0xe794  codep:0x224c parp:0xe794 size:0x000a C-string:'UNK_0xe794'
// 1721:      UNK_0xe7a0  codep:0x224c parp:0xe7a0 size:0x000a C-string:'UNK_0xe7a0'
// 1722:      UNK_0xe7ac  codep:0x224c parp:0xe7ac size:0x000a C-string:'UNK_0xe7ac'
// 1723:      UNK_0xe7b8  codep:0x224c parp:0xe7b8 size:0x000a C-string:'UNK_0xe7b8'
// 1724:      UNK_0xe7c4  codep:0x744d parp:0xe7c4 size:0x0003 C-string:'UNK_0xe7c4'
// 1725:      UNK_0xe7c9  codep:0x744d parp:0xe7c9 size:0x0003 C-string:'UNK_0xe7c9'
// 1726:      UNK_0xe7ce  codep:0x224c parp:0xe7ce size:0x0018 C-string:'UNK_0xe7ce'
// 1727:      UNK_0xe7e8  codep:0x224c parp:0xe7e8 size:0x0018 C-string:'UNK_0xe7e8'
// 1728:      UNK_0xe802  codep:0x224c parp:0xe802 size:0x005a C-string:'UNK_0xe802'
// 1729:      UNK_0xe85e  codep:0x224c parp:0xe85e size:0x002e C-string:'UNK_0xe85e'
// 1730:       SET-SPEED  codep:0x224c parp:0xe89a size:0x002a C-string:'SET_dash_SPEED'
// 1731:      UNK_0xe8c6  codep:0x224c parp:0xe8c6 size:0x0016 C-string:'UNK_0xe8c6'
// 1732:            CBLT  codep:0x1d29 parp:0xe8e5 size:0x0004 C-string:'CBLT'
// 1733:           CBLTP  codep:0xe8f3 parp:0xe8f3 size:0x002c C-string:'CBLTP'
// 1734:      UNK_0xe921  codep:0x224c parp:0xe921 size:0x00c2 C-string:'UNK_0xe921'
// 1735:      UNK_0xe9e5  codep:0x224c parp:0xe9e5 size:0x005c C-string:'UNK_0xe9e5'
// 1736:          %STORM  codep:0x1d29 parp:0xea4c size:0x0130 C-string:'_pe_STORM'
// 1737:      UNK_0xeb7e  codep:0x224c parp:0xeb7e size:0x001c C-string:'UNK_0xeb7e'
// 1738:      UNK_0xeb9c  codep:0x1d29 parp:0xeb9c size:0x0002 C-string:'UNK_0xeb9c'
// 1739:      UNK_0xeba0  codep:0x1d29 parp:0xeba0 size:0x0002 C-string:'UNK_0xeba0'
// 1740:      UNK_0xeba4  codep:0x2214 parp:0xeba4 size:0x000a C-string:'UNK_0xeba4'
// 1741:      UNK_0xebb0  codep:0x224c parp:0xebb0 size:0x0012 C-string:'UNK_0xebb0'
// 1742:      UNK_0xebc4  codep:0x224c parp:0xebc4 size:0x001e C-string:'UNK_0xebc4'
// 1743:      UNK_0xebe4  codep:0x224c parp:0xebe4 size:0x000c C-string:'UNK_0xebe4'
// 1744:      UNK_0xebf2  codep:0x224c parp:0xebf2 size:0x000c C-string:'UNK_0xebf2'
// 1745:      UNK_0xec00  codep:0x224c parp:0xec00 size:0x0086 C-string:'UNK_0xec00'
// 1746:      UNK_0xec88  codep:0x224c parp:0xec88 size:0x000c C-string:'UNK_0xec88'
// 1747:      UNK_0xec96  codep:0x224c parp:0xec96 size:0x000e C-string:'UNK_0xec96'
// 1748:      UNK_0xeca6  codep:0x224c parp:0xeca6 size:0x004a C-string:'UNK_0xeca6'
// 1749:      UNK_0xecf2  codep:0x224c parp:0xecf2 size:0x0020 C-string:'UNK_0xecf2'
// 1750:      UNK_0xed14  codep:0x224c parp:0xed14 size:0x001e C-string:'UNK_0xed14'
// 1751:      UNK_0xed34  codep:0x224c parp:0xed34 size:0x000c C-string:'UNK_0xed34'
// 1752:      UNK_0xed42  codep:0x224c parp:0xed42 size:0x0008 C-string:'UNK_0xed42'
// 1753:      UNK_0xed4c  codep:0x224c parp:0xed4c size:0x002c C-string:'UNK_0xed4c'
// 1754:      UNK_0xed7a  codep:0x224c parp:0xed7a size:0x002c C-string:'UNK_0xed7a'
// 1755:      UNK_0xeda8  codep:0x224c parp:0xeda8 size:0x005c C-string:'UNK_0xeda8'
// 1756:      UNK_0xee06  codep:0x224c parp:0xee06 size:0x002c C-string:'UNK_0xee06'
// 1757:      UNK_0xee34  codep:0x224c parp:0xee34 size:0x0024 C-string:'UNK_0xee34'
// 1758:         ?POPULA  codep:0x224c parp:0xee64 size:0x0068 C-string:'_ask_POPULA'
// 1759:      UNK_0xeece  codep:0x224c parp:0xeece size:0x004e C-string:'UNK_0xeece'
// 1760:      UNK_0xef1e  codep:0x224c parp:0xef1e size:0x0028 C-string:'UNK_0xef1e'
// 1761:      UNK_0xef48  codep:0x224c parp:0xef48 size:0x002a C-string:'UNK_0xef48'
// 1762:      UNK_0xef74  codep:0x224c parp:0xef74 size:0x00e4 C-string:'UNK_0xef74'
// 1763:      UNK_0xf05a  codep:0x224c parp:0xf05a size:0x0042 C-string:'UNK_0xf05a'
// 1764:      UNK_0xf09e  codep:0x224c parp:0xf09e size:0x0016 C-string:'UNK_0xf09e'
// 1765:      UNK_0xf0b6  codep:0x224c parp:0xf0b6 size:0x0008 C-string:'UNK_0xf0b6'
// 1766:      UNK_0xf0c0  codep:0x224c parp:0xf0c0 size:0x0060 C-string:'UNK_0xf0c0'
// 1767:      UNK_0xf122  codep:0x224c parp:0xf122 size:0x004a C-string:'UNK_0xf122'
// 1768:      UNK_0xf16e  codep:0x224c parp:0xf16e size:0x008e C-string:'UNK_0xf16e'
// 1769:      UNK_0xf1fe  codep:0x224c parp:0xf1fe size:0x0012 C-string:'UNK_0xf1fe'
// 1770:      UNK_0xf212  codep:0x224c parp:0xf212 size:0x007e C-string:'UNK_0xf212'
// 1771:      UNK_0xf292  codep:0x224c parp:0xf292 size:0x0008 C-string:'UNK_0xf292'
// 1772:      UNK_0xf29c  codep:0x224c parp:0xf29c size:0x001a C-string:'UNK_0xf29c'
// 1773:      UNK_0xf2b8  codep:0x224c parp:0xf2b8 size:0x0010 C-string:'UNK_0xf2b8'
// 1774:      UNK_0xf2ca  codep:0x224c parp:0xf2ca size:0x000c C-string:'UNK_0xf2ca'
// 1775:      UNK_0xf2d8  codep:0x224c parp:0xf2d8 size:0x0008 C-string:'UNK_0xf2d8'
// 1776:      UNK_0xf2e2  codep:0x224c parp:0xf2e2 size:0x0024 C-string:'UNK_0xf2e2'
// 1777:      UNK_0xf308  codep:0x224c parp:0xf308 size:0x0013 C-string:'UNK_0xf308'
// 1778:      UNK_0xf31d  codep:0x224c parp:0xf31d size:0x0036 C-string:'UNK_0xf31d'
// 1779:      UNK_0xf355  codep:0x224c parp:0xf355 size:0x0016 C-string:'UNK_0xf355'
// 1780:      UNK_0xf36d  codep:0x224c parp:0xf36d size:0x000c C-string:'UNK_0xf36d'
// 1781:      UNK_0xf37b  codep:0x224c parp:0xf37b size:0x000e C-string:'UNK_0xf37b'
// 1782:      UNK_0xf38b  codep:0x224c parp:0xf38b size:0x000a C-string:'UNK_0xf38b'
// 1783:      UNK_0xf397  codep:0x224c parp:0xf397 size:0x000e C-string:'UNK_0xf397'
// 1784:      UNK_0xf3a7  codep:0x224c parp:0xf3a7 size:0x004e C-string:'UNK_0xf3a7'
// 1785:         TV-MOVE  codep:0x224c parp:0xf401 size:0x0014 C-string:'TV_dash_MOVE'
// 1786:      plan-tseed  codep:0x73ea parp:0xf417 size:0x0006 C-string:'plan_dash_tseed'
// 1787:        APPROACH  codep:0x224c parp:0xf42a size:0x0000 C-string:'APPROACH'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char CBLT[4] = {0xff, 0x66, 0x00, 0x66}; // CBLT
unsigned char _pe_STORM[304] = {}; // %STORM
unsigned char UNK_0xeb9c[2] = {0x01, 0x00}; // UNK_0xeb9c
unsigned char UNK_0xeba0[2] = {0x00, 0x00}; // UNK_0xeba0

const unsigned short int cc_UNK_0xeba4 = 0x07d0; // UNK_0xeba4


// 0xe692: db 0xe1 0x00 '  '

// ================================================
// 0xe694: WORD 'UNK_0xe696' codep=0x224c parp=0xe696
// ================================================

void UNK_0xe696() // UNK_0xe696
{
  _ask_NEW_dash_HO(); // ?NEW-HO
  if (Pop() == 0) return;
  TIME(); // TIME
  _2_at_(); // 2@
  Push(0x6201); // pointer to LAST-UPDATE
  D_ex_(); // D!
  Push(1); // 1
  Push(0x5e66); // pointer to STAR-HR
  _plus__ex_(); // +!
  Push(0x5e66); // pointer to STAR-HR
  Push(Read16(Pop())); // @
  Push(0x0018);
  _eq_(); // =
  if (Pop() == 0) return;
  Push(0x5e66); // pointer to STAR-HR
  OFF(); // OFF
  Push(1); // 1
  Push(0x5e74); // pointer to STARDATE
  _plus__ex_(); // +!
}


// ================================================
// 0xe6c4: WORD 'UNK_0xe6c6' codep=0x224c parp=0xe6c6
// ================================================

void UNK_0xe6c6() // UNK_0xe6c6
{
  Push(0x0040);
  Push(cc__3); // 3
  Push(cc__7); // 7
  Push(0x0026);
  WINDOW(); // WINDOW
  Push(cc__dash_1); // -1
  Push(0x5bd2); // pointer to WBOTTOM
  _plus__ex_(); // +!
  _gt_TVCT(); // >TVCT
}


// ================================================
// 0xe6de: WORD 'UNK_0xe6e0' codep=0x224c parp=0xe6e0
// ================================================

void UNK_0xe6e0() // UNK_0xe6e0
{
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  Push(0x5bc4); // pointer to WTOP
  Push(Read16(Pop())); // @
  Push(0x5beb); // pointer to WLEFT
  Push(Read16(Pop())); // @
  Push(0x5bd2); // pointer to WBOTTOM
  Push(Read16(Pop())); // @
  Push(Pop()-1); // 1-
  Push(0x5bdf); // pointer to WRIGHT
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  SetColor("BLACK");
  POLY_dash_WINDOW_dash_FILL(); // POLY-WINDOW-FILL
  _ex_COLOR(); // !COLOR
}


// ================================================
// 0xe700: WORD 'UNK_0xe702' codep=0x224c parp=0xe702
// ================================================

void UNK_0xe702() // UNK_0xe702
{
  UNK_0xe6c6(); // UNK_0xe6c6
  UNK_0xe6e0(); // UNK_0xe6e0
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  Push(0x5beb); // pointer to WLEFT
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  Push(0x5bc4); // pointer to WTOP
  Push(Read16(Pop())); // @
  Push(Pop()-1); // 1-
  POS_dot_(); // POS.
}

// 0xe71a: db 0x4c 0x22 0x89 0x12 0x89 0x12 0x90 0x16 'L"      '

// ================================================
// 0xe722: WORD 'UNK_0xe724' codep=0x224c parp=0xe724
// ================================================

void UNK_0xe724() // UNK_0xe724
{
  Push(0x01e0);
  _dash_(); // -
  Push(0x000a);
  Push(0x0035);
  _star__slash_(); // */
  SWAP(); // SWAP
  Push(0x0480);
  _dash_(); // -
  Push(0x000a);
  Push(0x0040);
  _star__slash_(); // */
  SWAP(); // SWAP
}


// ================================================
// 0xe74a: WORD 'UNK_0xe74c' codep=0x224c parp=0xe74c
// ================================================

void UNK_0xe74c() // UNK_0xe74c
{
  if (Read16(regsp) != 0) Push(Read16(regsp)); // ?DUP
  if (Pop() == 0) goto label1;
  _0_st_(); // 0<
  if (Pop() == 0) goto label2;
  PRINT("W", 1); // (.")
  goto label3;

  label2:
  PRINT("E", 1); // (.")

  label3:
  return;

  label1:
  PRINT(" ", 1); // (.")
}


// ================================================
// 0xe76e: WORD 'UNK_0xe770' codep=0x224c parp=0xe770
// ================================================

void UNK_0xe770() // UNK_0xe770
{
  if (Read16(regsp) != 0) Push(Read16(regsp)); // ?DUP
  if (Pop() == 0) goto label1;
  _0_st_(); // 0<
  if (Pop() == 0) goto label2;
  PRINT("S", 1); // (.")
  goto label3;

  label2:
  PRINT("N", 1); // (.")

  label3:
  return;

  label1:
  PRINT(" ", 1); // (.")
}


// ================================================
// 0xe792: WORD 'UNK_0xe794' codep=0x224c parp=0xe794
// ================================================

void UNK_0xe794() // UNK_0xe794
{
  Push(0x63fc); // IFIELD(INST-X)
  Push(Read16(Pop())); // @
  Push(0x63fe); // IFIELD(INST-Y)
  Push(Read16(Pop())); // @
}


// ================================================
// 0xe79e: WORD 'UNK_0xe7a0' codep=0x224c parp=0xe7a0
// ================================================

void UNK_0xe7a0() // UNK_0xe7a0
{
  Push(0x626f); // pointer to TVEHICLE
  _at__gt_C_plus_S(); // @>C+S
  UNK_0xe794(); // UNK_0xe794
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xe7aa: WORD 'UNK_0xe7ac' codep=0x224c parp=0xe7ac
// ================================================

void UNK_0xe7ac() // UNK_0xe7ac
{
  Push(0x63fe); // IFIELD(INST-Y)
  _ex__3(); // !_3
  Push(0x63fc); // IFIELD(INST-X)
  _ex__3(); // !_3
}


// ================================================
// 0xe7b6: WORD 'UNK_0xe7b8' codep=0x224c parp=0xe7b8
// ================================================

void UNK_0xe7b8() // UNK_0xe7b8
{
  Push(0x5dae); // pointer to XABS
  Push(Read16(Pop())); // @
  Push(0x5db9); // pointer to YABS
  Push(Read16(Pop())); // @
}


// ================================================
// 0xe7c2: WORD 'UNK_0xe7c4' codep=0x744d parp=0xe7c4
// ================================================
// 0xe7c4: db 0x11 0x17 0x03 '   '

// ================================================
// 0xe7c7: WORD 'UNK_0xe7c9' codep=0x744d parp=0xe7c9
// ================================================
// 0xe7c9: db 0x10 0x1b 0x01 '   '

// ================================================
// 0xe7cc: WORD 'UNK_0xe7ce' codep=0x224c parp=0xe7ce
// ================================================

void UNK_0xe7ce() // UNK_0xe7ce
{
  Push2Words("*ASSIGN-CREW");
  _gt_C_plus_S(); // >C+S
  Push(0x6406); // IFIELD(UNK_0xe7c4)
  _at__gt_C_plus_S(); // @>C+S
  Push(0x640a); // IFIELD(UNK_0xe7c9)
  Push(Read8(Pop())&0xFF); // C@
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
  Push(0x00c7);
  _gt_(); // >
}


// ================================================
// 0xe7e6: WORD 'UNK_0xe7e8' codep=0x224c parp=0xe7e8
// ================================================

void UNK_0xe7e8() // UNK_0xe7e8
{
  _gt_R(); // >R
  Push(0x00c5);
  Push(0x000c);
  Push(0x00c1);
  Push(0x0044);
  R_gt_(); // R>
  POLY_dash_WINDOW_dash_FILL(); // POLY-WINDOW-FILL
}


// ================================================
// 0xe800: WORD 'UNK_0xe802' codep=0x224c parp=0xe802
// ================================================

void UNK_0xe802() // UNK_0xe802
{
  Push(0x5af4); // pointer to #STORM
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(cc__4); // 4
  _st_(); // <
  SWAP(); // SWAP
  Push(0x000b);
  _gt_(); // >
  Push(Pop() | Pop()); // OR
  UNK_0xe7ce(); // UNK_0xe7ce
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) return;
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  UNK_0xe7a0(); // UNK_0xe7a0
  UNK_0xe724(); // UNK_0xe724
  Push(0x0010);
  Push(0x00c5);
  POS_dot_(); // POS.
  Push(Read16(regsp)); // DUP
  Push(Read16(regsp)); // DUP
  _0_st_(); // 0<
  if (Pop() == 0) goto label1;
  Push(-Pop()); // NEGATE

  label1:
  Push(cc__4); // 4
  _dot_R(); // .R
  UNK_0xe770(); // UNK_0xe770
  PRINT(" * ", 3); // (.")
  Push(Read16(regsp)); // DUP
  Push(Read16(regsp)); // DUP
  _0_st_(); // 0<
  if (Pop() == 0) goto label2;
  Push(-Pop()); // NEGATE

  label2:
  Push(0); // 0
  _dot_R(); // .R
  UNK_0xe74c(); // UNK_0xe74c
}


// ================================================
// 0xe85c: WORD 'UNK_0xe85e' codep=0x224c parp=0xe85e
// ================================================

void UNK_0xe85e() // UNK_0xe85e
{
  Push(0x6ac1);
  SETLARRAY(); // SETLARRAY
  Push(0x5dae); // pointer to XABS
  Push(Read16(Pop())); // @
  Push(0x5916); // pointer to XCON
  Push(Read16(Pop())); // @
  Push(Pop()+1); // 1+
  _dash_(); // -
  Push(0x5db9); // pointer to YABS
  Push(Read16(Pop())); // @
  Push(0x5921); // pointer to YCON
  Push(Read16(Pop())); // @
  _dash_(); // -
}

// 0xe87c: db 0x4c 0x22 0x5c 0xe8 0x4e 0x4d 0x90 0x4d 0x5d 0x17 0x10 0x00 0x46 0x12 0x90 0x16 'L"\ NM M]   F   '

// ================================================
// 0xe88c: WORD 'SET-SPEED' codep=0x224c parp=0xe89a
// ================================================
// entry

void SET_dash_SPEED() // SET-SPEED
{
  UNK_0xe85e(); // UNK_0xe85e
  ACELLADDR(); // ACELLADDR
  A_at_(); // A@
  Push(Read16(regsp)); // DUP
  Push(0x0010);
  _st_(); // <
  if (Pop() == 0) goto label1;
  Pop(); // DROP
  Push(0xff56);
  goto label2;

  label1:
  Push(cc__3); // 3
  Push(2); // 2
  _star__slash_(); // */
  Push(-Pop()); // NEGATE

  label2:
  Push(0x5ad9); // pointer to E/M
  _ex__3(); // !_3
}


// ================================================
// 0xe8c4: WORD 'UNK_0xe8c6' codep=0x224c parp=0xe8c6
// ================================================

void UNK_0xe8c6() // UNK_0xe8c6
{
  Push(0x5944); // pointer to XVIS
  Push(Read16(Pop())); // @
  Push(0x5916); // pointer to XCON
  Push(Read16(Pop())); // @
  _dash_(); // -
  Push(0x594f); // pointer to YVIS
  Push(Read16(Pop())); // @
  Push(0x5921); // pointer to YCON
  Push(Read16(Pop())); // @
  _dash_(); // -
}


// ================================================
// 0xe8dc: WORD 'CBLT' codep=0x1d29 parp=0xe8e5
// ================================================
// 0xe8e5: db 0xff 0x66 0x00 0x66 ' f f'

// ================================================
// 0xe8e9: WORD 'CBLTP' codep=0xe8f3 parp=0xe8f3
// ================================================
// 0xe8f3: pop    ax
// 0xe8f4: add    ax,0003
// 0xe8f7: mov    [5863],ax
// 0xe8fb: pop    ax
// 0xe8fc: add    ax,0006
// 0xe8ff: mov    [586E],ax
// 0xe903: mov    word ptr [5892],0004
// 0xe909: mov    word ptr [5887],0006
// 0xe90f: mov    word ptr [589D],E8E5
// 0xe915: push   ds
// 0xe916: pop    word ptr [58AA]
// 0xe91a: lodsw
// 0xe91b: mov    bx,ax
// 0xe91d: jmp    word ptr [bx]

// ================================================
// 0xe91f: WORD 'UNK_0xe921' codep=0x224c parp=0xe921
// ================================================

void UNK_0xe921() // UNK_0xe921
{
  Push(0x58aa); // pointer to BLTSEG
  Push(Read16(Pop())); // @
  Push(0x000e);
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(cc__8); // 8
  Push(0); // 0

  signed short int j = Pop();
  signed short int jmax = Pop();
  do // (DO)
  {
  Push(0x4e49); // pointer to XLL
  Push(Read16(Pop())); // @
  Push(j); // I
  Push(Pop() + Pop()); // +
  Push(0x4e53); // pointer to YLL
  Push(Read16(Pop())); // @
  Push(i); // J
  Push(Pop() + Pop()); // +
  _2DUP(); // 2DUP
  CELLCOLOR(); // CELLCOLOR
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  _gt_R(); // >R
  _2DUP(); // 2DUP
  Push(Pop()+1); // 1+
  SWAP(); // SWAP
  Push(Pop()+1); // 1+
  SWAP(); // SWAP
  CELLCOLOR(); // CELLCOLOR
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  Push(j); // I
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  R_gt_(); // R>
  if (Pop() == 0) Push(1); else Push(0); // NOT
  _gt_R(); // >R

  label1:
  _2DUP(); // 2DUP
  Push(Pop()+1); // 1+
  CELLCOLOR(); // CELLCOLOR
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  _gt_R(); // >R
  _2DUP(); // 2DUP
  SWAP(); // SWAP
  Push(Pop()+1); // 1+
  SWAP(); // SWAP
  CELLCOLOR(); // CELLCOLOR
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  Push(j); // I
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label2;
  R_gt_(); // R>
  if (Pop() == 0) Push(1); else Push(0); // NOT
  _gt_R(); // >R

  label2:
  Push(j); // I
  Push(j); // I'
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label3;
  Push(j); // I
  Push(j); // I'
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label3;
  Push(j); // I
  _0_gt_(); // 0>
  if (Pop() == 0) goto label4;
  Push(j); // I
  goto label5;

  label4:
  Push(j); // I'

  label5:
  Push(0x55f2); // pointer to COLOR
  _st__ex__gt_(); // <!>
  _2DUP(); // 2DUP
  Push(cc__3); // 3
  Push(cc__3); // 3
  BUFFERXY(); // BUFFERXY
  CBLTP(); // CBLTP
  BLT(); // BLT

  label3:
  R_gt_(); // R>
  R_gt_(); // R>
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
  j++;
  } while(j<jmax); // (LOOP) 0xff5c

  i++;
  } while(i<imax); // (LOOP) 0xff52

  Push(0x58aa); // pointer to BLTSEG
  _ex__3(); // !_3
}


// ================================================
// 0xe9e3: WORD 'UNK_0xe9e5' codep=0x224c parp=0xe9e5
// ================================================

void UNK_0xe9e5() // UNK_0xe9e5
{
  Push(0x6ac1);
  SETLARRAY(); // SETLARRAY
  UNK_0xe8c6(); // UNK_0xe8c6
  Push(0x0016);
  Push(Pop() + Pop()); // +
  SWAP(); // SWAP
  Push(0x000d);
  Push(Pop() + Pop()); // +
  SWAP(); // SWAP
  OVER(); // OVER
  Push(cc__8); // 8
  Push(Pop() + Pop()); // +
  OVER(); // OVER
  Push(0x000e);
  Push(Pop() + Pop()); // +
  SETREGION(); // SETREGION
  _dot_REGION_plus_(); // .REGION+
  UNK_0xe921(); // UNK_0xe921
}

// 0xea13: db 0xea 0x73 0x20 0x11 0x02 0x16 0x8f 0x65 0x4c 0x22 0xf2 0x5a 0xae 0x0b 0x8b 0x3b 0x5d 0x17 0x0b 0x00 0x57 0x40 0xfa 0x15 0x14 0x00 0x20 0x0f 0x73 0x3b 0xd8 0x4a 0x89 0x12 0xfa 0x15 0x08 0x00 0xd9 0x53 0xae 0x0b 0xce 0x84 0x90 0x16 ' s     eL" Z   ;]   W@      s; J       S      '

// ================================================
// 0xea41: WORD '%STORM' codep=0x1d29 parp=0xea4c
// ================================================
// 0xea4c: db  '    ( < U L" Z  s;-     ]       L"+T    ~  Z       ; Z mV ` d  b z    J  r r   . ] d  J-      Z    ` 8 . {; J Zvm]     .      ;    uI J]          Zvm] d  Z K y  `    Z m Z m] d  Z K  L"zX  zX m] t ]   + ]    OC  O  r^  ] , ` ]   r   ]   `       A ]   F        0   '   -A ]   F        0   '   -   'zXvm  '

// ================================================
// 0xeb7c: WORD 'UNK_0xeb7e' codep=0x224c parp=0xeb7e
// ================================================

void UNK_0xeb7e() // UNK_0xeb7e
{
  Push(0x5e74); // pointer to STARDATE
  Push(Read16(Pop())); // @
  Push(0x5455); // pointer to (FLARE)
  Push(Read16(Pop())); // @
  _eq_(); // =
  Push(0x567c); // pointer to ?WIN
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) return;
  Push(0x5447); // pointer to 'FLARE
  _at_EXECUTE(); // @EXECUTE
}


// ================================================
// 0xeb9a: WORD 'UNK_0xeb9c' codep=0x1d29 parp=0xeb9c
// ================================================
// 0xeb9c: db 0x01 0x00 '  '

// ================================================
// 0xeb9e: WORD 'UNK_0xeba0' codep=0x1d29 parp=0xeba0
// ================================================
// 0xeba0: db 0x00 0x00 '  '

// ================================================
// 0xeba2: WORD 'UNK_0xeba4' codep=0x2214 parp=0xeba4
// ================================================
// 0xeba4: db 0xd0 0x07 0xea 0x73 0x44 0x90 0x02 0x9c 0xd8 0x69 '   sD    i'

// ================================================
// 0xebae: WORD 'UNK_0xebb0' codep=0x224c parp=0xebb0
// ================================================

void UNK_0xebb0() // UNK_0xebb0
{
  Push(0x626f); // pointer to TVEHICLE
  _at__gt_C_plus_S(); // @>C+S
  Push(0x5dae); // pointer to XABS
  Push(Read16(Pop())); // @
  Push(0x5db9); // pointer to YABS
  Push(Read16(Pop())); // @
  UNK_0xe7ac(); // UNK_0xe7ac
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xebc2: WORD 'UNK_0xebc4' codep=0x224c parp=0xebc4
// ================================================

void UNK_0xebc4() // UNK_0xebc4
{
  Push(0xeb9c); // pointer to UNK_0xeb9c
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(0x58ff); // pointer to DXVIS
  _st__ex__gt_(); // <!>
  Push(0x5944); // pointer to XVIS
  _st__plus__ex__gt_(); // <+!>
  Push(0xeba0); // pointer to UNK_0xeba0
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(0x590b); // pointer to DYVIS
  _st__ex__gt_(); // <!>
  Push(0x594f); // pointer to YVIS
  _st__plus__ex__gt_(); // <+!>
}


// ================================================
// 0xebe2: WORD 'UNK_0xebe4' codep=0x224c parp=0xebe4
// ================================================

void UNK_0xebe4() // UNK_0xebe4
{
  Push(Read16(regsp)); // DUP
  Push(0x592d); // pointer to DXCON
  _st__ex__gt_(); // <!>
  Push(0x5916); // pointer to XCON
  _st__plus__ex__gt_(); // <+!>
}


// ================================================
// 0xebf0: WORD 'UNK_0xebf2' codep=0x224c parp=0xebf2
// ================================================

void UNK_0xebf2() // UNK_0xebf2
{
  Push(Read16(regsp)); // DUP
  Push(0x5939); // pointer to DYCON
  _st__ex__gt_(); // <!>
  Push(0x5921); // pointer to YCON
  _st__plus__ex__gt_(); // <+!>
}


// ================================================
// 0xebfe: WORD 'UNK_0xec00' codep=0x224c parp=0xec00
// ================================================

void UNK_0xec00() // UNK_0xec00
{
  Push(0x5944); // pointer to XVIS
  Push(Read16(Pop())); // @
  Push(0x5916); // pointer to XCON
  Push(Read16(Pop())); // @
  _2DUP(); // 2DUP
  Push(Pop()-1); // 1-
  _eq_(); // =
  if (Pop() == 0) goto label1;
  Pop(); Pop();// 2DROP
  Push(0xfff4);
  goto label2;

  label1:
  Push(0x001a);
  Push(Pop() + Pop()); // +
  _eq_(); // =
  if (Pop() == 0) goto label3;
  Push(0x000c);
  goto label2;

  label3:
  Push(0); // 0

  label2:
  UNK_0xebe4(); // UNK_0xebe4
  Push(0x594f); // pointer to YVIS
  Push(Read16(Pop())); // @
  Push(0x5921); // pointer to YCON
  Push(Read16(Pop())); // @
  _2DUP(); // 2DUP
  Push(Pop()-1); // 1-
  _eq_(); // =
  if (Pop() == 0) goto label4;
  Pop(); Pop();// 2DROP
  Push(0xffec);
  goto label5;

  label4:
  Push(0x002a);
  Push(Pop() + Pop()); // +
  _eq_(); // =
  if (Pop() == 0) goto label6;
  Push(0x0014);
  goto label5;

  label6:
  Push(0); // 0

  label5:
  UNK_0xebf2(); // UNK_0xebf2
  Push(0x592d); // pointer to DXCON
  Push(Read16(Pop())); // @
  Push(0x5939); // pointer to DYCON
  Push(Read16(Pop())); // @
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) return;
  Push(0xc33e);
  MODULE(); // MODULE
}

// 0xec7e: db 0x4c 0x22 0xf0 0x0d 0x12 0x7e 0x90 0x16 'L"   ~  '

// ================================================
// 0xec86: WORD 'UNK_0xec88' codep=0x224c parp=0xec88
// ================================================

void UNK_0xec88() // UNK_0xec88
{
  _2DUP(); // 2DUP
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) return;
  _2DUP(); // 2DUP
}


// ================================================
// 0xec94: WORD 'UNK_0xec96' codep=0x224c parp=0xec96
// ================================================

void UNK_0xec96() // UNK_0xec96
{
  Push(0xec78);
  Push(Read16(regsp)); // DUP
  Push(0x6230); // pointer to ANCHOR-CONTOUR
  _2_ex_(); // 2!
  ORGLIST(); // ORGLIST
}


// ================================================
// 0xeca4: WORD 'UNK_0xeca6' codep=0x224c parp=0xeca6
// ================================================

void UNK_0xeca6() // UNK_0xeca6
{
  _gt_C(); // >C
  SET_dash_CURRENT(); // SET-CURRENT
  CI(); // CI
  _ask_ICON_eq_I(); // ?ICON=I
  if (Pop() == 0) goto label1;
  POINT_gt_ICON(); // POINT>ICON
  UNK_0xe794(); // UNK_0xe794
  _ex_IY(); // !IY
  _ex_IX(); // !IX

  label1:
  ICLOSE(); // ICLOSE
}

// 0xecbe: db 0x4c 0x22 0xb7 0x74 0xae 0x0b 0xc5 0x74 0xae 0x0b 0x5d 0x17 0xfe 0x00 0x14 0x51 0x75 0x75 0x90 0x16 0x4c 0x22 0x1e 0x75 0x5d 0x17 0x1b 0x00 0x5f 0x12 0xfa 0x15 0x08 0x00 0xbe 0xec 0x60 0x16 0x08 0x00 0x5d 0x17 0x37 0xc4 0xce 0x84 0x0e 0x9f 0x90 0x16 'L" t   t  ]    Quu  L" u]   _       `   ] 7       '

// ================================================
// 0xecf0: WORD 'UNK_0xecf2' codep=0x224c parp=0xecf2
// ================================================

void UNK_0xecf2() // UNK_0xecf2
{
  IOPEN(); // IOPEN
  _ask_NULL(); // ?NULL
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  Push(0xecd4);
  ALL(); // ALL
  Push(0xec78);
  Push(Read16(regsp)); // DUP
  Push(0x6230); // pointer to ANCHOR-CONTOUR
  _2_ex_(); // 2!
  ORGLIST(); // ORGLIST

  label1:
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xed12: WORD 'UNK_0xed14' codep=0x224c parp=0xed14
// ================================================

void UNK_0xed14() // UNK_0xed14
{
  Push(0x001e);
  _slash_(); // /
  Push(0x001e);
  Push(Pop() * Pop()); // *
  SWAP(); // SWAP
  Push(0x0012);
  _slash_(); // /
  Push(0x0012);
  Push(Pop() * Pop()); // *
  SWAP(); // SWAP
}


// ================================================
// 0xed32: WORD 'UNK_0xed34' codep=0x224c parp=0xed34
// ================================================

void UNK_0xed34() // UNK_0xed34
{
  Push(0x63fc); // IFIELD(INST-X)
  Push(Read16(Pop())); // @
  Push(0x63fe); // IFIELD(INST-Y)
  Push(Read16(Pop())); // @
  _ex_XYSEED(); // !XYSEED
}


// ================================================
// 0xed40: WORD 'UNK_0xed42' codep=0x224c parp=0xed42
// ================================================

void UNK_0xed42() // UNK_0xed42
{
  UNK_0xe7b8(); // UNK_0xe7b8
  UNK_0xed14(); // UNK_0xed14
  UNK_0xe7ac(); // UNK_0xe7ac
}


// ================================================
// 0xed4a: WORD 'UNK_0xed4c' codep=0x224c parp=0xed4c
// ================================================

void UNK_0xed4c() // UNK_0xed4c
{
  Push(0x000b);
  Push(0x000b);
  ICREATE(); // ICREATE
  _2DUP(); // 2DUP
  Push(0x62bf); // pointer to (PLANET
  _1_dot_5_at_(); // 1.5@
  IINSERT(); // IINSERT
  _2DUP(); // 2DUP
  Push(0x630f); // pointer to THIS-RE
  _1_dot_5_ex_(); // 1.5!
  _2DUP(); // 2DUP
  _gt_C(); // >C
  SET_dash_CURRENT(); // SET-CURRENT
  UNK_0xed42(); // UNK_0xed42
  UNK_0xed34(); // UNK_0xed34
  Push(0); // 0
  Push(0x63fa); // IFIELD(INST-QTY)
  _ex__3(); // !_3
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xed78: WORD 'UNK_0xed7a' codep=0x224c parp=0xed7a
// ================================================

void UNK_0xed7a() // UNK_0xed7a
{
  Push(0x630f); // pointer to THIS-RE
  _1_dot_5_at_(); // 1.5@
  UNK_0xec88(); // UNK_0xec88
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1;
  _gt_C_plus_S(); // >C+S
  UNK_0xe7b8(); // UNK_0xe7b8
  UNK_0xed14(); // UNK_0xed14
  UNK_0xe794(); // UNK_0xe794
  D_eq_(); // D=
  if (Pop() == 0) goto label2;
  Push(0); // 0
  goto label3;

  label2:
  Push(1); // 1

  label3:
  ICLOSE(); // ICLOSE
  return;

  label1:
  Push(1); // 1
}


// ================================================
// 0xeda6: WORD 'UNK_0xeda8' codep=0x224c parp=0xeda8
// ================================================

void UNK_0xeda8() // UNK_0xeda8
{
  UNK_0xe7b8(); // UNK_0xe7b8
  UNK_0xed14(); // UNK_0xed14
  Push2Words("NULL");
  Push(0x62bf); // pointer to (PLANET
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN

  label4:
  _2DUP(); // 2DUP
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // 0=
  _ask_LAST(); // ?LAST
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1;
  _at_INST_dash_CLASS(); // @INST-CLASS
  Push(0x000b);
  _eq_(); // =
  _at_INST_dash_SPECIES(); // @INST-SPECIES
  Push(0x000b);
  _eq_(); // =
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label2;
  Pop(); Pop();// 2DROP
  _2DUP(); // 2DUP
  UNK_0xe794(); // UNK_0xe794
  D_eq_(); // D=
  if (Pop() == 0) goto label3;
  CI(); // CI
  goto label2;

  label3:
  Push2Words("0.");

  label2:
  INEXT(); // INEXT
  goto label4;

  label1:
  _gt_R(); // >R
  _gt_R(); // >R
  Pop(); Pop();// 2DROP
  R_gt_(); // R>
  R_gt_(); // R>
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xee04: WORD 'UNK_0xee06' codep=0x224c parp=0xee06
// ================================================

void UNK_0xee06() // UNK_0xee06
{
  Push(0x5944); // pointer to XVIS
  Push(Read16(Pop())); // @
  Push(0x000e);
  Push(Pop() + Pop()); // +
  Push(0x594f); // pointer to YVIS
  Push(Read16(Pop())); // @
  Push(0x0016);
  Push(Pop() + Pop()); // +
  Push(0x5944); // pointer to XVIS
  Push(Read16(Pop())); // @
  Push(0x0016);
  Push(Pop() + Pop()); // +
  Push(0x594f); // pointer to YVIS
  Push(Read16(Pop())); // @
  Push(0x0029);
  Push(Pop() + Pop()); // +
  _ex_VISWINDOW(); // !VISWINDOW
}


// ================================================
// 0xee32: WORD 'UNK_0xee34' codep=0x224c parp=0xee34
// ================================================

void UNK_0xee34() // UNK_0xee34
{
  _2DUP(); // 2DUP
  _gt_C(); // >C
  SET_dash_CURRENT(); // SET-CURRENT
  IOPEN(); // IOPEN
  CI(); // CI
  _ask_ICON_eq_I(); // ?ICON=I
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1;
  ICLOSE(); // ICLOSE
  UNK_0xecf2(); // UNK_0xecf2
  IOPEN(); // IOPEN
  goto label2;

  label1:
  Pop(); // DROP

  label2:
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xee58: WORD '?POPULA' codep=0x224c parp=0xee64
// ================================================
// entry

void _ask_POPULA() // ?POPULA
{
  UNK_0xed7a(); // UNK_0xed7a
  if (Pop() == 0) return;
  Push(0x5783); // pointer to ?NEW
  ON_3(); // ON_3
  UNK_0xeda8(); // UNK_0xeda8
  _2DUP(); // 2DUP
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1;
  Pop(); Pop();// 2DROP
  UNK_0xed4c(); // UNK_0xed4c
  Push(0x62bf); // pointer to (PLANET
  _at__gt_C_plus_S(); // @>C+S
  IOPEN(); // IOPEN
  CDROP(); // CDROP
  _gt_C_plus_S(); // >C+S
  Push(0x5807); // pointer to MOVED
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label2;
  UNK_0xe702(); // UNK_0xe702
  PRINT("SCANNING NEW TERRAIN ", 21); // (.")

  label2:
  Push(0xce7c);
  MODULE(); // MODULE
  UNK_0xecf2(); // UNK_0xecf2
  CDROP(); // CDROP
  ICLOSE(); // ICLOSE
  Push(0x5807); // pointer to MOVED
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label3;
  UNK_0xe6e0(); // UNK_0xe6e0

  label3:
  return;

  label1:
  UNK_0xee34(); // UNK_0xee34
  Push(0x630f); // pointer to THIS-RE
  _1_dot_5_ex_(); // 1.5!
}


// ================================================
// 0xeecc: WORD 'UNK_0xeece' codep=0x224c parp=0xeece
// ================================================

void UNK_0xeece() // UNK_0xeece
{
  SetColor("PINK");
  UNK_0xe7e8(); // UNK_0xe7e8
  UNK_0xe802(); // UNK_0xe802
  Push(0x5783); // pointer to ?NEW
  ON_3(); // ON_3
  Push(0x5807); // pointer to MOVED
  ON_3(); // ON_3
  Push(Read16(regsp)); // DUP
  Push(0xeb9c); // pointer to UNK_0xeb9c
  _st__ex__gt_(); // <!>
  Push(0x5dae); // pointer to XABS
  _st__plus__ex__gt_(); // <+!>
  Push(Read16(regsp)); // DUP
  Push(0xeba0); // pointer to UNK_0xeba0
  _st__ex__gt_(); // <!>
  Push(0x5db9); // pointer to YABS
  _st__plus__ex__gt_(); // <+!>
  UNK_0xebc4(); // UNK_0xebc4
  UNK_0xec00(); // UNK_0xec00
  _ask_POPULA(); // ?POPULA
  _gt_MAINVIEW(); // >MAINVIEW
  VCLIPSET(); // VCLIPSET
  UNK_0xee06(); // UNK_0xee06
  UNK_0xebb0(); // UNK_0xebb0
  Push(0x626f); // pointer to TVEHICLE
  _1_dot_5_at_(); // 1.5@
  UNK_0xeca6(); // UNK_0xeca6
  UNK_0xec96(); // UNK_0xec96
  UNK_0xe9e5(); // UNK_0xe9e5
  _dot_LOCAL_dash_ICONS(); // .LOCAL-ICONS
  V_gt_DISPLAY(); // V>DISPLAY
  _gt_DISPLAY(); // >DISPLAY
  DCLIPSET(); // DCLIPSET
  SetColor("GREEN");
  UNK_0xe7e8(); // UNK_0xe7e8
  UNK_0xe802(); // UNK_0xe802
  UNK_0xe696(); // UNK_0xe696
  UNK_0xeb7e(); // UNK_0xeb7e
}


// ================================================
// 0xef1c: WORD 'UNK_0xef1e' codep=0x224c parp=0xef1e
// ================================================

void UNK_0xef1e() // UNK_0xef1e
{
  _2OVER(); // 2OVER
  ROT(); // ROT
  SWAP(); // SWAP
  _dash_(); // -
  Push(Read16(regsp)); // DUP
  if (Pop() == 0) goto label1;
  Push(Read16(regsp)); // DUP
  ABS(); // ABS
  _slash_(); // /

  label1:
  _gt_R(); // >R
  _dash_(); // -
  Push(Read16(regsp)); // DUP
  if (Pop() == 0) goto label2;
  Push(Read16(regsp)); // DUP
  ABS(); // ABS
  _slash_(); // /

  label2:
  R_gt_(); // R>
}


// ================================================
// 0xef46: WORD 'UNK_0xef48' codep=0x224c parp=0xef48
// ================================================

void UNK_0xef48() // UNK_0xef48
{
  Push(0x001c);
  Push(0x0033);
  ICREATE(); // ICREATE
  _2DUP(); // 2DUP
  Push(0x630f); // pointer to THIS-RE
  _1_dot_5_at_(); // 1.5@
  IINSERT(); // IINSERT
  _gt_C_plus_S(); // >C+S
  UNK_0xe7b8(); // UNK_0xe7b8
  UNK_0xe7ac(); // UNK_0xe7ac
  Push(0x0bb8);
  Push(Read16(regsp)); // DUP
  LoadData("ART-VOL"); // from 'ARTIFACT    '
  _ex__3(); // !_3
  Push(0x63fa); // IFIELD(INST-QTY)
  _ex__3(); // !_3
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xef72: WORD 'UNK_0xef74' codep=0x224c parp=0xef74
// ================================================

void UNK_0xef74() // UNK_0xef74
{
  Push(0); // 0
  Push(1); // 1
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label1;
  Pop(); Pop();// 2DROP
  PRINT("NORTH", 5); // (.")
  return;

  label1:
  Push(1); // 1
  Push(1); // 1
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label2;
  Pop(); Pop();// 2DROP
  PRINT("NORTHEAST", 9); // (.")
  return;

  label2:
  Push(1); // 1
  Push(0); // 0
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label3;
  Pop(); Pop();// 2DROP
  PRINT("EAST", 4); // (.")
  return;

  label3:
  Push(1); // 1
  Push(cc__dash_1); // -1
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label4;
  Pop(); Pop();// 2DROP
  PRINT("SOUTHEAST", 9); // (.")
  return;

  label4:
  Push(0); // 0
  Push(cc__dash_1); // -1
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label5;
  Pop(); Pop();// 2DROP
  PRINT("SOUTH", 5); // (.")
  return;

  label5:
  Push(cc__dash_1); // -1
  Push(cc__dash_1); // -1
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label6;
  Pop(); Pop();// 2DROP
  PRINT("SOUTHWEST", 9); // (.")
  return;

  label6:
  Push(cc__dash_1); // -1
  Push(0); // 0
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label7;
  Pop(); Pop();// 2DROP
  PRINT("WEST", 4); // (.")
  return;

  label7:
  Push(cc__dash_1); // -1
  Push(1); // 1
  _2OVER(); // 2OVER
  D_eq_(); // D=
  if (Pop() == 0) goto label8;
  Pop(); Pop();// 2DROP
  PRINT("NORTHWEST", 9); // (.")
  return;

  label8:
  Pop(); // DROP
  Pop(); // DROP
}


// ================================================
// 0xf058: WORD 'UNK_0xf05a' codep=0x224c parp=0xf05a
// ================================================

void UNK_0xf05a() // UNK_0xf05a
{
  Push2Words("*SHIP");
  _gt_C_plus_S(); // >C+S
  UNK_0xe7b8(); // UNK_0xe7b8
  Push(0x63fe); // IFIELD(INST-Y)
  Push(Read16(Pop())); // @
  _dash_(); // -
  ABS(); // ABS
  Push(Read16(regsp)); // DUP
  U_star_(); // U*
  ROT(); // ROT
  Push(0x63fc); // IFIELD(INST-X)
  Push(Read16(Pop())); // @
  _dash_(); // -
  ABS(); // ABS
  Push(Read16(regsp)); // DUP
  U_star_(); // U*
  D_plus_(); // D+
  SQRT(); // SQRT
}

// 0xf080: db 0x4c 0x22 0x69 0x54 0xae 0x0b 0x64 0x5e 0xae 0x0b 0x92 0x0f 0x41 0x0e 0xfa 0x15 0x0a 0x00 0x64 0x5e 0xae 0x0b 0x69 0x54 0x76 0x6d 0x90 0x16 'L"iT  d^    A     d^  iTvm  '

// ================================================
// 0xf09c: WORD 'UNK_0xf09e' codep=0x224c parp=0xf09e
// ================================================

void UNK_0xf09e() // UNK_0xf09e
{
  Push(0x5af4); // pointer to #STORM
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(cc__4); // 4
  _st_(); // <
  SWAP(); // SWAP
  Push(0x000b);
  _gt_(); // >
  Push(Pop() | Pop()); // OR
}


// ================================================
// 0xf0b4: WORD 'UNK_0xf0b6' codep=0x224c parp=0xf0b6
// ================================================

void UNK_0xf0b6() // UNK_0xf0b6
{
  UNK_0xf09e(); // UNK_0xf09e
  UNK_0xe7ce(); // UNK_0xe7ce
  Push(Pop() | Pop()); // OR
}


// ================================================
// 0xf0be: WORD 'UNK_0xf0c0' codep=0x224c parp=0xf0c0
// ================================================

void UNK_0xf0c0() // UNK_0xf0c0
{
  _gt_DISPLAY(); // >DISPLAY
  Push(0x0074);
  Push(0x0096);
  POS_dot_(); // POS.
  Push(cc__8); // 8
  SetColor("BLACK");
  POLY_dash_ERASE_dash_TEXT(); // POLY-ERASE-TEXT
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  UNK_0xf0b6(); // UNK_0xf0b6
  if (Pop() == 0) goto label1;
  UNK_0xf05a(); // UNK_0xf05a
  Push(cc__5); // 5
  _dot_R(); // .R
  PRINT("KM.", 3); // (.")
  goto label2;

  label1:
  PRINT("LOST!", 5); // (.")

  label2:
  Push(0x0074);
  Push(0x008f);
  POS_dot_(); // POS.
  Push(0x0009);
  SetColor("BLACK");
  POLY_dash_ERASE_dash_TEXT(); // POLY-ERASE-TEXT
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  UNK_0xf0b6(); // UNK_0xf0b6
  if (Pop() == 0) return;
  UNK_0xe7b8(); // UNK_0xe7b8
  UNK_0xe794(); // UNK_0xe794
  ICLOSE(); // ICLOSE
  UNK_0xef1e(); // UNK_0xef1e
  _2SWAP(); // 2SWAP
  Pop(); Pop();// 2DROP
  UNK_0xef74(); // UNK_0xef74
}


// ================================================
// 0xf120: WORD 'UNK_0xf122' codep=0x224c parp=0xf122
// ================================================

void UNK_0xf122() // UNK_0xf122
{
  Push(Read16(regsp)); // DUP
  Push(0xeba0); // pointer to UNK_0xeba0
  _st__ex__gt_(); // <!>
  Push(0x5db9); // pointer to YABS
  _st__plus__ex__gt_(); // <+!>
  Push(Read16(regsp)); // DUP
  Push(0xeb9c); // pointer to UNK_0xeb9c
  _st__ex__gt_(); // <!>
  Push(0x5dae); // pointer to XABS
  _st__plus__ex__gt_(); // <+!>
  UNK_0xebc4(); // UNK_0xebc4
  UNK_0xec00(); // UNK_0xec00
  SET_dash_SPEED(); // SET-SPEED
  UNK_0xf0c0(); // UNK_0xf0c0
  Push(0); // 0
  Push(cc__5); // 5
  RRND(); // RRND
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1;
  Push(0xc03f);
  MODULE(); // MODULE
  _gt_DISPLAY(); // >DISPLAY
  Push(0x53db); // pointer to 'INJURE
  Push(Read16(Pop())); // @
  MODULE(); // MODULE

  label1:
  _gt_MAINVIEW(); // >MAINVIEW
  VCLIPSET(); // VCLIPSET
  UNK_0xee06(); // UNK_0xee06
  NOP(); // NOP
  UNK_0xec96(); // UNK_0xec96
  UNK_0xe9e5(); // UNK_0xe9e5
  _dot_LOCAL_dash_ICONS(); // .LOCAL-ICONS
  V_gt_DISPLAY(); // V>DISPLAY
  DCLIPSET(); // DCLIPSET
}


// ================================================
// 0xf16c: WORD 'UNK_0xf16e' codep=0x224c parp=0xf16e
// ================================================

void UNK_0xf16e() // UNK_0xf16e
{
  UNK_0xe702(); // UNK_0xe702
  PRINT("OUT OF FUEL ", 12); // (.")
  GCR(); // GCR
  Push(1); // 1
  Push(0x586e); // pointer to XBLT
  _plus__ex_(); // +!
  PRINT("RETURNING TO SHIP ON FOOT ", 26); // (.")
  Push(0x03e8);
  MS(); // MS
  BEEPOFF(); // BEEPOFF
  Push(0xcf10);
  MODULE(); // MODULE
  Push(0x545f); // pointer to ?TV
  OFF_2(); // OFF_2
  UNK_0xef48(); // UNK_0xef48
  Push(0x543a); // pointer to LCOLOR
  OFF(); // OFF
  SetColor("BLACK");
  UNK_0xe7e8(); // UNK_0xe7e8
  UNK_0xe802(); // UNK_0xe802
  Push2Words("*SHIP");
  _gt_C_plus_S(); // >C+S
  UNK_0xe794(); // UNK_0xe794
  ICLOSE(); // ICLOSE

  label2:
  UNK_0xe7b8(); // UNK_0xe7b8
  _2OVER(); // 2OVER
  UNK_0xef1e(); // UNK_0xef1e
  _2DUP(); // 2DUP
  Push2Words("NULL");
  D_eq_(); // D=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  UNK_0xf122(); // UNK_0xf122
  Pop(); Pop();// 2DROP
  goto label2;

  label1:
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
  Push(0x626f); // pointer to TVEHICLE
  _at__gt_C_plus_S(); // @>C+S
  _dash_ICON(); // -ICON
  ICLOSE(); // ICLOSE
  Push(0x5c7e); // pointer to FTRIG
  ON_2(); // ON_2
  UNK_0xebb0(); // UNK_0xebb0
  BEEPOFF(); // BEEPOFF
}


// ================================================
// 0xf1fc: WORD 'UNK_0xf1fe' codep=0x224c parp=0xf1fe
// ================================================

void UNK_0xf1fe() // UNK_0xf1fe
{
  Push(0x0074);
  Push(0x00ab);
  POS_dot_(); // POS.
  Push(cc__7); // 7
  SetColor("BLACK");
  POLY_dash_ERASE_dash_TEXT(); // POLY-ERASE-TEXT
}


// ================================================
// 0xf210: WORD 'UNK_0xf212' codep=0x224c parp=0xf212
// ================================================

void UNK_0xf212() // UNK_0xf212
{
  UNK_0xf1fe(); // UNK_0xf1fe
  Push(0x626f); // pointer to TVEHICLE
  _at__gt_C_plus_S(); // @>C+S
  Push(0x63fa); // IFIELD(INST-QTY)
  Push(Read16(Pop())); // @
  ICLOSE(); // ICLOSE
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  Push(cc_UNK_0xeba4); // UNK_0xeba4
  _dash_(); // -
  Push(0x0064);
  Push(0x782f);
  _star__slash_(); // */
  Push(Read16(regsp)); // DUP
  _0_gt_(); // 0>
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  Pop(); // DROP
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  SetColor("PINK");
  _ex_COLOR(); // !COLOR
  PRINT("RESERVE", 7); // (.")
  _ex_COLOR(); // !COLOR
  goto label2;

  label1:
  Push(cc__3); // 3
  _dot_R(); // .R
  PRINT("%", 1); // (.")

  label2:
  Push(0x0074);
  Push(0x00a4);
  POS_dot_(); // POS.
  Push(cc__3); // 3
  SetColor("BLACK");
  POLY_dash_ERASE_dash_TEXT(); // POLY-ERASE-TEXT
  Push(0x5aab); // pointer to %EFF
  Push(Read16(Pop())); // @
  Push(0x0064);
  Push(0x5ad9); // pointer to E/M
  Push(Read16(Pop())); // @
  Push(cc__4); // 4
  Push(Pop() * Pop()); // *
  Push(-Pop()); // NEGATE
  _star__slash_(); // */
  Push(0x0063);
  MIN(); // MIN
  Push(cc__3); // 3
  _dot_R(); // .R
  PRINT("%", 1); // (.")
}


// ================================================
// 0xf290: WORD 'UNK_0xf292' codep=0x224c parp=0xf292
// ================================================

void UNK_0xf292() // UNK_0xf292
{
  UNK_0xf212(); // UNK_0xf212
  UNK_0xf0c0(); // UNK_0xf0c0
  UNK_0xe802(); // UNK_0xe802
}


// ================================================
// 0xf29a: WORD 'UNK_0xf29c' codep=0x224c parp=0xf29c
// ================================================

void UNK_0xf29c() // UNK_0xf29c
{
  Push(0x5ad9); // pointer to E/M
  Push(Read16(Pop())); // @
  Push(0x0064);
  Push(0x5aab); // pointer to %EFF
  Push(Read16(Pop())); // @
  _star__slash_(); // */
  Push(0x626f); // pointer to TVEHICLE
  _at__gt_C_plus_S(); // @>C+S
  Push(0x63fa); // IFIELD(INST-QTY)
  _plus__ex_(); // +!
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xf2b6: WORD 'UNK_0xf2b8' codep=0x224c parp=0xf2b8
// ================================================

void UNK_0xf2b8() // UNK_0xf2b8
{
  Push(0x626f); // pointer to TVEHICLE
  _at__gt_C_plus_S(); // @>C+S
  Push(0x63fa); // IFIELD(INST-QTY)
  Push(Read16(Pop())); // @
  ICLOSE(); // ICLOSE
  _0_gt_(); // 0>
  if (Pop() == 0) Push(1); else Push(0); // NOT
}


// ================================================
// 0xf2c8: WORD 'UNK_0xf2ca' codep=0x224c parp=0xf2ca
// ================================================

void UNK_0xf2ca() // UNK_0xf2ca
{
  Push(0x587c); // pointer to XORMODE
  OFF(); // OFF
  SetColor("GREEN");
  UNK_0xe7e8(); // UNK_0xe7e8
  UNK_0xe802(); // UNK_0xe802
}


// ================================================
// 0xf2d6: WORD 'UNK_0xf2d8' codep=0x224c parp=0xf2d8
// ================================================

void UNK_0xf2d8() // UNK_0xf2d8
{
  SetColor("BLACK");
  UNK_0xe7e8(); // UNK_0xe7e8
  UNK_0xe802(); // UNK_0xe802
}


// ================================================
// 0xf2e0: WORD 'UNK_0xf2e2' codep=0x224c parp=0xf2e2
// ================================================

void UNK_0xf2e2() // UNK_0xf2e2
{
  Push(0x5c95); // pointer to LKEY
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(0x0152);
  _eq_(); // =
  SWAP(); // SWAP
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) return;
  Push(0x5c7e); // pointer to FTRIG
  OFF(); // OFF
  Pop(); Pop();// 2DROP
  Push(0x5d76); // pointer to SKEY
  Push(Read16(Pop())); // @
  Pop();
  switch(Pop()) // (XYSCAN
  {
  case 327:
    UNK_0xa98d(); // UNK_0xa98d
    break;
  case 328:
    UNK_0xa995(); // UNK_0xa995
    break;
  case 329:
    UNK_0xa99d(); // UNK_0xa99d
    break;
  case 331:
    UNK_0xa9a5(); // UNK_0xa9a5
    break;
  case 333:
    UNK_0xa9b1(); // UNK_0xa9b1
    break;
  case 335:
    UNK_0xa9bd(); // UNK_0xa9bd
    break;
  case 336:
    UNK_0xa9c5(); // UNK_0xa9c5
    break;
  case 337:
    UNK_0xa9cd(); // UNK_0xa9cd
    break;
  case 0:
    UNK_0xa9d5(); // UNK_0xa9d5
    break;
  case 92:
    UNK_0xa98d(); // UNK_0xa98d
    break;
  case 126:
    UNK_0xa995(); // UNK_0xa995
    break;
  case 124:
    UNK_0xa9a5(); // UNK_0xa9a5
    break;
  case 500:
    UNK_0xa9b1(); // UNK_0xa9b1
    break;
  case 335:
    UNK_0xa9bd(); // UNK_0xa9bd
    break;
  case 96:
    UNK_0xa9c5(); // UNK_0xa9c5
    break;
  default:
    UNK_0xa9db(); // UNK_0xa9db
    break;

  }
}


// ================================================
// 0xf306: WORD 'UNK_0xf308' codep=0x224c parp=0xf308
// ================================================

void UNK_0xf308() // UNK_0xf308
{
  UNK_0xf1fe(); // UNK_0xf1fe
  SetColor("YELLOW");
  _ex_COLOR(); // !COLOR
  PRINT("NONE", 4); // (.")
  Pop(); Pop();// 2DROP
  UNK_0xf16e(); // UNK_0xf16e
}


// ================================================
// 0xf31b: WORD 'UNK_0xf31d' codep=0x224c parp=0xf31d
// ================================================

void UNK_0xf31d() // UNK_0xf31d
{
  Push(0xf082);
  Push(0x5f40); // pointer to 'VEHICLE-CYCLE
  _st__ex__gt_(); // <!>
  Push(0xea6e);
  Push(0x5f6a); // pointer to 'REPAIR
  _st__ex__gt_(); // <!>
  Push(0xf292);
  Push(0x5f32); // pointer to '.VEHICLE-STATUS
  _st__ex__gt_(); // <!>
  Push(0x5ec4); // pointer to 'SIMULA
  Push(Read16(Pop())); // @
  Push(0x5f5c); // pointer to 'EXTERNAL-EVENTS
  _st__ex__gt_(); // <!>
  Push(0x266b);
  Push(0x5edf); // pointer to 'ENDING
  _st__ex__gt_(); // <!>
  Push(0xeb06);
  Push(0x5f24); // pointer to '.DATE
  _st__ex__gt_(); // <!>
  Push(0x5e82); // pointer to TIME-PASSING
  ON_3(); // ON_3
}


// ================================================
// 0xf353: WORD 'UNK_0xf355' codep=0x224c parp=0xf355
// ================================================

void UNK_0xf355() // UNK_0xf355
{
  _ask_TRIG(); // ?TRIG
  Pop(); // DROP
  UNK_0xf2ca(); // UNK_0xf2ca
  UNK_0xf31d(); // UNK_0xf31d
  SetColor("GREEN");
  Push(0x543a); // pointer to LCOLOR
  _ex__3(); // !_3
  Push(cc__dash_1); // -1
  TONE(); // TONE
  BEEPON(); // BEEPON
}


// ================================================
// 0xf36b: WORD 'UNK_0xf36d' codep=0x224c parp=0xf36d
// ================================================

void UNK_0xf36d() // UNK_0xf36d
{
  Push(0x5ac2); // pointer to 'TVT
  Push(Read16(Pop())); // @
  MODULE(); // MODULE
  Push(0x5e82); // pointer to TIME-PASSING
  OFF(); // OFF
}


// ================================================
// 0xf379: WORD 'UNK_0xf37b' codep=0x224c parp=0xf37b
// ================================================

void UNK_0xf37b() // UNK_0xf37b
{
  UNK_0xf36d(); // UNK_0xf36d
  UNK_0xf2d8(); // UNK_0xf2d8
  SetColor("BLACK");
  Push(0x543a); // pointer to LCOLOR
  _ex__3(); // !_3
  BEEPOFF(); // BEEPOFF
}


// ================================================
// 0xf389: WORD 'UNK_0xf38b' codep=0x224c parp=0xf38b
// ================================================

void UNK_0xf38b() // UNK_0xf38b
{
  UNK_0xeece(); // UNK_0xeece
  SET_dash_SPEED(); // SET-SPEED
  UNK_0xf29c(); // UNK_0xf29c
  UNK_0xf292(); // UNK_0xf292
}


// ================================================
// 0xf395: WORD 'UNK_0xf397' codep=0x224c parp=0xf397
// ================================================

void UNK_0xf397() // UNK_0xf397
{
  _2DUP(); // 2DUP
  Push(Pop() | Pop()); // OR
  Push(0x5c7e); // pointer to FTRIG
  Push(Read16(Pop())); // @
  if (Pop() == 0) Push(1); else Push(0); // 0=
  Push(Pop() & Pop()); // AND
}


// ================================================
// 0xf3a5: WORD 'UNK_0xf3a7' codep=0x224c parp=0xf3a7
// ================================================

void UNK_0xf3a7() // UNK_0xf3a7
{
  Pop();
  switch(Pop()) // (XYSCAN
  {
  case 327:
    UNK_0xa98d(); // UNK_0xa98d
    break;
  case 328:
    UNK_0xa995(); // UNK_0xa995
    break;
  case 329:
    UNK_0xa99d(); // UNK_0xa99d
    break;
  case 331:
    UNK_0xa9a5(); // UNK_0xa9a5
    break;
  case 333:
    UNK_0xa9b1(); // UNK_0xa9b1
    break;
  case 335:
    UNK_0xa9bd(); // UNK_0xa9bd
    break;
  case 336:
    UNK_0xa9c5(); // UNK_0xa9c5
    break;
  case 337:
    UNK_0xa9cd(); // UNK_0xa9cd
    break;
  case 0:
    UNK_0xa9d5(); // UNK_0xa9d5
    break;
  case 92:
    UNK_0xa98d(); // UNK_0xa98d
    break;
  case 126:
    UNK_0xa995(); // UNK_0xa995
    break;
  case 124:
    UNK_0xa9a5(); // UNK_0xa9a5
    break;
  case 500:
    UNK_0xa9b1(); // UNK_0xa9b1
    break;
  case 335:
    UNK_0xa9bd(); // UNK_0xa9bd
    break;
  case 96:
    UNK_0xa9c5(); // UNK_0xa9c5
    break;
  default:
    UNK_0xa9db(); // UNK_0xa9db
    break;

  }
  UNK_0xf397(); // UNK_0xf397
  if (Pop() == 0) goto label1;
  UNK_0xf2b8(); // UNK_0xf2b8
  if (Pop() == 0) goto label2;
  UNK_0xf308(); // UNK_0xf308
  goto label3;

  label2:
  UNK_0xf38b(); // UNK_0xf38b

  label5:
  XYSCAN(); // XYSCAN
  UNK_0xf2e2(); // UNK_0xf2e2
  UNK_0xf397(); // UNK_0xf397
  UNK_0xf2b8(); // UNK_0xf2b8
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label4;
  UNK_0xf38b(); // UNK_0xf38b
  goto label5;

  label4:
  Pop(); Pop();// 2DROP
  UNK_0xf2b8(); // UNK_0xf2b8
  if (Pop() == 0) goto label3;
  Push2Words("0.");
  UNK_0xf308(); // UNK_0xf308

  label3:
  goto label6;

  label1:
  Pop(); Pop();// 2DROP

  label6:
  _ask_TRIG(); // ?TRIG
  if (Pop() == 0) return;
  Pop(); // DROP
  Push(0); // 0
  Push(0x5e82); // pointer to TIME-PASSING
  OFF(); // OFF
}


// ================================================
// 0xf3f5: WORD 'TV-MOVE' codep=0x224c parp=0xf401
// ================================================
// entry

void TV_dash_MOVE() // TV-MOVE
{
  Push(0xf37b);
  Push(0xf3a7);
  Push(0xf355);
  DOTASKS(UNK_0xf355, UNK_0xf3a7, UNK_0xf37b);
  Push(0x5e82); // pointer to TIME-PASSING
  ON_3(); // ON_3
}


// ================================================
// 0xf415: WORD 'plan-tseed' codep=0x73ea parp=0xf417
// ================================================
// 0xf417: db 0x20 0x07 0x02 0x16 0x8f 0x65 '     e'

// ================================================
// 0xf41d: WORD 'APPROACH' codep=0x224c parp=0xf42a
// ================================================

void APPROACH() // APPROACH
{
  _gt_LORES(); // >LORES
  DARK(); // DARK
  Push(0x62bf); // pointer to (PLANET
  _1_dot_5_at_(); // 1.5@
  _gt_C(); // >C
  SET_dash_CURRENT(); // SET-CURRENT
  _at_INST_dash_SPECIES(); // @INST-SPECIES
  Push(0x0012);
  _st_(); // <
  if (Pop() == 0) goto label1;
  CI(); // CI
  Pop(); // DROP
  goto label2;

  label1:
  LoadData("plan-tseed"); // from 'PLANET      '
  Push(Read16(Pop())); // @

  label2:
  Push(Read16(regsp)); // DUP
  Push(0xc302);
  MODULE(); // MODULE
  SHIP_dash_CONSOLE(); // SHIP-CONSOLE
  Push(0xc48c);
  MODULE(); // MODULE
  _dot_REGION_plus_(); // .REGION+
  Push(0xc4a2);
  MODULE(); // MODULE
  Push(0x5e90); // pointer to #CLRMAP
  Push(Read16(Pop())); // @
  SWAP(); // SWAP
  Push(0xc380);
  MODULE(); // MODULE
  Push(0xc395);
  MODULE(); // MODULE
  Push(0xc3a7);
  MODULE(); // MODULE
  _gt_DISPLAY(); // >DISPLAY
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  DCLIPSET(); // DCLIPSET
  Push(0x5a5c); // pointer to CONTEXT-ID#
  OFF(); // OFF
  ICLOSE(); // ICLOSE
}

// 0xf48e: db  'MOVE-VOC________________________NT OVERLAY -------( rfg28apr86) 1225  O    `   j     /O    XQ a m8  u y   ;   K O      ~ ` 6 B    ;  S    NOT ENOUGH STORAGE FOR FUNCTION]    *    ITEMS__________________________ '
  