// ====== OVERLAY 'SENT-OV' ======
// store offset = 0xe2b0
// overlay size   = 0x12b0

#include"../emul/cpu.h"
#include"../emul/starflt1.h"


// =================================
// =========== DICTIONARY ==========
// =================================
// 1743:      UNK_0xe2e6  codep:0x224c parp:0xe2e6 size:0x000a C-string:'UNK_0xe2e6'
// 1744:      UNK_0xe2f2  codep:0x224c parp:0xe2f2 size:0x002b C-string:'UNK_0xe2f2'
// 1745:           CARDS  codep:0xe312 parp:0xe327 size:0x003c C-string:'CARDS'
// 1746:        NICHE-SE  codep:0xe312 parp:0xe370 size:0x002d C-string:'NICHE_dash_SE'
// 1747:      UNK_0xe39f  codep:0x224c parp:0xe39f size:0x002e C-string:'UNK_0xe39f'
// 1748:      UNK_0xe3cf  codep:0x224c parp:0xe3cf size:0x0042 C-string:'UNK_0xe3cf'
// 1749:      UNK_0xe413  codep:0x224c parp:0xe413 size:0x0008 C-string:'UNK_0xe413'
// 1750:      UNK_0xe41d  codep:0x224c parp:0xe41d size:0x0016 C-string:'UNK_0xe41d'
// 1751:      UNK_0xe435  codep:0x224c parp:0xe435 size:0x002e C-string:'UNK_0xe435'
// 1752:        PROBABLE  codep:0x1d29 parp:0xe470 size:0x0002 C-string:'PROBABLE'
// 1753:      UNK_0xe474  codep:0x224c parp:0xe474 size:0x0006 C-string:'UNK_0xe474'
// 1754:      UNK_0xe47c  codep:0x224c parp:0xe47c size:0x0029 C-string:'UNK_0xe47c'
// 1755:            HITS  codep:0x744d parp:0xe4ae size:0x0008 C-string:'HITS'
// 1756:      UNK_0xe4b8  codep:0x73ea parp:0xe4b8 size:0x0016 C-string:'UNK_0xe4b8'
// 1757:      UNK_0xe4d0  codep:0x73ea parp:0xe4d0 size:0x000e C-string:'UNK_0xe4d0'
// 1758:      UNK_0xe4e0  codep:0x73ea parp:0xe4e0 size:0x0016 C-string:'UNK_0xe4e0'
// 1759:      UNK_0xe4f8  codep:0x73ea parp:0xe4f8 size:0x0046 C-string:'UNK_0xe4f8'
// 1760:      UNK_0xe540  codep:0x73ea parp:0xe540 size:0x0006 C-string:'UNK_0xe540'
// 1761:      UNK_0xe548  codep:0x73ea parp:0xe548 size:0x001e C-string:'UNK_0xe548'
// 1762:      UNK_0xe568  codep:0x73ea parp:0xe568 size:0x0006 C-string:'UNK_0xe568'
// 1763:      UNK_0xe570  codep:0x73ea parp:0xe570 size:0x0006 C-string:'UNK_0xe570'
// 1764:      UNK_0xe578  codep:0x73ea parp:0xe578 size:0x0006 C-string:'UNK_0xe578'
// 1765:      UNK_0xe580  codep:0x73ea parp:0xe580 size:0x000e C-string:'UNK_0xe580'
// 1766:      UNK_0xe590  codep:0x73ea parp:0xe590 size:0x00e6 C-string:'UNK_0xe590'
// 1767:      UNK_0xe678  codep:0x73ea parp:0xe678 size:0x0006 C-string:'UNK_0xe678'
// 1768:      UNK_0xe680  codep:0x744d parp:0xe680 size:0x0003 C-string:'UNK_0xe680'
// 1769:      UNK_0xe685  codep:0x744d parp:0xe685 size:0x0003 C-string:'UNK_0xe685'
// 1770:      UNK_0xe68a  codep:0x744d parp:0xe68a size:0x0003 C-string:'UNK_0xe68a'
// 1771:      UNK_0xe68f  codep:0x224c parp:0xe68f size:0x000c C-string:'UNK_0xe68f'
// 1772:      UNK_0xe69d  codep:0x224c parp:0xe69d size:0x0020 C-string:'UNK_0xe69d'
// 1773:      UNK_0xe6bf  codep:0x3b68 parp:0xe6bf size:0x0004 C-string:'UNK_0xe6bf'
// 1774:      UNK_0xe6c5  codep:0x1d29 parp:0xe6c5 size:0x0002 C-string:'UNK_0xe6c5'
// 1775:      UNK_0xe6c9  codep:0x1d29 parp:0xe6c9 size:0x0191 C-string:'UNK_0xe6c9'
// 1776:            SHOW  codep:0x1d29 parp:0xe863 size:0x0002 C-string:'SHOW'
// 1777:      UNK_0xe867  codep:0x224c parp:0xe867 size:0x004e C-string:'UNK_0xe867'
// 1778:      UNK_0xe8b7  codep:0x224c parp:0xe8b7 size:0x000e C-string:'UNK_0xe8b7'
// 1779:      UNK_0xe8c7  codep:0x224c parp:0xe8c7 size:0x011a C-string:'UNK_0xe8c7'
// 1780:      UNK_0xe9e3  codep:0x224c parp:0xe9e3 size:0x0028 C-string:'UNK_0xe9e3'
// 1781:      UNK_0xea0d  codep:0x224c parp:0xea0d size:0x000c C-string:'UNK_0xea0d'
// 1782:      UNK_0xea1b  codep:0x224c parp:0xea1b size:0x0012 C-string:'UNK_0xea1b'
// 1783:      UNK_0xea2f  codep:0x3b68 parp:0xea2f size:0x0004 C-string:'UNK_0xea2f'
// 1784:      UNK_0xea35  codep:0x224c parp:0xea35 size:0x0006 C-string:'UNK_0xea35'
// 1785:      UNK_0xea3d  codep:0x224c parp:0xea3d size:0x0008 C-string:'UNK_0xea3d'
// 1786:      UNK_0xea47  codep:0x224c parp:0xea47 size:0x001a C-string:'UNK_0xea47'
// 1787:      UNK_0xea63  codep:0x1d29 parp:0xea63 size:0x0023 C-string:'UNK_0xea63'
// 1788:             VTB  codep:0xea73 parp:0xea8e size:0x0008 C-string:'VTB'
// 1789:        POSSESSI  codep:0xea73 parp:0xeaa3 size:0x0008 C-string:'POSSESSI'
// 1790:         PRONOUN  codep:0xea73 parp:0xeab7 size:0x0008 C-string:'PRONOUN'
// 1791:        DETERMIN  codep:0xea73 parp:0xeacc size:0x0008 C-string:'DETERMIN'
// 1792:        POSSES.P  codep:0xea73 parp:0xeae1 size:0x0008 C-string:'POSSES_dot_P'
// 1793:      UNK_0xeaeb  codep:0x224c parp:0xeaeb size:0x000e C-string:'UNK_0xeaeb'
// 1794:      UNK_0xeafb  codep:0x224c parp:0xeafb size:0x0050 C-string:'UNK_0xeafb'
// 1795:      UNK_0xeb4d  codep:0x224c parp:0xeb4d size:0x0010 C-string:'UNK_0xeb4d'
// 1796:      UNK_0xeb5f  codep:0x224c parp:0xeb5f size:0x000e C-string:'UNK_0xeb5f'
// 1797:      UNK_0xeb6f  codep:0x224c parp:0xeb6f size:0x0008 C-string:'UNK_0xeb6f'
// 1798:      UNK_0xeb79  codep:0x224c parp:0xeb79 size:0x0010 C-string:'UNK_0xeb79'
// 1799:      UNK_0xeb8b  codep:0x224c parp:0xeb8b size:0x0024 C-string:'UNK_0xeb8b'
// 1800:      UNK_0xebb1  codep:0x224c parp:0xebb1 size:0x001c C-string:'UNK_0xebb1'
// 1801:      UNK_0xebcf  codep:0x224c parp:0xebcf size:0x000a C-string:'UNK_0xebcf'
// 1802:      UNK_0xebdb  codep:0x224c parp:0xebdb size:0x005e C-string:'UNK_0xebdb'
// 1803:      UNK_0xec3b  codep:0x224c parp:0xec3b size:0x0016 C-string:'UNK_0xec3b'
// 1804:      UNK_0xec53  codep:0x224c parp:0xec53 size:0x0024 C-string:'UNK_0xec53'
// 1805:      UNK_0xec79  codep:0x224c parp:0xec79 size:0x0028 C-string:'UNK_0xec79'
// 1806:      UNK_0xeca3  codep:0x224c parp:0xeca3 size:0x0020 C-string:'UNK_0xeca3'
// 1807:      UNK_0xecc5  codep:0x224c parp:0xecc5 size:0x0036 C-string:'UNK_0xecc5'
// 1808:      UNK_0xecfd  codep:0x224c parp:0xecfd size:0x002c C-string:'UNK_0xecfd'
// 1809:      UNK_0xed2b  codep:0x224c parp:0xed2b size:0x002c C-string:'UNK_0xed2b'
// 1810:      UNK_0xed59  codep:0x224c parp:0xed59 size:0x003a C-string:'UNK_0xed59'
// 1811:      UNK_0xed95  codep:0x224c parp:0xed95 size:0x0056 C-string:'UNK_0xed95'
// 1812:      UNK_0xeded  codep:0x224c parp:0xeded size:0x0066 C-string:'UNK_0xeded'
// 1813:      UNK_0xee55  codep:0x1d29 parp:0xee55 size:0x0010 C-string:'UNK_0xee55'
// 1814:      UNK_0xee67  codep:0x1d29 parp:0xee67 size:0x0010 C-string:'UNK_0xee67'
// 1815:      UNK_0xee79  codep:0x1d29 parp:0xee79 size:0x0010 C-string:'UNK_0xee79'
// 1816:      UNK_0xee8b  codep:0x1d29 parp:0xee8b size:0x0010 C-string:'UNK_0xee8b'
// 1817:      UNK_0xee9d  codep:0x1d29 parp:0xee9d size:0x0002 C-string:'UNK_0xee9d'
// 1818:      UNK_0xeea1  codep:0x224c parp:0xeea1 size:0x0006 C-string:'UNK_0xeea1'
// 1819:      UNK_0xeea9  codep:0x1d29 parp:0xeea9 size:0x0002 C-string:'UNK_0xeea9'
// 1820:      UNK_0xeead  codep:0x224c parp:0xeead size:0x001c C-string:'UNK_0xeead'
// 1821:      UNK_0xeecb  codep:0x224c parp:0xeecb size:0x00b6 C-string:'UNK_0xeecb'
// 1822:      UNK_0xef83  codep:0x224c parp:0xef83 size:0x0042 C-string:'UNK_0xef83'
// 1823:        POSSIBIL  codep:0xe963 parp:0xefd2 size:0x0018 C-string:'POSSIBIL'
// 1824:      UNK_0xefec  codep:0x224c parp:0xefec size:0x0080 C-string:'UNK_0xefec'
// 1825:      UNK_0xf06e  codep:0x224c parp:0xf06e size:0x0056 C-string:'UNK_0xf06e'
// 1826:           SAYIT  codep:0x224c parp:0xf0ce size:0x0014 C-string:'SAYIT'
// 1827:         ANALOGS  codep:0xe963 parp:0xf0ee size:0x0076 C-string:'ANALOGS'
// 1828:      UNK_0xf166  codep:0x224c parp:0xf166 size:0x0084 C-string:'UNK_0xf166'
// 1829:      UNK_0xf1ec  codep:0x224c parp:0xf1ec size:0x0020 C-string:'UNK_0xf1ec'
// 1830:      UNK_0xf20e  codep:0x224c parp:0xf20e size:0x000e C-string:'UNK_0xf20e'
// 1831:      UNK_0xf21e  codep:0x224c parp:0xf21e size:0x0011 C-string:'UNK_0xf21e'
// 1832:      UNK_0xf231  codep:0x224c parp:0xf231 size:0x0012 C-string:'UNK_0xf231'
// 1833:      UNK_0xf245  codep:0x224c parp:0xf245 size:0x0034 C-string:'UNK_0xf245'
// 1834:      UNK_0xf27b  codep:0x224c parp:0xf27b size:0x001c C-string:'UNK_0xf27b'
// 1835:      UNK_0xf299  codep:0x224c parp:0xf299 size:0x0024 C-string:'UNK_0xf299'
// 1836:      UNK_0xf2bf  codep:0x224c parp:0xf2bf size:0x000e C-string:'UNK_0xf2bf'
// 1837:      UNK_0xf2cf  codep:0x224c parp:0xf2cf size:0x0011 C-string:'UNK_0xf2cf'
// 1838:        CLASS>DE  codep:0x4a4f parp:0xf2ed size:0x0024 C-string:'CLASS_gt_DE'
// 1839:        CLASS>TY  codep:0x4a4f parp:0xf31e size:0x0024 C-string:'CLASS_gt_TY'
// 1840:           ?TALK  codep:0x224c parp:0xf34c size:0x0000 C-string:'_ask_TALK'

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char PROBABLE[2] = {0x03, 0x9c}; // PROBABLE
unsigned char UNK_0xe6c5[2] = {0x3a, 0x20}; // UNK_0xe6c5
unsigned char UNK_0xe6c9[401] = {}; // UNK_0xe6c9
unsigned char SHOW[2] = {0x00, 0x00}; // SHOW
unsigned char UNK_0xea63[35] = {0x00, 0x00, 0x4c, 0x22, 0x4f, 0x06, 0x93, 0x1f, 0x93, 0x1f, 0x93, 0x1f, 0x93, 0x1f, 0xfa, 0x1b, 0xe8, 0xd3, 0x2b, 0x61, 0xea, 0xae, 0x0b, 0xfa, 0x15, 0x06, 0x00, 0x7b, 0x3b, 0x72, 0x0f, 0x38, 0x0c, 0x90, 0x16}; // UNK_0xea63
unsigned char UNK_0xee55[16] = {0x00, 0xe5, 0x08, 0xe5, 0xc8, 0xe5, 0xd0, 0xe5, 0xd8, 0xe5, 0xe0, 0xe5, 0xe8, 0xe5, 0xf0, 0xe5}; // UNK_0xee55
unsigned char UNK_0xee67[16] = {0x10, 0xe5, 0x18, 0xe5, 0x98, 0xe5, 0xa0, 0xe5, 0xa8, 0xe5, 0xb0, 0xe5, 0xb8, 0xe5, 0xc0, 0xe5}; // UNK_0xee67
unsigned char UNK_0xee79[16] = {0x30, 0xe5, 0x38, 0xe5, 0x28, 0xe6, 0x30, 0xe6, 0x38, 0xe6, 0x40, 0xe6, 0x48, 0xe6, 0x50, 0xe6}; // UNK_0xee79
unsigned char UNK_0xee8b[16] = {0x20, 0xe5, 0x28, 0xe5, 0xf8, 0xe5, 0x00, 0xe6, 0x08, 0xe6, 0x10, 0xe6, 0x18, 0xe6, 0x20, 0xe6}; // UNK_0xee8b
unsigned char UNK_0xee9d[2] = {0x3a, 0x20}; // UNK_0xee9d
unsigned char UNK_0xeea9[2] = {0x3a, 0x20}; // UNK_0xeea9



// 0xe2c2: db 0x10 0x01 0x14 0x22 0x20 0x00 0x14 0x22 0x18 0x00 0x14 0x22 0x43 0x00 0x14 0x22 0x44 0x00 0x14 0x22 0x1c 0x00 0x14 0x22 0x39 0x00 0x4c 0x22 0xf0 0x0d 0x12 0x7e 0x90 0x16 '   "   "   "C  "D  "   "9 L"   ~  '

// ================================================
// 0xe2e4: WORD 'UNK_0xe2e6' codep=0x224c parp=0xe2e6
// ================================================

void UNK_0xe2e6() // UNK_0xe2e6
{
  Push(0x549d); // pointer to RECORD#
  Push(Read16(Pop())); // @
  Push(0x548f); // pointer to FILE#
  Push(Read16(Pop())); // @
}


// ================================================
// 0xe2f0: WORD 'UNK_0xe2f2' codep=0x224c parp=0xe2f2
// ================================================

void UNK_0xe2f2() // UNK_0xe2f2
{
  Push(0x548f); // pointer to FILE#
  _ex__3(); // !_3
  Push(0x549d); // pointer to RECORD#
  _ex__3(); // !_3
}


// ================================================
// 0xe2fc: WORD 'UNK_0xe2fe' codep=0x224c parp=0xe2fe
// ================================================
// 0xe2fe: db 0xed 0x22 0x73 0x3b 0x8c 0x21 0x92 0x6d 0x90 0x16 0x4c 0x22 0x4f 0x06 0x20 0x0f 0xfc 0xe2 0xfa 0x1b ' "s; ! m  L"O       '
// 0xe312: call   1649
  SWAP(); // SWAP
  _3_star_(); // 3*
  Push(Pop() + Pop()); // +
}


// ================================================
// 0xe31d: WORD 'CARDS' codep=0xe312 parp=0xe327
// ================================================
// 0xe327: db 0x13 0x00 0x00 0xbb 0xc3 0x01 0xd0 0xc3 0x01 0xe2 0xc3 0x01 0x00 0xc4 0x01 0x12 0xc4 0x01 0x29 0xc4 0x01 0x40 0xc4 0x01 0x5c 0xc4 0x01 0x70 0xc4 0x01 0x85 0xc4 0x01 0x9b 0xc4 0x01 0xb1 0xc4 0x01 0xc8 0xc4 0x01 0xdd 0xc4 0x01 0xf3 0xc4 0x01 0x09 0xc5 0x01 0x20 0xc5 0x01 0x36 0xc5 0x01 0x51 0xc5 0x01 '                  )  @  \  p                          6  Q  '

// ================================================
// 0xe363: WORD 'NICHE-SE' codep=0xe312 parp=0xe370
// ================================================
// 0xe370: db 0x0e 0x00 0x00 0x61 0xc7 0x01 0x7d 0xc7 0x01 0x00 0x00 0x00 0x92 0xc7 0x01 0x00 0x00 0x00 0xa8 0xc7 0x01 0x00 0x00 0x00 0xc9 0xc7 0x01 0x00 0x00 0x00 0xdf 0xc7 0x01 0x00 0x00 0x00 0x00 0xc8 0x01 0x00 0x00 0x00 0x15 0xc8 0x01 '   a  }                                      '

// ================================================
// 0xe39d: WORD 'UNK_0xe39f' codep=0x224c parp=0xe39f
// ================================================

void UNK_0xe39f() // UNK_0xe39f
{
  UNK_0xe2e6(); // UNK_0xe2e6
  _gt_R(); // >R
  _gt_R(); // >R
  _2DUP(); // 2DUP
  _gt_C_plus_S(); // >C+S
  Push(0x63fa); // IFIELD(PHR-CNT)
  Push(Read8(Pop())&0xFF); // C@
  Push(Pop()-1); // 1-
  Push(0x63fb); // IFIELD(PHRASE-MEM)
  Push(Pop() + Pop()); // +
  Push(Read8(Pop())&0xFF); // C@
  Push(0x0053);
  _eq_(); // =
  _at_INST_dash_SPECIES(); // @INST-SPECIES
  Push(cc__4); // 4
  _eq_(); // =
  Push(Pop() | Pop()); // OR
  ICLOSE(); // ICLOSE
  R_gt_(); // R>
  R_gt_(); // R>
  UNK_0xe2f2(); // UNK_0xe2f2
}


// ================================================
// 0xe3cd: WORD 'UNK_0xe3cf' codep=0x224c parp=0xe3cf
// ================================================

void UNK_0xe3cf() // UNK_0xe3cf
{
  UNK_0xe2e6(); // UNK_0xe2e6
  _gt_R(); // >R
  _gt_R(); // >R
  _2DUP(); // 2DUP
  _gt_C_plus_S(); // >C+S
  _at_INST_dash_SPECIES(); // @INST-SPECIES
  Push(cc__5); // 5
  _eq_(); // =
  ICLOSE(); // ICLOSE
  R_gt_(); // R>
  R_gt_(); // R>
  UNK_0xe2f2(); // UNK_0xe2f2
}

// 0xe3e9: db 0x4c 0x22 0xe4 0xe2 0xb4 0x0d 0xb4 0x0d 0xf0 0x0d 0xf2 0x79 0x30 0x75 0x73 0x3b 0x5f 0x12 0xc9 0x79 0x90 0x0e 0x90 0x0e 0xf0 0xe2 0x90 0x16 0x4c 0x22 0x20 0x0f 0x5d 0x17 0x00 0x01 0xd8 0x4a 0x90 0x16 'L"         y0us;_  y        L"  ]    J  '

// ================================================
// 0xe411: WORD 'UNK_0xe413' codep=0x224c parp=0xe413
// ================================================

void UNK_0xe413() // UNK_0xe413
{
  Push(0); // 0
  Push(2); // 2
  RRND(); // RRND
}


// ================================================
// 0xe41b: WORD 'UNK_0xe41d' codep=0x224c parp=0xe41d
// ================================================

void UNK_0xe41d() // UNK_0xe41d
{
  Push(1); // 1
  Push(0x0064);
  RRND(); // RRND
}

// 0xe427: db 0x4c 0x22 0xad 0x11 0x89 0x11 0x90 0x16 0x14 0x22 0x2e 0x00 'L"       ". '

// ================================================
// 0xe433: WORD 'UNK_0xe435' codep=0x224c parp=0xe435
// ================================================

void UNK_0xe435() // UNK_0xe435
{
  Push(0xe431);
  _ex__3(); // !_3
  Push(0xe431);
  CFA(); // CFA
  Push(0x2790);
  Push(0x000e);
  Push(Pop() + Pop()); // +
  _ex__3(); // !_3
  _dash_TRAILING(); // -TRAILING
  Push(0x099a);
  CFA(); // CFA
  Push(0x2790);
  Push(0x000e);
  Push(Pop() + Pop()); // +
  _ex__3(); // !_3
}


// ================================================
// 0xe463: WORD 'PROBABLE' codep=0x1d29 parp=0xe470
// ================================================
// 0xe470: db 0x03 0x9c '  '

// ================================================
// 0xe472: WORD 'UNK_0xe474' codep=0x224c parp=0xe474
// ================================================

void UNK_0xe474() // UNK_0xe474
{
  Push(0xe470); // pointer to PROBABLE
  _ex__3(); // !_3
}


// ================================================
// 0xe47a: WORD 'UNK_0xe47c' codep=0x224c parp=0xe47c
// ================================================

void UNK_0xe47c() // UNK_0xe47c
{
  UNK_0xe41d(); // UNK_0xe41d
  Push(0xe470); // pointer to PROBABLE
  Push(Read16(Pop())); // @
  _st_(); // <
}

// 0xe486: db 0x4c 0x22 0x4f 0x06 0x20 0x0f 0x93 0x1f 0xfa 0x1b 0xe8 0xb6 0x31 0x7f 0x0e 0x72 0x0f 0x72 0x0f 0xae 0x0b 0x90 0x16 0xea 0x73 0x18 0x00 0x03 0x00 0x00 0x00 'L"O         1 r r      s      '

// ================================================
// 0xe4a5: WORD 'HITS' codep=0x744d parp=0xe4ae
// ================================================
// 0xe4ae: db 0x44 0x11 0x01 0x4d 0x74 0x43 0x0b 0x02 'D  MtC  '

// ================================================
// 0xe4b6: WORD 'UNK_0xe4b8' codep=0x73ea parp=0xe4b8
// ================================================
// 0xe4b8: db 0x1a 0x00 0x10 0x17 0x0a 0x65 0xea 0x73 0x1c 0x1d 0x01 0x1e 0x2c 0x65 0xea 0x73 0x1c 0x18 0x01 0x1e 0x2c 0x65 '     e s    ,e s    ,e'

// ================================================
// 0xe4ce: WORD 'UNK_0xe4d0' codep=0x73ea parp=0xe4d0
// ================================================
// 0xe4d0: db 0x44 0x07 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x0b 0x01 0x9c 0xd8 0x69 'D    i sD    i'

// ================================================
// 0xe4de: WORD 'UNK_0xe4e0' codep=0x73ea parp=0xe4e0
// ================================================
// 0xe4e0: db 0x44 0x0c 0x01 0x9c 0xd8 0x69 0xea 0x73 0x44 0x0d 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x10 0x03 0x9c 0xd8 0x69 'D    i sD    i sD    i'

// ================================================
// 0xe4f6: WORD 'UNK_0xe4f8' codep=0x73ea parp=0xe4f8
// ================================================
// 0xe4f8: db 0x44 0x13 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x16 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x19 0x01 0x9c 0xd8 0x69 0xea 0x73 0x44 0x1a 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x1d 0x01 0x9c 0xd8 0x69 0xea 0x73 0x44 0x1e 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x21 0x01 0x9c 0xd8 0x69 0xea 0x73 0x44 0x22 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x25 0x01 0x9c 0xd8 0x69 'D    i sD    i sD    i sD    i sD    i sD    i sD!   i sD"   i sD%   i'

// ================================================
// 0xe53e: WORD 'UNK_0xe540' codep=0x73ea parp=0xe540
// ================================================
// 0xe540: db 0x44 0x26 0x03 0x9c 0xd8 0x69 'D&   i'

// ================================================
// 0xe546: WORD 'UNK_0xe548' codep=0x73ea parp=0xe548
// ================================================
// 0xe548: db 0x44 0x29 0x03 0x9c 0xd8 0x69 0xea 0x73 0x44 0x30 0x01 0x9c 0xd8 0x69 0xea 0x73 0x44 0x31 0x01 0x9c 0xd8 0x69 0xea 0x73 0x44 0x32 0x01 0x9c 0xd8 0x69 'D)   i sD0   i sD1   i sD2   i'

// ================================================
// 0xe566: WORD 'UNK_0xe568' codep=0x73ea parp=0xe568
// ================================================
// 0xe568: db 0x44 0x33 0x01 0x9c 0xd8 0x69 'D3   i'

// ================================================
// 0xe56e: WORD 'UNK_0xe570' codep=0x73ea parp=0xe570
// ================================================
// 0xe570: db 0x44 0x36 0x01 0x9c 0xd8 0x69 'D6   i'

// ================================================
// 0xe576: WORD 'UNK_0xe578' codep=0x73ea parp=0xe578
// ================================================
// 0xe578: db 0x44 0x37 0x01 0x9c 0xd8 0x69 'D7   i'

// ================================================
// 0xe57e: WORD 'UNK_0xe580' codep=0x73ea parp=0xe580
// ================================================
// 0xe580: db 0x44 0x38 0x02 0x9c 0xd8 0x69 0xea 0x73 0x44 0x3a 0x02 0x9c 0xd8 0x69 'D8   i sD:   i'

// ================================================
// 0xe58e: WORD 'UNK_0xe590' codep=0x73ea parp=0xe590
// ================================================
// 0xe590: db  'D<   i sD?   i sDB   i sDE   i sDH   i sDK   i sDN   i sDQ   i sDT   i sDW   i sDZ   i sD]   i sD`   i sDc   i sDf   i sDi   i sDl   i sDo   i sDr   i sDu   i sDx   i sD{   i sD~   i sD    i sD    i sD    i sD    i sD    i sD    i'

// ================================================
// 0xe676: WORD 'UNK_0xe678' codep=0x73ea parp=0xe678
// ================================================
// 0xe678: db 0x44 0x95 0x03 0x9c 0xd8 0x69 'D    i'

// ================================================
// 0xe67e: WORD 'UNK_0xe680' codep=0x744d parp=0xe680
// ================================================
// 0xe680: db 0x44 0x12 0x01 'D  '

// ================================================
// 0xe683: WORD 'UNK_0xe685' codep=0x744d parp=0xe685
// ================================================
// 0xe685: db 0x44 0x15 0x01 'D  '

// ================================================
// 0xe688: WORD 'UNK_0xe68a' codep=0x744d parp=0xe68a
// ================================================
// 0xe68a: db 0x44 0x1b 0x01 'D  '

// ================================================
// 0xe68d: WORD 'UNK_0xe68f' codep=0x224c parp=0xe68f
// ================================================

void UNK_0xe68f() // UNK_0xe68f
{
  Push(Read16(regsp)); // DUP
  _gt_R(); // >R
  PICK(); // PICK
  R_gt_(); // R>
  PICK(); // PICK
}


// ================================================
// 0xe69b: WORD 'UNK_0xe69d' codep=0x224c parp=0xe69d
// ================================================

void UNK_0xe69d() // UNK_0xe69d
{
  _2DUP(); // 2DUP
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) return;
  _2DUP(); // 2DUP
}

// 0xe6a9: db 0x4c 0x22 0xd0 0xe2 0x41 0x0e 0x8d 0x54 0x76 0x6d 0x9b 0x54 0xae 0x0b 0x99 0x73 0x32 0x0e 0x90 0x16 'L"  A  Tvm T   s2   '

// ================================================
// 0xe6bd: WORD 'UNK_0xe6bf' codep=0x3b68 parp=0xe6bf
// ================================================
// 0xe6bf: db 0x01 0x00 0x28 0xdb '  ( '

// ================================================
// 0xe6c3: WORD 'UNK_0xe6c5' codep=0x1d29 parp=0xe6c5
// ================================================
// 0xe6c5: db 0x3a 0x20 ': '

// ================================================
// 0xe6c7: WORD 'UNK_0xe6c9' codep=0x1d29 parp=0xe6c9
// ================================================
// 0xe6c9: db  ':  ALLOT   0   9   C   d    ,       .  #Z  ,n  5   =Z  F  1520  e   u                      !   $   '   *   -   0/  3   6   9   =   A   d        (          !Q  (   //  5n  ;   B   I=  d    Z   n      &   0Z  9   C   d                   !   (   /   5/  ;Z  B   I   dn   Y   s                      $   (   -   1|  5   :   ?/  CZ  d                   '   .   6   =Z  E   d               &   0   9   C   d '

// ================================================
// 0xe85a: WORD 'SHOW' codep=0x1d29 parp=0xe863
// ================================================
// 0xe863: db 0x00 0x00 '  '

// ================================================
// 0xe865: WORD 'UNK_0xe867' codep=0x224c parp=0xe867
// ================================================

void UNK_0xe867() // UNK_0xe867
{
  Push(0xe6c9); // pointer to UNK_0xe6c9
  Push(0x5d67); // pointer to PM-PTR
  _ex__3(); // !_3
  Push(0x5d5a); // pointer to LINE-COUNT
  Push(Read16(Pop())); // @
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(0x5d67); // pointer to PM-PTR
  Push(Read16(Pop())); // @
  Push(0xe6c5); // pointer to UNK_0xe6c5
  Push(Read16(Pop())); // @
  Push(0x5bf8); // pointer to WLINES
  Push(Read16(Pop())); // @
  Push(Pop()-1); // 1-
  _gt_(); // >
  if (Pop() == 0) goto label1;
  Push(0x03e8);
  MS(); // MS

  label1:
  Push(0xe863); // pointer to SHOW
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label2;
  COUNT(); // COUNT
  Exec(TYPE); // call of word 0x2690 '(TYPE)'
  goto label3;

  label2:
  WLINE_dash_UP(); // WLINE-UP

  label3:
  Push(cc_TEXTC_slash_L); // TEXTC/L
  Push(Pop()+1); // 1+
  Push(0x5d67); // pointer to PM-PTR
  _plus__ex_(); // +!
  Push(1); // 1
  Push(0xe6c5); // pointer to UNK_0xe6c5
  _plus__ex_(); // +!
  i++;
  } while(i<imax); // (LOOP) 0xffc4

}


// ================================================
// 0xe8b5: WORD 'UNK_0xe8b7' codep=0x224c parp=0xe8b7
// ================================================

void UNK_0xe8b7() // UNK_0xe8b7
{
  _2DUP(); // 2DUP
  _eq_(); // =
  _gt_R(); // >R
  _gt_(); // >
  R_gt_(); // R>
  Push(Pop() | Pop()); // OR
}


// ================================================
// 0xe8c5: WORD 'UNK_0xe8c7' codep=0x224c parp=0xe8c7
// ================================================

void UNK_0xe8c7() // UNK_0xe8c7
{
  Push(0x5d67); // pointer to PM-PTR
  Push(Read16(Pop())); // @
  Push(0xe6c9); // pointer to UNK_0xe6c9
  _dash_(); // -
  Push(cc_TEXTC_slash_L); // TEXTC/L
  Push(Pop()+1); // 1+
  _slash_(); // /
  Push(cc_TEXTC_slash_L); // TEXTC/L
  Push(Pop()+1); // 1+
  Push(Pop() * Pop()); // *
  Push(0xe6c9); // pointer to UNK_0xe6c9
  Push(Pop() + Pop()); // +
  Push(Read16(regsp)); // DUP
  Push(Read8(Pop())&0xFF); // C@
  Push(cc__3); // 3
  PICK(); // PICK
  Push(Read16(regsp)); // DUP
  Push(1); // 1
  _gt_(); // >
  if (Pop() == 0) goto label1;
  Push(Pop()+2); // 2+

  label1:
  Push(Pop() + Pop()); // +
  Push(cc_TEXTC_slash_L); // TEXTC/L
  UNK_0xe8b7(); // UNK_0xe8b7
  if (Pop() == 0) goto label2;
  Push(cc_TEXTC_slash_L); // TEXTC/L
  Push(Pop()+1); // 1+
  Push(Pop() + Pop()); // +
  Push(Read16(regsp)); // DUP
  Push(0); // 0
  SWAP(); // SWAP
  C_ex_(); // C!
  Push(Read16(regsp)); // DUP
  Push(Pop()+1); // 1+
  Push(0x5d67); // pointer to PM-PTR
  _ex__3(); // !_3
  Push(1); // 1
  Push(0x5d5a); // pointer to LINE-COUNT
  _plus__ex_(); // +!

  label2:
  Push(Read16(regsp)); // DUP
  Push(Read8(Pop())&0xFF); // C@
  Push(cc__3); // 3
  PICK(); // PICK
  Push(Pop() + Pop()); // +
  SWAP(); // SWAP
  C_ex_(); // C!
  Push(Read16(regsp)); // DUP
  _gt_R(); // >R
  Push(0x5d67); // pointer to PM-PTR
  Push(Read16(Pop())); // @
  SWAP(); // SWAP
  CMOVE(); // CMOVE
  R_gt_(); // R>
  Push(0x5d67); // pointer to PM-PTR
  _plus__ex_(); // +!
}


// ================================================
// 0xe93b: WORD 'UNK_0xe93d' codep=0x224c parp=0xe93d
// ================================================

void UNK_0xe93d() // UNK_0xe93d
{
  Push(Read16(regsp)); // DUP
  Push(Read16(Pop())); // @
  Push(Pop()+2); // 2+
  RANGE(); // RANGE
  Push(Pop()+2); // 2+
}

// 0xe949: db 0x4c 0x22 0x4f 0x06 0x8b 0x3b 0xb3 0x0f 0x41 0x0e 0x93 0x1f 0xed 0x22 0x7f 0x0e 0x8c 0x21 0xf2 0x0e 0x20 0x0f 0xfd 0x6d 0xfa 0x1b 'L"O  ;  A    "  !     m  '
// 0xe963: call   1649
  Push(0); // 0
  Push(cc__4); // 4
  ROLL(); // ROLL
  Push(cc__4); // 4
  ROLL(); // ROLL
  Push(cc__4); // 4
  ROLL(); // ROLL
  UNK_0xe93d(); // UNK_0xe93d

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  Push(i); // I
  _2_at_(); // 2@
  Push(cc__3); // 3
  PICK(); // PICK
  _eq_(); // =
  _gt_R(); // >R
  Push(cc__3); // 3
  PICK(); // PICK
  _eq_(); // =
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1;
  Push(i); // I
  Push(cc__4); // 4
  ROLL(); // ROLL
  Pop(); // DROP
  ROT(); // ROT
  ROT(); // ROT
  LEAVE(); // LEAVE

  label1:
  Push(cc__6); // 6
  int step = Pop();
  i += step;
  } while(((step>=0) && (i<imax)) || ((step<0) && (i>imax))); // (+LOOP) 0xffd4

  Pop(); Pop();// 2DROP
  if (Read16(regsp) != 0) Push(Read16(regsp)); // ?DUP
  if (Pop() == 0) goto label2;
  Push(cc__4); // 4
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  return;

  label2:
  PRINT("INDEX NOT IN PROBABILITY ARRAY! ", 32); // (.")
  UNRAVEL(); // UNRAVEL
  QUIT(); // QUIT
}


// ================================================
// 0xe9e1: WORD 'UNK_0xe9e3' codep=0x224c parp=0xe9e3
// ================================================

void UNK_0xe9e3() // UNK_0xe9e3
{

  label1:
  OVER(); // OVER
  OVER(); // OVER
  Push(Read8(Pop())&0xFF); // C@
  Push(Read16(regsp)); // DUP
  Push(0x0064);
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  _gt_R(); // >R
  _gt_(); // >
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) return;
  Push(Pop()+1); // 1+
  Push(cc__3); // 3
  Push(Pop() + Pop()); // +
  goto label1;
}


// ================================================
// 0xea0b: WORD 'UNK_0xea0d' codep=0x224c parp=0xea0d
// ================================================

void UNK_0xea0d() // UNK_0xea0d
{
  UNK_0xe9e3(); // UNK_0xe9e3
  Push(Pop()+1); // 1+
  _1_dot_5_at_(); // 1.5@
  ROT(); // ROT
  Pop(); // DROP
}


// ================================================
// 0xea19: WORD 'UNK_0xea1b' codep=0x224c parp=0xea1b
// ================================================

void UNK_0xea1b() // UNK_0xea1b
{
  Push(cc__4); // 4
  MOD(); // MOD
  if (Pop() == 0) goto label1;
  Push(0x5814); // pointer to PLANTS
  return;

  label1:
  Push(0x5822); // pointer to ANIMALS
}


// ================================================
// 0xea2d: WORD 'UNK_0xea2f' codep=0x3b68 parp=0xea2f
// ================================================
// 0xea2f: db 0x01 0x00 0xac 0xdc '    '

// ================================================
// 0xea33: WORD 'UNK_0xea35' codep=0x224c parp=0xea35
// ================================================

void UNK_0xea35() // UNK_0xea35
{
  Push(cc__dash_1); // -1
  Push(cc__dash_1); // -1
}


// ================================================
// 0xea3b: WORD 'UNK_0xea3d' codep=0x224c parp=0xea3d
// ================================================

void UNK_0xea3d() // UNK_0xea3d
{
  _2DUP(); // 2DUP
  UNK_0xea35(); // UNK_0xea35
  D_eq_(); // D=
}


// ================================================
// 0xea45: WORD 'UNK_0xea47' codep=0x224c parp=0xea47
// ================================================

void UNK_0xea47() // UNK_0xea47
{
  Push(2); // 2
  UNK_0xe474(); // UNK_0xe474
  UNK_0xe47c(); // UNK_0xe47c
  if (Pop() == 0) return;
  Push2Words("UNK_0xe6bf");
  Push(1); // 1
  Push(0x0014);
  RRND(); // RRND
  CARDS(); // CARDS
  _1_dot_5_at_(); // 1.5@
}


// ================================================
// 0xea61: WORD 'UNK_0xea63' codep=0x1d29 parp=0xea63
// ================================================
// 0xea63: db 0x00 0x00 '  '

// ================================================
// 0xea65: WORD 'UNK_0xea67' codep=0x224c parp=0xea67
// ================================================
// 0xea67: db 0x4f 0x06 0x93 0x1f 0x93 0x1f 0x93 0x1f 0x93 0x1f 0xfa 0x1b 'O           '
// 0xea73: call   1649
  Push(0xea63); // pointer to UNK_0xea63
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1;
  Push(cc__4); // 4
  Push(Pop() + Pop()); // +

  label1:
  _2_at_(); // 2@
}


// ================================================
// 0xea86: WORD 'VTB' codep=0xea73 parp=0xea8e
// ================================================
// 0xea8e: db 0x01 0x00 0x53 0xda 0x01 0x00 0x62 0xda '  S   b '

// ================================================
// 0xea96: WORD 'POSSESSI' codep=0xea73 parp=0xeaa3
// ================================================
// 0xeaa3: db 0x01 0x00 0xb2 0xda 0x01 0x00 0xa1 0xda '        '

// ================================================
// 0xeaab: WORD 'PRONOUN' codep=0xea73 parp=0xeab7
// ================================================
// 0xeab7: db 0x01 0x00 0x92 0xda 0x01 0x00 0x81 0xda '        '

// ================================================
// 0xeabf: WORD 'DETERMIN' codep=0xea73 parp=0xeacc
// ================================================
// 0xeacc: db 0x01 0x00 0xdf 0xda 0x01 0x00 0xed 0xda '        '

// ================================================
// 0xead4: WORD 'POSSES.P' codep=0xea73 parp=0xeae1
// ================================================
// 0xeae1: db 0x01 0x00 0x90 0xdb 0x01 0x00 0x7e 0xdb '      ~ '

// ================================================
// 0xeae9: WORD 'UNK_0xeaeb' codep=0x224c parp=0xeaeb
// ================================================

void UNK_0xeaeb() // UNK_0xeaeb
{
  Push(Read16(regsp)); // DUP
  Push(Pop()-1); // 1-
  Push(0); // 0
  MAX(); // MAX
  Push(0xea63); // pointer to UNK_0xea63
  _ex__3(); // !_3
}


// ================================================
// 0xeaf9: WORD 'UNK_0xeafb' codep=0x224c parp=0xeafb
// ================================================

void UNK_0xeafb() // UNK_0xeafb
{
  _2DUP(); // 2DUP
  Push2Words("0.");
  D_eq_(); // D=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  _2DUP(); // 2DUP
  _gt_C_plus_S(); // >C+S
  Push(0x63fb); // IFIELD(PHRASE-MEM)
  Push(Read8(Pop())&0xFF); // C@
  _gt_R(); // >R
  R_at_(); // R@
  Push(0x0041);
  _eq_(); // =
  R_at_(); // R@
  Push(0x0045);
  _eq_(); // =
  R_at_(); // R@
  Push(0x0049);
  _eq_(); // =
  R_at_(); // R@
  Push(0x004f);
  _eq_(); // =
  R_gt_(); // R>
  Push(0x0055);
  _eq_(); // =
  Push(Pop() | Pop()); // OR
  Push(Pop() | Pop()); // OR
  Push(Pop() | Pop()); // OR
  Push(Pop() | Pop()); // OR
  ICLOSE(); // ICLOSE
  return;

  label1:
  Push(0); // 0
}


// ================================================
// 0xeb4b: WORD 'UNK_0xeb4d' codep=0x224c parp=0xeb4d
// ================================================

void UNK_0xeb4d() // UNK_0xeb4d
{
  UNK_0xe413(); // UNK_0xe413
  if (Pop() == 0) return;
  Push2Words("UNK_0xe6bf");
  Push(0xdcba); Push(0x0001);
}


// ================================================
// 0xeb5d: WORD 'UNK_0xeb5f' codep=0x224c parp=0xeb5f
// ================================================

void UNK_0xeb5f() // UNK_0xeb5f
{
  UNK_0xe69d(); // UNK_0xe69d
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) return;
  Push2Words("UNK_0xe6bf");
  _2SWAP(); // 2SWAP
}


// ================================================
// 0xeb6d: WORD 'UNK_0xeb6f' codep=0x224c parp=0xeb6f
// ================================================

void UNK_0xeb6f() // UNK_0xeb6f
{
  _2DUP(); // 2DUP
  Push2Words("UNK_0xea2f");
  D_eq_(); // D=
}


// ================================================
// 0xeb77: WORD 'UNK_0xeb79' codep=0x224c parp=0xeb79
// ================================================

void UNK_0xeb79() // UNK_0xeb79
{
  UNK_0xeb6f(); // UNK_0xeb6f
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) return;
  Push(0xdbc3); Push(0x0001);
}


// ================================================
// 0xeb89: WORD 'UNK_0xeb8b' codep=0x224c parp=0xeb8b
// ================================================

void UNK_0xeb8b() // UNK_0xeb8b
{
  UNK_0xe3cf(); // UNK_0xe3cf
  if (Pop() == 0) Push(1); else Push(0); // 0=
  _gt_R(); // >R
  UNK_0xe39f(); // UNK_0xe39f
  if (Pop() == 0) Push(1); else Push(0); // 0=
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) return;
  Push(0xea63); // pointer to UNK_0xea63
  Push(Read16(Pop())); // @
  if (Pop() == 0) return;
  Push(0xdac2); Push(0x0001);
  _2SWAP(); // 2SWAP
}


// ================================================
// 0xebaf: WORD 'UNK_0xebb1' codep=0x224c parp=0xebb1
// ================================================

void UNK_0xebb1() // UNK_0xebb1
{
  UNK_0xeafb(); // UNK_0xeafb
  if (Pop() == 0) goto label1;
  Push(0xdad0); Push(0x0001);
  goto label2;

  label1:
  Push(0xdadf); Push(0x0001);

  label2:
  Push2Words("UNK_0xe6bf");
  _2SWAP(); // 2SWAP
}


// ================================================
// 0xebcd: WORD 'UNK_0xebcf' codep=0x224c parp=0xebcf
// ================================================

void UNK_0xebcf() // UNK_0xebcf
{
  _gt_C_plus_S(); // >C+S
  Push(0x63fb); // IFIELD(PHRASE-MEM)
  Push(0x63fa); // IFIELD(PHR-CNT)
  Push(Read8(Pop())&0xFF); // C@
}


// ================================================
// 0xebd9: WORD 'UNK_0xebdb' codep=0x224c parp=0xebdb
// ================================================

void UNK_0xebdb() // UNK_0xebdb
{
  UNK_0xeb6f(); // UNK_0xeb6f
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) return;

  label7:
  Push2Words("UNK_0xea2f");
  _2OVER(); // 2OVER
  D_dash_(); // D-
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1;
  UNK_0xea3d(); // UNK_0xea3d
  if (Pop() == 0) goto label2;
  Pop(); Pop();// 2DROP
  U_gt__do_(); // U>$
  UNK_0xe8c7(); // UNK_0xe8c7
  goto label3;

  label2:
  CI(); // CI
  Push2Words("UNK_0xe6bf");
  D_eq_(); // D=
  if (Pop() == 0) goto label4;
  Push(0x5d67); // pointer to PM-PTR
  Push(Read16(Pop())); // @
  Push(Read8(Pop())&0xFF); // C@
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label5;
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  goto label6;

  label5:
  Pop(); Pop();// 2DROP

  label6:
  goto label3;

  label4:
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE

  label3:
  goto label7;

  label1:
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
}


// ================================================
// 0xec39: WORD 'UNK_0xec3b' codep=0x224c parp=0xec3b
// ================================================

void UNK_0xec3b() // UNK_0xec3b
{
  Push(1); // 1
  Push(0x5d5a); // pointer to LINE-COUNT
  _ex__3(); // !_3
  Push(0); // 0
  Push(0xe6c9); // pointer to UNK_0xe6c9
  Push(Read16(regsp)); // DUP
  Push(Pop()+1); // 1+
  Push(0x5d67); // pointer to PM-PTR
  _ex__3(); // !_3
  C_ex_(); // C!
}


// ================================================
// 0xec51: WORD 'UNK_0xec53' codep=0x224c parp=0xec53
// ================================================

void UNK_0xec53() // UNK_0xec53
{
  Push(cc_TEXTC_slash_L); // TEXTC/L
  Push(Read16(regsp)); // DUP
  _gt_R(); // >R
  Push(0xe6c9); // pointer to UNK_0xe6c9
  Push(Read16(regsp)); // DUP
  _gt_R(); // >R
  C_ex_(); // C!
  R_gt_(); // R>
  Push(Pop()+1); // 1+
  R_gt_(); // R>
  Push(cc_BL); // BL
  FILL_1(); // FILL_1
  Push(1); // 1
  Push(0x5d5a); // pointer to LINE-COUNT
  _ex__3(); // !_3
  Push(0xe6c9); // pointer to UNK_0xe6c9
  WLINE_dash_UP(); // WLINE-UP
}


// ================================================
// 0xec77: WORD 'UNK_0xec79' codep=0x224c parp=0xec79
// ================================================

void UNK_0xec79() // UNK_0xec79
{
  _2DUP(); // 2DUP
  UNK_0xeb6f(); // UNK_0xeb6f
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1;
  UNK_0xec3b(); // UNK_0xec3b
  Pop(); Pop();// 2DROP
  UNK_0xe2e6(); // UNK_0xe2e6
  _gt_R(); // >R
  _gt_R(); // >R
  UNK_0xebdb(); // UNK_0xebdb
  UNK_0xe867(); // UNK_0xe867
  R_gt_(); // R>
  R_gt_(); // R>
  UNK_0xe2f2(); // UNK_0xe2f2
  return;

  label1:
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
}


// ================================================
// 0xeca1: WORD 'UNK_0xeca3' codep=0x224c parp=0xeca3
// ================================================

void UNK_0xeca3() // UNK_0xeca3
{
  LoadData("UNK_0xe4e0"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  UNK_0xea1b(); // UNK_0xea1b
  Push(0x5814); // pointer to PLANTS
  _eq_(); // =
  if (Pop() == 0) goto label1;
  Push(0xdb13); Push(0x0001);
  return;

  label1:
  Push(0xdafe); Push(0x0001);
}


// ================================================
// 0xecc3: WORD 'UNK_0xecc5' codep=0x224c parp=0xecc5
// ================================================

void UNK_0xecc5() // UNK_0xecc5
{
  LoadData("RESEMBLES"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  UNK_0xe69d(); // UNK_0xe69d
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) goto label1;
  LoadData("SHAPE"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  _2DUP(); // 2DUP
  Push(0xdddc); Push(0x0001);
  D_eq_(); // D=
  _gt_R(); // >R
  LoadData("UNK_0xe4e0"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  UNK_0xea1b(); // UNK_0xea1b
  Push(0x5814); // pointer to PLANTS
  _eq_(); // =
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1;
  Pop(); Pop();// 2DROP

  label1:
  Push2Words("UNK_0xe6bf");
}


// ================================================
// 0xecfb: WORD 'UNK_0xecfd' codep=0x224c parp=0xecfd
// ================================================

void UNK_0xecfd() // UNK_0xecfd
{
  Push2Words("UNK_0xe6bf");
  Push(0xdcf4); Push(0x0001);
  Push2Words("UNK_0xe6bf");
  UNK_0xeb79(); // UNK_0xeb79
  UNK_0xeca3(); // UNK_0xeca3
  Push2Words("UNK_0xe6bf");
  UNK_0xecc5(); // UNK_0xecc5
  Push(0x0028);
  UNK_0xe474(); // UNK_0xe474
  UNK_0xe47c(); // UNK_0xe47c
  if (Pop() == 0) goto label1;
  LoadData("UNK_0xe590"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  Push2Words("UNK_0xe6bf");

  label1:
  LoadData("UNK_0xe678"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  UNK_0xebb1(); // UNK_0xebb1
}


// ================================================
// 0xed29: WORD 'UNK_0xed2b' codep=0x224c parp=0xed2b
// ================================================

void UNK_0xed2b() // UNK_0xed2b
{
  UNK_0xe47c(); // UNK_0xe47c
  if (Pop() == 0) goto label1;
  ROT(); // ROT
  _gt_R(); // >R
  SWAP(); // SWAP
  _gt_R(); // >R
  _gt_R(); // >R
  Push2Words("UNK_0xe6bf");
  R_gt_(); // R>
  IFLD_at_(); // IFLD@
  R_gt_(); // R>
  IFLD_at_(); // IFLD@
  UNK_0xeb5f(); // UNK_0xeb5f
  R_gt_(); // R>
  IFLD_at_(); // IFLD@
  UNK_0xeb5f(); // UNK_0xeb5f
  return;

  label1:
  Pop(); // DROP
  Pop(); Pop();// 2DROP
}


// ================================================
// 0xed57: WORD 'UNK_0xed59' codep=0x224c parp=0xed59
// ================================================

void UNK_0xed59() // UNK_0xed59
{
  Push(1); // 1
  OVER(); // OVER
  _eq_(); // =
  if (Pop() == 0) goto label1;
  Pop(); // DROP
  UNK_0xebb1(); // UNK_0xebb1
  return;

  label1:
  Push(0x00ff);
  OVER(); // OVER
  _eq_(); // =
  if (Pop() == 0) goto label2;
  Pop(); // DROP
  UNK_0xeb4d(); // UNK_0xeb4d
  return;

  label2:
  _gt_R(); // >R
  R_at_(); // R@
  if (Pop() == 0) goto label3;
  Push2Words("UNK_0xe6bf");
  R_at_(); // R@
  UNK_0xea35(); // UNK_0xea35

  label3:
  R_gt_(); // R>
  Pop(); // DROP
}


// ================================================
// 0xed93: WORD 'UNK_0xed95' codep=0x224c parp=0xed95
// ================================================

void UNK_0xed95() // UNK_0xed95
{
  Push2Words("UNK_0xea2f");
  LoadData("UNK_0xe4d0"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  _2DUP(); // 2DUP
  _gt_R(); // >R
  _gt_R(); // >R
  Push(0xe668);
  Push(0xe660);
  Push(0xe658);
  Push(0x003c);
  UNK_0xe474(); // UNK_0xe474
  UNK_0xed2b(); // UNK_0xed2b
  LoadData("UNK_0xe4f8"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  UNK_0xeb5f(); // UNK_0xeb5f
  R_gt_(); // R>
  R_gt_(); // R>
  UNK_0xe39f(); // UNK_0xe39f
  if (Pop() == 0) Push(1); else Push(0); // 0=
  _gt_R(); // >R
  UNK_0xe3cf(); // UNK_0xe3cf
  if (Pop() == 0) Push(1); else Push(0); // 0=
  _gt_R(); // >R
  Push(0xdfb1); Push(0x0001);
  D_eq_(); // D=
  if (Pop() == 0) Push(1); else Push(0); // NOT
  R_gt_(); // R>
  R_gt_(); // R>
  Push(Pop() & Pop()); // AND
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label1;
  UNK_0xebb1(); // UNK_0xebb1

  label1:
  Push(0); // 0
  Push(0xea63); // pointer to UNK_0xea63
  _ex__3(); // !_3
}


// ================================================
// 0xedeb: WORD 'UNK_0xeded' codep=0x224c parp=0xeded
// ================================================

void UNK_0xeded() // UNK_0xeded
{
  Push2Words("UNK_0xea2f");
  Push(0x6400); // IFIELD(HITS)
  Push(Read8(Pop())&0xFF); // C@
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label1;
  Push(0xcaa8); Push(0x0001);
  goto label2;

  label1:
  Push(0x6401); // IFIELD(UNK_0xe680)
  Push(Read8(Pop())&0xFF); // C@
  LoadData("UNK_0xe568"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  _st_(); // <
  if (Pop() == 0) goto label3;
  Push(0xe262); Push(0x0001);
  goto label2;

  label3:
  Push(0x640a); // IFIELD(UNK_0xe68a)
  Push(Read8(Pop())&0xFF); // C@
  Push(1); // 1
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label4;
  Push(0xe24c); Push(0x0001);
  goto label2;

  label4:
  Push(0x6404); // IFIELD(UNK_0xe685)
  Push(Read8(Pop())&0xFF); // C@
  if (Pop() == 0) goto label5;
  LoadData("UNK_0xe548"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@
  goto label2;

  label5:
  LoadData("UNK_0xe540"); // from 'CREATURE    '
  _1_dot_5_at_(); // 1.5@

  label2:
  Push2Words("UNK_0xe6bf");
  Push(0xdd05); Push(0x0001);
  Push2Words("UNK_0xe6bf");
  PRONOUN(); // PRONOUN
}


// ================================================
// 0xee53: WORD 'UNK_0xee55' codep=0x1d29 parp=0xee55
// ================================================
// 0xee55: db 0x00 0xe5 0x08 0xe5 0xc8 0xe5 0xd0 0xe5 0xd8 0xe5 0xe0 0xe5 0xe8 0xe5 0xf0 0xe5 '                '

// ================================================
// 0xee65: WORD 'UNK_0xee67' codep=0x1d29 parp=0xee67
// ================================================
// 0xee67: db 0x10 0xe5 0x18 0xe5 0x98 0xe5 0xa0 0xe5 0xa8 0xe5 0xb0 0xe5 0xb8 0xe5 0xc0 0xe5 '                '

// ================================================
// 0xee77: WORD 'UNK_0xee79' codep=0x1d29 parp=0xee79
// ================================================
// 0xee79: db 0x30 0xe5 0x38 0xe5 0x28 0xe6 0x30 0xe6 0x38 0xe6 0x40 0xe6 0x48 0xe6 0x50 0xe6 '0 8 ( 0 8 @ H P '

// ================================================
// 0xee89: WORD 'UNK_0xee8b' codep=0x1d29 parp=0xee8b
// ================================================
// 0xee8b: db 0x20 0xe5 0x28 0xe5 0xf8 0xe5 0x00 0xe6 0x08 0xe6 0x10 0xe6 0x18 0xe6 0x20 0xe6 '  (             '

// ================================================
// 0xee9b: WORD 'UNK_0xee9d' codep=0x1d29 parp=0xee9d
// ================================================
// 0xee9d: db 0x3a 0x20 ': '

// ================================================
// 0xee9f: WORD 'UNK_0xeea1' codep=0x224c parp=0xeea1
// ================================================

void UNK_0xeea1() // UNK_0xeea1
{
  Push(0xee9d); // pointer to UNK_0xee9d
  Push(Read16(Pop())); // @
}


// ================================================
// 0xeea7: WORD 'UNK_0xeea9' codep=0x1d29 parp=0xeea9
// ================================================
// 0xeea9: db 0x3a 0x20 ': '

// ================================================
// 0xeeab: WORD 'UNK_0xeead' codep=0x224c parp=0xeead
// ================================================

void UNK_0xeead() // UNK_0xeead
{
  Push(0xeea9); // pointer to UNK_0xeea9
  Push(Read16(Pop())); // @
  if (Pop() == 0) goto label1;
  Push(0xdb5b); Push(0x0001);
  Push2Words("UNK_0xe6bf");
  goto label2;

  label1:
  UNK_0xeb79(); // UNK_0xeb79

  label2:
  Push(0xeea9); // pointer to UNK_0xeea9
  OFF(); // OFF
}


// ================================================
// 0xeec9: WORD 'UNK_0xeecb' codep=0x224c parp=0xeecb
// ================================================

void UNK_0xeecb() // UNK_0xeecb
{
  UNK_0xeea1(); // UNK_0xeea1
  Push(Read16(Pop())); // @
  IFLD_at_(); // IFLD@
  UNK_0xe69d(); // UNK_0xe69d
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) goto label1;
  _2OVER(); // 2OVER
  UNK_0xeb6f(); // UNK_0xeb6f
  _gt_R(); // >R
  Pop(); Pop();// 2DROP
  R_gt_(); // R>
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label2;
  _gt_R(); // >R
  _gt_R(); // >R
  Push2Words("UNK_0xe6bf");
  UNK_0xeead(); // UNK_0xeead
  R_gt_(); // R>
  R_gt_(); // R>
  goto label3;

  label2:
  Push(0xeea9); // pointer to UNK_0xeea9
  ON_3(); // ON_3

  label3:
  UNK_0xeea1(); // UNK_0xeea1
  Push(2); // 2
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  IFLD_at_(); // IFLD@
  UNK_0xeaeb(); // UNK_0xeaeb
  Pop(); // DROP
  UNK_0xeb8b(); // UNK_0xeb8b
  UNK_0xeea1(); // UNK_0xeea1
  _gt_R(); // >R
  R_at_(); // R@
  Push(cc__8); // 8
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  R_at_(); // R@
  Push(cc__6); // 6
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  R_gt_(); // R>
  Push(cc__4); // 4
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  Push(0x0032);
  UNK_0xe474(); // UNK_0xe474
  UNK_0xed2b(); // UNK_0xed2b
  UNK_0xea47(); // UNK_0xea47
  UNK_0xeea1(); // UNK_0xeea1
  Push(0x000a);
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  IFLD_at_(); // IFLD@
  UNK_0xe69d(); // UNK_0xe69d
  Push(Pop() | Pop()); // OR
  if (Pop() == 0) Push(1); else Push(0); // 0=
  if (Pop() == 0) goto label4;
  UNK_0xe413(); // UNK_0xe413
  if (Pop() == 0) goto label5;
  UNK_0xeea1(); // UNK_0xeea1
  Push(0x000c);
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  IFLD_at_(); // IFLD@
  UNK_0xeb5f(); // UNK_0xeb5f

  label5:
  goto label6;

  label4:
  UNK_0xeb5f(); // UNK_0xeb5f

  label6:
  UNK_0xeea1(); // UNK_0xeea1
  Push(0x000e);
  Push(Pop() + Pop()); // +
  Push(Read16(Pop())); // @
  IFLD_at_(); // IFLD@
  UNK_0xeb5f(); // UNK_0xeb5f
  UNK_0xeea1(); // UNK_0xeea1
  Push(Pop()+2); // 2+
  Push(Read16(Pop())); // @
  IFLD_at_(); // IFLD@
  UNK_0xed59(); // UNK_0xed59

  label1:
  Push(0xea63); // pointer to UNK_0xea63
  OFF(); // OFF
}


// ================================================
// 0xef81: WORD 'UNK_0xef83' codep=0x224c parp=0xef83
// ================================================

void UNK_0xef83() // UNK_0xef83
{
  Push2Words("UNK_0xea2f");
  LoadData("UNK_0xe578"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  Push(0x001e);
  _gt_(); // >
  if (Pop() == 0) goto label1;
  Push(0xee55); // pointer to UNK_0xee55
  Push(0xee9d); // pointer to UNK_0xee9d
  _ex__3(); // !_3
  UNK_0xeecb(); // UNK_0xeecb

  label1:
  Push(0xee67); // pointer to UNK_0xee67
  Push(0xee9d); // pointer to UNK_0xee9d
  _ex__3(); // !_3
  UNK_0xeecb(); // UNK_0xeecb
  Push(0xee8b); // pointer to UNK_0xee8b
  Push(0xee9d); // pointer to UNK_0xee9d
  _ex__3(); // !_3
  UNK_0xeecb(); // UNK_0xeecb
  Push(0xee79); // pointer to UNK_0xee79
  Push(0xee9d); // pointer to UNK_0xee9d
  _ex__3(); // !_3
  UNK_0xeecb(); // UNK_0xeecb
  UNK_0xeb6f(); // UNK_0xeb6f
  if (Pop() == 0) Push(1); else Push(0); // NOT
  if (Pop() == 0) return;
  Push2Words("UNK_0xe6bf");
  POSSESSI(); // POSSESSI
  Push2Words("UNK_0xe6bf");
  PRONOUN(); // PRONOUN
}


// ================================================
// 0xefc5: WORD 'POSSIBIL' codep=0xe963 parp=0xefd2
// ================================================
// 0xefd2: db 0x06 0x00 0x00 0x00 0x01 0x00 0xda 0xef 0x14 0x49 0xdb 0x01 0x27 0x68 0xdc 0x01 0x39 0x4e 0xdc 0x01 0x64 0x36 0xdb 0x01 '         I  'h  9N  d6  '

// ================================================
// 0xefea: WORD 'UNK_0xefec' codep=0x224c parp=0xefec
// ================================================

void UNK_0xefec() // UNK_0xefec
{
  UNK_0xe413(); // UNK_0xe413
  if (Pop() == 0) return;
  UNK_0xe41d(); // UNK_0xe41d
  _gt_R(); // >R
  Push(cc__9); // 9
  UNK_0xe68f(); // UNK_0xe68f
  Push(0xdc9d); Push(0x0001);
  D_eq_(); // D=
  if (Pop() == 0) goto label1;
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
  Pop(); Pop();// 2DROP
  Pop(); // DROP
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>

  label1:
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  _gt_R(); // >R
  Push(0x000a);
  OVER(); // OVER
  U_st_(); // U<
  if (Pop() == 0) goto label2;
  Push(0x000a);
  _slash_(); // /
  Push(0x000a);
  Push(Pop() * Pop()); // *

  label2:
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  R_gt_(); // R>
  Push(0x0001); Push(0x0000);
  POSSIBIL(); // POSSIBIL
  UNK_0xea0d(); // UNK_0xea0d
  Push2Words("UNK_0xe6bf");
}


// ================================================
// 0xf06c: WORD 'UNK_0xf06e' codep=0x224c parp=0xf06e
// ================================================

void UNK_0xf06e() // UNK_0xf06e
{
  Push2Words("UNK_0xea2f");
  Push(0xdc7c); Push(0x0001);
  Push2Words("UNK_0xe6bf");
  LoadData("UNK_0xe580"); // from 'CREATURE    '
  Push(Read16(Pop())); // @
  Push(Read16(regsp)); // DUP
  Push(0x0064);
  _st_(); // <
  if (Pop() == 0) goto label1;
  _gt_R(); // >R
  Push(0xdc8d); Push(0x0001);
  goto label2;

  label1:
  Push(0x0064);
  _slash_MOD(); // /MOD
  _gt_R(); // >R
  Push(0xdc8d); Push(0x0001);
  Push2Words("UNK_0xe6bf");
  Push(cc__5); // 5
  ROLL(); // ROLL
  UNK_0xea35(); // UNK_0xea35
  Push2Words("UNK_0xe6bf");
  Push(0xdc9d); Push(0x0001);

  label2:
  Push2Words("UNK_0xe6bf");
  R_gt_(); // R>
  UNK_0xea35(); // UNK_0xea35
  Push2Words("UNK_0xe6bf");
  UNK_0xefec(); // UNK_0xefec
  Push(0xcab9); Push(0x0001);
}


// ================================================
// 0xf0c4: WORD 'SAYIT' codep=0x224c parp=0xf0ce
// ================================================

void SAYIT() // SAYIT
{
  Push(0xe6c5); // pointer to UNK_0xe6c5
  OFF_2(); // OFF_2
  UNK_0xed95(); // UNK_0xed95
  UNK_0xecfd(); // UNK_0xecfd
  UNK_0xec79(); // UNK_0xec79
  UNK_0xef83(); // UNK_0xef83
  UNK_0xec79(); // UNK_0xec79
  UNK_0xeded(); // UNK_0xeded
  UNK_0xec79(); // UNK_0xec79
}


// ================================================
// 0xf0e2: WORD 'ANALOGS' codep=0xe963 parp=0xf0ee
// ================================================
// 0xf0ee: db 0x0c 0x00 0x00 0x00 0x01 0x00 0xfc 0xf0 0x00 0x00 0x02 0x00 0x34 0xf1 0x05 0x35 0xc8 0x01 0x0a 0x47 0xc8 0x01 0x0f 0x58 0xc8 0x01 0x14 0x69 0xc8 0x01 0x1e 0x7e 0xc8 0x01 0x1f 0x90 0xc8 0x01 0x20 0xa0 0xc8 0x01 0x2d 0xb6 0xc8 0x01 0x3c 0xc8 0xc8 0x01 0x4b 0xdc 0xc8 0x01 0x55 0xed 0xc8 0x01 0x5a 0xff 0xc8 0x01 0x5f 0x13 0xc9 0x01 0x64 0x27 0xc9 0x01 0x04 0x3d 0xc9 0x01 0x08 0x50 0xc9 0x01 0x0c 0x74 0xc9 0x01 0x14 0x61 0xc9 0x01 0x1e 0x84 0xc9 0x01 0x1f 0x90 0xc8 0x01 0x20 0x96 0xc9 0x01 0x2d 0xaa 0xc9 0x01 0x3c 0xbd 0xc9 0x01 0x4b 0xcd 0xc9 0x01 0x5a 0xdd 0xc9 0x01 0x64 0xf0 0xc9 0x01 '            4  5   G   X   i   ~          -   <   K   U   Z   _   d'   =   P   t   a              -   <   K   Z   d   '

// ================================================
// 0xf164: WORD 'UNK_0xf166' codep=0x224c parp=0xf166
// ================================================

void UNK_0xf166() // UNK_0xf166
{
  UNK_0xe2e6(); // UNK_0xe2e6
  _gt_R(); // >R
  _gt_R(); // >R
  Push(0xe6c5); // pointer to UNK_0xe6c5
  OFF(); // OFF
  UNK_0xec3b(); // UNK_0xec3b
  UNK_0xf06e(); // UNK_0xf06e
  UNK_0xec79(); // UNK_0xec79
  UNK_0xec3b(); // UNK_0xec3b
  Push(0xca39); Push(0x0001);
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  LoadData("UNK_0xe4e0"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  NICHE_dash_SE(); // NICHE-SE
  _1_dot_5_at_(); // 1.5@
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  UNK_0xe867(); // UNK_0xe867
  UNK_0xec3b(); // UNK_0xec3b
  Push(0xca66); Push(0x0001);
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  LoadData("UNK_0xe578"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  Push(0x0001); Push(0x0000);
  ANALOGS(); // ANALOGS
  UNK_0xea0d(); // UNK_0xea0d
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  UNK_0xe867(); // UNK_0xe867
  UNK_0xec3b(); // UNK_0xec3b
  Push(0xca86); Push(0x0001);
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  LoadData("UNK_0xe570"); // from 'CREATURE    '
  Push(Read8(Pop())&0xFF); // C@
  Push(0x0002); Push(0x0000);
  ANALOGS(); // ANALOGS
  UNK_0xea0d(); // UNK_0xea0d
  UNK_0xebcf(); // UNK_0xebcf
  UNK_0xe8c7(); // UNK_0xe8c7
  ICLOSE(); // ICLOSE
  UNK_0xe867(); // UNK_0xe867
  Push(cc__3); // 3
  Push(0xe6c5); // pointer to UNK_0xe6c5
  _plus__ex_(); // +!
  R_gt_(); // R>
  R_gt_(); // R>
  UNK_0xe2f2(); // UNK_0xe2f2
}


// ================================================
// 0xf1ea: WORD 'UNK_0xf1ec' codep=0x224c parp=0xf1ec
// ================================================

void UNK_0xf1ec() // UNK_0xf1ec
{
  SWAP(); // SWAP
  Push(cc__4); // 4
  Push(Pop() * Pop()); // *
  Push(0x5beb); // pointer to WLEFT
  Push(Read16(Pop())); // @
  Push(Pop()+2); // 2+
  Push(Pop() + Pop()); // +
  Push(0x5bc4); // pointer to WTOP
  Push(Read16(Pop())); // @
  Push(Pop()-1); // 1-
  ROT(); // ROT
  Push(cc__7); // 7
  Push(Pop() * Pop()); // *
  _dash_(); // -
  POS_dot_(); // POS.
}


// ================================================
// 0xf20c: WORD 'UNK_0xf20e' codep=0x224c parp=0xf20e
// ================================================

void UNK_0xf20e() // UNK_0xf20e
{
  Push(0); // 0
  Push(cc__6); // 6
  UNK_0xf1ec(); // UNK_0xf1ec
  Push(1); // 1
  Push(0xe6c5); // pointer to UNK_0xe6c5
  _ex__3(); // !_3
}


// ================================================
// 0xf21c: WORD 'UNK_0xf21e' codep=0x224c parp=0xf21e
// ================================================

void UNK_0xf21e() // UNK_0xf21e
{
  UNK_0xf20e(); // UNK_0xf20e
  PRINT("YOUR SHIP ", 10); // (.")
}


// ================================================
// 0xf22f: WORD 'UNK_0xf231' codep=0x224c parp=0xf231
// ================================================

void UNK_0xf231() // UNK_0xf231
{
  UNK_0xf20e(); // UNK_0xf20e
  LoadData("ART-NAME"); // from 'ARTIFACT    '
  Push(0x0018);
  Push(0x002e);
  UNK_0xe435(); // UNK_0xe435
  Exec(TYPE); // call of word 0x2690 '(TYPE)'
}


// ================================================
// 0xf243: WORD 'UNK_0xf245' codep=0x224c parp=0xf245
// ================================================

void UNK_0xf245() // UNK_0xf245
{
  UNK_0xf20e(); // UNK_0xf20e
  Push(0x63fa); // IFIELD(INST-QTY)
  Push(Read16(Pop())); // @
  Push(0x000a);
  _slash_(); // /
  Push(1); // 1
  MAX(); // MAX
  _dot_(); // .
  PRINT("CUBIC METER(S) ", 15); // (.")
  LoadData("UNK_0xe4b8"); // from 'ELEMENT     '
  Push(0x0010);
  Push(0x002e);
  UNK_0xe435(); // UNK_0xe435
  Exec(TYPE); // call of word 0x2690 '(TYPE)'
}


// ================================================
// 0xf279: WORD 'UNK_0xf27b' codep=0x224c parp=0xf27b
// ================================================

void UNK_0xf27b() // UNK_0xf27b
{
  UNK_0xf20e(); // UNK_0xf20e
  PRINT("YOUR TERRAIN VEHICLE ", 21); // (.")
}


// ================================================
// 0xf297: WORD 'UNK_0xf299' codep=0x224c parp=0xf299
// ================================================

void UNK_0xf299() // UNK_0xf299
{
  UNK_0xf20e(); // UNK_0xf20e
  PRINT("A ", 2); // (.")
  LoadData("SPEC-NAME"); // from 'SPECIMEN    '
  Push(0x0010);
  Push(0x002e);
  UNK_0xe435(); // UNK_0xe435
  Exec(TYPE); // call of word 0x2690 '(TYPE)'
  PRINT(" SPECIMEN ", 10); // (.")
}


// ================================================
// 0xf2bd: WORD 'UNK_0xf2bf' codep=0x224c parp=0xf2bf
// ================================================

void UNK_0xf2bf() // UNK_0xf2bf
{
  UNK_0xf20e(); // UNK_0xf20e
  PRINT("A RUIN ", 7); // (.")
}


// ================================================
// 0xf2cd: WORD 'UNK_0xf2cf' codep=0x224c parp=0xf2cf
// ================================================

void UNK_0xf2cf() // UNK_0xf2cf
{
  UNK_0xf20e(); // UNK_0xf20e
  PRINT("A MESSAGE ", 10); // (.")
}


// ================================================
// 0xf2e0: WORD 'CLASS>DE' codep=0x4a4f parp=0xf2ed
// ================================================
// entry
// 0xf2ed: db 0x08 0x00 0x6b 0x26 0x44 0x00 0xce 0xf0 0x2a 0x00 0x7b 0xf2 0x28 0x00 0x99 0xf2 0x14 0x00 0x1e 0xf2 0x29 0x00 0xbf 0xf2 0x1a 0x00 0x45 0xf2 0x1c 0x00 0x31 0xf2 0x1b 0x00 0xcf 0xf2 '  k&D   * { (       )     E   1     '

// ================================================
// 0xf311: WORD 'CLASS>TY' codep=0x4a4f parp=0xf31e
// ================================================
// 0xf31e: db 0x08 0x00 0x6b 0x26 0x44 0x00 0x66 0xf1 0x2a 0x00 0x7b 0xf2 0x28 0x00 0x99 0xf2 0x14 0x00 0x1e 0xf2 0x29 0x00 0xbf 0xf2 0x1a 0x00 0x45 0xf2 0x1c 0x00 0x31 0xf2 0x1b 0x00 0xcf 0xf2 '  k&D f * { (       )     E   1     '

// ================================================
// 0xf342: WORD '?TALK' codep=0x224c parp=0xf34c
// ================================================
// entry

void _ask_TALK() // ?TALK
{
  Push(0x55f2); // pointer to COLOR
  Push(Read16(Pop())); // @
  _gt_R(); // >R
  SetColor("WHITE");
  _ex_COLOR(); // !COLOR
  if (Read16(regsp) != 0) Push(Read16(regsp)); // ?DUP
  if (Pop() == 0) goto label1;
  Push(0); // 0

  signed short int i = Pop();
  signed short int imax = Pop();
  do // (DO)
  {
  POINT_gt_ICON(); // POINT>ICON
  _at_IL(); // @IL
  _at_IH(); // @IH
  _gt_C_plus_S(); // >C+S
  _at_INST_dash_CLASS(); // @INST-CLASS
  Push(0x5bae); // pointer to NCRS
  Push(Read16(Pop())); // @
  Push(cc__3); // 3
  _eq_(); // =
  if (Pop() == 0) goto label2;
  Pop();
  switch(Pop()) // CLASS>DE
  {
  case 68:
    SAYIT(); // SAYIT
    break;
  case 42:
    UNK_0xf27b(); // UNK_0xf27b
    break;
  case 40:
    UNK_0xf299(); // UNK_0xf299
    break;
  case 20:
    UNK_0xf21e(); // UNK_0xf21e
    break;
  case 41:
    UNK_0xf2bf(); // UNK_0xf2bf
    break;
  case 26:
    UNK_0xf245(); // UNK_0xf245
    break;
  case 28:
    UNK_0xf231(); // UNK_0xf231
    break;
  case 27:
    UNK_0xf2cf(); // UNK_0xf2cf
    break;
  default:
    BEEP(); // BEEP
    break;

  }
  goto label3;

  label2:
  Pop();
  switch(Pop()) // CLASS>TY
  {
  case 68:
    UNK_0xf166(); // UNK_0xf166
    break;
  case 42:
    UNK_0xf27b(); // UNK_0xf27b
    break;
  case 40:
    UNK_0xf299(); // UNK_0xf299
    break;
  case 20:
    UNK_0xf21e(); // UNK_0xf21e
    break;
  case 41:
    UNK_0xf2bf(); // UNK_0xf2bf
    break;
  case 26:
    UNK_0xf245(); // UNK_0xf245
    break;
  case 28:
    UNK_0xf231(); // UNK_0xf231
    break;
  case 27:
    UNK_0xf2cf(); // UNK_0xf2cf
    break;
  default:
    BEEP(); // BEEP
    break;

  }

  label3:
  ICLOSE(); // ICLOSE
  Push(i); // I'
  Push(1); // 1
  _gt_(); // >
  Push(i); // I
  Push(Pop()+1); // 1+
  Push(i); // I'
  _eq_(); // =
  if (Pop() == 0) Push(1); else Push(0); // NOT
  Push(Pop() & Pop()); // AND
  if (Pop() == 0) goto label4;
  Push(0x05dc);
  MS(); // MS
  UNK_0xec53(); // UNK_0xec53
  UNK_0xec53(); // UNK_0xec53

  label4:
  i++;
  } while(i<imax); // (LOOP) 0xffbe


  label1:
  R_gt_(); // R>
  _ex_COLOR(); // !COLOR
}

// 0xf3aa: db  'SENTS___________________________`      I      9@L  I      9@    2272    `   $  I      9@  & <  I  Y   9@          `     z   `      I      9@      s;F       b   ` v \  I      9@  d   A s;_   {;_ >       L    I      Y   9@    k   9@>     2   6   `     M    ;-           `   XQ`   XQ`   XQ  L"    {;        `   T              m     SPEC.PAR L"   T   J K`   =      /   H x   5         LOPARMS________________________________---------2218---- '
  