// store offset = 0xea30
// overlay size   = 0x0b30
// name = 'XCOM-VOC________________________ SPEC -------------8/13/85 AWK )1175Ï‹COMM-VOC'

// =================================
// =========== DICTIONARY ==========
// =================================
// 1703:      UNK_0xea84 bitfield: 0 0 0  codep:0x224c parp:0xea84 size:0x0008
// 1704:      UNK_0xea8e bitfield: 0 0 0  codep:0x224c parp:0xea8e size:0x005e
// 1705:      UNK_0xeaee bitfield: 0 0 0  codep:0x224c parp:0xeaee size:0x008b
// 1706:      UNK_0xeb7b bitfield: 0 0 0  codep:0x224c parp:0xeb7b size:0x000e
// 1707:          ?VOWEL bitfield: 0 0 0  codep:0x4a4f parp:0xeb94 size:0x0134
// 1708:      UNK_0xecca bitfield: 0 0 0  codep:0x1d29 parp:0xecca size:0x01e3
// 1709:            XY/N bitfield: 0 0 0  codep:0x224c parp:0xeeb6 size:0x05ce
// 1710:      UNK_0xf486 bitfield: 0 0 0  codep:0x224c parp:0xf486 size:0x002e
// 1711:         SA-CASE bitfield: 0 0 0  codep:0x4a4f parp:0xf4c0 size:0x001c
// 1712:      UNK_0xf4de bitfield: 0 0 0  codep:0x224c parp:0xf4de size:0x0014
// 1713:         (XCOMM) bitfield: 0 0 0  codep:0x224c parp:0xf4fe size:0x0062

// =================================
// =========== VARIABLES ===========
// =================================
unsigned char UNK_0xecca[483] = {};



// 0xea42: db 0xaf 0x00 0x4c 0x22 0x5d 0x17 0xd8 0xbd 0xce 0x84 0x90 0x16 0x4c 0x22 0x5d 0x17 0x0d 0x00 0x44 0xea 0xe2 0x5c 0x76 0x6d 0xb3 0x5f 0xae 0x0b 0x93 0x3b 0x44 0xea 0xfa 0x15 0x04 0x00 0x35 0x10 0x99 0x55 0x76 0x6d 0x90 0x16 0x4c 0x22 0x5d 0x17 0xd1 0xc1 0xce 0x84 0x90 0x16 0x4c 0x22 0x5d 0x17 0xe1 0xc1 0xce 0x84 0x90 0x16 '  L"]       L"]   D  \vm _   ;D     5  Uvm  L"]       L"]       '
  
// ================================================
// 0xea82: WORD 'UNK_0xea84' codep=0x224c parp=0xea84
// ================================================

void UNK_0xea84()
{
  Push(0xc205); MODULE();
  
}


// ================================================
// 0xea8c: WORD 'UNK_0xea8e' codep=0x224c parp=0xea8e
// ================================================

void UNK_0xea8e()
{
  >C+S();
  PHRASE$ pp_LSCAN $!();
  pp_LSCAN COUNT();
  ICLOSE();
  
}

// 0xea9e: db 0x4d 0x74 0x34 0x12 0x01 0x4d 0x74 0x34 0x13 0x01 0x4d 0x74 0x14 0x43 0x02 0x68 0x3b 0x01 0x00 0x90 0x60 0x68 0x3b 0x01 0x00 0x72 0x82 0x68 0x3b 0x00 0x00 0xc0 0xda 0x4c 0x22 0x28 0xa5 0x39 0x3f 0x0a 0x52 0x45 0x43 0x45 0x49 0x56 0x49 0x4e 0x47 0x3a 0x47 0xa5 0x5d 0x17 0xf4 0x01 0xef 0x2a 0x90 0x16 0x4c 0x22 0xbf 0xea 0x6e 0xea 0x78 0xea 0x5d 0x17 0x16 0xc2 0xce 0x84 0xc9 0x79 0x90 0x16 'Mt4  Mt4  Mt C h;   `h;  r h;    L"( 9? RECEIVING:G ]    *  L"  n x ]      y  '
  
// ================================================
// 0xeaec: WORD 'UNK_0xeaee' codep=0x224c parp=0xeaee
// ================================================

void UNK_0xeaee()
{
  pp_THIS-BU @ Push(2); < 
  if (Pop() == 0) goto label1042;
  BLUE pp_THIS-BU @ .ON();
  CTINIT();
  Push(0x118a); Pust(0x0002); UNK_0xea8e();
  .TTY();
  Push(0x01f4); MS();
  
  if (Pop() == 0) goto label1043;
  
  UNK_0x3f39("YES.");
  
  goto label1045;
  
  label1043:
  
  UNK_0x3f39("NO.");
  
  label1045:
  .TTY();
  TTY-SCR();
  
  goto label1044;
  
  label1042:
  DROP 
  label1044:
  BLACK pp_THIS-BU @ .ON();
  
}

// 0xeb3d: db 0x4c 0x22 0xf2 0x79 0x5d 0x17 0xd5 0x02 0x5d 0x17 0xde 0x04 0xb7 0x74 0xac 0x6d 0x75 0x75 0x30 0x75 0x2e 0x0f 0x5f 0x12 0xc9 0x79 0x6f 0x56 0xae 0x0b 0x9f 0x12 0xf5 0x12 0xfd 0x62 0x95 0x49 0xb9 0xea 0x39 0x40 0xf5 0x12 0xfa 0x15 0x0a 0x00 0x37 0x52 0x6f 0x7b 0x60 0x16 0x04 0x00 0xac 0x7e 0x90 0x16 'L" y]   ]    t muu0u. _  yoV       b I  9@      7Ro{`    ~  '
  
// ================================================
// 0xeb79: WORD 'UNK_0xeb7b' codep=0x224c parp=0xeb7b
// ================================================

void UNK_0xeb7b()
{
  XYSCAN();
  DROP ?DUP 
  if (Pop() == 0) goto label1039;
  NEW-BUT();
  
  label1039:
  
}


// ================================================
// 0xeb89: WORD '?VOWEL' codep=0x4a4f parp=0xeb94
// ================================================
// 0xeb94: db  '  " A 0 E 0 I 0 O 0 U 0 L"      L"HZ m] d     L"] J     ) : L"Pl y z]   ]    ~  L"    .  z     ~     t  A   vm    x   `   2    y`        u y  L"  2  z     ~2   L"uuPl9@       u`    y  L""    t m t           ~4   f   L"]     Y  y tvmXuPl y7  y  f   L"Pl y z]   ]    ~2  z     ~2 H = 2 4   N   L" Q y       y  '
  
// ================================================
// 0xecc8: WORD 'UNK_0xecca' codep=0x1d29 parp=0xecca
// ================================================
// 0xecca: db  ': L"  2 .  ;           ;Q . ] d  JA   ]       r ] ?   jm  oO    ]        *8    :  e  ` my |\   *8  `8 F@>     P      [  A     `   =    ;    _   A       J   t oO          G          DENIED.  `      GRANTED.HZ m \ m6^ m  L" T  {;       T  . -       n x    m             y  L"   y        F  y     T  . -       n x    m    R        y  L" T  .         n x    m             y  L" T   ;       T  . -       n x    m    N        y  L"   y        F  y     T  . -       n x    m             y  '
  
// ================================================
// 0xeead: WORD 'XY/N' codep=0x224c parp=0xeeb6
// ================================================

void XY/N()
{
  'KEY();
  DROP Push(0x000a); .BTN-TE();
  INIT-BU();
  TIME 2@ Push(0xea60); Pust(0x0000); D+ pp_TIRED-T D!();
  
  label1040:
  UNK_0xeb7b();
  pp_FTRIG @ TIME 2@ pp_TIRED-T 2@ D>();
  OR 
  if (Pop() == 0) goto label1040;
  pp_THIS-BU @ DUP Push(2); = pp_FTRIG @ * pp_(STOP-C !();
  ?TRIG();
  0= 2* + 0= 
  if (Pop() == 0) goto label1041;
  DROP UNK_0xea84();
  Push(1); 
  goto label1046;
  
  label1041:
  UNK_0xea84();
  DROP Push(0); 
  label1046:
  DUP UNK_0xeaee();
  Push(cc_9); .BTN-TE();
  
}

// 0xef24: db  ') : L"          L"]   D  T  . -     h n x (   ]   ] 2       ]     ]   `      m  ] 2 $         y   pz  P    y ~ y y$  m    r ] d         jm y  L"          L". D  T  . -     . n x       ] d       .   `      m  ] d   jm y  ) : L"          L"s;]       j n x       ] T ] d     A   vm]   L   ]      @2 ] ] r A s;   m   m   y   ;          ]   L      ;l `      m   "  L"  ?       L"          L"n x  T  = _       f             N ] ,  ;    6 ] ,  ;L ]   ]   Y  y     tvmuu yPl y7  y]    m  `        ;  `     N        y  L"          L" U        n x    m  "   ] d   ] d   jm y  L"          L"          L"n x  T  = _   >                   * ]    ;       ;  Z `      mf ]      ;l    y  L"          L"n x  T  ]     ] (  ;      \    T   ;_  ;  s;r ] 0 r s; ?]   r jm    ] d     $      T   ;_ ] <   ]   r    ;L `      m   y  L"          L"  A  #]    m      ] N `      g] ; r jm g] = r A ]      m #]   ] . g        m  L"      ] !   ]       A   vm]   L   ]      g]   r A {;   m   m2     ]   -     ] S `      g]   r =    m  L"n x Pl y z]   ]    ~  f  z. ]    J     z    0u]   _      z0u]   _   A   * L   X    g       ;        H  ~2      ;l  z`z>     4  y  L"          L"n x       ] d     < (  O   O]  Ovm  9? CREW ARRESTEDG 9? GAME OVERG   2  H2      m   y  L"          L"          L"n x     ]    * _     _            ]    *   mHZ m    jm T m y  )  0     t )  ? s       )    6   )    )    )      '
  
// ================================================
// 0xf484: WORD 'UNK_0xf486' codep=0x224c parp=0xf486
// ================================================

void UNK_0xf486()
{
  ?DUP 
  if (Pop() == 0) goto label1048;
  pp_UNK_0xecca OFF();
  COUNT();
  2* OVER + SWAP 
  do // (DO)
  {
  I @EXECUTE pp_UNK_0xecca @ 
  if (Pop() == 0) goto label1049;
  LEAVE 
  label1049:
  .SORD();
  Push(2); 
  } while(...); // (+LOOP) 0xffec
  
  label1048:
  
}


// ================================================
// 0xf4b4: WORD 'SA-CASE' codep=0x4a4f parp=0xf4c0
// ================================================
// 0xf4c0: db 0x06 0x00 0x22 0x0f 0x01 0x00 0x54 0xf4 0x07 0x00 0x5f 0xf4 0x08 0x00 0x6c 0xf4 0x03 0x00 0x75 0xf4 0x0a 0x00 0x7a 0xf4 0x15 0x00 0x7f 0xf4 '  "   T   _   l   u   z    '
  
// ================================================
// 0xf4dc: WORD 'UNK_0xf4de' codep=0x224c parp=0xf4de
// ================================================

  label1047:
  
void UNK_0xf4de()
{
  TIME 2@ pp_OK-TALK 2@ D>();
  ?TERMINAL OR 
  if (Pop() == 0) goto label1047;
  
}


// ================================================
// 0xf4f2: WORD '(XCOMM)' codep=0x224c parp=0xf4fe
// ================================================

void (XCOMM)()
{
  UNK_0xf4de();
  pp_(AORIGI @>C+S();
  @INST-S();
  ICLOSE();
  SA-CASE UNK_0xf486();
  UNK_0xf4de();
  
}

// 0xf510: db 0x58 0x43 0x4f 0x4d 0x2d 0x56 0x4f 0x43 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x5f 0x20 0x53 0x50 0x45 0x43 0x20 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x2d 0x38 0x2f 0x31 0x33 0x2f 0x38 0x35 0x20 0x41 0x57 0x4b 0x20 0x29 0x31 0x31 0x37 0x35 0xcf 0x8b 0x90 0x16 0x43 0x4f 0x4d 0x4d 0x2d 0x56 0x4f 0x43 0x00 'XCOM-VOC________________________ SPEC -------------8/13/85 AWK )1175    COMM-VOC '
  